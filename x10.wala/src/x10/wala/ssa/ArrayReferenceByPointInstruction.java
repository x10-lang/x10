package x10.wala.ssa;

import com.ibm.wala.types.TypeReference;
import com.ibm.wala.util.debug.Assertions;

public abstract class ArrayReferenceByPointInstruction extends ArrayReferenceInstruction {
    protected final int pointIndex;

    public ArrayReferenceByPointInstruction(int arrayRef, int pointIndex, TypeReference declaredType) {
	super(arrayRef, declaredType);
	this.pointIndex= pointIndex;
    }

    public int getPointIndex() {
	return pointIndex;
    }

    @Override
    public int getNumberOfUses() {
	return 2;
    }

    @Override
    public int getUse(int j) throws UnsupportedOperationException {
    Assertions.productionAssertion(j <= 2);
	return (j == 0) ? arrayRef : pointIndex;
    }

    @Override
    public int hashCode() {
        return 5087 + 1801 * pointIndex + 6221 * super.hashCode();
    }
}
