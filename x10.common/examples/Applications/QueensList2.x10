...
class loadBalancer {
	...
	// in each place
	const Random[.] Ran= new Random[UniqueD]
            (point p[i]) {return new Random((long)i*17L);};
	/**
	 * find an appropriate place to ship a task
	 */
	public static place choosePlace() {
		// choose a random place (includes here)
		return UniqueD[Ran[(here).id].nextInt(NP)];
	}
}
...
public value class QueensList2 {
   // Operations and data structures for a list of queen coordinates

   // Since QueensList2 is an immutable value class, 
    ...
    */
   public QueensList2(final int value [.] oldList, final int newItem) {
