...
public class BlockDist {
	...
	final int L=5;
	   for(point [tries]:1:COUNT) {
		...
		if ((Math.abs(ub1-lb1) + 1) % np != 0) {
		    // System.out.println("Current implementation allows blocking only over one dimension -- that must be sufficiently wide (dim1.size() == " + (Math.abs(ub1-lb1) + 1) + " < #places=" + place.MAX_PLACES);
		} else {
