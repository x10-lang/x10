...
public final value Ft {
	...
	private final static dist(:rank==1 && unique) UNIQUE = (dist(:rank==1 && unique))dist.UNIQUE;
	...
	public void FFT2DComm_Slab(final DoubleArray local2d, final DoubleArray[.] dist1d, final int dir, final int orientation, final int placeID, final clock clk) {
	...
	public void FFT2DComm_Pencil(final DoubleArray local2d, final DoubleArray[.] dist1d,
	...
	public void FFT2DComm(final DoubleArray local2d, final DoubleArray[.] dist1d,
			final int dir, final int orientation, final int placeID, final clock clk) {
	...
	*/
	...
	public static void  main(String[] args) {
		...
		final DoubleArray[.] Planes2d = new DoubleArray[UNIQUE]
			(point p) { return new DoubleArray((int)(FtSolver.MAX_PADDED_SIZE/NUMPLACES), OFFSET); };
		final DoubleArray[.] Planes1d = new DoubleArray[UNIQUE]
			(point p) { return new DoubleArray((int)(FtSolver.MAX_PADDED_SIZE/NUMPLACES), 0); };
		final DoubleArray[.] V = new DoubleArray[UNIQUE]
			(point p) { return new DoubleArray((int)(FtSolver.MAX_PADDED_SIZE/NUMPLACES), 0); };
		final DoubleArray[.] ex = new DoubleArray[UNIQUE]
			(point p) { return new DoubleArray((int)(FtSolver.TOTALSIZE/NUMPLACES), 0); };
		...
		finish async {
			...
			ateach (point [PID]: UNIQUE) clocked(clk) {
				...
				initializeC(NUMPLACES, FtSolver.NX, FtSolver.NY, FtSolver.NZ, OFFSET, CPAD_COLS);
				double cputime2;
				...
				FFTInit(FtSolver.FT_COMM, localPlanes2d.m_array, localPlanes1d.m_array, PID);
				...
				FtSolver.FFT2DComm(localPlanes2d, Planes1d, FFT_FWD, current_orientation, PID, clk);
				next;
				FT_1DFFT(FtSolver.FT_COMM, localPlanes1d.m_array, localPlanes2d.m_array, 1, FFT_FWD, current_orientation, PID);
				next;

				if (PID == 0) System.out.println("Start timing of IBM X10 NAS FT: class "+FtSolver.class_id_char+" ("+FtSolver.class_id_str+", FT_COMM = "+FtSolver.FT_COMM+")");
				cputime2 = -mysecond();
				...
				FtSolver.FFT2DComm(localPlanes2d, Planes1d, FFT_FWD, current_orientation, PID, clk);
				next;
				FT_1DFFT(FtSolver.FT_COMM, localPlanes1d.m_array, local_V.m_array, 0, FFT_FWD, current_orientation, PID);
				next; // NOT redundant

				current_orientation = switch_view(current_orientation, PID);
				int saved_orientation = current_orientation;

				for (int iter = 1; iter <= FtSolver.MAX_ITER; iter ++) {
					...
					FtSolver.FFT2DComm(localPlanes2d, Planes1d, FFT_BWD, current_orientation, PID, clk);
					next;
					FT_1DFFT(FtSolver.FT_COMM, localPlanes1d.m_array, localPlanes2d.m_array, 1, FFT_BWD, current_orientation, PID);
					current_orientation = switch_view(current_orientation, PID);
					FtSolver.checksum(localPlanes2d, PID, iter, clk);
					next;
					if (PID == 0) {
						System.out.println(" Iter = "+iter+" checksum_real = "+
								FtSolver.checksum_real[iter]+" checksum_imag = "+FtSolver.checksum_imag[iter]);
					}
				}
				cputime2 += mysecond();
		...
		if (FtSolver.class_id_char !='T') checksum_verify(FtSolver.NX, FtSolver.NY, FtSolver.NZ, FtSolver.MAX_ITER, FtSolver.checksum_real, FtSolver.checksum_imag);
