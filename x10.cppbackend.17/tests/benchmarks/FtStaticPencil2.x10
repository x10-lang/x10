...
public final value FtStatic {
	...
	// [IP] For Tong: Made static
	private static int switch_view(int orientation, int PID) {
	...
	// [IP] For Tong: Made static
	private static int set_view(int orientation, int PID) {
	...
	public static void FFT2DComm_Slab(final DoubleArray local2d, final DoubleArray[.] dist1d,
					  final int dir, final int orientation, final int placeID,
					  final clock clk, final int [:self.rect && self.rank==1 && self.zeroBased] dims) {

		final int dim0 = dims[orientation*3], dim1 = dims[orientation*3+1], dim2 = dims[orientation*3+2];
		//System.out.println(" orientation ="+orientation+" dim0 = "+dim0+" dim1 = "+dim1+" dim2 = "+dim2);
		final int plane_size = dim0*dim1;
		final int CHUNK_SZ = (dim0/NUMPLACES)*dim1;
		final int numrows = dim0/NUMPLACES;
		...
		for (int p = 0; p < dim2/NUMPLACES; p++) {
			...
			for (int t = 0; t <NUMPLACES; t++) {
				...
				final int destID = t;
				final int offset2 = offset1 + destID*CHUNK_SZ;
				...
				//int destStart = dist1d[destID].m_start +
	...
	public static void FFT2DComm_Pencil(final DoubleArray local2d, final DoubleArray[.] dist1d,
					    final int dir, final int orientation, final int placeID, final clock clk,
	...
	public static void FFT2DComm(final DoubleArray local2d, final DoubleArray[.] dist1d,
				     final int dir, final int orientation, final int placeID, final clock clk,
				     final int [:self.rect && self.rank==1 && self.zeroBased] dims, final int FT_COMM) {
	    if (FT_COMM == FT_COMM_SLABS)
		FFT2DComm_Slab(local2d, dist1d, dir, orientation, placeID, clk, dims);
	    else
		FFT2DComm_Pencil(local2d, dist1d, dir, orientation, placeID, clk, dims);
	}
	...
	/*
	private void printArray(final DoubleArray[.] DDA) {
		finish ateach (point [PID]: UNIQUE) {
			DoubleArray da = DDA[PID];
	...
	*/
	...
	static final int OFFSET = 3, CPAD_COLS = 0;
		...
		PLANES_ORIENTED_Y_Z_X = 1, /*assumes that Y Z planes are contiguous in the memory*/
		PLANES_ORIENTED_Z_X_Y = 2; /*assumes that Z X planes are contiguous in the memory*/

	static final  int SS = 1, WW=2, AA=3, BB=4, CC=5, DD=6;
	static final  int NUMPLACES = place.MAX_PLACES;
	static final  dist(:rank==1 && unique) UNIQUE = (dist(:rank==1 && unique))dist.UNIQUE;
	public static void  main(String[] args) {
		...
		int CLASS_TMP = 0;
		int FT_COMM_TMP = FT_COMM_SLABS;
		for (int q = 0; q < args.length; ++q) {
			if (args[q].equals("-s") || args[q].equals("-S")) CLASS_TMP = 1;
			if (args[q].equals("-w") || args[q].equals("-W")) CLASS_TMP = 2;
			if (args[q].equals("-a") || args[q].equals("-A")) CLASS_TMP = 3;
			if (args[q].equals("-b") || args[q].equals("-B")) CLASS_TMP = 4;
			if (args[q].equals("-c") || args[q].equals("-C")) CLASS_TMP = 5;
			if (args[q].equals("-d") || args[q].equals("-D")) CLASS_TMP = 6;
			if (args[q].equals("-p") || args[q].equals("-P")) FT_COMM_TMP = FT_COMM_PENCILS;
		}
		...
		/*
			(int[:self.rect && self.rank==1 && self.zeroBased]) new int [[0:8]];
		...
		*/
		...
		finish async {
			...
			ateach (point [PID]: UNIQUE) clocked(clk) {
				...
				double cputime2, cputime3;
				int current_orientation = set_view(PLANES_ORIENTED_X_Y_Z,PID);

				final DoubleArray local_ex = ex[PID];
				final DoubleArray localPlanes2d = Planes2d[PID];
				final DoubleArray localPlanes1d = Planes1d[PID];
				...
				cputime2 = -mysecond(); cputime3=cputime2;
				...
				cputime3 += mysecond(); timers[PID]+=cputime3;
				next;
				...
				for (int iter = 1; iter <= MAX_ITER; iter ++) {
					...
					//  dims, FT_COMM);
					// vj Mon Sep 24 13:13:00 2007 manually inlined
					{
					    final int dim0 = dims[current_orientation*3],
						dim1 = dims[current_orientation*3+1], dim2 = dims[current_orientation*3+2],
					    ...
					    for (int p = 0; p < dim2/NUMPLACES; p++) {
						final int offset1 = plane_size*p;
						FFT2DLocalCols(local2darray, offset1, FFT_BWD, current_orientation, PID);
						for (int i = 0; i < numrows; i++)
						    for (int t = 0; t <NUMPLACES; t++) {
							//final int destID = (PID + t) % NUMPLACES; //the MPI order
							final int destID = t;
							final int offset2 = offset1 + destID*CHUNK_SZ + i*dim1;
							FFT2DLocalRow(local2darray, offset2, FFT_BWD, current_orientation, PID);
							//int srcStart = localPlanes2d.m_start + offset2*2;
							final int srcStart = offset2*2;
							//int destStart = Planes1d[destID].m_start +
							//        2*(PID*dim2/NUMPLACES*dim1+p*dim1 + i*dim1*dim2);
					...
					// vj Mon Sep 24 13:08:00 2007 manually inlined
					{
					double sum_real = 0;
					double sum_imag = 0;
	...
	// [IP] For Tong: Made static
	private static void checksum_verify(final int d1, final int d2, final int d3, final int nt,
...
//@@X101X@@TSTAMP@@20082605.105957
//@@X101X@@TCASE@@FtStaticPencil2
//@@X101X@@VCODE@@FAIL_BUILD
//@@X101X@@TOUT@@0 60
//@@X101X@@SRCS@@FtStatic.cc
