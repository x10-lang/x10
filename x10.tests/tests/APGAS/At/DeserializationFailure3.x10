...
public class DeserializationFailure3 extends x10Test {
       ...
       for (victim in Place.places()) {
           ...
           try {
               ...
               passed = false;
                ...
                if (e.exceptions().size == 2) {
                    Console.OUT.println("Sub-test pass: got exactly 2 exceptions");
                } else {
                    Console.OUT.println("Sub-test fail: got "+e.exceptions().size+" exceptions; not 2");
                    passed = false;
                }
            }
       }                          
    ...
    public static def main(Rail[String]) {
        new DeserializationFailure3().execute();
    }
}
