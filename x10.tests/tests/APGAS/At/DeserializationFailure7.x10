 ...
 */
public class DeserializationFailure7 extends x10Test {
       ...
       for (victim in Place.places()) {
           ...
           try {
               for (p in Place.places()) {
                   val res = at (p) (victim == here ? new TimeBomb(there) : new TimeBomb(there.next()));
    ...
    public static def main(Rail[String]) {
        new DeserializationFailure7().execute();
    }
}
