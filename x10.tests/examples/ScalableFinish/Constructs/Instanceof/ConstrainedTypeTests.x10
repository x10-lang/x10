 ...
 * @author igorp
 **/
public class ConstrainedTypeTests   {
	...
	public def run(): boolean = {
		...
		x10.io.Console.OUT.println("X10DepTypeSubClassOneB{a==2} <: X10DepTypeClassOneB{p==2} -> "+(!res4));
		x10.io.Console.OUT.println("X10DepTypeSubClassOneB{a==3} <: X10DepTypeClassOneB{p==2} -> "+(!res5));
		x10.io.Console.OUT.println("X10DepTypeSubClassOneB{p==2} <: X10DepTypeClassOneB{p==2} -> "+res6);
		
		return (res1 && res2 && res3 && res4 && res5 && res6);
	}

	public static def main(var args: Rail[String]): void = {
		new ConstrainedTypeTests().run ();
	}
