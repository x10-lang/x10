...
class Linpack {
	...
	var kflops: int;
	val opInt = Int.+;
	var ipvt: Array[int @ Clocked[double] (c, op, 0)](1);

	final def read(val a: Array[double @ Clocked[Double] (c,op, 0.0)](2), val i: int, val j: int): double = @ ClockedM (c) {
		return a(i, j);
	}
	final def write(val a: Array[double @ Clocked[Double] (c, op, 0.0)](2), val i: int, val j: int, val val: double): void = @ ClockedM (c){
		 a(i, j) = val;
	}
	final def plusWrite(val a: Array[double @ Clocked[Double] (c, op, 0.0)](2), val i: int, val j: int, val val: double): void = @ ClockedM (c){
		 a(i, j) = val;
	}
	final def plusWrite(val a: Array[double @ Clocked[Double] (c, op, 0.0)](1) , val i: int, val val: double): void = @ ClockedM (c){
		  a(i) = val;
	}
	/*final def timesWrite(val a: Array[double @ Clocked[Double](c, op, 0.0)](2) , val i: int, val j: int, val val: double): void = @ ClockedM (c) {
		 atomic a(i, j) *= val;
	}
	final def timesWrite(val a: Array[double @ Clocked[Int] (c, opInt, 0)](1) , val i: int, val val: double): void = @ ClockedM (c) {
		 atomic a(i) *= val;
	}*/
	...
	final def matgen(val a: Array[double @ Clocked[double](c, op, 0.0)](2), val lda: int, val n: int, val b: Array[double @ Clocked[double](c, op, 0.0)](2)): double = @ ClockedM(c){
		var init: int = 1325;
	 ...
	 */
	final def dgefa(val a: Array[double @ Clocked[Int](c, op, 0.0)](2), val lda: int, val n: int, val ipvt: Array[int @ Clocked[Int] (c, opInt, 0)](1)): int = @ ClockedM (c){
	...
	/**
	 ...
	 blas daxpy, ddot
	 */
	final def dgesl(val a: Array[double @ Clocked[double](c, op, 0.0)](2), val lda: int, val n: int, val ipvt: Array[int @ Clocked[Int] (c, opInt, 0)](1), val b: Array[double @ Clocked[Double](c, op, 0.0)](2), val job: int): void = @ ClockedM(c) {
		val nm1: int = n - 1;
		if (job == 0) {
	 ...
	 */
	final def daxpy(var n: int, var da: double, val dx: Array[double](2), val dxCol: int, val dx_off: int, var incx: int, var dy: Array[double](2), var dyCol: int, var dy_off: int, var incy: int): void = @ ClockedM (c) {
		if ((n > 0) && (da != 0)) {
			if (incx != 1 || incy != 1) {
	 ...
	 */
	final def idamax(val n: int, val dx: Array[double](2), val dxk: int, val dx_off: int, val incx: int): int = @ ClockedM(c) {
		if (n < 1) return -1;
		if (n == 1) return 0;
		if (incx != 1) {
			// code for increment not equal to 1
	 ...
	 */
	final def dmxpy(val n1: int, val y: Array[double](2), val n2: int, val ldm: int, val x: Array[double](2), val m: Array[double](2)): void = @ ClockedM(c) {
		// cleanup odd vector
...
public class LUFact extends Linpack {
	...
	public def JGFinitialise(): void = @ClockedM (Linpack.c) {
		n = datasizes(size);
	...
	public def JGFkernel(): void = @ ClockedM(Linpack.c) {
	...
	public def JGFvalidate(): void = @ClockedM(Linpack.c) {
		var i: int;
		var eps: double;
		var residn: double;
	...
	public def JGFrun(var size: int): void = @ ClockedM (Linpack.c) {
