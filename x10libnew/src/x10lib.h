...
enum error_code { X10_OK, X10_NOT_OK, X10_ILLEGAL_ARG};


extern int init(X10_async_handler_t* handlers, int num);
extern int finalize();
extern int cleanup();
 ...
 */
extern int get(X10_gas_ref_t src,  void* dest, int nbytes);
 ...
 */
extern int get_nb(X10_gas_ref_t src, void* dest, int nbytes, 
                      X10_switch_t handle);
...
template <typename T>
extern T getValue_int (x10_gas_ref_t src);
 ...
 */
extern int put(void* src, X10_gas_ref_t dest, int bytes);
 ...
 * should simply create a "default" switch and use that explicitly.
 */
extern int put_nb(void* src, X10_gas_ref_t dest, int bytes, 
                      X10_switch_t handle);
 ...
 */             
template <typename T>
extern int putValue_int(T value, X10_gas_ref_t dest);
