...
public class PolyglotNodeLocator implements ISourcePositionLocator {
    ...
    private NodeVisitor fVisitor= new NodeVisitor() {
        public NodeVisitor enter(Node n) {
            if (DEBUG)
                System.out.println("Entering node type = " + n.getClass().getName() + ", first token '"
                        + fLS.getIPrsStream().getTokenAtCharacter(n.position().offset()) + "'");
            ...
            if (DEBUG)
                System.out.println("Node extent: " + pos.offset() + " => " + pos.endOffset() + " [" + pos.line() + ":" + pos.column() + " => [" + pos.endLine()
                        + ":" + pos.endColumn() + "]");
    ...
    /**
     * Like fParentVisitor, but looks for the parent of a given node, rather than the parent of the node with a given offset/length.
     */
