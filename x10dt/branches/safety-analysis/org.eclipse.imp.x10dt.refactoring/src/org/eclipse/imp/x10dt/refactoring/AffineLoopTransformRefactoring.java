...
public class AffineLoopTransformRefactoring extends X10RefactoringBase {
    private static final String AFFINE_LOOP_TRANSFORM_REFACTORING_NAME= "Affine Loop Transform";
    ...
    private Expr fTransformExpr;

    public AffineLoopTransformRefactoring(ITextEditor editor) {
        super(editor);
    }

    @Override
    public String getName() {
        return AFFINE_LOOP_TRANSFORM_REFACTORING_NAME;
    }
    ...
    public Change createChange(IProgressMonitor pm) throws CoreException, OperationCanceledException {
        CompositeChange outerChange = new CompositeChange(AFFINE_LOOP_TRANSFORM_REFACTORING_NAME);
        TextFileChange tfc = new TextFileChange("Affine loop transform", fSourceFile);

        tfc.setEdit(new MultiTextEdit());

        createAddAffineTransformAnnotationChange(tfc);
    ...
    private void createAddAffineTransformAnnotationChange(TextFileChange tfc) {
        int loopOffset = fLoop.position().offset();

        tfc.addEdit(new InsertEdit(loopOffset, "@Transform(" + fTransformExpr + ")"));
    }
}
