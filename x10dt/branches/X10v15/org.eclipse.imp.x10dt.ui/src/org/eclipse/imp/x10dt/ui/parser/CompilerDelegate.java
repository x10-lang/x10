...
public class CompilerDelegate {
	...
	public polyglot.frontend.ExtensionInfo getExtensionInfo() {
		return extInfo;
	}

	public X10Lexer getLexer() {
		return extInfo.getLexer();
	}

	public X10Parser getParser() {
		return extInfo.getParser();
	}

	public Job getJob(Source source) {
		return extInfo.getJob(source);
	}

	public PolyglotFrontEnd getFrontEnd() {
		return fe;
	}
	...
	CompilerDelegate(Monitor monitor, IMessageHandler handler, IProject project) {
		...
		extInfo = new org.eclipse.imp.x10dt.ui.parser.ExtensionInfo(monitor,
				new MessageHandlerAdapter(handler)); // new
														// ExtensionInfo(monitor);
	...
	private String pathListToPathString(List<IPath> pathList) {
		...
		for (Iterator<IPath> iter = pathList.iterator(); iter.hasNext();) {
			...
			if (projectRef != null && projectRef.exists()) {
				// This is a workspace-relative path, but the project may not
				// actually
				// live inside the workspace, so use its actual location as the
				// prefix
				// for the rest of the specified path.
				buff.append(projectRef.getLocation().append(
						path.removeFirstSegments(1)).toOSString());
			} else if (x10Project.getProject().exists(path)) {
				buff.append(x10Project.getProject().getLocation().append(path)
						.toOSString());
			} else {
	...
	private void buildOptions(ExtensionInfo extInfo) {
		...
		try {
			...
			opts.parseCommandLine(new String[] { "-assert", "-noserial", "-cp",
					buildClassPathSpec(), "-sourcepath", projectSrcPath },
					new HashSet());
		} catch (UsageError e) {
			if (!e.getMessage().equals("must specify at least one source file"))
				System.err.println(e.getMessage());
		} catch (JavaModelException e) {
			X10UIPlugin.getInstance().writeErrorMsg(
					"Unable to obtain resolved class path: " + e.getMessage());
		}
	...
	private String buildClassPathSpec() {
		StringBuffer buff = new StringBuffer();
		// RMF 8/2/2006 - Determine whether an X10 runtime is on the project
		// classpath,
		// and if not, silently add the default X10 runtime, so that various IDE
		// services
		...
		try {
			IClasspathEntry[] classPath = (x10Project != null) ? x10Project
					.getResolvedClasspath(true) : new IClasspathEntry[0];
			...
			// String runtimePath= commonPath.substring(0,
			// commonPath.lastIndexOf(File.separator) + 1) + "x10.runtime" +
	...
	private String getRuntimePath() {
		...
		String bundleVersion = (String) x10RuntimeBundle.getHeaders().get(
				"Bundle-Version");
		String x10RuntimePath = Platform.getInstallLocation().getURL()
				.getPath()
				+ "plugins/x10.runtime_" + bundleVersion + ".jar";
