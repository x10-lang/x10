...
import org.eclipse.imp.x10dt.ui.editor.X10LabelProvider;
...
//PORT1.7 move images to X10LabelProvider?
public class Outliner extends OutlinerBase implements IOutliner
{
	...
	if (ast != null)
	{
		...
		parent.setImage(X10LabelProvider._DESC_OBJS_PACKDECL);
        ...
        for (Iterator i = decls.iterator(); i.hasNext();)
        {
            ...
            for (Iterator it = type_set.iterator(); it.hasNext();)
            {
                ...
                if (field_list.size() > 0)
                {
                    ...
                    field_item.setImage(X10LabelProvider._DESC_ELCL_VIEW_MENU);
    ...
    /**
     * Convenience method to take a FlagsNode 
     * PORT1.7 add method with this signature for convenience
     * @param flagsNode
     * @param images
     * @param tree_item
     */
    private void setImageFromAccessQualifiers(FlagsNode flagsNode, Image[] images, TreeItem tree_item) {
    	setImageFromAccessQualifiers(flagsNode.flags(), images, tree_item);
    }
    ...
    void outlineField(TreeItem parent_item, FieldDecl_c field)
    {
        ...
        setImageFromAccessQualifiers(field.flags(), X10LabelProvider.FIELD_DESCS, tree_item);
        ...
        public NodeVisitor enter(Node parent, Node n)
        {
            if (n instanceof ClassDecl_c)
            {
                ...
                if (parent == null) // top-level type declaration
                {
                    ...
                    if (type.flags().flags().isInterface())  //PORT1.7 flags()->flags().flags()
                         tree_item.setImage(X10LabelProvider._DESC_OBJS_CFILEINT);
                    else tree_item.setImage(X10LabelProvider._DESC_OBJS_CFILECLASS);
                    tree_item.setText(type.name().id().toString());//PORT1.7 name() no longer returns a String
                }
                else // an inner class of some sort
                {
                    ...
                    if (type.flags().flags().isInterface())   //PORT1.7 flags()->flags().flags()
                    {
                    	setImageFromAccessQualifiers(type.flags(), X10LabelProvider.INNER_INTF_DESCS, tree_item); 
                    }
                    else
                    {
                    	setImageFromAccessQualifiers(type.flags(), X10LabelProvider.INNER_CLASS_DESCS, tree_item);   
                    }
                }
...
//              int right_token_index = ((JPGPosition) type.body().position()).getLeftIToken().getTokenIndex() - 1;
            ...
            else if (n instanceof New) // PORT1.7 --- ask Bob   New??note next 'else if' clause does 'New'; or can we ignore all this b/c Outliner not used?
            {
                ...
                {
                    TreeItem tree_item = new TreeItem((TreeItem) tree_item_of.get(parent), SWT.NONE);
                    tree_item.setImage(X10LabelProvider._DESC_OBJS_INNER_CLASS_DEFAULT);
                    tree_item_of.put(cons, tree_item);
                    IToken left_token = ((JPGPosition) cons.position()).getLeftIToken();
...
//                override(n);
            ...
            */  //PORT1.7 --- end ignore
            else if (n instanceof MethodDecl_c)
            {
                ...
                setImageFromAccessQualifiers(method.flags(), X10LabelProvider.MISC_DESCS, tree_item);  
                tree_item.setText(text);
            }
            ...
            else if (n instanceof AtEach_c)
            {
                ...
                tree_item.setData(pos(left_token, left_token.getPrsStream().getIToken(right_token_index)));
                tree_item.setImage(X10LabelProvider._DESC_MISC_PUBLIC);
                tree_item.setText(text);
            }
            ...
            else if (n instanceof Finish_c)
            {
                ...
                tree_item.setImage(X10LabelProvider._DESC_MISC_PRIVATE);
                tree_item.setText(text);
            }
            ...
            else if (n instanceof Next_c)
            {
                ...
                tree_item.setImage(X10LabelProvider._DESC_MISC_PROTECTED);
                tree_item.setText(text);
            }
            else if (n instanceof Async_c)
            {
                ...
                tree_item.setImage(X10LabelProvider._DESC_MISC_DEFAULT);
                tree_item.setText(text);
            }
