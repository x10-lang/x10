...
public class ExtractVarsVisitor extends NodeVisitor {
	...
	public class VarUseType {
		 ...
		 * fresh dereference and parameter maps.
		 *
		 */
		public VarUseType(){
		 ...
		 * 
		 * @param vn Variable which is having its type generated
		 * @param basetype Basic type (usually LVAL or RVAL)
		 * @param fieldtype Type for Variables which are fields
		 * @param arraytype Type for Variables which are Java arrays
		 * @param x10arraytype Type for Variables which are X10 point-indexed arrays
		 * @param x10array1type Type for Variables which are other X10 arrays 
		 * @return Variable type
		 */
		private final int getType(Variable vn, int basetype, int fieldtype, int arraytype, int x10arraytype, int x10array1type) {
			return basetype | ((vn instanceof Field)?fieldtype:0) | ((vn instanceof ArrayAccess)?arraytype:0) | ((vn instanceof X10ArrayAccess)?x10arraytype:0) | ((vn instanceof X10ArrayAccess1)?x10array1type:0);
		}
	
		/**
		 * Generates standard type for parameter Variable and also
		 * stores the parameter number in the parameter map
		 * 
		 * @param vn Variable which is having its type generated
		...
		/**
		 * Generates standard type for dereferenced Variable and also
		 * stores the dereference number in the dereference map
		 * 
		 * @param vn Variable which is having its type generated
		 * @param basetype Basic type (usually LVAL or RVAL)
		 * @param drefNum Dereference number
		 * @return Variable type
		 */
		 ...
		 * 
		 * @param vn Variable which is having its type generated
		 * @param basetype Basic type (usually LVAL)
		 * @return Variable type
		 */
		 ...
		 * 
		 * @param vn Key into parameter map
		 ...
		 * 
		 * @param vn Key into parameter map
		 * @param paramNum New parameter number
		 */
	...
	public NodeVisitor enter(Node par, Node n) {
		...
		if (n instanceof Formal){
			...
			Local_c nlocal = new Local_c(nformal.position(),nformal.id());
			nlocal.localInstance(nformal.localInstance());
			evCol.add(nlocal);
			evTypeMap.put(nlocal, VarUseType.LVAL | VarUseType.DECL | ((par instanceof X10Loop)?VarUseType.LOOP_VAR:0));
		}
		if (n instanceof LocalDecl) {
			LocalDecl ndecl = (LocalDecl) n;
			if (ndecl.type().type().isReference()){
				Position npos = n.position();
				Position p = new Position(npos.path(),npos.file(),npos.line(),npos.column(),npos.endLine(),npos.column()+ndecl.id().id().length()-1);
				Local_c nlocal = new Local_c(npos,ndecl.id());
		...
		if (n instanceof Variable) {
			...
			if (vn.type().isReference()){
				...
				if (par instanceof ArrayAccess) {
					...
					parType = ((parType & VarUseType.PARAM) != 0)?parType^VarUseType.PARAM|VarUseType.RVAL:parType;
					...
					if (vn.equals(apar.array()))
						if (((parType & VarUseType.DREF) != 0) && ((parType & VarUseType.LVAL) != 0))
							evTypeMap.put(vn, vutype.getStandardType(vn, parType ^ VarUseType.ARRAY ^ VarUseType.LVAL | VarUseType.RVAL));
						else
...
//						evTypeMap.put(vn, VarUseType.RIGHT | ((vn instanceof Field)?VarUseType.GETFIELD:0)); }
					...
					if ((parType & VarUseType.PUTFIELD) != 0) {
						...
						evTypeMap.put(vn, vutype.getStandardDrefType(vn, VarUseType.RVAL, 1));//(parType ^ VarUseType.GETFIELD)));
					}
	...
	/**
	 * Fixes derefence information for Variables by counting from
	 * the base dereferenced Variable up to the ultimate Variable.
	 */
	@Override
	...
	public HashMap<RefactoringPosition, Variable> getVarMap() {
		if (evMap == null) {
			...
			for (Variable v : evCol) {
				RefactoringPosition rp = new RefactoringPosition(v
						.position());
				rp.setCheckMode();
				hm.put(rp, v);
			}
