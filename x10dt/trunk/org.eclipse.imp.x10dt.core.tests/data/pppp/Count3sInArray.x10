...
public class Count3sInArray {
     ...
     */  
	 ...
	 this.sizePerAsync = size/numAsyncs;
     }
    ...
    public def run() {
	  ...
	  count = 0;
	  if (numAsyncs==1) // sequential
	      count3s(0);
	  else 
	      finish {
		  for((i) in 0..numAsyncs-2) 
		      async
			  count3s(i);
		  count3s(numAsyncs-1);
	  }
	...
	if (args.length < 2) 
	    Console.OUT.println("Usage: Count3sInArray <Size; should be multiple of 4> <NumAsyncs; >=1 >");
