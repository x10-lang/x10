...
public class SocketTransport {
	...
	public static enum PROBE_TYPE {ACCEPT, ACCEPTORWRITE, ALL};
	...
	public static enum MSGTYPE {STANDARD(0), PUT(1), GET(2), GET_COMPLETED(3), GET_PLACE_REQUEST(6), GET_PLACE_RESPONSE(7), CONNECT_DATASTORE(8);
	    private int value;
	    private MSGTYPE(int value) {this.value = value;}
	    public int getValue(){return this.value;}
	}; // note that GET_PLACE_REQUEST does not overlap with CTRL_MSG_TYPE
	public static enum CALLBACKID {closureMessageID, simpleAsyncMessageID, uncountedPutID};
	...
	private byte[] dataStoreLocation = null;
	private final NetworkTransportCallbacks runtimeLink; // API to notify the runtime of places added, etc.  Can be null
	
	public SocketTransport(NetworkTransportCallbacks runtimeLink) {
		...
		if (nplaces > 1) {
			for (int i=0; i<myPlaceId; i++) {
				try {
					...
					String re = System.getenv("X10_RESILIENT_MODE");
					String rs = System.getProperty("X10_RESILIENT_MODE", "0");
					if ("0".equals(rs) && (re == null || "0".equals(re)))
						e.printStackTrace();
...
*/	
    ...
    public OutputStream getOutputStream(int place) throws IOException {
    	CommunicationLink link = channels.get(place);
    	if (link == null || link.sc == null) 
    		throw new IOException("Invalid link to place "+place);
    	return link.sc.socket().getOutputStream();
    }
    
	public void setDataStoreLocation(byte[] dataStoreLocation) {
	...
	private void markPlaceDead(int place) {
    	...
    	if (runtimeLink != null) runtimeLink.runPlaceRemovedHandler(place);
    }
        
    // request to start more places.  Forward to the launcher, if possible
    public long addPlaces(long newPlaces) {
	...
	public Message x10rt_probe(PROBE_TYPE probeType, boolean blocking) {
/* a single place may have other places join it later
...
*/    	
    	...
    	try {
			...
			if (key.isAcceptable()) {
				...
				if (sc == null)
					return null; // nothing actually here
				int remote = -1;
				...
				if (CTRL_MSG_TYPE.HELLO.ordinal() == msgtype) {
					...
					if (to == myPlaceId) {
				    	...
				    	if (runtimeLink != null) runtimeLink.runPlaceAddedHandler(remote);
					}
					else if (to == -1) {
						remote = controlMsg.getInt();
						if (remote == -1) {
							...
							if (myPlaceId == lowestValidPlaceId) {
								...
								if (runtimeLink != null) 
									controlMsg.putLong(runtimeLink.getEpoch());
								else
									controlMsg.putLong(-1);
				...
				return new Message(-1, 0); // blank message
			}
			if (probeType == PROBE_TYPE.ACCEPT) return null;
			if (key.isWritable()) {
			...
			if (probeType == PROBE_TYPE.ACCEPTORWRITE) return null;
			if (key.isReadable()) {
						...
						if (m != null) { // there is an existing message partially read in from this socket
							...
							if (msgType == MSGTYPE.STANDARD.getValue()) {
								...
								if (runtimeLink != null) {
									...
									runtimeLink.initDataStore(linkString);
								}
							}
							else
		...
		if (remotePlace == myPlaceId && myPlaceId != -1) { // send connection details to launcher
			...
			if (controlMsg.getInt() == CTRL_MSG_TYPE.HELLO.ordinal()) {
				...
				if (this.myPlaceId == -1 && toPlace > -1) {
					...
					epochBuffer.flip();
					if (runtimeLink != null) {
						runtimeLink.setEpoch(epochBuffer.getLong());
						if (DEBUG) System.err.println("Set epoch to "+runtimeLink.getEpoch());
					}
					...
					setSocketOptions(sc);
					registerOnSelector(sc, SelectionKey.OP_READ, null);
					if (runtimeLink != null) runtimeLink.runPlaceAddedHandler(remotePlace);
					if (DEBUG) System.err.println("Place "+this.myPlaceId+" established a link to place "+remotePlace+" of "+this.nplaces+" places at "+connectionInfo);
				}
			}
			else
