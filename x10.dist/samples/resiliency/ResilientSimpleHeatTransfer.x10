 ...
 * Uses simple ResilientDistArray
 * @author kawatiya
 * 
 * For Managed X10:
 *   $ x10 ResilientSimpleHeatTransfer.x10
 *   $ X10_RESILIENT_PLACE_ZERO=1 X10_NPLACES=4 run.sh x10 ResilientSimpleHeatTransfer [size]
 * For Native X10:
 *   $ x10c++ ResilientSimpleHeatTransfer.x10 -o ResilientSimpleHeatTransfer
 *   $ X10_RESILIENT_PLACE_ZERO=1 X10_NPLACES=4 run.sh runx10 ResilientSimpleHeatTransfer [size]
 */
public class ResilientSimpleHeatTransfer {
        ...
        for (i in 1..ITERATIONS) {
            ...
            try {
                ...
                if (restore_needed()) {
                    ...
                    Console.OUT.println("Create new PlaceGroup over available " + (Place.MAX_PLACES-Place.numDead()) + " places");
    ...
    private static def stencil_1(A:ResilientDistArray_BlockBlock_2[Double], [x,y]:Point(2)): Double {
    ...
    //     }
    // }
    private static def prettyPrint(A:ResilientDistArray_BlockBlock_2[Double], x:Long, y:Long) {
        for (i in 0..(x-1)) {
            for (j in 0..(y-1)) {
    ...
    private static def printDist(A:ResilientDistArray_BlockBlock_2[Double], x:Long, y:Long) {
        for (i in 0..(x-1)) {
            for (j in 0..(y-1)) {
