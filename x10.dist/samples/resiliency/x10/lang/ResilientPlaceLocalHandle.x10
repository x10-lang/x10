...
/**
 * ResilientPlaceLocalHandle which supports snapshot and restore
 * Currently limited methods necessary for ResilientHeatTransfer are implemented
 * @author kawatiya
 */
public class ResilientPlaceLocalHandle[T]{T isref, T haszero} { // struct cannot have var field
    private var pg:PlaceGroup;
    private var plh:PlaceLocalHandle[T];
    private def this(pg:PlaceGroup, plh:PlaceLocalHandle[T]) {
        this.pg = pg;
        this.plh = plh;
    }
    public static def make[T](pg:PlaceGroup, init:()=>T){T isref, T haszero}
        = new ResilientPlaceLocalHandle[T](pg, PlaceLocalHandle.make[T](pg, init));
     ...
     */
    /**
     * Remake the ResilientPlaceLocalHandle over a new PlaceGroup using specified initializer
     */
    public def remake(newPg:PlaceGroup, init:()=>T) {
        if (pg.size()!=newPg.size()) throw new Exception("PlaceGroup size should be same");
        this.pg = newPg;
        this.plh = PlaceLocalHandle.make[T](newPg, init);
    }
    
    /*
     * Note that snapshot/restore should be executed for the master copy of PlaceLocalHandle
     */
    private val snapshots = new Rail[ResilientStoreForDistArray[Long,T]](2, (Long)=>ResilientStoreForDistArray.make[Long,T]());
    ...
    public def restore(newPg:PlaceGroup) {
        ...
        this.pg = newPg;
        this.plh = PlaceLocalHandle.make[T](newPg, ()=>{
     ...
     *         x10 x10.lang.ResilientPlaceLocalHandle
     */
    public static def main(ars:Rail[String]) {
        ...
        val A = ResilientPlaceLocalHandle.make[Cell[Long]](pg, ()=>new Cell(pg.indexOf(here)));
        A.snapshot();
