 ...
 * @author kawatiya
 */
public class DistArray_BlockBlock_2[T] implements (Long,Long)=>T {
    public property rank() = 2;
    private var da:x10.array.DistArray_BlockBlock_2[T];
    public def this(m:Long, n:Long, pg:PlaceGroup{self!=null}, init:(Long,Long)=>T) {
        this.da = new x10.array.DistArray_BlockBlock_2[T](m, n, pg, init);
        this.savedPg = null;
    }
    ...
    public final def map[U](dst:DistArray_BlockBlock_2[U], op:(T)=>U):DistArray_BlockBlock_2[U]{self==dst} {
        da.map[U](dst.da, op); return dst;
    }
    public final def map[S,U](src2:DistArray_BlockBlock_2[S], dst:DistArray_BlockBlock_2[U], op:(T,S)=>U):DistArray_BlockBlock_2[U]{self==dst} {
     ...
     */
    /**
     * Remake the Resilient DistArray over a new PlaceGroup using specified initializer
     */
    public def remake(pg:PlaceGroup{self!=null}, init:(Long,Long)=>T) {
        val globalIndices = da.globalIndices();
        val m = globalIndices.max(0) - globalIndices.min(0) + 1; // = da.numElems_1;
        val n = globalIndices.max(1) - globalIndices.min(1) + 1; // = da.numElems_2;
        this.da = new x10.array.DistArray_BlockBlock_2[T](m, n, pg, init);
        this.savedPg = null;
    }
    /**
     * Remake the Resilient DistArray over a new PlaceGroup with zero
     */
    public def remake(pg:PlaceGroup{self!=null}){T haszero} {
        val globalIndices = da.globalIndices();
        val m = globalIndices.max(0) - globalIndices.min(0) + 1; // = da.numElems_1;
        val n = globalIndices.max(1) - globalIndices.min(1) + 1; // = da.numElems_2;
        this.da = new x10.array.DistArray_BlockBlock_2[T](m, n, pg);
        this.savedPg = null;
    }
    ...
    private val snapshots = new Rail[ResilientStoreForApp[Place,LocalInfo[T]]](2, (Long)=>ResilientStoreForApp.make[Place,LocalInfo[T]]());
    ...
    public def restore(newPg:PlaceGroup{self!=null}) {
        ...
        val init = (i:Long,j:Long)=>{
            ...
            //     return 0 as T;
            // }
            // code from x10.array.DistArray_BlockBlock_2.place(i,j)
             ...
             */
            // code from x10.array.DistArray_BlockBlock_2.offset(i,j)
        ...
        val m = globalIndices.max(0) - globalIndices.min(0) + 1; // = da.numElems_1;
        val n = globalIndices.max(1) - globalIndices.min(1) + 1; // = da.numElems_2;
        this.da = new x10.array.DistArray_BlockBlock_2[T](m, n, newPg, init);
     ...
     *         x10 x10.resilient.array.DistArray_BlockBlock_2
     */
    public static def main(ars:Rail[String]) {
        ...
        val A = new DistArray_BlockBlock_2[Long](10, 1, pg, (x:Long,y:Long)=>x);
        A.snapshot();
