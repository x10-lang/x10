...
public class ResilientPlhHeatTransfer {
    ...
    public static def main(args:Rail[String]) {
        ...
        val A = ResilientPlaceLocalHandle.make[PartialArray[Double]](pg, ()=>{
            val i = usePlaces.indexOf(here); // get my index
            return new PartialArray[Double](part(i)-1, part(i+1), n+2, (x:Long, y:Long)=>( (x==0) ? 1.0 : 0.0 ));
        });
        val B = ResilientPlaceLocalHandle.make[PartialArray[Double]](pg, ()=>{ // temporary array not to be backed up
            val i = usePlaces.indexOf(here); // get my index
            return new PartialArray[Double](part(i)-1, part(i+1), n+2, (x:Long, y:Long)=>( (x==0) ? 1.0 : 0.0 ));
        });
...
//              }; /* end of collective finish */
     ...
     */
    private static def prettyPrint(A:ResilientPlaceLocalHandle[PartialArray[Double]], usePlaces:ArrayList[Place]) {
