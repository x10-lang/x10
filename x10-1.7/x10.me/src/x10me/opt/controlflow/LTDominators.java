...
public class LTDominators extends Stack<BasicBlock> {
  ...
  private void step1() {
    ...
    DFSCounter = 0;
    if (DEBUG) { ir.dumpFile.current.println("Initializing blocks:"); }
  ...
  protected void DFS(BasicBlock block) {
    ...
    while (!empty()) {
      ...
      block = peek();
      if (DEBUG) { ir.dumpFile.current.println(" Processing (peek)" + block); }

      if (block == null) {
        if (DEBUG) { ir.dumpFile.current.println(" Popping"); }
        pop();   // return
        continue;
      }
      ...
      if (e == null) {
        if (DEBUG) { ir.dumpFile.current.println(" Initial processing of " + block); }
        ...
        if (DEBUG) { ir.dumpFile.current.println(" Resuming processing of " + block); }
      }

      while (e.hasMoreElements()) {
        BasicBlock next = e.next();

        if (DEBUG) { ir.dumpFile.current.println("    Inspecting next node: " + next); }
        ...
        if (getSemi(next) == 0) {
          ...
          if (DEBUG) { ir.dumpFile.current.println(" Pushing" + next); }
          push(next);
          continue recurse;
        }
      }           // while more nexts
   ...
   */
  private void step2() {
    if (DEBUG) { ir.dumpFile.current.println(" ******* Beginning STEP 2 *******\n"); }
    ...
    for (int i = DFSCounter; i > 1; i--) {
      ...
      if (DEBUG) { ir.dumpFile.current.println(" Processing: " + block + "\n"); }

      // visit each predecessor
      BasicBlockEnumeration e = getPrevNodes(block);
      while (e.hasMoreElements()) {
        BasicBlock prev = e.next();
        if (DEBUG) { ir.dumpFile.current.println("    Inspecting prev: " + prev); }
