...
public class ValueClass6 extends x10Test {
	...
	public def run(): boolean = {
		...
		val X2: Array[complex] = Array.make[complex](D, (p(i): Point): complex => {
			x10.io.Console.OUT.println("The currentplace for X2" + i  + " is " + ( here));
			return new complex(i,i);
		});
		val Y2: Array[complex] = Array.make[complex](D, (p(i): Point): complex => {
			x10.io.Console.OUT.println("The currentplace for Y2" + i  + " is " + ( here));
			return new complex(i,i);
		});
		...
		val X3: Array[foo] = Array.make[foo](D, (p(i): Point): foo => {
			x10.io.Console.OUT.println("The currentplace for X3" + i  + " is " + ( here));
			return new foo();
		});
		val Y3: Array[foo] = Array.make[foo](D, (p(i): Point): foo => {
			x10.io.Console.OUT.println("The currentplace for Y3" + i  + " is " + ( here));
			return new foo();
		});
