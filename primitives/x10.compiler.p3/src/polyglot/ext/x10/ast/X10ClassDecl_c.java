...
public class X10ClassDecl_c extends ClassDecl_c implements X10ClassDecl {
	...
	protected void setSuperClass(TypeSystem ts, ClassDef thisType) throws SemanticException {
		...
		if (thisType.fullName().equals(QName.make("x10.lang.Object"))) {
			...
			if (superClass != null)
				throw new SemanticException("x10.lang.Object cannot have a superclas.", position());
		}
		else if (flags().flags().isInterface()) {
			thisType.superType(null);
		}
		else if (superClass == null && X10Flags.isPrimitive(flags().flags())) {
			thisType.superType(null);
		}
	...
	public Node conformanceCheck(ContextVisitor tc) throws SemanticException {
		...
		if (X10Flags.isPrimitive(flags.flags())) {
			if (superClass != null) {
				throw new SemanticException("Value class " + type + " cannot have a superclass.", position());
			}
			if (! interfaces.isEmpty()) {
				throw new SemanticException("Value class " + type + " cannot implement an interface.", position());
			}
		}
		else {
		...
		if (X10Flags.toX10Flags(flags.flags()).isPrimitive()) {
			checkForPrimitiveCycles((X10ClassDef) type, (X10ClassDef) type, null);
		}
	...
	private void checkForPrimitiveCycles(X10ClassDef cd0, X10ClassDef cd, QName path) throws SemanticException {
		if (cd0 == cd && path != null)
			throw new SemanticException("Value class " + cd0 + " cannot transitively contain a non-static field of its own type; path to field is " + path + ".", cd0.position());
		for (FieldDef fd : cd.fields()) {
			...
			if (t instanceof X10ClassType) {
				X10ClassDef cd1 = ((X10ClassType) t).x10Def();
				if (X10Flags.isPrimitive(cd1.flags())) {
					checkForPrimitiveCycles(cd0, cd1, QName.make(path, fd.name()));
				}
			}
		}		
	}
