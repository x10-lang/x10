...
private:
	TFrame(const TFrame& f) 
	      : Frame(f), u(f.u), k(f.k), ownerClosure(f.ownerClosure) {} // Barrier needed as k is volatile TODO RAJ
public:
	int u; // vertex
	volatile int k; // TODO Raj -- may need MEM_BARRIER
	Closure *ownerClosure;
		
	TFrame(int u1, Closure *cl) : u(u1), ownerClosure(cl){}
...
public:
	...
	static void traverse(Worker *w, int u, Closure *cl) /*throws StealAbort*/ {
		TFrame *frame = new TFrame(u, cl);
		frame->k=1;
		V_ARR *G = graph->G;
