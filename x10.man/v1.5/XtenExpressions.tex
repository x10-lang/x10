...
evaluates \xcd"x" to a location, then evaluates \xcd"e",
...
(\Sref{Promotion}) is performed on the operands before the
comparison.
...
\begin{grammar}
RegionExpression \: Expression \xcd".." Expression \\
RegionExpression \: Expression \xcd"*" Expression \\
\end{grammar}
...
Thus, \xcd"(1..2) * (3..4)"
is the region consistent of the points $\{$\xcd"(1,3)",
\xcd"(1,4)", \xcd"(2,3)", \xcd"(2,4)"$\}$.
