...
distribution \xcd"d" and point \xcd"p" (\S~\ref{XtenPlaces}).  
...
the same place as \xcd"a.distribution[i]". The programmer must be 
...
by the place expression \xcd"a.distribution[i]".
...
\xcd"finish S" interacts with clocks (\S~\ref{XtenClocks}). 
...
public class Latch implements future {
  ...
  protected var result: nullable[T] = null;
  protected var z: nullable[Exception] = null;

  public atomic def setValue(val: nullable[Object]): boolean {
    return setValue(val, null);
  }
  public atomic def setValue(z: nullable[Exception]): boolean {
    return setValue(null, z);
  }
  public atomic def setValue(val: nullable[Object],
                             z: nullable[Exception]): boolean {
...
\xcd"region".  Expressions \xcd"e" of type \xcd"distribution" and
...
Here the expression is intended to be of type \xcd"distribution".
Expressions \xcd"e" of type \xcd"array" are also accepted, and treated
as if they were \xcd"e.distribution". The compiler throws a type error
