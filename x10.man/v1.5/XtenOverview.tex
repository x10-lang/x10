...
(\Sref{XtenInterfaces}) and \emph{classes}
(\Sref{XtenClasses}).

An \Xten{} class (\Sref{XtenClasses}) has fields, methods and
...
activitiy \emph{asynchronously} (\Sref{AsynchronousActivity}). This
...
(\Sref{XtenDistributions}) to a base type (which may itself be an
array type).

A distribution is a map from a \emph{region} (\Sref{XtenRegions}) to a
