...
Programmers write \Xten{} code by writing \emph{generic interfaces}
(\S~\ref{XtenInterfaces}) and \emph{generic classes}
(\S~\ref{XtenClasses}). Generic interfaces and classes may be
defined over a collection of \emph{type parameters}. Instances can be
...
There are two kinds of scalar classes: \emph{reference} classes
...
\Xten{} has a \xcd"nullable" type constructor which can be applied
uniformly to scalar (value or reference) and array types. This type
constructor returns a new type which adds a special value \xcd"null"
