...
instances of anonymous classes that implement an interface
corresponding to the appropriate instantiation of the
type constructor \xcd"=>"
as described in \Sref{FunctionTypes}.
...
\eat{
\subsection{Closures in variable initializers}

XXX conflicts with exploded syntax

\begin{xten}
val p(x: Int): Point = ...;   # exploded point
val f(x: Int): Point = ...;   # closure
\end{xten}

\begin{grammar}
VariableDeclarator \:
        Identifier TypeParameters\opt \xcd"(" Formals\opt \xcd")"
                \xcd":" Type Throws\opt \xcd"=" ClosureBody
        \\
\end{grammar}
...
subsequent assignment to \xcd"x", the value may not refer to \xcd"x".
}
