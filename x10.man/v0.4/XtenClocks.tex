...
%%
%%Each activity is created with reference to zero or more ``old'' clocks 
%%(clocks that exist when the activity is created) with which the creating
%%activity is registered. Since clocks cannot be stored in objects, 
%%the only old clocks an activity
%%an activity cannot acquire a reference to any old clock to whi
%%
%%Because of the condition above, an activity cannot
...
462 Statement ::= ClockedStatement
472 StatementNoShortIf ::= ClockedStatementNoShortIf
480 ClockedStatement ::= clocked ( ClockListopt ) Statement
490 ClockedStatementNoShortIf ::= clocked ( ClockListopt ) StatementNoShortIf
503 ClockList ::= Clock
...
\todo{Shift this to a qualifier on activities, e.g. 
{\tt clocked(C1,...,Cn) async(P) {S}}
and
{\tt clocked(C1,...,Cn) ateach( Formal : Domain) S}
 and
{\tt clocked(C1,...,Cn) foreach( Formal : Domain) S}
}

\todo{Make sure that {\tt clocked() S} works correctly.}
...
\todo{change this to c.registered().}
