...
aliases,
ownership, arrays and clocks. We have implemented the type
system
...
\begin{tabular}{rrcl}
  (Constraint)   &\xcd{c} &::=& \xcd{r.contains(r)} \\
  (Region) &\xcd{r} &::=& \xcd{t} \bnf [${\tt b}_1$:${\tt d}_1$,\ldots,${\tt b}_k$:${\tt d}_k$] \bnf  \\
           &        &   & \xcd{r || r} \bnf \xcd{r && r} \bnf
           \xcd{r - r} \bnf \xcd{r + p} \bnf \xcd{r - p} \\
...
over-relaxation~\cite{sor} of an $n \times n$ matrix \xcd{G}.
The type-checker establishes that the \xcd{region}
property of the point \xcd{ij} (line 17) is
\xcd{inner && [i:i,d1min:d1max]}, and that this
region is a subset of \xcd{outer}, the region of the array \xcd{G}.
...
\end{displayxten}
The constraint on \xcd{left} states that if
the \xcd{localLeft} property is true, then the location 
of the \xcd{left} child must be \xcd{here} and its
\xcd{localLeft} property must be set.
...
Figure~\ref{fig:ownership} shows a fragment of a \xcd{List} class, 
...
Each \xcd{Owned} object has an \xcd{owner} property.
...
in the \xcd{head} field of each element.
The \xcd{tail} of the list is owned by the list object itself.
...
The \xcd{tail()} method for instance, incorrecty leaks the
