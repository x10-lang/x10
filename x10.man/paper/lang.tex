...
The precondition
says that
\xcd{tail} must be passed a non-empty list.  The method returns a list
...
in the body of an interface may have method constraints
as well.
...
class List(int(:self >= 0) n) {
  Object head;
  List(:self!=null & self.n==this.n-1) tail;
  ...
}
\end{xten}
