...
(Constraint) & \xcd{c},\xcd{d} &{::=}& \xcd{true} \alt \xcd{t == t} \alt \xcd{c 
&& c}\alt \xcd{T x; c} \\\\\end{tabular}
\noindent
...
\begin{displayxten}
int(:self >= 0) product(int(:self >= 0) a, int(:self >= 0) b) {
    return a*b;
}
\end{displayxten}
...
constraint on the return type.  If the new constraint cannot be
represented by any installed constraint system, rather than
report an error, the constraint is simply removed---in this
case, the type of \xcd{a*b} becomes \xcd{int}.
