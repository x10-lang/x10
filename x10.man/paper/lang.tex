...
class Region(rank: int) {
  def contains(p: Point{rank==this.rank}):
      boolean { ... }
  ...
}
...
\caption{Sketch of \Xten{} support for regions, distributions, and arrays}\label{fig:region}
\end{figure}
...
\xcd{m$_1$} in a class \xcd{C} overrides a method \xcd{m$_2$} in a
superclass \xcd{D} if \xcd{m$_1$} and \xcd{m$_2$} have signatures
...
is type-checked against \xcd{m$_2$} at the superclass type \xcd{D},
the precondition is satisfied if the method is dispatched to the
method \xcd{m$_1$} in the subclass \xcd{C}.
