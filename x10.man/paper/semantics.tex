...
substitution $\theta=[\bar{t}/\bar{x}]$ to a constraint $c$ by
$c[\bar{t}/\bar{x}]$. 

\begin{tabular}{rrcl}
&&&\\
(C Terms) & t&{::=}& {\tt  x}\alt \self \alt \this \alt {\tt t.f} \alt \new\ {\tt C($\bar{\tt t}) \alt {\tt g}(\bar{\tt t})$}\\
(Constraint) & c,d&{::=}&$\true\alt {\tt p}(\bar{\tt t})\alt {\tt t=t}\alt {\tt c,c}\alt{\tt  T\,x;c}$\\
&&&\\
\end{tabular}
...
The definitions are based on those in \cite{FJ}. 
...
\paragraph{\CFJ{} subtyping judgement}\label{CFJ-subtyping}
...
\paragraph{\CFJ{} typing judgement}\label{CFJ-typing}
...
used in the syntax of a type.}  and constraints. Typing judgements are
...
essentially as specified in \cite{FJ}, with the difference that the
