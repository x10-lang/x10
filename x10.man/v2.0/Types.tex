...
%~~type~~`~~`~~ ~~
\xcd`(b:Rail[String]{b.length==a}, a:Int) => Boolean`
and 
%~~type~~`~~`~~ ~~
\xcd`(a:Rail[String]{a.length==b}, b:Int) => Boolean`
are equivalent types.

The formal parameter names are in scope throughout the function
signature---they may be used in the types of other formal parameters
and in the return type.  In particular, the common pattern that variables must
be
declared before they are used does not apply within a type, as the previous
example shows. 
