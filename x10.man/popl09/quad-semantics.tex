...
\infrule[And]
	{\Gamma  \vdash {\tt c}: {\tt o} \andalso \Gamma \vdash {\tt d}: {\tt o}}
...
\infax[Object]{\Gamma \vdash {\tt Object}: {\tt type}}
...
\infrule[Exists]
	{\Gamma \vdash {\tt t}: {\tt V} \andalso \Gamma \vdash {\tt U}[{\tt t}/{\tt x}]: \type}
	{\Gamma \vdash \exists {\tt x}:{\tt V}.~{\tt U}: \type} 
...
\infax[Fields-B]
{{\tt x}:{\tt Object} \vdash \fields({\tt x})=\bullet}

\infrule[Fields-I]
{\Gamma, {\tt x}:{\tt D} \vdash \fields({\tt x})=\tbar{g}: \tbar{V}
 \\
 {\tt class}~{\tt C}(\tbar{f}:\tbar{U})\{{\tt c}\}~{\tt extends}~{\tt D}~\{~\tbar{M}~\} \in {\tt P}}
{\Gamma, {\tt x}:{\tt C} \vdash \fields({\tt x})=\tbar{g}: \tbar{V}, \tbar{f}: \tbar{U}[{\tt x}/{\tt this}]}

\infrule[Fields-C,E]
{\Gamma, {\tt x}: {\tt S} \vdash \fields({\tt x})=\tbar{f}: \tbar{V}}
{\Gamma, {\tt x}: {\tt S}\{{\tt d}\} \vdash \fields({\tt x}) = \tbar{f}:
                                  \tbar{V}\{{\tt d}[{\tt x}/{\tt self}]\}
\\
\Gamma, {\tt x}: \exists {\tt y}: {\tt U}.~{\tt S} \vdash
\fields({\tt x})=\tbar{f}:\exists {\tt y}:{\tt U}.~\tbar{V}
	}

\infrule[Method-B]
{{\tt class}~{\tt C}(\tbar{f}:\tbar{V})\{{\tt c}\}~{\tt extends}~{\tt D}~\{~\tbar{M}~\} \in {\tt P}
 \\
 {\tt def}~{\tt m}(\tbar{z}:\tbar{U})\{{\tt d}\}:{\tt T} = {\tt e} \in \tbar{M} \andalso
 \theta=[{\tt x}/\this]}
{\Gamma, {\tt x}:{\tt C} \vdash {\tt x}~\has~({\tt m}(\tbar{z}:\tbar{U}\theta)\{{\tt d}\theta\}:{\tt T}\theta={\tt e})}

\infrule[Method-I]
{\Gamma, {\tt x}:{\tt D} \vdash {\tt x}~\has~({\tt m}(\tbar{z}:\tbar{U})\{{\tt d}\}:{\tt T}={\tt e})
\\
 {\tt class}~{\tt C}(\tbar{f}:\tbar{V})\{{\tt c}\}~{\tt extends}~{\tt D}~\{~\tbar{M}~\} \in {\tt P}
 \andalso
 {\tt m} \not\in \tbar{M}}
{\Gamma, {\tt x}:{\tt C} \vdash {\tt x}~\has~({\tt m}(\tbar{z}:\tbar{U})\{{\tt d}\}:{\tt T}={\tt e})}

\infrule[Method-C,E]
{\Gamma, {\tt x}:{\tt S} \vdash {\tt x}~\has~({\tt m}(\tbar{z}:\tbar{U})\{{\tt c}\}:{\tt T}={\tt e})}
{\Gamma, {\tt x}: {\tt S}\{{\tt d}\} \vdashO 
                                 {\tt x}~\has~({\tt
                                 m}(\tbar{z}:\tbar{U})\{{\tt
                                 c}\}:{\tt T\{d[{\tt x}/{\tt self}]\}}={\tt e})
\\
\Gamma, {\tt x}: \exists {\tt y}: {\tt U}.~{\tt S} \vdash
                                 {\tt x}~\has~({\tt
                                 m}(\tbar{z}:\tbar{U})\{{\tt
                                 c}\}:{\tt \exists {\tt y}: {\tt U}.~{\tt T}}={\tt e})}
...
[XXX define!]
%% Well, it isn't really since its assumptions are ordered??!
...
\infrule[Class]
{{\tt class}~{\tt C}(\tbar{f}:\tbar{V})\{{\tt c}\}~{\tt extends}~{\tt D}~\{~\tbar{M}~\} \in {\tt P}}
{\tt \vdashO \klass({\tt C}),{\tt C} \extends {\tt D}}

\infrule[Has-F]
{\Gamma \vdashO \fields({\tt x})=\tbar{f}:\tbar{V}} 
{\Gamma \vdashO {\tt x}\ \has\ {\tt f}_i:{\tt V}_i}

\infax[Sel]{\vdashO {\tt new}~{\tt C}(\tbar{t}).{\tt f}_i={\tt t}_i}

\infrule[Inv]
{\Gamma \vdashO {\tt x:C}
\andalso
{\tt class}~{\tt C}(\tbar{f}:\tbar{V})\{{\tt c}\}~{\tt extends}~{\tt D}~\{~\tbar{M}~\} \in {\tt P}}
{\Gamma \vdashO {\tt c}[{\tt x}/{\tt this}]}

\infax[V-Id]
{\vdashO {\tt C} \extends {\tt C}} 

\infrule[V-Trans]
{\Gamma\tt \vdashO T_1 \extends T_2, T_2 \extends T_3}
{\Gamma\tt \vdashO T_1 \extends T_3}

\infrule[Sub-X]
{\Gamma \vdashO {\tt t} \extends {\tt T}}
{\Gamma \vdashO {\tt t} \subtype {\tt T}}
...
\infrule[S-Const-R]
	{\Gamma\tt  \vdashO {\tt S} \subtype {\tt
        T}\andalso\Gamma,{\tt \self:S} \vdashO {\tt c} }
...
{\Gamma \vdashO {\tt S} \subtype {\tt T}
 \andalso
 \mbox{({\tt x} fresh})}
{\Gamma \vdashO \exists {\tt x}: {\tt U}.~{\tt S} \subtype {\tt T}}

\infrule[S-Exists-R]
{\Gamma \vdash {\tt t}:{\tt U}
 \andalso
 \Gamma \vdashO {\tt S} \subtype {\tt T}[{\tt t}/{\tt x}]}
{\Gamma  \vdashO {\tt S}\subtype \exists {\tt x}: {\tt U}.~{\tt T}}

\eat{
\infrule[Type-equiv]
{\Gamma  \vdashO {\tt S} \subtype {\tt T}
 \andalso
 \Gamma \vdashO {\tt T} \subtype {\tt S}}
{\Gamma  \vdashO {\tt S} \equiv {\tt T}}
}
...
Once fields of type \type{} are permitted, it is natural to have
...
\infax[Equals]{{\tt S==T} \vdashO S \extends T, T \extends S}

\infrule[Inh-p]
	{\Gamma \vdashO {\tt p} \extends {\tt T} \andalso
        \Gamma, {\tt x}:{\tt T} \vdashO {\tt x}\ \has\ {\tt I}}
	{\Gamma,{\tt x}:{\tt p} \vdashO {\tt x}\ \has\ {\tt I}}

\infrule[Inh-X]
	{\Gamma \vdashO {\tt X} \extends {\tt T} \andalso
        \Gamma, {\tt x}:{\tt T} \vdashO {\tt x}\ \has\ {\tt I}}
	{\Gamma,{\tt x}:{\tt X} \vdashO {\tt x}\ \has\ {\tt I}}
