...
\infrule[X-New]
  {{\tt class}~{\tt C}(\tbar{f}:\tbar{T})\{{\tt c}\}~{\tt extends}~{\tt D}~\{~\tbar{M}~\}\in {\tt P}
   ...
   \andalso
   \Gamma\vdash\exists{\tt x}:{\tt C}\{{\tt d}\}.~{\tt e}}
  {\Gamma\vdash\exists \tbar{y}:\tbar{U}.~\tbar{U}\subtype\tbar{V},{\tt x}==\new~{\tt C}(\tbar{y}),{\tt c}[{\tt x}/\this],{\tt d}[{\tt x}/\self],{\tt e}}
\end{minipage}%
...
\infrule[T-Class]
  {\this:{\tt C}\vdash\tbar{T}~\type \andalso \this:{\tt C},\tbar{f}:\tbar{T}\vdash{\tt c}~\constraint \andalso \wj{}{\tt D} \andalso \tbar{M}~{\rm OK~in}~{\tt C}}
...
\quad\typicallabel{XXXXX}
\infrule[S-Const-L]
	{\wj{\Gamma}{{\tt T}\{{\tt c}\}}, {\tt T}\subtype {\tt U}}
