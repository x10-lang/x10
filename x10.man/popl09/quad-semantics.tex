...
We now describe the semantics of languages in the \FX{} family. We start with a core \FXZ{} language that support simple \FJ-like types, then add value-dependent types, type-dependent types, and both. For uniformity we declare type-valued parameters and properties to be of ``type'' \type, instead of using square brackets to demarcate them.
...
list $x_1, \dots, x_n$ and $\bullet$ to denote the empty
list.
...
nor method invocations are permitted.
...
dynamic semantics of \FXZ{} (see, e.g., the rule \TField{} in Figure~\ref{fig:FX}).
...
We enforce a call-by-value-like semantics in the sense
that rules \RField, \RInvk, and \RCast{} requires receivers of the form
``$\new~{\tt C}(\tbar{t})$'' rather than the more general ``$\new~{\tt C}(\tbar{e})$''.
In other words, we consider expressions involving casts or method calls to be incomparable.}
%Constraint terms have the property that they can be reconstructed from their types.
...
Each language in the family is defined over a given input constraint system $\mathcal{X}$ that is required to to support the trivial constraint \true{}, conjunction, existential quantification, and equality on constraint terms. Given a program {\tt P}, we now show how to derive from $\mathcal{X}$ a larger deduction system that captures the object-oriented structure of {\tt P} and let us decide whether {\tt P} is well typed.
...
We say a program, context, or judgment is well formed iff all the constraints and types involved are well formed. By design, every judgment derived from a well formed context is also well formed. As a consequence, if a program type checks then it is well formed. 

\paragraph{Member lookup.} Figure~\ref{fig:lookup} specifies the fields and methods available on each class. Note that classes may only be extended by classes with stronger invariants. Fields cannot be overridden.
...
We distinguish members available on variables from class members to prepare for the introduction of generic types. For now, ``${\tt x}:{\tt C}~\underline\has~{\tt I}$'' iff ``${\tt C}~\has~{\tt I}$.''
...
In order to capture the object-oriented structure of the program {\tt P}, we add:
\infrule[Sel]
...
The following rule governs the introduction of existential constraints:
\infrule[Exists]
  {\Gamma\vdash {\tt e}:{\tt T} \andalso \Gamma, {\tt x}:{\tt T}\vdash{\tt c}}
...
{\sc T-Method} and {\sc T-Class} ensure that the types and constraints occurring in a program are well formed. Following from \FJ{}, the typing rules do not preclude the existence of cycles in the type declarations. We assume acyclicity. {\sc T-Method} checks that the method body {\tt e} can be shown to have a type {\tt W} that is a subtype of its declared return type {\tt V}. {\sc T-Class} makes sure all methods of the class type check.
...
\infrule[S-Exists-R]
  {\Gamma\vdash {\tt e}:{\tt T} \andalso \Gamma,{\tt x}:{\tt T}\vdash {\tt U}\subtype {\tt V}}
...
Turning \FXZ{} into a language with value-dependent types is easy. The construction of the previous section is parametric in the underlying constraint system $\mathcal{X}$. Therefore, we just have to replace $\mathcal{X}$ with a richer constraint system.

We assume given a constraint system $\cal A$, with a vocabulary of primitive types ${\tt A}$,
predicates ${\tt p}:({\tt A})$ and functions ${\tt f}:({\tt A})=>{\tt A}$, 0-ary functions encoding constants.

We extend the productions of \FXZ{} as follows.
\begin{center}
\begin{tabular}{r@{\quad}rcl}
  (Type)& {\tt T} &{::=}& {\tt A} \\
...
As we did before, we delegate the role of defining and verifying constraint well-formedness to the underlying constraint system.

The obvious rule is needed to type function calls.

\infrule[T-Fun]
	{{\tt f}:(\tbar{A})\Rightarrow{\tt B}\in \mathcal{A} \andalso \Gamma\vdash\tbar{e}:\tbar{A}} 
	{\Gamma\vdash{{\tt f}(\tbar{e})}:{\tt B}}

Finally, we have to declare that all constraints except for existential constraints are user constraints, that is, are permitted in programs.
