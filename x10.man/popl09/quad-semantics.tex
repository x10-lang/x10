...
types). Users are permitted to specify ``$==$'' and ``$\extends$'' constraints
...
We require the underlying constraint system $\mathcal{G}$ to treat ``$\extends$'' as a partial order relation (reflexive, antisymmetric, and transitive). If type {\tt T} extends type {\tt U} then
\begin{itemize}
...
\item{\sc L-Extends}. If {\tt x} has type {\tt U} then {\tt x} has all the members of type {\tt T}. Note we only extend the ``$\underline\has$'' predicate that is used in typing judgements. On the other hand, the ``$\has$'' predicate used for method lookup in the oprational semantics is not affected.
\end{itemize}

The modification of the $\underline\has$ lookup predicate is
...
%
\begin{xten}
class A() extends Object { def m():A }
class B() extends A { def m():B }
class C(f:type){this.f<=A} extends Object {}
class D(){this.f<=B} extends C { ..this.f.m().. }
\end{xten}
%
occurrences of $\this.{\tt f}$ in {\tt D} are bound both by {\tt A} and {\tt B} hence 
$\this.{\tt f}.{\tt m}()$ may either be typed using the declaration of {\tt m} in {\tt A} or {\tt B}.

It is also possible for a program to specify inconsistent constraints such as ``${\tt x}\extends{\tt A}, {\tt x}\extends{\tt D}$'' using the class definitions above. The type system we have defined for \FXG{} does not necessarily reject programs embedding such constraints. It simply precludes generic classes with inconsistent constraints or generic methods with inconsistent guards to be instantiated on concrete types (since concrete types cannot satisfy these constraints).
