...
\begin{enumerate}
	...
	\item Typing:\\
	  ...
	  $\vdash {\tt class}~{\tt C}(\tbar{f}:\tbar{V})\{{\tt c}\}~{\tt extends}~{\tt D}~\{~\tbar{M}~\}~{\rm OK}$ \\ $~$ \hfill class {\tt C} type checks
	\item Subtyping:\\
...
\paragraph{Well-formedness.} The rules for well-formedness of types in Figure~\ref{fig:well} are straightforward. A constraint $\tt c$ is well-formed for context $\Gamma$ iff its free variables are declared in $\Gamma$, i.e., $\tt\mathcal{F}(c)\subseteq\mathcal{D}(\Gamma)$. $\mathcal{D}$ and $\mathcal{F}$ are defined as usual.\footnote{Constraints such as ``$\new~{\tt C}(\bullet)==\new~{\tt D}(\bullet)$'' are well formed even if inconsistent. By design, it will not be possible to derive an inconsistent constraint from a consistent context.}
...
\infrule[\RInvk]
	{{\tt x}:{\tt C}\vdash {\tt x}~\has~{\tt m}(\tbar{f}:\tbar{T})\{{\tt c}\}:{\tt U}={\tt b}}
	{\new~{\tt C}(\tbar{e}).{\tt m}(\tbar{a})\derives {\tt b}[\new~{\tt C}(\tbar{e}),\tbar{a}/{\tt x},\tbar{f}]}
...
\infrule[X-Inv]
  {{\tt class}~{\tt C}(\tbar{f}:\tbar{T})\{{\tt c}\}~{\tt extends}~{\tt D}~\{~\tbar{M}~\}\in {\tt P}
   \andalso
   \Gamma\vdash {\tt t}:{\tt T},{\tt T}\subtype{\tt C}}
...
\infrule[L-Field-C,E]
  ...
  {\Gamma,{\tt x}\ty {\tt T}\{{\tt c}\}\vdash {\tt x}~\has~{\tt f}\ty {\tt U}\{{\tt c}[{\tt x}/{\self}]\} \\
   \Gamma,{\tt x}\ty \exists {\tt y}\ty {\tt V}.~{\tt T}\vdash {\tt x}~\has~{\tt f}:\exists {\tt y}:{\tt V}.~{\tt U}}
...
\infrule[L-Method-C,E]
  ...
  {\Gamma,{\tt x}:{\tt T}\{{\tt c}\}\vdash {\tt x}~\has~{\tt m}(\tbar{y}:\tbar{U})\{{\tt d}\}:{\tt V}\{{\tt c}[{\tt x}/\self]\}={\tt e} \\
   \Gamma,{\tt x}:\exists {\tt z}:{\tt W}.~{\tt T}\vdash {\tt x}~\has~{\tt m}(\tbar{y}:\tbar{U})\{{\tt d}\}:\exists {\tt z}:{\tt W}.~{\tt V}={\tt e}}
...
\infrule[T-New]
    ...
    \tbar{U}\subtype\tbar{V},{\tt c}[{\tt x}/\this],\fields({\tt x})=\tbar{g}:\tbar{V} \andalso {\tt x},\tbar{y}~\rm fresh}
...
\infrule[T-Method]
  {{\tt class}~{\tt C}(\tbar{f}:\tbar{T})\{{\tt c}\}~{\tt extends}~{\tt D}~\{~\tbar{M}~\}\in {\tt P} \andalso
    ...
    \this:{\tt C},\tbar{y}:\tbar{U},{\tt d}\vdash {\tt V}~\type,\tbar{U}~\type,{\tt d}~\constraint,{\tt e}:{\tt W},{\tt W}\subtype {\tt V}}
  {{\tt def}~{\tt m}(\tbar{y}:\tbar{U})\{{\tt d}\}:{\tt V}={\tt e}~{\rm OK~in}~{\tt C}}

\infrule[T-Class]
  {\tt\tbar{M}~{\rm ok~in}~C \andalso \this:C,\tbar{f}:\tbar{T},{\tt c}\vdash\tbar{T}~\type,{\tt c}~\constraint}
...
\infrule[S-Const-R]
	{\Gamma\vdash {\tt T}\subtype {\tt U} \andalso\Gamma,\self:{\tt T}\vdash {\tt c}}
	{\Gamma\vdash {\tt T}\subtype {\tt U}\{{\tt c}\}}

\infrule[S-Exists-L]
  {\Gamma\vdash {\tt T}\subtype {\tt U} \andalso {\tt x}~\rm fresh}
...
\infrule[W-Extends]
	{{\tt class}~{\tt C}(\tbar{f}:\tbar{T})\{{\tt c}\}~{\tt extends}~{\tt D}\{~\tbar{M}~\}\in {\tt P}
	 \andalso \wj{\Gamma}{\tt t}
	 \andalso \cj{\Gamma}{\tt d}}
	{\cj{\Gamma}{{\tt t}\extends {\tt C}\{{\tt d}\}}}
