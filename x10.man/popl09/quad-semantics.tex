...
\infrule[T-Method]
  {{\tt class}~{\tt C}(\tbar{f}:\tbar{T})\{{\tt c}\}~{\tt extends}~{\tt D}~\{~\tbar{M}~\}\in {\tt P} \andalso
    ...
    \this:{\tt C}\vdash \tbar{U}~\type
    \andalso
    \this:{\tt C},\tbar{y}:\tbar{U}\vdash {\tt d}~\constraint
    \andalso
    \this:{\tt C},\tbar{y}:\tbar{U},{\tt d}\vdash {\tt e}:{\tt W},{\tt W}\subtype {\tt V}}
  {{\tt def}~{\tt m}(\tbar{y}:\tbar{U})\{{\tt d}\}:{\tt V}={\tt e}~{\rm OK~in}~{\tt C}}

\infrule[T-Class]
  {\tt\tbar{M}~{\rm OK~in}~C \andalso \this:C\vdash\tbar{T}~\type \andalso \this:C,\tbar{f}:\tbar{T}\vdash{\tt c}~\constraint}
...
\quad\typicallabel{XXXXX}
\infrule[S-Const-L]
	{\wj{\Gamma}{{\tt T}\{{\tt c}\}}\andalso \Gamma\vdash {\tt T}\subtype {\tt U}}
	{\Gamma\vdash {\tt T}\{{\tt c}\}\subtype {\tt U}}

\infrule[S-Const-R]
	{\wj{\Gamma}{{\tt U}\{{\tt c}\}}, {\tt T}\subtype {\tt U} \andalso\Gamma,\self:{\tt T}\vdash {\tt c}}
