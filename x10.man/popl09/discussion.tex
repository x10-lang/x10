...
For example, the \xcd"List" class of Figure~\ref{fig:list0}
could be written as shown in Figure~\ref{fig:list0prop}.
...
\begin{xtennoindent}
      class List(T: *, len: int) {
        var head: T;
        val tail: List{self.T==this.T}
        def get(i: int): T {
          if (i == 0) return head;
          else return tail.get(i-1);
        }
        def this(S: *, hd: T, tl: List{self.T==S}):
            List{self.T==S,len==tl.len+1} {
          super();
          property(S, tl.len+1);
          head = hd; tail = tl;
        }
...
\caption{List example, with type properties}
\label{fig:list0prop}
\end{figure}
