...
The use of constraints for type inference and subtyping was
developed by
Mitchell~\cite{mitchell84}
and by Reynolds~\cite{reynolds85}.  These and subsequent systems 
are based on constraints over types, but not over values.
Trifonov and Smith~\cite{trifonov96}
proposed a type system in which types 
are refined using subtyping
constraints.
...
\hmx~\cite{sulzmann97type,pottier01a,pottier-remy-attapl},
...
Constraints in \hmx are over types, not values.
%
...
PolyJ~\cite{java-popl97} also uses structual constraints to bound type
parameters.

\paragraph{Dependent types.}

Dependent type
systems~\cite{xi99dependent,calculus-of-constructions,epigram,cayenne}
...
constraints yields a legal ML program.  This differs from generic constrained
types, where erasure of subtyping constraints can prevent the program from
type-checking.
DML does not permit any runtime checking of constraints
(dynamic casts).
...
X10's, that is, two liquid types are in the subtyping relation if their base
...
The types of certain expressions are overapproximated to ensure inference
is decidable.
...
In \gxx, dynamic type checks, including tests of dependent
...
\csharp also supports generics via ~\cite{csharp-generics}.
...
\gxx does not support \emph{bivariance}~\cite{variant-parametric-types}; a
