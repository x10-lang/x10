...
MixGen~\cite{allen03} supported mixins through type parameterization.
...
parameterized on values as well as types.  Parameterization on values is
...
Consider the \xcd"Vector" class in Figure~\ref{fig:vector}.
If the class is instantiated on a constrained type \xcd"C{c}",
i.e., \xcd"Vector[C{c}]",
then all elements of the vector satisfy \xcd"c".
