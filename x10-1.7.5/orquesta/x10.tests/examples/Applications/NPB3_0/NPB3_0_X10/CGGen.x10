	...
	public def makea(var n: int, var nz: int, var a: Rail[double], var colidx: Rail[int], var rowstr: Rail[int], var nonzer: int, var rcond: double, var shift: double): void = {
		...
		if (false)
			System.out.println("na=" + n + " nz=" + nz + " a.length=" + a.length + " colidx.length="  + colidx.length
					...
					+ " rng=" + rng);
		if (false) {
			for (var i: int = 0; i < a.length; i++) if (a(i) != 0) System.out.println("a[" + i + "]=" + a(i));
			for (var i: int = 0; i < colidx.length; i++) if (colidx(i) != 0) System.out.println("colidx[" + i + "]=" + colidx(i));
			for (var i: int = 0; i < rowstr.length; i++) if (rowstr(i) != 0) System.out.println("rowstr[" + i + "]=" + rowstr(i));
			for (var i: int = 0; i < arow.length; i++) if (arow(i) != 0) System.out.println("arow[" + i + "]=" + arow(i));
			for (var i: int = 0; i < acol.length; i++) if (acol(i) != 0) System.out.println("acol[" + i + "]=" + acol(i));
			for (var i: int = 0; i < aelt.length; i++) if (aelt(i) != 0) System.out.println("aelt[" + i + "] = " + aelt(i));
			for (var i: int = 0; i < v.length; i++) if (v(i) != 0) System.out.println("v[" + i + "]=" + v(i));
			for (var i: int = 0; i < iv.length; i++) if (iv(i) != 0) System.out.println("iv[" + i + "]=" + iv(i));
		}
