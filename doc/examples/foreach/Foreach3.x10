 ...
 * </ul>
 */
// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX Foreach2 with additional functions and optimizations
public final class Foreach3 {


    /**
     * Implementation where iterations are executes in sequence. This
     * may be used for debugging purposes or as a compiler target
     * where there is no benefit to be gained from parallelizing an
     * iteration.
     */
    public static final class Sequential {

	/**
         * Iterate over elements of a collection in sequence in a single activity.
         * @param collection the iteration space
         * @param body a closure that executes over an element of the collection
         */
        public static @Inline operator for[T](collection: Iterable[T],
					      body:(T)=>void) {
            for (x in collection) {
		body(x);
	    }
         ...
         * @param identity the identity value for the reduction operation such that combine(identity,f)=f
         */
         ...
         */
        public static final class Reduce[T] {
            ...
            /**
             * Constructor collecting loop with reducer.
             * @param reduce the reduction operation
    ...
    public static final class Block {
        ...
        public static @Inline operator for(range: LongRange,
            ...
            Foreach3.Block.slice(range.min, range.max, executeRange);
        }
             ...
             if (Runtime.NTHREADS == 1n) {
                 ...
                 finish for (var t:Long = Runtime.NTHREADS-1; t >= 0; t--) {
                     ...
                     async {
                         ...
                         Foreach3.Sequential.operator for(block, body);
                     }
                 }
             }
         }
        ...
        public static @Inline def reduce[T](range:LongRange,
            ...
            return Foreach3.Block.reduceSlice(range.min, range.max, executeRange, combine);
        }
    ...
    public static final class Bisect {
        ...
        public static @Inline def slice(min:Long, max:Long,
            ...
            Foreach3.Bisect.slice(min, max, grainSize, body);
        }
        ...
        public static @Inline operator for(range: LongRange,
            ...
            Foreach3.Bisect.for_(range, grainSize, body);
        }
        ...
        public static @Inline def reduceSlice[T](min:Long, max:Long,
                                                 ...
                                                 reduce:(a:T,b:T)=>T):T {
            val grainSize = Math.max(1, (max-min) / (Runtime.NTHREADS*8));
            return Foreach3.Bisect.reduceSlice(min, max, grainSize, body, reduce);
        }
        ...
        public static @Inline def reduce[T](range:LongRange,
					    ...
					    reduce:(a:T,b:T)=>T, identity:T):T {
            val grainSize = Math.max(1, (range.max-range.min) / (Runtime.NTHREADS*8));
            return Foreach3.Bisect.for_(range, grainSize, body, reduce, identity);
        }
	...
	public static @Inline def slice(min0:Long, max0:Long,
	    ...
	    Foreach3.Bisect.slice(min0, max0, min1, max1, grainSize0, grainSize1, body);
	}
	 ...
	 */
        public static @Inline operator for (space:DenseIterationSpace_2,
            ...
            Foreach3.Bisect.slice(space.min0, space.max0, space.min1, space.max1, grainSize0, grainSize1, executeRange);
        }
	 ...
	 */
	    ...
	    Foreach3.Bisect.operator for (space, grainSize0, grainSize1, body);
	}
