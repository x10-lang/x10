import x10.util.Random;
	...
	private const datasize_M = 100;
	private const datasize_N = 100;
	private const datasize_nz = 500;
	...
	public def JGFinitialise(): void = {
		val ds_N: int = datasize_N;
		val ds_M: int = datasize_M;
		val ds_nz: int = datasize_nz;
	...
	public def JGFvalidate(): void = {
		//double refval[] = { 75.02484945753453, 150.0130719633895, 749.5245870753752 };
	     val refval = 0.16484213206275583;
		var dev: double = Math.abs(ytotal - refval);
		if (dev > 1.0e-10) {
			Console.OUT.println("Validation failed");
			Console.OUT.println("ytotal = " + ytotal + "  refval =" + refval + "  " + size);
...
val c = new Clock();
for (val (id): Point in [0..nthreads-1]) async clocked(c)
