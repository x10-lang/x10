...
out_file=dfs_random_altair_color.jpg
...
printf "set xlabel \" \"\n" >> $tmp_file
printf "set title \"[DFS/Random/Altair] 250K\"\n" >> $tmp_file
...
printf "set size 0.25,1\n" >> $tmp_file
printf "plot \"${java_tbl}\" index 2 using 3:5 title \"XWS\" with linespoints lw 2 pt 4, \\" >> $tmp_file
printf "\n" >> $tmp_file
printf "\"${c_tbl}\" index 2 using 3:5 title \"C\" with linespoints lw 2 pt 4, \\" >> $tmp_file
printf "\n" >> $tmp_file
printf "\"${cilk_tbl}\" index 2 using 3:5 title \"Cilk\" with linespoints lw 2 pt 4" >> $tmp_file
printf "\n" >> $tmp_file
