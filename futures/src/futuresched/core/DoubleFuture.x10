...
public final class DoubleFuture implements Notifier {
...
//      throw new Exception("Future is already set.");
  ...
  public def asyncSet(futures: ArrayList[DoubleFuture], fun: ()=>Double) {
    FTask.asyncAnd(
      futures,
      ()=>{ set(fun()); }
    );
...
//    );
  ...
  public def asyncSet(futures: ArrayList[Notifier], fun: ()=>Double) {
    FTask.asyncAnd(
      futures,
      ()=>{ set(fun()); }
    );
...
//    throw new Exception("Future is not ready yet.");
  ...
  public def register(fun: (Double)=>void) {
    val newBlock = () => {
      val v = get();
      fun(v);
    };
