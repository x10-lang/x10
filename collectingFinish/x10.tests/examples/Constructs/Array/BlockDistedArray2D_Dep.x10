...
public class BlockDistedArray2D_Dep extends x10Test {
    ...
    public def run(): boolean = {
        var D: Dist{rank==2};
        D=Dist.makeConstant([0..SIZE-1, 0..SIZE-1], Place.place(0));
        for(var i: int = 1;i<N_PLACES;i++) D=D||(Dist.makeConstant([0..SIZE-1, i*SIZE..((i+1)*SIZE-1)], Place.place(i)));
        val intArray: DistArray[int]{rank==2} = DistArray.make[int](D, ((i,j): Point): int => {return i+j;}); 
        val dblArray: DistArray[double]{rank==2} = DistArray.make[double](D, ((i,j): Point): double => {return (i+j)*0.1;});
