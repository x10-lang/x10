...
public class EffectsVisitor extends NodeVisitor {
    ...
    private final Map<ProcedureDef,Effect> fMethodEffects= new HashMap<ProcedureDef, Effect>();

    private XConstraint fMethodContext;
...
//    private final ValueMap fValueMap; // TODO Do we really need this in the first place?
	...
	private final X10TypeSystem fTypeSystem;

	private final ErrorQueue fEQ;

	public EffectsVisitor(Job job /*, ValueMap valueMap*/) {
		fJob = job;
		fTypeSystem= (X10TypeSystem) ((fJob != null) ? fJob.extensionInfo().typeSystem() : null);
		fEQ= (fJob != null) ? fJob.extensionInfo().compiler().errorQueue() : null;
    }

	public EffectsVisitor(X10ProcedureDecl proc /*, ValueMap valueMap*/) throws XFailure {
		fJob= null;
		fTypeSystem= null;
		fEQ= null;
    ...
    /**
     * Intended to be used to produce XTerms within the given ProcedureDecl.
     * @return a constraint representing the conjunction of the procedure guard
     * with all of the formal parameters' constraints
     * @throws XFailure
     */
    private final XConstraint computeMethodContextConstraint(X10ProcedureDecl proc) throws XFailure {
        ...
        for(Formal formal: formals) {
            ...
            if (xc != null) {
                result= conjunction(result, xc); // is this conjunction sensible, or might each clause end up constraining "self" out of scope?
            }
        }
        return result;
    }
    ...
    private XTerm createTermForReceiver(Receiver r) {
        ...
        XTypeTranslator xtt= new XTypeTranslator(fTypeSystem);
...
//      throw new UnsupportedOperationException("Can't produce an XTerm for type references.");
    ...
    public NodeVisitor enter(Node parent, Node n) {
    	if (n instanceof X10ProcedureDecl) {
			X10ProcedureDecl pd = (X10ProcedureDecl) n;
    		try {
				fMethodContext= computeMethodContextConstraint(pd);
			} catch (XFailure e) {
				// TODO What to do with this when invoked from outside of a compiler context?
				throw new InternalCompilerError("Caught exception while computing method context constraint for " + pd.name(), n.position(), e);
			}
    	}
    	return this;
    }
    ...
    public Node leave(Node parent, Node old, Node n, NodeVisitor v) {
        ...
        try {
            ...
            } else if (old instanceof ProcedureDecl) {
            	ProcedureDecl pd= (ProcedureDecl) old;

            	result= fEffects.get(pd.body());
            	fMethodEffects.put(pd.procedureInstance(), result);

            	if (result == null) {
            		emitMessage("Unable to compute effects of method " + pd.name(), pd.position());
            	} else if (result == Effects.BOTTOM_EFFECT) {
            		emitMessage("Method " + pd.name() + " has 'bottom' effect", pd.position());
            	} else if (isMainMethod(pd) && !result.isFun()) {
            		emitMessage("Main method is not safely parallelized; effect is: " + result, pd.position());
            	}
        	...
        	throw new InternalCompilerError(f.getMessage(), old.position(), f);
        }
        return super.leave(parent, old, n, v);
    }

	private void emitMessage(String msg, Position pos) {
		if (fEQ != null) {
	...
	private boolean isMainMethod(ProcedureDecl pd) {
		...
		return md.name().id().toString().equals("main") &&
			...
			fTypeSystem.isVoid(md.returnType().type()) &&
			md.formals().size() == 1 &&
			fTypeSystem.isRail(pd.formals().get(0).type().type());
	}
