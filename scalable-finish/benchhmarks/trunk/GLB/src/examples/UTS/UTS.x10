  ...
  public static struct Constants {
	...
	public static def main (args : Rail[String]!) {
		...
		if (seq != 0U) {
		    ...
		    if (Constants.BINOMIAL==t) {
			val runner = 
			   new SeqRunner[SHA1Rand,UInt](new Binomial(b0,qq,mf));
			result=runner(SHA1Rand(r), reducer);
		    } else {
			val runner = 
			   new SeqRunner[TreeNode, UInt](new Geometric(b0,a,d));
		...
		if (Constants.BINOMIAL==t) {
		    val runner = 
		new GlobalRunner[SHA1Rand, UInt](args,  Dist.makeUnique(), 
		...
		val runner =  
		    new GlobalRunner[TreeNode, UInt](args, Dist.makeUnique(), 
