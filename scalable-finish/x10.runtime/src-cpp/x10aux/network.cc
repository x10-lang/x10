...
static void cuda_post (const x10rt_msg_params *p, void *env)
{
    ...
    (fs.operator->()->*(x10aux::findITable<x10::lang::Runtime__RootFinishState>(fs->_getITables())->notifyActivityCreation))();
    (fs.operator->()->*(x10aux::findITable<x10::lang::Runtime__RootFinishState>(fs->_getITables())->notifyActivityTermination))();
}
