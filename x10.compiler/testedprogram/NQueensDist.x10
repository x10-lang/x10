...
public class NQueensDist {
    ...
    class Board {
        /*The element of q may be updated by different threads*/
        val  q: Rail[Int];
        ...
        def searchOne(k:Int) {
            if (safe(k)) {
                if (fixed==(N-1)) {
                    ...
                    /*this atomic can be removed*/
                    var array:DistArray[Int](1) = NQueensDist.this.results;
                    atomic(array) array(here.id)++;
                } else {
                    /*update of q should be atomically*/
                    q(fixed++) = k;
                    search();
                    fixed--;
                }
