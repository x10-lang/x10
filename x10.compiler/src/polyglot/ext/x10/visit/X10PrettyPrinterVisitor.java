 ...
 */
public class X10PrettyPrinterVisitor 
    extends Runabout {
    ...
    public X10PrettyPrinterVisitor(CodeWriter w,
                                   PrettyPrinter pp) {
        this.w = w;
        this.pp = pp;
    }
    ...
    public void visit(ForLoop_c f) {   	
        ...
        dump("forloop", new Object[] { f.formal().flags().translate(), 
        		f.formal().type(), f.formal().name(), f.domain(), f.body() });
    }
    ...
    public void visit(ArrayConstructor_c a) {
        ...
        if (base_type.isBoolean()) {
            ...
            if (a.hasLocal1DimDistribution()) {
                if (a.hasInitializer()) {
                    dump("boolean_array_initializer", new Object[] {a.initializer(),
                            new Boolean(a.isSafe()),
                            new Boolean(a.isValue())});
                    return;
                }
                dump("boolean_array_local", new Object[] { a.distribution(),
                        new Boolean(a.isSafe()),
                        new Boolean(a.isValue()) });
                return;
            }
            Object init = a.initializer();
            dump("boolean_array_dist_op", new Object[] { a.distribution(), 
                    init != null ? init : "null",
                            new Boolean(a.isSafe()),
                            new Boolean(a.isValue()) });
            return;
        ...
        if (base_type.isChar()) {
            ...
            if (a.hasLocal1DimDistribution()) {
                if (a.hasInitializer()) {
                    dump("char_array_initializer", new Object[] {a.initializer(),
                           new Boolean(a.isSafe()),
                           new Boolean(a.isValue())});
                    return;
                }
                dump("char_array_local", new Object[] { a.distribution(),
                           new Boolean(a.isSafe()),
                           new Boolean(a.isValue()) });
                return;
            }
            Object init = a.initializer();
            dump("char_array_dist_op", new Object[] { a.distribution(), 
                                init != null ? init : "null",
                                new Boolean(a.isSafe()),
                                new Boolean(a.isValue()) });
            return;
        ...
        if (base_type.isByte()) {
            ...
            if (a.hasLocal1DimDistribution()) {
                if (a.hasInitializer()) {
                    dump("byte_array_initializer", new Object[] {a.initializer(),
                           new Boolean(a.isSafe()),
                           new Boolean(a.isValue())});
                    return;
                }
                dump("byte_array_local", new Object[] { a.distribution(),
                           new Boolean(a.isSafe()),
                           new Boolean(a.isValue()) });
                return;
            }
            Object init = a.initializer();
            dump("byte_array_dist_op", new Object[] { a.distribution(), 
                                init != null? init : "null",
                                new Boolean(a.isSafe()),
                                new Boolean(a.isValue()) });
            return;
        ...
        if (base_type.isShort()) {
            ...
            if (a.hasLocal1DimDistribution()) {
                if (a.hasInitializer()) {
                    dump("short_array_initializer", new Object[] {a.initializer(),
                           new Boolean(a.isSafe()),
                           new Boolean(a.isValue())});
                    return;
                }
                dump("short_array_local", new Object[] { a.distribution(),
                           new Boolean(a.isSafe()),
                           new Boolean(a.isValue()) });
                return;
            }
            Object init = a.initializer();
            dump("short_array_dist_op", new Object[] { a.distribution(), 
                                init != null? init : "null",
                                new Boolean(a.isSafe()),
                                new Boolean(a.isValue()) });
            return;
    		...
    		if (a.hasLocal1DimDistribution()) {
    			if (a.hasInitializer()) {
    				dump("int_array_initializer", 
				     new Object[] {a.initializer(),
						   new Boolean(a.isSafe()),
						   new Boolean(a.isValue())
				});
    				return;
    			}
    			dump("int_array_local", new Object[] { a.distribution(),
							       new Boolean(a.isSafe()),
							       new Boolean(a.isValue()) });
    			return;
    		}
    		Object init = a.initializer();
    		dump("int_array_dist_op", new Object[] { a.distribution(), 
							 init != null? init : "null",
							 new Boolean(a.isSafe()),
							 new Boolean(a.isValue())
		});
        ...
        if (base_type.isFloat()) {
            ...
            if (a.hasLocal1DimDistribution()) {
                if (a.hasInitializer()) {
                    dump("float_array_initializer", new Object[] {a.initializer(),
                           new Boolean(a.isSafe()),
                           new Boolean(a.isValue())});
                    return;
                }
                dump("float_array_local", new Object[] { a.distribution(),
                           new Boolean(a.isSafe()),
                           new Boolean(a.isValue()) });
                return;
            }
            Object init = a.initializer();
            dump("float_array_dist_op", new Object[] { a.distribution(), 
                                init != null? init : "null",
                                new Boolean(a.isSafe()),
                                new Boolean(a.isValue()) });
            return;
    	...
    	if (base_type.isDouble()) {
    		...
    		if (a.hasLocal1DimDistribution()) {
    			if (a.hasInitializer()) {
    				dump("double_array_initializer", new Object[] {a.initializer(),
						   new Boolean(a.isSafe()),
						   new Boolean(a.isValue())});
    				return;
    			}
    			dump("double_array_local", new Object[] { a.distribution(),
						   new Boolean(a.isSafe()),
						   new Boolean(a.isValue()) });
    			return;
    		}
    		Object init = a.initializer();
    		dump("double_array_dist_op", new Object[] { a.distribution(), 
							    init != null? init : "null",
							    new Boolean(a.isSafe()),
							    new Boolean(a.isValue()) });
    		return;
    	...
    	if (base_type.isLong()) {
    		...
    		if (a.hasLocal1DimDistribution()) {
    			if (a.hasInitializer()) {
    				dump("long_array_initializer", new Object[] {a.initializer(),
						   new Boolean(a.isSafe()),
						   new Boolean(a.isValue())});
    				return;
    			}
    			dump("long_array_local", new Object[] { a.distribution(), 
						   new Boolean(a.isSafe()),
						   new Boolean(a.isValue()) });
    			return;
    		}
    		Object init = a.initializer();
    		dump("long_array_dist_op", new Object[] { a.distribution(), init != null? init : "null",
						   new Boolean(a.isSafe()),
						   new Boolean(a.isValue()) });
    		return;
    	}
    	if (! base_type.isPrimitive()) {
    	    ...
    	    if (a.hasLocal1DimDistribution()) {
    	        if (a.hasInitializer()) {
    	            dump("generic_array_initializer", new Object[] {a.initializer(),
    	                    new Boolean(a.isSafe()),
    	                    new Boolean(a.isValue()),
                            new Boolean(refs_to_values) });
    	            return;
    	        }
    	        dump("generic_array_local", new Object[] { a.distribution(), 
    	                new Boolean(a.isSafe()),
    	                new Boolean(a.isValue()),
                        new Boolean(refs_to_values) });
    	        return;
    	    ...
    	    dump("generic_array_dist_op", 
    	            new Object[] { a.distribution(), init != null? init : "(x10.compilergenerated.Parameter1)null",
    	                    new Boolean(a.isSafe()),
    	                    new Boolean(a.isValue()),
                            new Boolean(refs_to_values) });
    	    return;
    ...
    public void visit(PlaceCast_c  a) {
        ...
        dump("cast_place", new Node[] { a.expr(), a.placeCastType(), 
    ...
    public void visit(X10ArrayAccess_c a) {
    	...
    	assert size > 1;
    	if (size == 2) 
    		dump("array_get2", new Node[] { a.array(), (Node) index.get(0), (Node) index.get(1)});
    	else if (size == 3)
    		dump("array_get3", new Node[] { a.array(), (Node) index.get(0), (Node) index.get(1), (Node) index.get(2)});
    	else if (size == 4)
    		dump("array_get4", new Node[] { a.array(), (Node) index.get(0), (
    				Node) index.get(1), (Node) index.get(2), (Node) index.get(3)});
    	else 
    ...
    public void visit(X10ArrayAccess1Assign_c a) {
        ...
        dump("array_set1", new Node[] { left.array(), a.right(), left.index() });
    }
   /*
    * 
     public void visit(X10ArrayAccessAssign_c a) {
    	...
    	assert size > 1;
    	if (size == 2) 
    		dump("array_set2", new Node[] { left.array(), a.right(), (Node) index.get(0), (Node) index.get(1)});
    	else if (size == 3)
    		dump("array_set3", new Node[] { left.array(), a.right(), (Node) index.get(0), (Node) index.get(1), (Node) index.get(2)});
    	else if (size == 4)
    		dump("array_set4", new Node[] { left.array(), a.right(), (Node) index.get(0), (
    				Node) index.get(1), (Node) index.get(2), (Node) index.get(3)});
    	else 
    		throw new Error("TODO: vj->cvp/cg ... Please implement general case.");
    ...
    */
     ...
     *        loop body template.
     */
    private void dumpLoop(String id,
                          String lid,
                          Object[] components) {
