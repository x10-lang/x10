...
public class VarsBoxer extends ContextVisitor {
    ...
    private Stmt addBoxValWriteBackCallToBody(Context context2, AtStmt atSt) {
        Stmt body = (Stmt) atSt.body().visit(new ContextVisitor(job, ts, nf) {
            public Node override(Node parent, Node n) {
                if (n instanceof AtStmt || n instanceof AtExpr) { // TODO closure
                    return n;
                }
                return null;
            };
    ...
    private Stmt addBoxValWriteBackCallToBody(Context context2, AtExpr atEx) {
        Stmt body = (Stmt) atEx.body().visit(new ContextVisitor(job, ts, nf) {
            public Node override(Node parent, Node n) {
