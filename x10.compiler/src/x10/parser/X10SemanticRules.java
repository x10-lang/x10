...
public class X10SemanticRules implements Parser, ParseErrorCodes {
	...
	public X10Parser getX10Parser() {
		return p;
	}

	public X10Lexer getX10Lexer() {
		return lexer;
	}

	// private Object setResult(Object o) { return o; } // todo: refactor it out
	private void setResult(Object object) {
		getParser().setSym1(object);
	}

	public Object getRhsSym(int i) {
		return p.getRhsSym(i);
	}

	public int getRhsTokenIndex(int i) {
		return p.getRhsTokenIndex(i);
	}

	public IToken getRhsIToken(int i) {
		return p.getRhsIToken(i);
	}

	public int getRhsFirstTokenIndex(int i) {
		return p.getRhsFirstTokenIndex(i);
	}

	public IToken getRhsFirstIToken(int i) {
		return p.getRhsFirstIToken(i);
	}

	public int getRhsLastTokenIndex(int i) {
		return p.getRhsLastTokenIndex(i);
	}

	public IToken getRhsLastIToken(int i) {
		return p.getRhsLastIToken(i);
	}

	public int getLeftSpan() {
		return p.getLeftSpan();
	}

	public IToken getLeftIToken() {
		return p.getLeftIToken();
	}

	public int getRightSpan() {
		return p.getRightSpan();
	}

	public IToken getRightIToken() {
		return p.getRightIToken();
	}

	public int getRhsErrorTokenIndex(int i) {
		return p.getRhsErrorTokenIndex(i);
	}

	public ErrorToken getRhsErrorIToken(int i) {
		return p.getRhsErrorIToken(i);
	}
	...
	// public String[] orderedTerminalSymbols() { return
	// X10Parsersym.orderedTerminalSymbols; }
	// public String getTokenKindName(int kind) { return
	// X10Parsersym.orderedTerminalSymbols[kind]; }
	// public int getEOFTokenKind() { return prsTable.getEoftSymbol(); }
	public IPrsStream getIPrsStream() {
		return p.getIPrsStream();
	}
	...
	public void initialize(TypeSystem t, NodeFactory n, FileSource source,
			ErrorQueue q) {
	...
	public X10SemanticRules(X10Lexer lexer, TypeSystem t, NodeFactory n,
			FileSource source, ErrorQueue q) {
		this.lexer = lexer;
		ILexStream lexStream = this.lexer.getILexStream();
		p = new X10Parser(lexStream);
		p.r = this;
		initialize((TypeSystem) t, (NodeFactory) n, source, q);
	...
	public static class MessageHandler implements IMessageHandler {
		...
		public void handleMessage(int errorCode, int[] msgLocation,
			...
			Position pos = new JPGPosition("", file.getPath(), l0, c0, l1,
					c1 + 1, o0, o1);
	...
	public String getErrorLocation(int lefttok, int righttok) {
		return prsStream.getFileName() + ':' + prsStream.getLine(lefttok) + ":"
				+ prsStream.getColumn(lefttok) + ":"
				+ prsStream.getEndLine(righttok) + ":"
				+ prsStream.getEndColumn(righttok) + ": ";
	}
	...
	private static class FlagModifier extends Modifier {
		...
		public Flags flags() {
			...
			if (flag == STATIC)
				return Flags.STATIC;
			if (flag == UNITFOR)
				return Flags.UNITFOR;
			if (flag == ATOMICPLUS)
				return Flags.ATOMICPLUS; /* data-centric */
			if (flag == LINKED)
				return Flags.LINKED; /* data-centric */
			assert (false);
			return null;
		}
		...
		public String name() {
			...
			if (flag == UNITFOR)
				return "unitfor";
			if (flag == ATOMICPLUS)
				return "atomicplus"; /* data-centric */
			if (flag == LINKED)
				return "linked"; /* data-centric */
			assert (false);
			return "?";
		}
	...
	// TODO: Say something!
	//
	private List<Node> checkModifiers(String kind, List<Modifier> modifiers,
			boolean legal_flags[]) {
		...
		for (int i = 0; i < modifiers.size(); i++) {
			...
			if (element instanceof FlagModifier) {
				...
				l.addAll(Collections.singletonList(nf.FlagsNode(
						modifier.position(), modifier.flags())));

				if (!flags[modifier.flag()]) {
					flags[modifier.flag()] = true;
				} else {
					syntaxError("Duplicate specification of modifier: "
							+ modifier.name(), modifier.position());
				}

				if (!legal_flags[modifier.flag()]) {
					syntaxError("\"" + modifier.name() + "\" is not a valid "
							+ kind + " modifier", modifier.position());
				}
			} else {
	...
	private List<Node> checkClassModifiers(List<Modifier> modifiers) {
		return (modifiers.size() == 0 ? Collections.<Node> singletonList(nf
				.FlagsNode(JPGPosition.COMPILER_GENERATED, Flags.NONE))
				: checkModifiers("class", modifiers,
	...
	public static class JPGPosition extends Position {
		...
		public JPGPosition(String path, String filename, IToken leftToken,
				IToken rightToken) {
			super(path, filename, leftToken.getLine(), leftToken.getColumn(),
					rightToken.getEndLine(), rightToken.getEndColumn(),
					leftToken.getStartOffset(), rightToken.getEndOffset());
			this.leftIToken = null; // BRT -- was null, need to keep leftToken
									// for later reference
			this.rightIToken = null; // BRT -- was null, need to keep rightToken
										// for later reference
		}

		public JPGPosition(Position start, Position end) {
			super(start, end);
			this.leftIToken = (start instanceof JPGPosition) ? ((JPGPosition) start).leftIToken
					: null;
			this.rightIToken = (end instanceof JPGPosition) ? ((JPGPosition) end).rightIToken
					: null;
		}

		JPGPosition(String path, String filename, int line, int column,
				int endLine, int endColumn, int offset, int endOffset) {
			super(path, filename, line, column, endLine, endColumn, offset,
					endOffset);
		...
		public static final JPGPosition COMPILER_GENERATED = (JPGPosition) (new JPGPosition()
		...
		public String toText() {
			...
			return new String(prsStream.getInputChars(), offset(), endOffset()
					- offset() + 1);
		}
	}
		...
		try {
			...
			if (sf != null) {
				...
				eq.enqueue(ErrorInfo.SYNTAX_ERROR,
						"Unable to parse " + source.name() + ".",
						new JPGPosition(null, file(), 1, 1, 1, 1, 0, 0)
								.markCompilerGenerated());
			}
			...
			eq.enqueue(ErrorInfo.SYNTAX_ERROR, e.getMessage(), new JPGPosition(
					null, file(), 1, 1, 1, 1, 0, 0).markCompilerGenerated());
		}
	...
	public JPGPosition pos(JPGPosition start, JPGPosition end) {
		return new JPGPosition(start.path(), start.file(), start.leftIToken,
				end.rightIToken);
	}
	...
	private void checkTypeName(Id identifier) {
		...
		int dot = filename.lastIndexOf('.'), slash = filename.lastIndexOf('/',
				dot);
		if (slash == -1)
			slash = filename.lastIndexOf('\\', dot);
		String clean_filename = (slash >= 0 && dot >= 0 ? filename.substring(
				slash + 1, dot) : "");
		if ((!clean_filename.equals(idname))
				&& clean_filename.equalsIgnoreCase(idname))
			eq.enqueue(ErrorInfo.SYNTAX_ERROR,
					"This type name does not match the name of the containing file: "
	...
	private polyglot.lex.Identifier id(int i) {
		return new Identifier(pos(i), prsStream.getName(i),
				X10Parsersym.TK_IDENTIFIER);
	}
	...
	private List<Formal> toFormals(List<Formal> l) {
		return l;
	}
	...
	private List<TypeParamNode> toTypeParams(List<TypeParamNode> l) {
		return l;
	}
	...
	private FlagsNode extractFlags(List<? extends Node> l1,
			List<? extends Node> l2) {
	...
	private FlagsNode extractFlags(List<? extends Node> l) {
		...
		for (Node n : l) {
			if (n instanceof FlagsNode) {
				...
				pos = pos == null ? fn.position() : new JPGPosition(pos,
						fn.position());
				Flags f = fn.flags();
				xf = xf.set(f);
			}
		}
		return nf.FlagsNode(pos == null ? JPGPosition.COMPILER_GENERATED : pos,
				xf);
	}

	/*
	 * Roll our own integer parser. We can't use Long.parseLong because it
	 * doesn't handle numbers greater than 0x7fffffffffffffff correctly.
	 */
	...
	private long parseLong(String s, Position pos) {
		...
		while (end_index > 0) {
			...
			if (lastCh == 'l' || lastCh == 'L')
				isUnsigned = true; // for signed values that start with 0, we
									// need to make them negative if they are
									// above max value
			if (lastCh == 'y' || lastCh == 'Y')
				min = Byte.MIN_VALUE;
			if (lastCh == 's' || lastCh == 'S')
				min = Short.MIN_VALUE;
			if (lastCh != 'y' && lastCh != 'Y' && lastCh != 's'
					&& lastCh != 'S' && lastCh != 'l' && lastCh != 'L'
					&& lastCh != 'u' && lastCh != 'U') {
				break;
			}
			end_index--;
		...
		if (!isUnsigned && radix != 10 && res >= max) {
			...
			// e.g., 0xffUY == 255, 0xffY== 255-256 = -1 , 0xfeYU==254, 0xfeY==
			// 254-256 = -2
			return res + min * 2;
		}
		return res;
	...
	private void setIntLit(IntLit.Kind k) {
		setResult(nf.IntLit(pos(), k,
				parseLong(prsStream.getName(getRhsFirstTokenIndex(1)), pos())));
	}

	private polyglot.lex.FloatLiteral float_lit(int i) {
		try {
			String s = prsStream.getName(i);
			int end_index = (s.charAt(s.length() - 1) == 'f'
					|| s.charAt(s.length() - 1) == 'F' ? s.length() - 1 : s
					.length());
			float x = Float.parseFloat(s.substring(0, end_index));
			return new FloatLiteral(pos(i), x,
					X10Parsersym.TK_FloatingPointLiteral);
		} catch (NumberFormatException e) {
			unrecoverableSyntaxError = true;
			eq.enqueue(ErrorInfo.LEXICAL_ERROR, "Illegal float literal \""
					+ prsStream.getName(i) + "\"", pos(i));
			return null;
		}
	}
	...
	private polyglot.lex.DoubleLiteral double_lit(int i) {
		try {
			String s = prsStream.getName(i);
			int end_index = (s.charAt(s.length() - 1) == 'd'
					|| s.charAt(s.length() - 1) == 'D' ? s.length() - 1 : s
					.length());
	...
	private polyglot.lex.BooleanLiteral boolean_lit(int i) {
		return new BooleanLiteral(pos(i),
				prsStream.getKind(i) == X10Parsersym.TK_true,
				prsStream.getKind(i));
	}
	...
	private polyglot.lex.StringLiteral string_lit(int i, Position pos) {
			...
			if (s.charAt(j) != '\\')
				...
				default: {
					...
					if (c > 255) {
						unrecoverableSyntaxError = true;
						eq.enqueue(ErrorInfo.LEXICAL_ERROR,
								"Illegal character (" + s.substring(j, n)
										+ ") in string literal " + s, pos(i));
					}
					x[k++] = c;
					j = n;
				}
		...
		return new StringLiteral(pos(i), new String(x, 0, k),
				X10Parsersym.TK_StringLiteral);
	}
	...
	// Production: ClosureExpression ::= FormalParameters WhereClauseopt
	// HasResultTypeopt OBSOLETE_Offersopt '=>' ClosureBody
	void rule_ClosureExpression0(Object _FormalParameters,
			Object _WhereClauseopt, Object _HasResultTypeopt,
			Object _OBSOLETE_Offersopt, Object _ClosureBody) {
		...
		setResult(nf.Closure(pos(), FormalParameters, WhereClauseopt,
				HasResultTypeopt == null ? nf.UnknownTypeNode(pos())
						: HasResultTypeopt, ClosureBody));
	}
	...
	void rule_PackageOrTypeName0(Object _PackageOrTypeName) {
		...
		setResult(new ParsedName(nf, ts, pos(getLeftSpan(), getRightSpan()),
				PackageOrTypeName, nf.Id(pos(getRightSpan()), "*")));
	}
	...
	void rule_Property0(Object _Annotationsopt, Object _Identifier,
			Object _ResultType) {
		...
		PropertyDecl cd = nf.PropertyDecl(pos(),
				nf.FlagsNode(pos(), Flags.PUBLIC.Final()), ResultType,
				Identifier);
	...
	void rule_FieldDeclarator0(Object _Identifier, Object _HasResultType) {
		...
		setResult(new Object[] { pos(), Identifier,
				Collections.<Id> emptyList(), HasResultType, null });
	}

	// Production: FieldDeclarator ::= Identifier HasResultTypeopt '='
	// VariableInitializer
	void rule_FieldDeclarator1(Object _Identifier, Object _HasResultTypeopt,
			Object _VariableInitializer) {
		Id Identifier = (Id) _Identifier;
		TypeNode HasResultTypeopt = (TypeNode) _HasResultTypeopt;
	...
	// Production: AtCaptureDeclarator ::= Modifiersopt VarKeywordopt
	// VariableDeclarator
	void rule_AtCaptureDeclarator0(Object _Modifiersopt, Object _VarKeywordopt,
			Object _VariableDeclarator) {
		...
		FlagsNode fn = VarKeywordopt == null ? extractFlags(modifiers,
				Flags.FINAL) : extractFlags(modifiers, VarKeywordopt);
		...
		if (type == null)
			type = nf.UnknownTypeNode(name != null ? name.position() : pos);
		Expr init = (Expr) o[5];
		LocalDecl ld = nf.LocalDecl(pos, fn, type, name, init, exploded);
		ld = (LocalDecl) ((X10Ext) ld.ext())
				.annotations(extractAnnotations(modifiers));
	...
	void rule_FullyQualifiedName0(Object _FullyQualifiedName) {
		...
		setResult(new ParsedName(nf, ts, pos(getLeftSpan(), getRightSpan()),
				FullyQualifiedName, nf.Id(pos(getRightSpan()), "*")));
	}
	...
	// Production: VariableDeclaratorWithType ::= Identifier HasResultType '='
	// VariableInitializer
	void rule_VariableDeclaratorWithType0(Object _Identifier,
			Object _HasResultType, Object _VariableInitializer) {
		Id Identifier = (Id) _Identifier;
		TypeNode HasResultType = (TypeNode) _HasResultType;
		Expr VariableInitializer = (Expr) _VariableInitializer;
		setResult(new Object[] { pos(), Identifier,
				Collections.<Id> emptyList(), null, HasResultType,
				VariableInitializer });
	}

	// Production: VariableDeclaratorWithType ::= '[' IdentifierList ']'
	// HasResultType '=' VariableInitializer
	void rule_VariableDeclaratorWithType1(Object _IdentifierList,
			Object _HasResultType, Object _VariableInitializer) {
		List<Id> IdentifierList = (List<Id>) _IdentifierList;
		TypeNode HasResultType = (TypeNode) _HasResultType;
		Expr VariableInitializer = (Expr) _VariableInitializer;
		setResult(new Object[] { pos(), null, IdentifierList, null,
				HasResultType, VariableInitializer });
	}

	// Production: VariableDeclaratorWithType ::= Identifier '[' IdentifierList
	// ']' HasResultType '=' VariableInitializer
	void rule_VariableDeclaratorWithType2(Object _Identifier,
			Object _IdentifierList, Object _HasResultType,
			Object _VariableInitializer) {
		Id Identifier = (Id) _Identifier;
		List<Id> IdentifierList = (List<Id>) _IdentifierList;
		TypeNode HasResultType = (TypeNode) _HasResultType;
		Expr VariableInitializer = (Expr) _VariableInitializer;
		setResult(new Object[] { pos(), Identifier, IdentifierList, null,
				HasResultType, VariableInitializer });
	}
	...
	void rule_AnnotationStatement0(Object _Annotationsopt,
			Object _NonExpressionStatement) {
		List<AnnotationNode> Annotationsopt = (List<AnnotationNode>) _Annotationsopt;
		Stmt NonExpressionStatement = (Stmt) _NonExpressionStatement;
		if (NonExpressionStatement.ext() instanceof X10Ext) {
			NonExpressionStatement = (Stmt) ((X10Ext) NonExpressionStatement
					.ext()).annotations(Annotationsopt);
		}
	...
	void rule_TypeDeclarations1(Object _TypeDeclarations,
	...
	void rule_TypeImportOnDemandDeclaration0(Object _PackageOrTypeName) {
		ParsedName PackageOrTypeName = (ParsedName) _PackageOrTypeName;
		setResult(nf.Import(pos(getLeftSpan(), getRightSpan()), Import.PACKAGE,
				QName.make(PackageOrTypeName.toString())));
	}
	...
	// Production: ConditionalOrExpression ::= ConditionalOrExpression '||'
	// ConditionalAndExpression
	void rule_ConditionalOrExpression1(Object _ConditionalOrExpression,
			Object _ConditionalAndExpression) {
		Expr ConditionalOrExpression = (Expr) _ConditionalOrExpression;
		Expr ConditionalAndExpression = (Expr) _ConditionalAndExpression;
		setResult(nf.Binary(pos(), ConditionalOrExpression, Binary.COND_OR,
				ConditionalAndExpression));
	}

	// Production: LocalVariableDeclaration ::= Modifiersopt VarKeyword
	// VariableDeclarators
	void rule_LocalVariableDeclaration0(Object _Modifiersopt,
			Object _VarKeyword, Object _VariableDeclarators) {
		List<Modifier> Modifiersopt = (List<Modifier>) _Modifiersopt;
		List<FlagsNode> VarKeyword = (List<FlagsNode>) _VarKeyword;
		List<Object[]> VariableDeclarators = (List<Object[]>) _VariableDeclarators;
		List<Node> modifiers = checkVariableModifiers(Modifiersopt);
		FlagsNode fn = VarKeyword == null ? extractFlags(modifiers, Flags.FINAL)
				: extractFlags(modifiers, VarKeyword);
		List<LocalDecl> l = new TypedList<LocalDecl>(
				new LinkedList<LocalDecl>(), LocalDecl.class, false);
		for (Object[] o : VariableDeclarators) {
			...
			FlagsNode efn = extractFlags(modifiers, Flags.FINAL); // exploded
																	// vars are
																	// always
																	// final
			for (Id id : exploded) {
				TypeNode tni = init == null ? nf.CanonicalTypeNode(compilerGen,
						ts.Int()) : // we infer the type of the exploded
									// components, however if there is no init,
									// then we just assume Int to avoid
									// cascading errors.
				...
				l.add(nf.LocalDecl(
						...
						tni,
						id,
						init != null ? nf.ClosureCall(compilerGen, nf.Local(
								compilerGen, name), Collections
								.<Expr> singletonList(nf.IntLit(compilerGen,
										IntLit.INT, index))) : null));
				index++;
			}
		}
		setResult(l);
	...
	// Production: LocalVariableDeclaration ::= Modifiersopt
	// VariableDeclaratorsWithType
	void rule_LocalVariableDeclaration1(Object _Modifiersopt,
			Object _VariableDeclaratorsWithType) {
		rule_LocalVariableDeclaration0(_Modifiersopt, null,
				_VariableDeclaratorsWithType);
	}

	// Production: LocalVariableDeclaration ::= Modifiersopt VarKeyword
	// FormalDeclarators
	void rule_LocalVariableDeclaration2(Object _Modifiersopt,
			Object _VarKeyword, Object _FormalDeclarators) {
		rule_LocalVariableDeclaration0(_Modifiersopt, _VarKeyword,
				_FormalDeclarators);
	}

	// Production: InterfaceMemberDeclarationsopt ::= %Empty
	void rule_InterfaceMemberDeclarationsopt0() {
		setResult(new TypedList<ClassMember>(new LinkedList<ClassMember>(),
				ClassMember.class, false));
	}

	// Production: InterfaceTypeList ::= Type
	void rule_InterfaceTypeList0(Object _Type) {
		TypeNode Type = (TypeNode) _Type;
		List<TypeNode> l = new TypedList<TypeNode>(new LinkedList<TypeNode>(),
	...
	void rule_PackageName0(Object _PackageName) {
		...
		setResult(new ParsedName(nf, ts, pos(getLeftSpan(), getRightSpan()),
				PackageName, nf.Id(pos(getRightSpan()), "*")));
	}
	...
	// ShiftExpression
	void rule_RelationalExpression3(Object _RelationalExpression,
			Object _ShiftExpression) {
		Expr RelationalExpression = (Expr) _RelationalExpression;
		Expr ShiftExpression = (Expr) _ShiftExpression;
		setResult(nf.Binary(pos(), RelationalExpression, Binary.LT,
				ShiftExpression));
	}

	// Production: RelationalExpression ::= RelationalExpression '>'
	// ShiftExpression
	void rule_RelationalExpression4(Object _RelationalExpression,
			Object _ShiftExpression) {
		Expr RelationalExpression = (Expr) _RelationalExpression;
		Expr ShiftExpression = (Expr) _ShiftExpression;
		setResult(nf.Binary(pos(), RelationalExpression, Binary.GT,
				ShiftExpression));
	}

	// Production: RelationalExpression ::= RelationalExpression '<='
	// ShiftExpression
	void rule_RelationalExpression5(Object _RelationalExpression,
			Object _ShiftExpression) {
		Expr RelationalExpression = (Expr) _RelationalExpression;
		Expr ShiftExpression = (Expr) _ShiftExpression;
		setResult(nf.Binary(pos(), RelationalExpression, Binary.LE,
				ShiftExpression));
	}

	// Production: RelationalExpression ::= RelationalExpression '>='
	// ShiftExpression
	void rule_RelationalExpression6(Object _RelationalExpression,
			Object _ShiftExpression) {
		Expr RelationalExpression = (Expr) _RelationalExpression;
		Expr ShiftExpression = (Expr) _ShiftExpression;
		setResult(nf.Binary(pos(), RelationalExpression, Binary.GE,
				ShiftExpression));
	}
	...
	void rule_BlockInteriorStatement1(Object _ClassDeclaration) {
		...
		List<Stmt> l = new TypedList<Stmt>(new LinkedList<Stmt>(), Stmt.class,
				false);
	...
	void rule_UnaryExpression1(Object _Annotations,
			Object _UnannotatedUnaryExpression) {
	...
	// Production: ExclusiveOrExpression ::= ExclusiveOrExpression '^'
	// AndExpression
	void rule_ExclusiveOrExpression1(Object _ExclusiveOrExpression,
			Object _AndExpression) {
		Expr ExclusiveOrExpression = (Expr) _ExclusiveOrExpression;
		Expr AndExpression = (Expr) _AndExpression;
		setResult(nf.Binary(pos(), ExclusiveOrExpression, Binary.BIT_XOR,
				AndExpression));
	}
	...
	// Production: AdditiveExpression ::= AdditiveExpression '+'
	// MultiplicativeExpression
	void rule_AdditiveExpression1(Object _AdditiveExpression,
			Object _MultiplicativeExpression) {
		Expr AdditiveExpression = (Expr) _AdditiveExpression;
		Expr MultiplicativeExpression = (Expr) _MultiplicativeExpression;
		setResult(nf.Binary(pos(), AdditiveExpression, Binary.ADD,
				MultiplicativeExpression));
	}

	// Production: AdditiveExpression ::= AdditiveExpression '-'
	// MultiplicativeExpression
	void rule_AdditiveExpression2(Object _AdditiveExpression,
			Object _MultiplicativeExpression) {
		Expr AdditiveExpression = (Expr) _AdditiveExpression;
		Expr MultiplicativeExpression = (Expr) _MultiplicativeExpression;
		setResult(nf.Binary(pos(), AdditiveExpression, Binary.SUB,
				MultiplicativeExpression));
	}

	// Production: AssignPropertyCall ::= property TypeArgumentsopt '('
	// ArgumentListopt ')' ';'
	void rule_AssignPropertyCall0(Object _TypeArgumentsopt,
			Object _ArgumentListopt) {
		List<TypeNode> TypeArgumentsopt = (List<TypeNode>) _TypeArgumentsopt;
		List<Expr> ArgumentListopt = (List<Expr>) _ArgumentListopt;
		setResult(nf.AssignPropertyCall(pos(), TypeArgumentsopt,
				ArgumentListopt));
	}
	...
	void rule_ClosureBody0(Object _ConditionalExpression) {
		...
		setResult(nf.Block(pos(),
				nf.X10Return(pos(), ConditionalExpression, true)));
	}

	// Production: ClosureBody ::= Annotationsopt '{' BlockStatementsopt
	// LastExpression '}'
	void rule_ClosureBody1(Object _Annotationsopt, Object _BlockStatementsopt,
			Object _LastExpression) {
		List<AnnotationNode> Annotationsopt = (List<AnnotationNode>) _Annotationsopt;
	...
	// RangeExpression
	void rule_MultiplicativeExpression1(Object _MultiplicativeExpression,
			Object _RangeExpression) {
		Expr MultiplicativeExpression = (Expr) _MultiplicativeExpression;
		Expr RangeExpression = (Expr) _RangeExpression;
		setResult(nf.Binary(pos(), MultiplicativeExpression, Binary.MUL,
				RangeExpression));
	}

	// Production: MultiplicativeExpression ::= MultiplicativeExpression '/'
	// RangeExpression
	void rule_MultiplicativeExpression2(Object _MultiplicativeExpression,
			Object _RangeExpression) {
		Expr MultiplicativeExpression = (Expr) _MultiplicativeExpression;
		Expr RangeExpression = (Expr) _RangeExpression;
		setResult(nf.Binary(pos(), MultiplicativeExpression, Binary.DIV,
				RangeExpression));
	}

	// Production: MultiplicativeExpression ::= MultiplicativeExpression '%'
	// RangeExpression
	void rule_MultiplicativeExpression3(Object _MultiplicativeExpression,
			Object _RangeExpression) {
		Expr MultiplicativeExpression = (Expr) _MultiplicativeExpression;
		Expr RangeExpression = (Expr) _RangeExpression;
		setResult(nf.Binary(pos(), MultiplicativeExpression, Binary.MOD,
				RangeExpression));
	}

	// Production: MultiplicativeExpression ::= MultiplicativeExpression '**'
	// RangeExpression
	void rule_MultiplicativeExpression4(Object _MultiplicativeExpression,
			Object _RangeExpression) {
		Expr MultiplicativeExpression = (Expr) _MultiplicativeExpression;
		Expr RangeExpression = (Expr) _RangeExpression;
		setResult(nf.Binary(pos(), MultiplicativeExpression, Binary.STARSTAR,
				RangeExpression));
	}
	...
	void rule_FormalParameterList0(Object _FormalParameter) {
		...
		List<Formal> l = new TypedList<Formal>(new LinkedList<Formal>(),
				Formal.class, false);
		l.add(FormalParameter);
		setResult(l);
	}

	// Production: FormalParameterList ::= FormalParameterList ','
	// FormalParameter
	void rule_FormalParameterList1(Object _FormalParameterList,
			Object _FormalParameter) {
		List<Formal> FormalParameterList = (List<Formal>) _FormalParameterList;
		X10Formal FormalParameter = (X10Formal) _FormalParameter;
		FormalParameterList.add(FormalParameter);
	}

	// Production: SwitchBlock ::= '{' SwitchBlockStatementGroupsopt
	// SwitchLabelsopt '}'
	void rule_SwitchBlock0(Object _SwitchBlockStatementGroupsopt,
			Object _SwitchLabelsopt) {
	...
	// Production: UnannotatedUnaryExpression ::= '+'
	...
	// Production: VariableDeclarator ::= Identifier HasResultTypeopt '='
	// VariableInitializer
	void rule_VariableDeclarator0(Object _Identifier, Object _HasResultTypeopt,
			Object _VariableInitializer) {
		Id Identifier = (Id) _Identifier;
		TypeNode HasResultTypeopt = (TypeNode) _HasResultTypeopt;
		Expr VariableInitializer = (Expr) _VariableInitializer;
		setResult(new Object[] { pos(), Identifier,
				Collections.<Id> emptyList(), null, HasResultTypeopt,
				VariableInitializer });
	}

	// Production: VariableDeclarator ::= '[' IdentifierList ']'
	// HasResultTypeopt '=' VariableInitializer
	void rule_VariableDeclarator1(Object _IdentifierList,
			Object _HasResultTypeopt, Object _VariableInitializer) {
		List<Id> IdentifierList = (List<Id>) _IdentifierList;
		TypeNode HasResultTypeopt = (TypeNode) _HasResultTypeopt;
		Expr VariableInitializer = (Expr) _VariableInitializer;
		setResult(new Object[] { pos(), null, IdentifierList, null,
	...
	// Production: VariableDeclarator ::= Identifier '[' IdentifierList ']'
	// HasResultTypeopt '=' VariableInitializer
	void rule_VariableDeclarator2(Object _Identifier, Object _IdentifierList,
			Object _HasResultTypeopt, Object _VariableInitializer) {
		Id Identifier = (Id) _Identifier;
		List<Id> IdentifierList = (List<Id>) _IdentifierList;
		TypeNode HasResultTypeopt = (TypeNode) _HasResultTypeopt;
	...
	void rule_TypeParamWithVarianceList0(Object _TypeParameter) {
		...
		List<TypeParamNode> l = new TypedList<TypeParamNode>(
				new LinkedList<TypeParamNode>(), TypeParamNode.class, false);
		l.add(TypeParameter);
		setResult(l);
	}
	...
	// Production: TypeParamWithVarianceList ::= TypeParamWithVarianceList ','
	// TypeParameter
	void rule_TypeParamWithVarianceList2(Object _TypeParamWithVarianceList,
			Object _TypeParameter) {
	...
	void rule_TypeParamWithVarianceList3(Object _TypeParamWithVarianceList,
			Object _OBSOLETE_TypeParamWithVariance) {
		List<TypeParamNode> TypeParamWithVarianceList = (List<TypeParamNode>) _TypeParamWithVarianceList;
		TypeParamNode OBSOLETE_TypeParamWithVariance = (TypeParamNode) _OBSOLETE_TypeParamWithVariance;
	...
	void rule_Interfacesopt0() {
		setResult(new TypedList<TypeNode>(new LinkedList<TypeNode>(),
				TypeNode.class, false));
	}

	// Production: ConditionalExpression ::= ConditionalOrExpression '?'
	// Expression ':' ConditionalExpression
	void rule_ConditionalExpression4(Object _ConditionalOrExpression,
			Object _Expression, Object _ConditionalExpression) {
		Expr ConditionalOrExpression = (Expr) _ConditionalOrExpression;
		Expr Expression = (Expr) _Expression;
		Expr ConditionalExpression = (Expr) _ConditionalExpression;
		setResult(nf.Conditional(pos(), ConditionalOrExpression, Expression,
	...
	void rule_VariableDeclarators0(Object _VariableDeclarator) {
		Object[] VariableDeclarator = (Object[]) _VariableDeclarator;
		List<Object[]> l = new TypedList<Object[]>(new LinkedList<Object[]>(),
				Object[].class, false);
		l.add(VariableDeclarator);
		setResult(l);
	}

	// Production: VariableDeclarators ::= VariableDeclarators ','
	// VariableDeclarator
	void rule_VariableDeclarators1(Object _VariableDeclarators,
			Object _VariableDeclarator) {
		List<Object[]> VariableDeclarators = (List<Object[]>) _VariableDeclarators;
		Object[] VariableDeclarator = (Object[]) _VariableDeclarator;
	...
	void rule_AtCaptureDeclarators0(Object _AtCaptureDeclarator) {
		...
		List<Node> l = new TypedList<Node>(new LinkedList<Node>(), Node.class,
				false);
		l.add(AtCaptureDeclarator);
		setResult(l);
	}

	// Production: AtCaptureDeclarators ::= AtCaptureDeclarators ','
	// AtCaptureDeclarator
	void rule_AtCaptureDeclarators1(Object _AtCaptureDeclarators,
			Object _AtCaptureDeclarator) {
	...
	void rule_BlockStatements1(Object _BlockStatements,
			Object _BlockInteriorStatement) {
	...
	void rule_TypeParameterList1(Object _TypeParameterList,
			Object _TypeParameter) {
	...
	void rule_OBSOLETE_TypeParamWithVariance0(Object _TypeParameter) {
		...
		setResult(TypeParameter.variance(ParameterType.Variance.COVARIANT)
				.position(pos()));
	}
	...
	void rule_OBSOLETE_TypeParamWithVariance1(Object _TypeParameter) {
		...
		setResult(TypeParameter.variance(ParameterType.Variance.CONTRAVARIANT)
				.position(pos()));
	}
	...
	// Production: VariableDeclaratorsWithType ::= VariableDeclaratorsWithType
	// ',' VariableDeclaratorWithType
	void rule_VariableDeclaratorsWithType1(Object _VariableDeclaratorsWithType,
			Object _VariableDeclaratorWithType) {
	...
	// Production: OBSOLETE_MethodSelection ::= MethodName '.' '('
	// FormalParameterListopt ')'
	void rule_OBSOLETE_MethodSelection0(Object _MethodName,
			Object _FormalParameterListopt) {
		noMethodSelection();
	}

	// Production: OBSOLETE_MethodSelection ::= Primary '.' Identifier '.' '('
	// FormalParameterListopt ')'
	void rule_OBSOLETE_MethodSelection1(Object _Primary, Object _Identifier,
			Object _FormalParameterListopt) {
		noMethodSelection();
	}

	// Production: OBSOLETE_MethodSelection ::= super '.' Identifier '.' '('
	// FormalParameterListopt ')'
	void rule_OBSOLETE_MethodSelection2(Object _Identifier,
			Object _FormalParameterListopt) {
		noMethodSelection();
	}

	// Production: OBSOLETE_MethodSelection ::= ClassName '.' super '.'
	// Identifier '.' '(' FormalParameterListopt ')'
	void rule_OBSOLETE_MethodSelection3(Object _ClassName, Object _Identifier,
			Object _FormalParameterListopt) {
		noMethodSelection();
	}
	...
	private void noMethodSelection() {
		syntaxError(
				"Method selection syntax is no longer supported. Use closure syntax instead.",
				pos()); // see XTENLANG-2512
	}

	// Production: FunctionType ::= TypeParametersopt '(' FormalParameterListopt
	// ')' WhereClauseopt OBSOLETE_Offersopt '=>' Type
	void rule_FunctionType0(Object _TypeParametersopt,
			Object _FormalParameterListopt, Object _WhereClauseopt,
			Object _OBSOLETE_Offersopt, Object _Type) {
		...
		TypeNode OBSOLETE_Offersopt = (TypeNode) _OBSOLETE_Offersopt;
		TypeNode Type = (TypeNode) _Type;
		setResult(nf.FunctionTypeNode(pos(), TypeParametersopt,
				FormalParameterListopt, WhereClauseopt, Type,
				OBSOLETE_Offersopt));
	}
	...
	// Production: ConstraintConjunction ::= ConstraintConjunction ','
	// Expression
	void rule_ConstraintConjunction1(Object _ConstraintConjunction,
			Object _Expression) {
	...
	void rule_EqualityExpression1(Object _EqualityExpression,
			Object _RelationalExpression) {
		Expr EqualityExpression = (Expr) _EqualityExpression;
		Expr RelationalExpression = (Expr) _RelationalExpression;
		setResult(nf.Binary(pos(), EqualityExpression, Binary.EQ,
				RelationalExpression));
	}

	// Production: EqualityExpression ::= EqualityExpression '!='
	// RelationalExpression
	void rule_EqualityExpression2(Object _EqualityExpression,
			Object _RelationalExpression) {
		Expr EqualityExpression = (Expr) _EqualityExpression;
		Expr RelationalExpression = (Expr) _RelationalExpression;
		setResult(nf.Binary(pos(), EqualityExpression, Binary.NE,
				RelationalExpression));
	}
	...
	// Production: EqualityExpression ::= EqualityExpression '~'
	// RelationalExpression
	void rule_EqualityExpression4(Object _EqualityExpression,
			Object _RelationalExpression) {
		Expr EqualityExpression = (Expr) _EqualityExpression;
		Expr RelationalExpression = (Expr) _RelationalExpression;
		setResult(nf.Binary(pos(), EqualityExpression, Binary.TWIDDLE,
				RelationalExpression));
	}

	// Production: EqualityExpression ::= EqualityExpression '!~'
	// RelationalExpression
	void rule_EqualityExpression5(Object _EqualityExpression,
			Object _RelationalExpression) {
		Expr EqualityExpression = (Expr) _EqualityExpression;
		Expr RelationalExpression = (Expr) _RelationalExpression;
		setResult(nf.Binary(pos(), EqualityExpression, Binary.NTWIDDLE,
				RelationalExpression));
	}
	...
	void rule_ArgumentList0(Object _Expression) {
		Expr Expression = (Expr) _Expression;
		List<Expr> l = new TypedList<Expr>(new LinkedList<Expr>(), Expr.class,
				false);
		l.add(Expression);
		setResult(l);
	}
	...
	void rule_FormalParametersopt0() {
		setResult(new TypedList<Formal>(new LinkedList<Formal>(), Formal.class,
				false));
	}
	...
	void rule_FormalDeclarators1(Object _FormalDeclarators,
			Object _FormalDeclarator) {
	...
	void rule_SingleTypeImportDeclaration0(Object _TypeName) {
		...
		setResult(nf.Import(pos(getLeftSpan(), getRightSpan()), Import.CLASS,
				QName.make(TypeName.toString())));
	}
	...
	void rule_RangeExpression1(Object _RangeExpression, Object _UnaryExpression) {
		Expr RangeExpression = (Expr) _RangeExpression;
		Expr UnaryExpression = (Expr) _UnaryExpression;
		Expr regionCall = nf.Binary(pos(), RangeExpression, Binary.DOT_DOT,
				UnaryExpression);
	...
	void rule_ParameterizedNamedType1(Object _SimpleNamedType,
			Object _TypeArguments) {
		AmbTypeNode SimpleNamedType = (AmbTypeNode) _SimpleNamedType;
		List<TypeNode> TypeArguments = (List<TypeNode>) _TypeArguments;
		TypeNode type = nf.AmbMacroTypeNode(pos(), SimpleNamedType.prefix(),
				SimpleNamedType.name(), TypeArguments, new TypedList<Expr>(
						new LinkedList<Expr>(), Expr.class, false));
		// System.out.println("@X10SemanticRules what is the type node: " +
		// type);
		// for data-centric synchronization
		type.setFlagsNode(SimpleNamedType.getFlagsNode());
	...
	// Production: ParameterizedNamedType ::= SimpleNamedType TypeArguments
	// Arguments
	void rule_ParameterizedNamedType2(Object _SimpleNamedType,
			Object _TypeArguments, Object _Arguments) {
		AmbTypeNode SimpleNamedType = (AmbTypeNode) _SimpleNamedType;
		List<TypeNode> TypeArguments = (List<TypeNode>) _TypeArguments;
		List<Expr> Arguments = (List<Expr>) _Arguments;
		TypeNode type = nf.AmbMacroTypeNode(pos(), SimpleNamedType.prefix(),
	...
	void rule_DepNamedType0(Object _SimpleNamedType, Object _DepParameters) {
		...
		TypeNode type = nf.AmbDepTypeNode(pos(), SimpleNamedType.prefix(),
				SimpleNamedType.name(), new TypedList<TypeNode>(
						new LinkedList<TypeNode>(), TypeNode.class, false),
	...
	void rule_DepNamedType1(Object _ParameterizedNamedType,
			Object _DepParameters) {
		AmbMacroTypeNode ParameterizedNamedType = (AmbMacroTypeNode) _ParameterizedNamedType;
		DepParameterExpr DepParameters = (DepParameterExpr) _DepParameters;
		TypeNode type = nf.AmbDepTypeNode(pos(), ParameterizedNamedType,
				DepParameters);
		setResult(type);
	}
	...
	void rule_ClassMemberDeclaration1(Object _ConstructorDeclaration) {
		...
		List<ClassMember> l = new TypedList<ClassMember>(
				new LinkedList<ClassMember>(), ClassMember.class, false);
	...
	// Production: NormalClassDeclaration ::= Modifiersopt class Identifier
	// TypeParamsWithVarianceopt Propertiesopt WhereClauseopt Superopt
	// Interfacesopt ClassBody
	void rule_NormalClassDeclaration0(Object _Modifiersopt, Object _Identifier,
			Object _TypeParamsWithVarianceopt, Object _Propertiesopt,
			Object _WhereClauseopt, Object _Superopt, Object _Interfacesopt,
			Object _ClassBody) {
		List<Modifier> Modifiersopt = (List<Modifier>) _Modifiersopt;
		Id Identifier = (Id) _Identifier;
		...
		ClassDecl cd = nf.X10ClassDecl(pos(), f, Identifier, TypeParametersopt,
	...
	void rule_SimpleNamedType2(Object _ParameterizedNamedType,
			Object _Identifier) {
	...
	TypeNode explodedType(Position p) {
		return nf.UnknownTypeNode(p);// exploded formals/locals are either Int
										// or T (when exploding Array[T]).
	...
	List<Formal> createExplodedFormals(List<Id> exploded) {
		...
		for (Id id : exploded) {
			...
			explodedFormals.add(nf.Formal(id.position(),
					nf.FlagsNode(id.position(), Flags.FINAL),
					explodedType(id.position()), id));
		}
	...
	void rule_LoopIndex0(Object _Modifiersopt, Object _LoopIndexDeclarator) {
		...
		f = (Formal) ((X10Ext) f.ext())
				.annotations(extractAnnotations(modifiers));
		setResult(f);
	}

	// Production: LoopIndex ::= Modifiersopt VarKeyword LoopIndexDeclarator
	void rule_LoopIndex1(Object _Modifiersopt, Object _VarKeyword,
			Object _LoopIndexDeclarator) {
		List<Modifier> Modifiersopt = (List<Modifier>) _Modifiersopt;
		List<FlagsNode> VarKeyword = (List<FlagsNode>) _VarKeyword;
		Object[] LoopIndexDeclarator = (Object[]) _LoopIndexDeclarator;
		List<Node> modifiers = checkVariableModifiers(Modifiersopt);
	...
	void rule_Literal5() {
		...
		setResult(nf.FloatLit(pos(), FloatLit.DOUBLE, a.getValue()
				.doubleValue()));
	}
	...
	void rule_Literal6(Object _BooleanLiteral) {
		...
		setResult(nf
				.BooleanLit(pos(), BooleanLiteral.getValue().booleanValue()));
	}
	...
	void rule_TypeName0(Object _TypeName) {
		ParsedName TypeName = (ParsedName) _TypeName;
		setResult(new ParsedName(nf, ts, pos(getLeftSpan(), getRightSpan()),
				TypeName, nf.Id(pos(getRightSpan()), "*")));
	}
	...
	// #TODO this method is the old fashion, which requires programmers
	// to write the syntax like: var a:(atomicplus Type) = new (atomicplus
	// Type)();
	// , which seems to be ugly. The current implementation uses the below
	// method
	...
	void rule_TypeName4(Object _linkedModifier, Object _TypeName) {
		...
		// System.out.println("@SemanticRules, the modifier is: " + fm.flag());
		// get the type name
		ParsedName TypeName = (ParsedName) _TypeName;
		// extract the flag nodes
		List<Node> modifiers = checkTypeModifiers(Collections
				.<Modifier> singletonList(modifier));
		FlagsNode fn = extractFlags(modifiers, Flags.TYPE_FLAG);
	...
	// Production: InterfaceDeclaration ::= Modifiersopt interface Identifier
	// TypeParamsWithVarianceopt Propertiesopt WhereClauseopt
	// ExtendsInterfacesopt InterfaceBody
	void rule_InterfaceDeclaration0(Object _Modifiersopt, Object _Identifier,
			Object _TypeParamsWithVarianceopt, Object _Propertiesopt,
			Object _WhereClauseopt, Object _ExtendsInterfacesopt,
			Object _InterfaceBody) {
		List<Modifier> Modifiersopt = (List<Modifier>) _Modifiersopt;
		Id Identifier = (Id) _Identifier;
		List<TypeParamNode> TypeParamsWithVarianceopt = (List<TypeParamNode>) _TypeParamsWithVarianceopt;
		List<PropertyDecl> Propertiesopt = (List<PropertyDecl>) _Propertiesopt;
		...
		ClassDecl cd = nf.X10ClassDecl(pos(), fn, Identifier,
				TypeParametersopt, props, ci, null, ExtendsInterfacesopt,
				InterfaceBody);
		cd = (ClassDecl) ((X10Ext) cd.ext())
				.annotations(extractAnnotations(modifiers));
		setResult(cd);
	}
	...
	void rule_Propertiesopt0() {
		setResult(new TypedList<PropertyDecl>(new LinkedList<PropertyDecl>(),
				PropertyDecl.class, false));
	}
	...
	void rule_FieldAccess2(Object _ErrorClassNameSuperPrefix) {
		...
		setResult(nf.Field(pos(getRightSpan()),
				nf.Super(super_pos, ClassName.toType()),
				nf.Id(identifier.getPosition(), identifier.getIdentifier())));
	}
	...
	void rule_FieldAccess5(Object _ClassName, Object _Identifier) {
		ParsedName ClassName = (ParsedName) _ClassName;
		Id Identifier = (Id) _Identifier;
		setResult(nf.Field(pos(), nf.Super(
				pos(getRhsFirstTokenIndex(1), getRhsLastTokenIndex(3)),
				ClassName.toType()), Identifier));
	}
	...
	void rule_ForInit1(Object _LocalVariableDeclaration) {
		...
		List<ForInit> l = new TypedList<ForInit>(new LinkedList<ForInit>(),
				ForInit.class, false);
	...
	// Production: AtEachStatement ::= ateach '(' LoopIndex in Expression ')'
	// ClockedClauseopt Statement
	void rule_AtEachStatement0(Object _LoopIndex, Object _Expression,
			Object _ClockedClauseopt, Object _Statement) {
		...
		if (!fn.flags().isFinal()) {
			syntaxError("Enhanced ateach loop may not have var loop index. "
					+ LoopIndex, LoopIndex.position());
			fn = fn.flags(fn.flags().Final());
			LoopIndex = LoopIndex.flags(fn);
		}
		setResult(nf.AtEach(pos(), LoopIndex, Expression, ClockedClauseopt,
				Statement));
	}
	...
	void rule_AtEachStatement1(Object _Expression, Object _Statement) {
		...
		setResult(nf.AtEach(pos(), nf.X10Formal(pos(),
				nf.FlagsNode(pos(), Flags.FINAL), type, name, null, true),
				Expression, new TypedList<Expr>(new LinkedList<Expr>(),
						Expr.class, false), Statement));
	}
	...
	void rule_TypeDeclarationsopt0() {
		setResult(new TypedList<TopLevelDecl>(new LinkedList<TopLevelDecl>(),
				TopLevelDecl.class, false));
	}
	...
	// Production: ClassMemberDeclarations ::= ClassMemberDeclarations
	// ClassMemberDeclaration
	void rule_ClassMemberDeclarations1(Object _ClassMemberDeclarations,
			Object _ClassMemberDeclaration) {
	...
	// Production: InterfaceMemberDeclarations ::= InterfaceMemberDeclarations
	// InterfaceMemberDeclaration
	void rule_InterfaceMemberDeclarations1(Object _InterfaceMemberDeclarations,
			Object _InterfaceMemberDeclaration) {
	...
	// ')'
	void rule_MethodInvocation0(Object _ErrorPrimaryPrefix,
			Object _ArgumentListopt) {
		Object[] ErrorPrimaryPrefix = (Object[]) _ErrorPrimaryPrefix;
		List<Expr> ArgumentListopt = (List<Expr>) _ArgumentListopt;
		Expr Primary = (Expr) ErrorPrimaryPrefix[0];
	...
	void rule_MethodInvocation1(Object _ErrorSuperPrefix,
			Object _ArgumentListopt) {
		Object[] ErrorSuperPrefix = (Object[]) _ErrorSuperPrefix;
		List<Expr> ArgumentListopt = (List<Expr>) _ArgumentListopt;
		JPGPosition super_pos = (JPGPosition) ErrorSuperPrefix[0];
	...
	// Production: MethodInvocation ::= ErrorClassNameSuperPrefix '('
	// ArgumentListopt ')'
	void rule_MethodInvocation2(Object _ErrorClassNameSuperPrefix,
			Object _ArgumentListopt) {
		Object[] ErrorClassNameSuperPrefix = (Object[]) _ErrorClassNameSuperPrefix;
		List<Expr> ArgumentListopt = (List<Expr>) _ArgumentListopt;
		ParsedName ClassName = (ParsedName) ErrorClassNameSuperPrefix[0];
	...
	// Production: MethodInvocation ::= MethodName TypeArgumentsopt '('
	// ArgumentListopt ')'
	void rule_MethodInvocation3(Object _MethodName, Object _TypeArgumentsopt,
			Object _ArgumentListopt) {
		ParsedName MethodName = (ParsedName) _MethodName;
		List<TypeNode> TypeArgumentsopt = (List<TypeNode>) _TypeArgumentsopt;
		List<Expr> ArgumentListopt = (List<Expr>) _ArgumentListopt;
		setResult(nf.X10Call(pos(), MethodName.prefix == null ? null
				: MethodName.prefix.toReceiver(), MethodName.name,
				TypeArgumentsopt, ArgumentListopt));
	}

	// Production: MethodInvocation ::= Primary '.' Identifier TypeArgumentsopt
	// '(' ArgumentListopt ')'
	void rule_MethodInvocation4(Object _Primary, Object _Identifier,
			Object _TypeArgumentsopt, Object _ArgumentListopt) {
		Expr Primary = (Expr) _Primary;
		Id Identifier = (Id) _Identifier;
		List<TypeNode> TypeArgumentsopt = (List<TypeNode>) _TypeArgumentsopt;
		List<Expr> ArgumentListopt = (List<Expr>) _ArgumentListopt;
		setResult(nf.X10Call(pos(), Primary, Identifier, TypeArgumentsopt,
				ArgumentListopt));
	}

	// Production: MethodInvocation ::= super '.' Identifier TypeArgumentsopt
	// '(' ArgumentListopt ')'
	void rule_MethodInvocation5(Object _Identifier, Object _TypeArgumentsopt,
			Object _ArgumentListopt) {
		Id Identifier = (Id) _Identifier;
		List<TypeNode> TypeArgumentsopt = (List<TypeNode>) _TypeArgumentsopt;
		List<Expr> ArgumentListopt = (List<Expr>) _ArgumentListopt;
		setResult(nf.X10Call(pos(), nf.Super(pos(getLeftSpan())), Identifier,
				TypeArgumentsopt, ArgumentListopt));
	}

	// Production: MethodInvocation ::= ClassName '.' super '.' Identifier
	// TypeArgumentsopt '(' ArgumentListopt ')'
	void rule_MethodInvocation6(Object _ClassName, Object _Identifier,
			Object _TypeArgumentsopt, Object _ArgumentListopt) {
		ParsedName ClassName = (ParsedName) _ClassName;
		Id Identifier = (Id) _Identifier;
		List<TypeNode> TypeArgumentsopt = (List<TypeNode>) _TypeArgumentsopt;
		List<Expr> ArgumentListopt = (List<Expr>) _ArgumentListopt;
		setResult(nf.X10Call(pos(), nf.Super(
				pos(getRhsFirstTokenIndex(1), getRhsLastTokenIndex(3)),
				ClassName.toType()), Identifier, TypeArgumentsopt,
				ArgumentListopt));
	}

	// Production: MethodInvocation ::= Primary TypeArgumentsopt '('
	// ArgumentListopt ')'
	void rule_MethodInvocation7(Object _Primary, Object _TypeArgumentsopt,
			Object _ArgumentListopt) {
		Expr Primary = (Expr) _Primary;
		List<TypeNode> TypeArgumentsopt = (List<TypeNode>) _TypeArgumentsopt;
		List<Expr> ArgumentListopt = (List<Expr>) _ArgumentListopt;
		if (Primary instanceof Field) {
			...
			setResult(nf.X10Call(pos(), f.target(), f.name(), TypeArgumentsopt,
					ArgumentListopt));
		} else if (Primary instanceof AmbExpr) {
			AmbExpr f = (AmbExpr) Primary;
			setResult(nf.X10Call(pos(), null, f.name(), TypeArgumentsopt,
					ArgumentListopt));
		} else {
			setResult(nf.ClosureCall(pos(), Primary, TypeArgumentsopt,
					ArgumentListopt));
		}
	}

	// Production: MethodInvocation ::= OperatorPrefix TypeArgumentsopt '('
	// ArgumentListopt ')'
	void rule_MethodInvocation8(Object _OperatorPrefix,
			Object _TypeArgumentsopt, Object _ArgumentListopt) {
		...
		if (OperatorPrefix instanceof Field) {
			...
			throw new InternalCompilerError("Invalid operator prefix",
					OperatorPrefix.position());
		}
	}

	// Production: MethodInvocation ::= ClassName '.' operator as '[' Type ']'
	// TypeArgumentsopt '(' ArgumentListopt ')'
	void rule_OperatorPrefix25(Object _ClassName, Object _Type,
			Object _TypeArgumentsopt, Object _ArgumentListopt) {
		ParsedName ClassName = (ParsedName) _ClassName;
		TypeNode Type = (TypeNode) _Type;
		List<TypeNode> TypeArgumentsopt = (List<TypeNode>) _TypeArgumentsopt;
		...
		setResult(nf.X10ConversionCall(pos(), ClassName.toType(),
				nf.Id(pos(getRhsFirstTokenIndex(3), getRhsLastTokenIndex(5)),
						opName), Type, TypeArgumentsopt, ArgumentListopt));
	}

	// Production: MethodInvocation ::= ClassName '.' operator '[' Type ']'
	// TypeArgumentsopt '(' ArgumentListopt ')'
	void rule_OperatorPrefix26(Object _ClassName, Object _Type,
			Object _TypeArgumentsopt, Object _ArgumentListopt) {
		ParsedName ClassName = (ParsedName) _ClassName;
		TypeNode Type = (TypeNode) _Type;
		List<TypeNode> TypeArgumentsopt = (List<TypeNode>) _TypeArgumentsopt;
		...
		setResult(nf.X10ConversionCall(pos(), ClassName.toType(),
				nf.Id(pos(getRhsFirstTokenIndex(3), getRhsLastTokenIndex(4)),
						opName), Type, TypeArgumentsopt, ArgumentListopt));
	}
	...
	void rule_OperatorPrefix0(Object _BinOp) {
		...
		setResult(nf
				.Field(pos(), null, nf.Id(
						pos(getRhsFirstTokenIndex(1), getRhsLastTokenIndex(2)),
						opName)));
	}
	...
	void rule_OperatorPrefix1(Object _FullyQualifiedName, Object _BinOp) {
		...
		setResult(nf
				.Field(pos(), FullyQualifiedName.toReceiver(), nf.Id(
						pos(getRhsFirstTokenIndex(3), getRhsLastTokenIndex(4)),
						opName)));
	}
	...
	void rule_OperatorPrefix3(Object _BinOp) {
		...
		setResult(nf
				.Field(pos(), nf.Super(pos(getLeftSpan())), nf.Id(
						pos(getRhsFirstTokenIndex(3), getRhsLastTokenIndex(4)),
						opName)));
	}
	...
	void rule_OperatorPrefix4(Object _ClassName, Object _BinOp) {
		...
		setResult(nf
				.Field(pos(), nf.Super(
						pos(getRhsFirstTokenIndex(1), getRhsLastTokenIndex(3)),
						ClassName.toType()), nf.Id(
						pos(getRhsFirstTokenIndex(5), getRhsLastTokenIndex(6)),
						opName)));
	}
	...
	void rule_OperatorPrefix6(Object _FullyQualifiedName, Object _BinOp) {
		...
		setResult(nf
				.Field(pos(), FullyQualifiedName.toReceiver(), nf.Id(
						pos(getRhsFirstTokenIndex(3), getRhsLastTokenIndex(6)),
						opName)));
	}
	...
	void rule_OperatorPrefix9(Object _ClassName, Object _BinOp) {
		...
		setResult(nf
				.Field(pos(), nf.Super(
						pos(getRhsFirstTokenIndex(1), getRhsLastTokenIndex(3)),
						ClassName.toType()), nf.Id(
						pos(getRhsFirstTokenIndex(5), getRhsLastTokenIndex(8)),
						opName)));
	}
	...
	void rule_OperatorPrefix10(Object _PrefixOp) {
		...
		if (opName == null) {
			syntaxError("Cannot invoke binary operator '" + PrefixOp + "'.",
					pos());
			opName = Name.make("invalid operator");
		}
		setResult(nf
	...
	void rule_OperatorPrefix16(Object _FullyQualifiedName) {
		...
		setResult(nf
				.Field(pos(), FullyQualifiedName.toReceiver(), nf.Id(
						pos(getRhsFirstTokenIndex(3), getRhsLastTokenIndex(5)),
						opName)));
	}
	...
	void rule_OperatorPrefix19(Object _ClassName) {
		...
		setResult(nf
				.Field(pos(), nf.Super(
						pos(getRhsFirstTokenIndex(1), getRhsLastTokenIndex(3)),
						ClassName.toType()), nf.Id(
						pos(getRhsFirstTokenIndex(5), getRhsLastTokenIndex(7)),
						opName)));
	}
	...
	// Production: Modifier ::= linked data-centric synchronizations
	...
	void rule_TypeParamsWithVarianceopt0() {
		setResult(new TypedList<TypeParamNode>(new LinkedList<TypeParamNode>(),
				TypeParamNode.class, false));
	}
	...
	void rule_ClassBody0(Object _ClassMemberDeclarationsopt) {
		...
		setResult(nf.ClassBody(pos(getLeftSpan(), getRightSpan()),
	...
	void rule_ForUpdateopt0() {
		setResult(new TypedList<ForUpdate>(new LinkedList<ForUpdate>(),
				ForUpdate.class, false));
	}
	...
	void rule_AndExpression1(Object _AndExpression, Object _EqualityExpression) {
		Expr AndExpression = (Expr) _AndExpression;
		Expr EqualityExpression = (Expr) _EqualityExpression;
		setResult(nf.Binary(pos(), AndExpression, Binary.BIT_AND,
				EqualityExpression));
	}
	...
	void rule_Catchesopt0() {
		setResult(new TypedList<Catch>(new LinkedList<Catch>(), Catch.class,
				false));
	}

	// Production: MethodDeclaration ::= MethodModifiersopt def Identifier
	// TypeParametersopt FormalParameters WhereClauseopt HasResultTypeopt
	// OBSOLETE_Offersopt MethodBody
	void rule_MethodDeclaration0(Object _MethodModifiersopt,
			Object _Identifier, Object _TypeParametersopt,
			Object _FormalParameters, Object _WhereClauseopt,
			Object _HasResultTypeopt, Object _OBSOLETE_Offersopt,
			Object _MethodBody) {
		...
		Position bodyStart = MethodBody == null ? pos().endOf() : MethodBody
				.position().startOf();
		ProcedureDecl pd = nf.X10MethodDecl(
				pos(),
				extractFlags(modifiers),
				HasResultTypeopt == null ? nf.UnknownTypeNode(bodyStart
						.markCompilerGenerated()) : HasResultTypeopt,
				Identifier, TypeParametersopt, FormalParameters,
				WhereClauseopt, OBSOLETE_Offersopt, MethodBody);
		pd = (ProcedureDecl) ((X10Ext) pd.ext())
				.annotations(extractAnnotations(modifiers));
		setResult(pd);
	}

	// Production: BinaryOperatorDeclaration ::= MethodModifiersopt operator
	// TypeParametersopt '(' FormalParameter ')' BinOp '(' FormalParameter ')'
	// WhereClauseopt HasResultTypeopt OBSOLETE_Offersopt MethodBody
	void rule_MethodDeclaration1(Object _MethodModifiersopt,
			Object _TypeParametersopt, Object _fp1, Object _BinOp, Object _fp2,
			Object _WhereClauseopt, Object _HasResultTypeopt,
			Object _OBSOLETE_Offersopt, Object _MethodBody) {
		List<Modifier> MethodModifiersopt = (List<Modifier>) _MethodModifiersopt;
		List<TypeParamNode> TypeParametersopt = (List<TypeParamNode>) _TypeParametersopt;
		...
		Name opName = X10Binary_c.binaryMethodName(BinOp);
		if (opName == null) {
			syntaxError("Cannot override binary operator '" + BinOp + "'.",
					pos());
			opName = Name.make("invalid operator");
		}
		Position bodyStart = MethodBody == null ? pos().endOf() : MethodBody
				.position().startOf();
		MethodDecl md = nf.X10MethodDecl(
				pos(),
				extractFlags(modifiers),
				HasResultTypeopt == null ? nf.UnknownTypeNode(bodyStart
						.markCompilerGenerated()) : HasResultTypeopt, nf.Id(
						pos(getRhsFirstTokenIndex(7)), opName),
				TypeParametersopt, Arrays.<Formal> asList(fp1, fp2),
				WhereClauseopt, OBSOLETE_Offersopt, MethodBody);
		FlagsNode flags = md.flags();
		if (!flags.flags().isStatic()) {
			syntaxError("Binary operator with two parameters must be static.",
					md.position());
			md = md.flags(flags.flags(flags.flags().Static()));
		}
		md = (MethodDecl) ((X10Ext) md.ext())
				.annotations(extractAnnotations(modifiers));
		setResult(md);
	}

	// Production: PrefixOperatorDeclaration ::= MethodModifiersopt operator
	// TypeParametersopt PrefixOp '(' FormalParameter ')' WhereClauseopt
	// HasResultTypeopt OBSOLETE_Offersopt MethodBody
	void rule_MethodDeclaration2(Object _MethodModifiersopt,
			Object _TypeParametersopt, Object _PrefixOp, Object _fp2,
			Object _WhereClauseopt, Object _HasResultTypeopt,
			Object _OBSOLETE_Offersopt, Object _MethodBody) {
		List<Modifier> MethodModifiersopt = (List<Modifier>) _MethodModifiersopt;
		List<TypeParamNode> TypeParametersopt = (List<TypeParamNode>) _TypeParametersopt;
		...
		Name opName = X10Unary_c.unaryMethodName(PrefixOp);
		if (opName == null) {
			syntaxError("Cannot override unary operator '" + PrefixOp + "'.",
					pos());
			opName = Name.make("invalid operator");
		}
		Position bodyStart = MethodBody == null ? pos().endOf() : MethodBody
		...
		MethodDecl md = nf.X10MethodDecl(
				...
				HasResultTypeopt == null ? nf.UnknownTypeNode(bodyStart
						.markCompilerGenerated()) : HasResultTypeopt, nf.Id(
						pos(getRhsFirstTokenIndex(4)), opName),
				TypeParametersopt, Collections.<Formal> singletonList(fp2),
				WhereClauseopt, OBSOLETE_Offersopt, MethodBody);
		FlagsNode flags = md.flags();
		if (!flags.flags().isStatic()) {
			syntaxError("Unary operator with one parameter must be static.",
					md.position());
			md = md.flags(flags.flags(flags.flags().Static()));
		}
		md = (MethodDecl) ((X10Ext) md.ext())
	...
	// Production: BinaryOperatorDeclaration ::= MethodModifiersopt operator
	// TypeParametersopt this BinOp '(' FormalParameter ')' WhereClauseopt
	// HasResultTypeopt OBSOLETE_Offersopt MethodBody
	void rule_MethodDeclaration3(Object _MethodModifiersopt,
			Object _TypeParametersopt, Object _BinOp, Object _fp2,
			Object _WhereClauseopt, Object _HasResultTypeopt,
			Object _OBSOLETE_Offersopt, Object _MethodBody) {
		List<Modifier> MethodModifiersopt = (List<Modifier>) _MethodModifiersopt;
		List<TypeParamNode> TypeParametersopt = (List<TypeParamNode>) _TypeParametersopt;
		...
		MethodDecl md = nf.X10MethodDecl(
				...
				HasResultTypeopt == null ? nf.UnknownTypeNode(bodyStart
						.markCompilerGenerated()) : HasResultTypeopt, nf.Id(
						pos(getRhsFirstTokenIndex(5)), opName),
				TypeParametersopt, Collections.<Formal> singletonList(fp2),
				WhereClauseopt, OBSOLETE_Offersopt, MethodBody);
		FlagsNode flags = md.flags();
		if (flags.flags().isStatic()) {
			syntaxError(
					"Binary operator with this parameter cannot be static.",
					md.position());
	...
	// Production: BinaryOperatorDeclaration ::= MethodModifiersopt operator
	// TypeParametersopt '(' FormalParameter ')' BinOp this WhereClauseopt
	// HasResultTypeopt OBSOLETE_Offersopt MethodBody
	void rule_MethodDeclaration4(Object _MethodModifiersopt,
			Object _TypeParametersopt, Object _fp1, Object _BinOp,
			Object _WhereClauseopt, Object _HasResultTypeopt,
			Object _OBSOLETE_Offersopt, Object _MethodBody) {
		List<Modifier> MethodModifiersopt = (List<Modifier>) _MethodModifiersopt;
		List<TypeParamNode> TypeParametersopt = (List<TypeParamNode>) _TypeParametersopt;
		...
		MethodDecl md = nf.X10MethodDecl(
				...
				HasResultTypeopt == null ? nf.UnknownTypeNode(bodyStart
						.markCompilerGenerated()) : HasResultTypeopt, nf.Id(
						pos(getRhsFirstTokenIndex(7)), opName),
				TypeParametersopt, Collections.<Formal> singletonList(fp1),
				WhereClauseopt, OBSOLETE_Offersopt, MethodBody);
		FlagsNode flags = md.flags();
		if (flags.flags().isStatic()) {
			syntaxError(
	...
	// Production: PrefixOperatorDeclaration ::= MethodModifiersopt operator
	// TypeParametersopt PrefixOp this WhereClauseopt HasResultTypeopt
	// OBSOLETE_Offersopt MethodBody
	void rule_MethodDeclaration5(Object _MethodModifiersopt,
			Object _TypeParametersopt, Object _PrefixOp,
			Object _WhereClauseopt, Object _HasResultTypeopt,
			Object _OBSOLETE_Offersopt, Object _MethodBody) {
		List<Modifier> MethodModifiersopt = (List<Modifier>) _MethodModifiersopt;
		List<TypeParamNode> TypeParametersopt = (List<TypeParamNode>) _TypeParametersopt;
		...
		MethodDecl md = nf.X10MethodDecl(
				...
				HasResultTypeopt == null ? nf.UnknownTypeNode(bodyStart
						.markCompilerGenerated()) : HasResultTypeopt, nf.Id(
						pos(getRhsFirstTokenIndex(4)), opName),
				TypeParametersopt, Collections.<Formal> emptyList(),
				WhereClauseopt, OBSOLETE_Offersopt, MethodBody);
		FlagsNode flags = md.flags();
		if (flags.flags().isStatic()) {
			syntaxError("Unary operator with this parameter cannot be static.",
					md.position());
			md = md.flags(flags.flags(flags.flags().clearStatic()));
		}
		md = (MethodDecl) ((X10Ext) md.ext())
	...
	// Production: ApplyOperatorDeclaration ::= MethodModifiersopt operator this
	// TypeParametersopt FormalParameters WhereClauseopt HasResultTypeopt
	// OBSOLETE_Offersopt MethodBody
	void rule_MethodDeclaration6(Object _MethodModifiersopt,
			Object _TypeParametersopt, Object _FormalParameters,
			Object _WhereClauseopt, Object _HasResultTypeopt,
			Object _OBSOLETE_Offersopt, Object _MethodBody) {
		List<Modifier> MethodModifiersopt = (List<Modifier>) _MethodModifiersopt;
		List<TypeParamNode> TypeParametersopt = (List<TypeParamNode>) _TypeParametersopt;
		...
		MethodDecl md = nf.X10MethodDecl(
				...
				HasResultTypeopt == null ? nf.UnknownTypeNode(bodyStart
						.markCompilerGenerated()) : HasResultTypeopt, nf.Id(
						pos(), ClosureCall.APPLY), TypeParametersopt,
				FormalParameters, WhereClauseopt, OBSOLETE_Offersopt,
				MethodBody);
		FlagsNode flags = md.flags();
		if (flags.flags().isStatic()) {
	...
	// Production: SetOperatorDeclaration ::= MethodModifiersopt operator this
	// TypeParametersopt FormalParameters '=' '(' FormalParameter ')'
	// WhereClauseopt HasResultTypeopt OBSOLETE_Offersopt MethodBody
	void rule_MethodDeclaration7(Object _MethodModifiersopt,
			Object _TypeParametersopt, Object _FormalParameters, Object _fp2,
			Object _WhereClauseopt, Object _HasResultTypeopt,
			Object _OBSOLETE_Offersopt, Object _MethodBody) {
		List<Modifier> MethodModifiersopt = (List<Modifier>) _MethodModifiersopt;
		List<TypeParamNode> TypeParametersopt = (List<TypeParamNode>) _TypeParametersopt;
		...
		MethodDecl md = nf.X10MethodDecl(
				...
				HasResultTypeopt == null ? nf.UnknownTypeNode(bodyStart
						.markCompilerGenerated()) : HasResultTypeopt, nf.Id(
						pos(), SettableAssign.SET), TypeParametersopt,
				CollectionUtil.append(FormalParameters,
						Collections.singletonList(fp2)), WhereClauseopt,
				OBSOLETE_Offersopt, MethodBody);
		FlagsNode flags = md.flags();
		if (flags.flags().isStatic()) {
	...
	// Production: ExplicitConversionOperatorDeclaration ::= MethodModifiersopt
	// operator TypeParametersopt '(' FormalParameter ')' as Type WhereClauseopt
	// OBSOLETE_Offersopt MethodBody
	void rule_MethodDeclaration8(Object _MethodModifiersopt,
			Object _TypeParametersopt, Object _fp1, Object _Type,
			Object _WhereClauseopt, Object _OBSOLETE_Offersopt,
			Object _MethodBody) {
		List<Modifier> MethodModifiersopt = (List<Modifier>) _MethodModifiersopt;
		List<TypeParamNode> TypeParametersopt = (List<TypeParamNode>) _TypeParametersopt;
		X10Formal fp1 = (X10Formal) _fp1;
		...
		Block MethodBody = (Block) _MethodBody;
		List<Node> modifiers = checkMethodModifiers(MethodModifiersopt);
		MethodDecl md = nf.X10MethodDecl(pos(), extractFlags(modifiers), Type,
				nf.Id(pos(), Converter.operator_as), TypeParametersopt,
				Collections.<Formal> singletonList(fp1), WhereClauseopt,
				OBSOLETE_Offersopt, MethodBody);
		FlagsNode flags = md.flags();
		if (!flags.flags().isStatic()) {
	...
	// Production: ExplicitConversionOperatorDeclaration ::= MethodModifiersopt
	// operator TypeParametersopt '(' FormalParameter ')' as '?' WhereClauseopt
	// HasResultTypeopt OBSOLETE_Offersopt MethodBody
	void rule_MethodDeclaration9(Object _MethodModifiersopt,
			Object _TypeParametersopt, Object _fp1, Object _WhereClauseopt,
			Object _HasResultTypeopt, Object _OBSOLETE_Offersopt,
			Object _MethodBody) {
		List<Modifier> MethodModifiersopt = (List<Modifier>) _MethodModifiersopt;
		List<TypeParamNode> TypeParametersopt = (List<TypeParamNode>) _TypeParametersopt;
		...
		MethodDecl md = nf.X10MethodDecl(
				...
				HasResultTypeopt == null ? nf.UnknownTypeNode(bodyStart
						.markCompilerGenerated()) : HasResultTypeopt, nf.Id(
						pos(), Converter.operator_as), TypeParametersopt,
				Collections.<Formal> singletonList(fp1), WhereClauseopt,
				OBSOLETE_Offersopt, MethodBody);
		FlagsNode flags = md.flags();
		if (!flags.flags().isStatic()) {
	...
	// Production: ImplicitConversionOperatorDeclaration ::= MethodModifiersopt
	// operator TypeParametersopt '(' FormalParameter ')' WhereClauseopt
	// HasResultTypeopt OBSOLETE_Offersopt MethodBody
	void rule_MethodDeclaration10(Object _MethodModifiersopt,
			Object _TypeParametersopt, Object _fp1, Object _WhereClauseopt,
			Object _HasResultTypeopt, Object _OBSOLETE_Offersopt,
			Object _MethodBody) {
		List<Modifier> MethodModifiersopt = (List<Modifier>) _MethodModifiersopt;
		...
		MethodDecl md = nf.X10MethodDecl(
				...
				HasResultTypeopt == null ? nf.UnknownTypeNode(bodyStart
						.markCompilerGenerated()) : HasResultTypeopt, nf.Id(
						pos(), Converter.implicit_operator_as),
				TypeParametersopt, Collections.<Formal> singletonList(fp1),
				WhereClauseopt, OBSOLETE_Offersopt, MethodBody);
		FlagsNode flags = md.flags();
		if (!flags.flags().isStatic()) {
	...
	// Production: DepParameters ::= '{' FUTURE_ExistentialListopt
	// ConstraintConjunctionopt '}'
	void rule_DepParameters0(Object _FUTURE_ExistentialListopt,
			Object _ConstraintConjunctionopt) {
		...
		setResult(nf.DepParameterExpr(pos(), FUTURE_ExistentialListopt,
				ConstraintConjunctionopt));
	}
	...
	// Production: ExplicitConstructorInvocation ::= this TypeArgumentsopt '('
	// ArgumentListopt ')' ';'
	void rule_ExplicitConstructorInvocation0(Object _TypeArgumentsopt,
			Object _ArgumentListopt) {
		List<TypeNode> TypeArgumentsopt = (List<TypeNode>) _TypeArgumentsopt;
		List<Expr> ArgumentListopt = (List<Expr>) _ArgumentListopt;
	...
	// Production: ExplicitConstructorInvocation ::= super TypeArgumentsopt '('
	// ArgumentListopt ')' ';'
	void rule_ExplicitConstructorInvocation1(Object _TypeArgumentsopt,
			Object _ArgumentListopt) {
		List<TypeNode> TypeArgumentsopt = (List<TypeNode>) _TypeArgumentsopt;
		List<Expr> ArgumentListopt = (List<Expr>) _ArgumentListopt;
	...
	// Production: ExplicitConstructorInvocation ::= Primary '.' this
	// TypeArgumentsopt '(' ArgumentListopt ')' ';'
	void rule_ExplicitConstructorInvocation2(Object _Primary,
			Object _TypeArgumentsopt, Object _ArgumentListopt) {
		Expr Primary = (Expr) _Primary;
		List<TypeNode> TypeArgumentsopt = (List<TypeNode>) _TypeArgumentsopt;
		List<Expr> ArgumentListopt = (List<Expr>) _ArgumentListopt;
		setResult(nf.X10ThisCall(pos(), Primary, TypeArgumentsopt,
				ArgumentListopt));
	}

	// Production: ExplicitConstructorInvocation ::= Primary '.' super
	// TypeArgumentsopt '(' ArgumentListopt ')' ';'
	void rule_ExplicitConstructorInvocation3(Object _Primary,
			Object _TypeArgumentsopt, Object _ArgumentListopt) {
		Expr Primary = (Expr) _Primary;
		List<TypeNode> TypeArgumentsopt = (List<TypeNode>) _TypeArgumentsopt;
		List<Expr> ArgumentListopt = (List<Expr>) _ArgumentListopt;
		setResult(nf.X10SuperCall(pos(), Primary, TypeArgumentsopt,
				ArgumentListopt));
	}
	...
	void rule_FormalParameter1(Object _Modifiersopt, Object _VarKeyword,
			Object _FormalDeclarator) {
		List<Modifier> Modifiersopt = (List<Modifier>) _Modifiersopt;
		List<FlagsNode> VarKeyword = (List<FlagsNode>) _VarKeyword;
		Object[] FormalDeclarator = (Object[]) _FormalDeclarator;
	...
	void rule_FormalParameter2(Object _Type) {
		TypeNode Type = (TypeNode) _Type;
		Formal f;
		f = nf.X10Formal(pos().markCompilerGenerated(),
				nf.FlagsNode(pos().markCompilerGenerated(), Flags.FINAL), Type,
				nf.Id(pos().markCompilerGenerated(), Name.makeFresh("id$")),
				Collections.<Formal> emptyList(), true);
		setResult(f);
	}

	// Production: BasicForStatement ::= for '(' ForInitopt ';' Expressionopt
	// ';' ForUpdateopt ')' Statement
	void rule_BasicForStatement0(Object _ForInitopt, Object _Expressionopt,
			Object _ForUpdateopt, Object _Statement) {
		List<ForInit> ForInitopt = (List<ForInit>) _ForInitopt;
		Expr Expressionopt = (Expr) _Expressionopt;
		List<ForUpdate> ForUpdateopt = (List<ForUpdate>) _ForUpdateopt;
		Stmt Statement = (Stmt) _Statement;
		setResult(nf.For(pos(), ForInitopt, Expressionopt, ForUpdateopt,
				Statement));
	}
	...
	void rule_StatementExpressionList0(Object _StatementExpression) {
		Expr StatementExpression = (Expr) _StatementExpression;
		List<Eval> l = new TypedList<Eval>(new LinkedList<Eval>(), Eval.class,
				false);
		l.add(nf.Eval(pos(), StatementExpression));
		setResult(l);
	}

	// Production: StatementExpressionList ::= StatementExpressionList ','
	// StatementExpression
	void rule_StatementExpressionList1(Object _StatementExpressionList,
			Object _StatementExpression) {
		List<Eval> StatementExpressionList = (List<Eval>) _StatementExpressionList;
		Expr StatementExpression = (Expr) _StatementExpression;
		StatementExpressionList.add(nf.Eval(pos(), StatementExpression));
	}

	// Production: SwitchBlockStatementGroups ::= SwitchBlockStatementGroups
	// SwitchBlockStatementGroup
	void rule_SwitchBlockStatementGroups1(Object _SwitchBlockStatementGroups,
			Object _SwitchBlockStatementGroup) {
	...
	// Production: TypeDefDeclaration ::= Modifiersopt type Identifier
	// TypeParametersopt WhereClauseopt '=' Type ';'
	void rule_TypeDefDeclaration0(Object _Modifiersopt, Object _Identifier,
			Object _TypeParametersopt, Object _WhereClauseopt, Object _Type) {
		List<Modifier> Modifiersopt = (List<Modifier>) _Modifiersopt;
		Id Identifier = (Id) _Identifier;
		List<TypeParamNode> TypeParametersopt = (List<TypeParamNode>) _TypeParametersopt;
		DepParameterExpr WhereClauseopt = (DepParameterExpr) _WhereClauseopt;
		...
		TypeDecl cd = nf.TypeDecl(pos(), f, Identifier, TypeParametersopt,
				formals, WhereClauseopt, Type);
		cd = (TypeDecl) ((X10Ext) cd.ext()).annotations(annotations);
		setResult(cd);
	}

	// Production: TypeDefDeclaration ::= Modifiersopt type Identifier
	// TypeParametersopt ( FormalParameterList ) WhereClauseopt '=' Type ';'
	void rule_TypeDefDeclaration1(Object _Modifiersopt, Object _Identifier,
			Object _TypeParametersopt, Object _FormalParameterList,
			Object _WhereClauseopt, Object _Type) {
		List<Modifier> Modifiersopt = (List<Modifier>) _Modifiersopt;
		Id Identifier = (Id) _Identifier;
		List<TypeParamNode> TypeParametersopt = (List<TypeParamNode>) _TypeParametersopt;
		List<Formal> FormalParameterList = (List<Formal>) _FormalParameterList;
		...
		for (Formal v : FormalParameterList) {
			...
			if (!flags.flags().isFinal()) {
				syntaxError("Type definition parameters must be final.",
						v.position());
	...
	// Production: PropertyMethodDeclaration ::= MethodModifiersopt Identifier
	// TypeParametersopt FormalParameters WhereClauseopt HasResultTypeopt
	// MethodBody
	void rule_PropertyMethodDeclaration0(Object _MethodModifiersopt,
			Object _Identifier, Object _TypeParametersopt,
			Object _FormalParameters, Object _WhereClauseopt,
			Object _HasResultTypeopt, Object _MethodBody) {
		List<Modifier> MethodModifiersopt = (List<Modifier>) _MethodModifiersopt;
		Id Identifier = (Id) _Identifier;
		List<TypeParamNode> TypeParametersopt = (List<TypeParamNode>) _TypeParametersopt;
		List<Formal> FormalParameters = (List<Formal>) _FormalParameters;
		...
		MethodDecl md = nf.X10MethodDecl(pos(),
				...
				HasResultTypeopt == null ? nf.UnknownTypeNode(pos())
						: HasResultTypeopt, Identifier, TypeParametersopt,
				FormalParameters, WhereClauseopt, null, // offersOpt
				MethodBody);
		md = (MethodDecl) ((X10Ext) md.ext())
				.annotations(extractAnnotations(modifiers));
		setResult(md);
	...
	// Production: PropertyMethodDeclaration ::= MethodModifiersopt Identifier
	// WhereClauseopt HasResultTypeopt MethodBody
	void rule_PropertyMethodDeclaration1(Object _MethodModifiersopt,
			Object _Identifier, Object _WhereClauseopt,
			Object _HasResultTypeopt, Object _MethodBody) {
		syntaxError(
				"This syntax is no longer supported. You must supply the property method formals, and if there are none, you can use an empty parenthesis '()'.",
				pos());
		List<Modifier> MethodModifiersopt = (List<Modifier>) _MethodModifiersopt;
		Id Identifier = (Id) _Identifier;
		DepParameterExpr WhereClauseopt = (DepParameterExpr) _WhereClauseopt;
		...
		MethodDecl md = nf.X10MethodDecl(pos(),
				extractFlags(modifiers, Flags.PROPERTY),
				HasResultTypeopt == null ? nf.UnknownTypeNode(pos())
						: HasResultTypeopt, Identifier, Collections
						.<TypeParamNode> emptyList(), Collections
						.<Formal> emptyList(), WhereClauseopt, null, // offersOpt
				MethodBody);
		md = (MethodDecl) ((X10Ext) md.ext())
				.annotations(extractAnnotations(modifiers));
		setResult(md);
	...
	void rule_SwitchBlockStatementGroup0(Object _SwitchLabels,
			Object _BlockStatements) {
		List<SwitchElement> SwitchLabels = (List<SwitchElement>) _SwitchLabels;
		List<Stmt> BlockStatements = (List<Stmt>) _BlockStatements;
		List<SwitchElement> l = new TypedList<SwitchElement>(
				new LinkedList<SwitchElement>(), SwitchElement.class, false);
	...
	// Production: AtStatement ::= at ( Expression ; AtCaptureDeclaratorsopt )
	// Statement
	void rule_AtStatement1(Object _Expression, Object _AtCaptureDeclaratorsopt,
			Object _Statement) {
		Expr Expression = (Expr) _Expression;
		List<Node> AtCaptureDeclaratorsopt = (List<Node>) _AtCaptureDeclaratorsopt;
		Stmt Statement = (Stmt) _Statement;
		setResult(nf.AtStmt(pos(), Expression, AtCaptureDeclaratorsopt,
				Statement));
	}
	...
	// Production: AtStatement ::= athome ( HomeVariableList ;
	// AtCaptureDeclaratorsopt ) Statement
	void rule_AtStatement3(Object _HomeVariableList,
			Object _AtCaptureDeclaratorsopt, Object _Statement) {
		List<Expr> HomeVariableList = (List<Expr>) _HomeVariableList;
		List<Node> AtCaptureDeclaratorsopt = (List<Node>) _AtCaptureDeclaratorsopt;
		Stmt Statement = (Stmt) _Statement;
		setResult(nf.AtHomeStmt(pos(), HomeVariableList,
				AtCaptureDeclaratorsopt, Statement));
	}
	...
	// Production: AtStatement ::= athome ( * ; AtCaptureDeclaratorsopt )
	// Statement
	void rule_AtStatement5(Object _AtCaptureDeclaratorsopt, Object _Statement) {
		List<Node> AtCaptureDeclaratorsopt = (List<Node>) _AtCaptureDeclaratorsopt;
		Stmt Statement = (Stmt) _Statement;
		setResult(nf
				.AtHomeStmt(pos(), null, AtCaptureDeclaratorsopt, Statement));
	}
	...
	// Production: MethodBody ::= '=' Annotationsopt '{' BlockStatementsopt
	// LastExpression '}'
	void rule_MethodBody1(Object _Annotationsopt, Object _BlockStatementsopt,
			Object _LastExpression) {
		List<AnnotationNode> Annotationsopt = (List<AnnotationNode>) _Annotationsopt;
		List<Stmt> BlockStatementsopt = (List<Stmt>) _BlockStatementsopt;
		Stmt LastExpression = (Stmt) _LastExpression;
		...
		l.addAll(BlockStatementsopt);
		l.add(LastExpression);
		setResult((Block) ((X10Ext) nf.Block(pos(), l).ext())
	...
	// Production: FieldDeclaration ::= Modifiersopt FieldKeyword
	// FieldDeclarators ';'
	void rule_FieldDeclaration0(Object _Modifiersopt, Object _FieldKeyword,
			Object _FieldDeclarators) {
		List<Modifier> Modifiersopt = (List<Modifier>) _Modifiersopt;
		...
		for (Object[] o : FieldDeclarators) {
			...
			if (type == null)
				type = nf.UnknownTypeNode(name.position());
			Expr init = (Expr) o[4];
			FieldDecl fd = nf.FieldDecl(pos, fn, type, name, init);
			fd = (FieldDecl) ((X10Ext) fd.ext())
					.annotations(extractAnnotations(modifiers));
			fd = (FieldDecl) ((X10Ext) fd.ext())
					.setComment(comment(getRhsFirstTokenIndex(1)));
			l.add(fd);
		}
		setResult(l);
	}
	...
	void rule_FieldDeclaration1(Object _Modifiersopt, Object _FieldDeclarators) {
		rule_FieldDeclaration0(_Modifiersopt,
				Collections.singletonList(nf.FlagsNode(pos(), Flags.FINAL)),
				_FieldDeclarators);
	}
	...
	// Production: ShiftExpression ::= ShiftExpression '<<' AdditiveExpression
	void rule_ShiftExpression1(Object _ShiftExpression,
			Object _AdditiveExpression) {
		Expr ShiftExpression = (Expr) _ShiftExpression;
		Expr AdditiveExpression = (Expr) _AdditiveExpression;
		setResult(nf.Binary(pos(), ShiftExpression, Binary.SHL,
				AdditiveExpression));
	}

	// Production: ShiftExpression ::= ShiftExpression '>>' AdditiveExpression
	void rule_ShiftExpression2(Object _ShiftExpression,
			Object _AdditiveExpression) {
		Expr ShiftExpression = (Expr) _ShiftExpression;
		Expr AdditiveExpression = (Expr) _AdditiveExpression;
		setResult(nf.Binary(pos(), ShiftExpression, Binary.SHR,
				AdditiveExpression));
	}

	// Production: ShiftExpression ::= ShiftExpression '>>>' AdditiveExpression
	void rule_ShiftExpression3(Object _ShiftExpression,
			Object _AdditiveExpression) {
		Expr ShiftExpression = (Expr) _ShiftExpression;
		Expr AdditiveExpression = (Expr) _AdditiveExpression;
		setResult(nf.Binary(pos(), ShiftExpression, Binary.USHR,
				AdditiveExpression));
	}
	...
	// Production: StructDeclaration ::= Modifiersopt struct Identifier
	// TypeParamsWithVarianceopt Propertiesopt WhereClauseopt Interfacesopt
	// ClassBody
	void rule_StructDeclaration0(Object _Modifiersopt, Object _Identifier,
			Object _TypeParamsWithVarianceopt, Object _Propertiesopt,
			Object _WhereClauseopt, Object _Interfacesopt, Object _ClassBody) {
		List<Modifier> Modifiersopt = (List<Modifier>) _Modifiersopt;
		Id Identifier = (Id) _Identifier;
		List<TypeParamNode> TypeParamsWithVarianceopt = (List<TypeParamNode>) _TypeParamsWithVarianceopt;
		List<PropertyDecl> Propertiesopt = (List<PropertyDecl>) _Propertiesopt;
	...
	// Production: ConstructorBlock ::= '{' ExplicitConstructorInvocationopt
	// BlockStatementsopt '}'
	void rule_ConstructorBlock0(Object _ExplicitConstructorInvocationopt,
			Object _BlockStatementsopt) {
	...
	// Production: InclusiveOrExpression ::= InclusiveOrExpression '|'
	// ExclusiveOrExpression
	void rule_InclusiveOrExpression1(Object _InclusiveOrExpression,
			Object _ExclusiveOrExpression) {
		Expr InclusiveOrExpression = (Expr) _InclusiveOrExpression;
		Expr ExclusiveOrExpression = (Expr) _ExclusiveOrExpression;
		setResult(nf.Binary(pos(), InclusiveOrExpression, Binary.BIT_OR,
				ExclusiveOrExpression));
	}
	...
	void rule_PropertyList0(Object _Property) {
		...
		List<PropertyDecl> l = new TypedList<PropertyDecl>(
				new LinkedList<PropertyDecl>(), PropertyDecl.class, false);
		l.add(Property);
		setResult(l);
	}
	...
	// Production: ConditionalAndExpression ::= ConditionalAndExpression '&&'
	// InclusiveOrExpression
	void rule_ConditionalAndExpression1(Object _ConditionalAndExpression,
			Object _InclusiveOrExpression) {
		Expr ConditionalAndExpression = (Expr) _ConditionalAndExpression;
		Expr InclusiveOrExpression = (Expr) _InclusiveOrExpression;
		setResult(nf.Binary(pos(), ConditionalAndExpression, Binary.COND_AND,
				InclusiveOrExpression));
	}
	...
	void rule_SwitchLabels0(Object _SwitchLabel) {
		...
		List<Case> l = new TypedList<Case>(new LinkedList<Case>(), Case.class,
				false);
		l.add(SwitchLabel);
		setResult(l);
	}
	...
	void rule_ImportDeclarationsopt0() {
		setResult(new TypedList<Import>(new LinkedList<Import>(), Import.class,
				false));
	}

	// Production: IfThenElseStatement ::= if '(' Expression ')' Statement else
	// Statement
	...
	void rule_AnnotatedType0(Object _Type, Object _Annotations) {
		...
		List<AnnotationNode> Annotations = (List<AnnotationNode>) _Annotations;
		TypeNode tn = Type;
		tn = (TypeNode) ((X10Ext) tn.ext())
				.annotations((List<AnnotationNode>) Annotations);
		setResult(tn.position(pos()));
	}

	// Production: ConstructorDeclaration ::= Modifiersopt def this
	// TypeParametersopt FormalParameters WhereClauseopt HasResultTypeopt
	// OBSOLETE_Offersopt ConstructorBody
	void rule_ConstructorDeclaration0(Object _Modifiersopt,
			Object _TypeParametersopt, Object _FormalParameters,
			Object _WhereClauseopt, Object _HasResultTypeopt,
			Object _OBSOLETE_Offersopt, Object _ConstructorBody) {
		List<Modifier> Modifiersopt = (List<Modifier>) _Modifiersopt;
		List<TypeParamNode> TypeParametersopt = (List<TypeParamNode>) _TypeParametersopt;
		List<Formal> FormalParameters = (List<Formal>) _FormalParameters;
		DepParameterExpr WhereClauseopt = (DepParameterExpr) _WhereClauseopt;
		...
		ConstructorDecl cd = nf.X10ConstructorDecl(pos(),
				extractFlags(modifiers), nf.Id(pos(getRhsFirstTokenIndex(3)),
						TypeSystem.CONSTRUCTOR_NAME), HasResultTypeopt,
				TypeParametersopt, FormalParameters, WhereClauseopt,
				OBSOLETE_Offersopt, ConstructorBody);
		cd = (ConstructorDecl) ((X10Ext) cd.ext())
				.annotations(extractAnnotations(modifiers));
		setResult(cd);
	}
	...
	void rule_SwitchBlockStatementGroupsopt0() {
		setResult(new TypedList<SwitchElement>(new LinkedList<SwitchElement>(),
				SwitchElement.class, false));
	}

	// Production: Catches ::= CatchClause
	void rule_Catches0(Object _CatchClause) {
		Catch CatchClause = (Catch) _CatchClause;
		List<Catch> l = new TypedList<Catch>(new LinkedList<Catch>(),
				Catch.class, false);
		l.add(CatchClause);
		setResult(l);
	}
	...
	void rule_FieldDeclarators1(Object _FieldDeclarators,
			Object _FieldDeclarator) {
		List<Object[]> FieldDeclarators = (List<Object[]>) _FieldDeclarators;
		Object[] FieldDeclarator = (Object[]) _FieldDeclarator;
	...
	// Production: ClassInstanceCreationExpression ::= new TypeName
	// TypeArgumentsopt '(' ArgumentListopt ')' ClassBodyopt
	void rule_ClassInstanceCreationExpression0(Object _TypeName,
			Object _TypeArgumentsopt, Object _ArgumentListopt,
			Object _ClassBodyopt) {
		ParsedName TypeName = (ParsedName) _TypeName;
		List<TypeNode> TypeArgumentsopt = (List<TypeNode>) _TypeArgumentsopt;
		List<Expr> ArgumentListopt = (List<Expr>) _ArgumentListopt;
		ClassBody ClassBodyopt = (ClassBody) _ClassBodyopt;
		if (ClassBodyopt == null)
			setResult(nf.X10New(pos(), TypeName.toType(), TypeArgumentsopt,
					ArgumentListopt));
		else
			setResult(nf.X10New(pos(), TypeName.toType(), TypeArgumentsopt,
	...
	// Production: ClassInstanceCreationExpression ::= Primary '.' new
	// Identifier TypeArgumentsopt '(' ArgumentListopt ')' ClassBodyopt
	void rule_ClassInstanceCreationExpression2(Object _Primary,
			Object _Identifier, Object _TypeArgumentsopt,
			Object _ArgumentListopt, Object _ClassBodyopt) {
		Expr Primary = (Expr) _Primary;
		Id Identifier = (Id) _Identifier;
		List<TypeNode> TypeArgumentsopt = (List<TypeNode>) _TypeArgumentsopt;
		List<Expr> ArgumentListopt = (List<Expr>) _ArgumentListopt;
		...
		if (ClassBodyopt == null)
			setResult(nf.X10New(pos(), Primary, b.toType(), TypeArgumentsopt,
					ArgumentListopt));
		else
			setResult(nf.X10New(pos(), Primary, b.toType(), TypeArgumentsopt,
					ArgumentListopt, ClassBodyopt));
	...
	// Production: ClassInstanceCreationExpression ::= FullyQualifiedName '.'
	// new Identifier TypeArgumentsopt '(' ArgumentListopt ')' ClassBodyopt
	void rule_ClassInstanceCreationExpression3(Object _FullyQualifiedName,
			Object _Identifier, Object _TypeArgumentsopt,
			Object _ArgumentListopt, Object _ClassBodyopt) {
		ParsedName FullyQualifiedName = (ParsedName) _FullyQualifiedName;
		Id Identifier = (Id) _Identifier;
		...
		ParsedName b = new ParsedName(nf, ts, pos(), Identifier);
		if (ClassBodyopt == null)
			setResult(nf.X10New(pos(), FullyQualifiedName.toExpr(), b.toType(),
					TypeArgumentsopt, ArgumentListopt));
		else
			setResult(nf.X10New(pos(), FullyQualifiedName.toExpr(), b.toType(),
					TypeArgumentsopt, ArgumentListopt, ClassBodyopt));
	...
	// Production: AtExpression ::= at ( Expression ; AtCaptureDeclaratorsopt )
	// ClosureBody
	void rule_AtExpression1(Object _Expression,
			Object _AtCaptureDeclaratorsopt, Object _ClosureBody) {
		Expr Expression = (Expr) _Expression;
		List<Node> AtCaptureDeclaratorsopt = (List<Node>) _AtCaptureDeclaratorsopt;
		Block ClosureBody = (Block) _ClosureBody;
		setResult(nf.AtExpr(pos(), Expression, AtCaptureDeclaratorsopt,
				ClosureBody));
	}
	...
	// Production: AtExpression ::= athome ( HomeVariableList ;
	// AtCaptureDeclaratorsopt ) ClosureBody
	void rule_AtExpression3(Object _HomeVariableList,
			Object _AtCaptureDeclaratorsopt, Object _ClosureBody) {
		List<Expr> HomeVariableList = (List<Expr>) _HomeVariableList;
		List<Node> AtCaptureDeclaratorsopt = (List<Node>) _AtCaptureDeclaratorsopt;
		Block ClosureBody = (Block) _ClosureBody;
		setResult(nf.AtHomeExpr(pos(), HomeVariableList,
				AtCaptureDeclaratorsopt, ClosureBody));
	}
	...
	// Production: AtExpression ::= athome ( * ; AtCaptureDeclaratorsopt )
	// ClosureBody
	void rule_AtExpression5(Object _AtCaptureDeclaratorsopt, Object _ClosureBody) {
		List<Node> AtCaptureDeclaratorsopt = (List<Node>) _AtCaptureDeclaratorsopt;
		Block ClosureBody = (Block) _ClosureBody;
		setResult(nf.AtHomeExpr(pos(), null, AtCaptureDeclaratorsopt,
				ClosureBody));
	}

	// Production: CompilationUnit ::= PackageDeclarationopt TypeDeclarationsopt
	void rule_CompilationUnit0(Object _PackageDeclarationopt,
			Object _TypeDeclarationsopt) {
		...
		setResult(nf.SourceFile(pos(getLeftSpan(), getRightSpan()),
				PackageDeclarationopt, new TypedList<Import>(
						new LinkedList<Import>(), Import.class, false),
				TypeDeclarationsopt));
	}

	// Production: CompilationUnit ::= PackageDeclarationopt ImportDeclarations
	// TypeDeclarationsopt
	void rule_CompilationUnit1(Object _PackageDeclarationopt,
			Object _ImportDeclarations, Object _TypeDeclarationsopt) {
		PackageNode PackageDeclarationopt = (PackageNode) _PackageDeclarationopt;
	...
	// Production: CompilationUnit ::= ImportDeclarations PackageDeclaration
	// ImportDeclarationsopt TypeDeclarationsopt
	void rule_CompilationUnit2(Object _ImportDeclarations,
			Object _misplacedPackageDeclaration,
			Object _misplacedImportDeclarations, Object _TypeDeclarationsopt) {
		List<Import> ImportDeclarations = (List<Import>) _ImportDeclarations;
		...
		List<TopLevelDecl> TypeDeclarationsopt = (List<TopLevelDecl>) _TypeDeclarationsopt;
		syntaxError("Misplaced package declaration",
				misplacedPackageDeclaration.position());
		ImportDeclarations.addAll(misplacedImportDeclarations); // merge the two
																// import lists
		setResult(nf.SourceFile(pos(getLeftSpan(), getRightSpan()),
				misplacedPackageDeclaration, ImportDeclarations,
				TypeDeclarationsopt));
	}

	// Production: CompilationUnit ::= PackageDeclaration ImportDeclarations
	// PackageDeclaration ImportDeclarationsopt TypeDeclarationsopt
	void rule_CompilationUnit3(Object _PackageDeclaration,
			Object _ImportDeclarations, Object _misplacedPackageDeclaration,
			Object _misplacedImportDeclarations, Object _TypeDeclarationsopt) {
		...
		List<Import> misplacedImportDeclarations = (List<Import>) _misplacedImportDeclarations;
		List<TopLevelDecl> TypeDeclarationsopt = (List<TopLevelDecl>) _TypeDeclarationsopt;
		syntaxError("Misplaced package declaration, ignoring",
				misplacedPackageDeclaration.position());
		ImportDeclarations.addAll(misplacedImportDeclarations); // merge the two
																// import lists
		setResult(nf.SourceFile(pos(getLeftSpan(), getRightSpan()),
				PackageDeclaration, ImportDeclarations, TypeDeclarationsopt));
	}

	// Production: Assignment ::= LeftHandSide AssignmentOperator
	// AssignmentExpression
	void rule_Assignment0(Object _LeftHandSide, Object _AssignmentOperator,
			Object _AssignmentExpression) {
		...
		setResult(nf.Assign(pos(), LeftHandSide, AssignmentOperator,
				AssignmentExpression));
	}

	// Production: Assignment ::= ExpressionName '(' ArgumentListopt ')'
	// AssignmentOperator AssignmentExpression
	void rule_Assignment1(Object _e1, Object _ArgumentListopt,
			Object _AssignmentOperator, Object _AssignmentExpression) {
	...
	// Production: Assignment ::= Primary '(' ArgumentListopt ')'
	// AssignmentOperator AssignmentExpression
	void rule_Assignment2(Object _e1, Object _ArgumentListopt,
			Object _AssignmentOperator, Object _AssignmentExpression) {
		Expr e1 = (Expr) _e1;
		List<Expr> ArgumentListopt = (List<Expr>) _ArgumentListopt;
		Assign.Operator AssignmentOperator = (Assign.Operator) _AssignmentOperator;
	...
	void rule_MethodModifiersopt1(Object _MethodModifiersopt) {
		List<Modifier> MethodModifiersopt = (List<Modifier>) _MethodModifiersopt;
		MethodModifiersopt.add(new FlagModifier(pos(getRhsFirstTokenIndex(2)),
				FlagModifier.PROPERTY));
	}
	...
	void rule_Annotationsopt0() {
		setResult(new TypedList<AnnotationNode>(
				new LinkedList<AnnotationNode>(), AnnotationNode.class, false));
	}
	...
	void rule_LoopIndexDeclarator0(Object _Identifier, Object _HasResultTypeopt) {
		...
		TypeNode HasResultTypeopt = (TypeNode) _HasResultTypeopt;
		setResult(new Object[] { pos(), Identifier,
				Collections.<Id> emptyList(), null, HasResultTypeopt, null });
	}

	// Production: LoopIndexDeclarator ::= '[' IdentifierList ']'
	// HasResultTypeopt
	void rule_LoopIndexDeclarator1(Object _IdentifierList,
			Object _HasResultTypeopt) {
		List<Id> IdentifierList = (List<Id>) _IdentifierList;
		TypeNode HasResultTypeopt = (TypeNode) _HasResultTypeopt;
		setResult(new Object[] { pos(), null, IdentifierList, null,
				HasResultTypeopt, null });
	}

	// Production: LoopIndexDeclarator ::= Identifier '[' IdentifierList ']'
	// HasResultTypeopt
	void rule_LoopIndexDeclarator2(Object _Identifier, Object _IdentifierList,
			Object _HasResultTypeopt) {
		Id Identifier = (Id) _Identifier;
		List<Id> IdentifierList = (List<Id>) _IdentifierList;
		TypeNode HasResultTypeopt = (TypeNode) _HasResultTypeopt;
	...
	void rule_Annotations0(Object _Annotation) {
		AnnotationNode Annotation = (AnnotationNode) _Annotation;
		List<AnnotationNode> l = new TypedList<AnnotationNode>(
				new LinkedList<AnnotationNode>(), AnnotationNode.class, false);
		l.add(Annotation);
		setResult(l);
	}
	...
	void rule_ImportDeclarations1(Object _ImportDeclarations,
			Object _ImportDeclaration) {
		List<Import> ImportDeclarations = (List<Import>) _ImportDeclarations;
		Import ImportDeclaration = (Import) _ImportDeclaration;
	...
	// Production: EnhancedForStatement ::= for '(' LoopIndex in Expression ')'
	// Statement
	void rule_EnhancedForStatement0(Object _LoopIndex, Object _Expression,
			Object _Statement) {
		X10Formal LoopIndex = (X10Formal) _LoopIndex;
		Expr Expression = (Expr) _Expression;
		Stmt Statement = (Stmt) _Statement;
		FlagsNode fn = LoopIndex.flags();
		if (!fn.flags().isFinal()) {
			syntaxError("Enhanced for loop may not have var loop index. "
					+ LoopIndex, LoopIndex.position());
			fn = fn.flags(fn.flags().Final());
			LoopIndex = LoopIndex.flags(fn);
		}
		setResult(nf.ForLoop(pos(), LoopIndex, Expression, Statement));
	}
	...
	void rule_EnhancedForStatement1(Object _Expression, Object _Statement) {
		...
		Id name = nf.Id(pos(), Name.makeFresh());
		TypeNode type = nf.UnknownTypeNode(pos());
		setResult(nf.ForLoop(pos(), nf.X10Formal(pos(),
				nf.FlagsNode(pos(), Flags.FINAL), type, name, null, true),
	...
	void rule_FormalDeclarator0(Object _Identifier, Object _ResultType) {
		...
		TypeNode ResultType = (TypeNode) _ResultType;
		setResult(new Object[] { pos(), Identifier,
				Collections.<Id> emptyList(), null, ResultType, null });
	}
	...
	void rule_FormalDeclarator1(Object _IdentifierList, Object _ResultType) {
		List<Id> IdentifierList = (List<Id>) _IdentifierList;
		TypeNode ResultType = (TypeNode) _ResultType;
		setResult(new Object[] { pos(), null, IdentifierList, null, ResultType,
				null });
	}

	// Production: FormalDeclarator ::= Identifier '[' IdentifierList ']'
	// ResultType
	void rule_FormalDeclarator2(Object _Identifier, Object _IdentifierList,
			Object _ResultType) {
		Id Identifier = (Id) _Identifier;
		List<Id> IdentifierList = (List<Id>) _IdentifierList;
		TypeNode ResultType = (TypeNode) _ResultType;
	...
	void rule_ForInitopt0() {
		setResult(new TypedList<ForInit>(new LinkedList<ForInit>(),
				ForInit.class, false));
	}
	...
	void rule_FUTURE_ExistentialList0(Object _FormalParameter) {
		...
		List<Formal> l = new TypedList<Formal>(new LinkedList<Formal>(),
				Formal.class, false);
		l.add(FormalParameter.flags(nf.FlagsNode(
				Position.compilerGenerated(FormalParameter.position()),
				Flags.FINAL)));
		setResult(l);
	}

	// Production: FUTURE_ExistentialList ::= FUTURE_ExistentialList ';'
	// FormalParameter
	void rule_FUTURE_ExistentialList1(Object _FUTURE_ExistentialList,
			Object _FormalParameter) {
		List<Formal> FUTURE_ExistentialList = (List<Formal>) _FUTURE_ExistentialList;
		X10Formal FormalParameter = (X10Formal) _FormalParameter;
		FUTURE_ExistentialList.add(FormalParameter.flags(nf.FlagsNode(
				Position.compilerGenerated(FormalParameter.position()),
				Flags.FINAL)));
	}
