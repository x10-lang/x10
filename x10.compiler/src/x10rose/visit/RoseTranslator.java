...
*/
	...
	public RoseTranslator(Job job, TypeSystem ts, NodeFactory nf, TargetFactory tf) {
		super(job, ts, nf, tf);
		jobList.add(job);
//		System.out.println("RoseTranslator.<init> job = " + job);
	}
...
//				for (Enumeration<? extends ZipEntry> i = jarFile.entries(); i.hasMoreElements();) {
//					ZipEntry ei = i.nextElement();
//					String n = ei.getName();
//					if (n.toLowerCase().indexOf(fileName) >= 0) {
////						System.out.println("found " + fileName);
//						Job currentJob = null;
//						for (String jobName : jobMap.keySet()) {
////							System.out.println("jobName=" + jobName + ", fileName=" + fileName);
//							if (jobName.replaceAll("/", ".").indexOf(fileName) >= 0) {
//								currentJob = jobMap.get(jobName);
...
//						}
				...
				if (tVisitor.isFound()) {
					System.out.println(typeName + " found, ast=" + ast);
//					JNI.cactionBuildClassSupportEnd(typeName, createJavaToken((Node_c)ast, typeName));
//					try {
					tVisitor.createTypeReferenceEnd();
//						System.out.println("tVisitor=" + tVisitor);
//					} catch (Error e) {
//						e.printStackTrace();
//					} catch (Exception e) {
//						e.printStackTrace();
//					}
////					new ToRoseVisitor(null, null).visitAppropriate(ast);
//					System.out.println("====================================");
////					ast.visit(tVisitor);
////					System.out.println("====================================");
//					
...
//							new X10Lexer(source.path()) : new X10Lexer(reader, source.toString());
//					X10SemanticRules x10_parser = new X10SemanticRules(
//							x10_lexer, job.extensionInfo().typeSystem(), job.extensionInfo().nodeFactory(), source, eq); 
//					x10_lexer.lexer(x10_parser.getIPrsStream());
//					Node ast2 = x10_parser.parse();
//					System.out.println("ast2=" + ast2);
//					new ToRoseVisitor(null, null).visitAppropriate(ast2);
//					
//					source.close();
//					break;
					return;
...
//			}	
		...
		public boolean addFileIndex() {
			System.out.println("ADDFILE " + fileIndex);
			if (++fileIndex == numSourceFile) {
				--fileIndex;
				return false;
			}
			System.out.println("ADDFILE2 " + fileIndex);
			return true;
		}
		...
		public void subFileIndex() {
			--fileIndex;
			if (fileIndex < 0)
				throw new RuntimeException("File index cannot be less than zero");
		}
		...
		public void visit(SourceFile_c n) {
          ...
          visitChildren(n, n.decls());
			...
			System.out.println("SourceFile_c end : " + n);
		}
...
//			visitChildren(n, n.arguments());
		...
		public void visit(X10CanonicalTypeNode_c n) {
			...
			else if (	  n.nameString().indexOf("Rail[") == 0
					  || n.nameString().indexOf("GrowableRail[") >= 0
					  ) {
				String railString = n.type().toString();
				String type = railString.substring(railString.indexOf('[')+1, railString.indexOf(']'))
											.replaceAll("x10.lang.", "")  // so far, eliminate package name
											.replaceAll("x10.util.", ""); // so far, eliminate package name
//											.toLowerCase();
...
//			JNI.cactionInsertClassStart(class_name, createJavaToken(n, class_name));
			JNI.cactionInsertClassStart(class_name, false, false, false, createJavaToken(n, class_name));
//			JNI.cactionPushPackage("", createJavaToken(n, class_name));
			// does not consider nested class so far
			JNI.cactionInsertClassEnd(class_name, createJavaToken(n, class_name));
...
//		}
		...
		void visitChild(Node p, Node n) {
			if (n == null)
				return;
//			System.out.println("this.hashcode()=" + this.hashCode() + ", visitChild " + n.toString());
...
//			new TypeVisitor(package_name, class_name, null).visitAppropriate(n);
		...
		public void previsit(X10MethodDecl_c n) {
          ...
          if (typeParam.size() > 0) { // so far, skip processing type parameters
			...
			System.out.println("this.hashcode()=" + this.hashCode() + ", before visit return type ----------" + method_name + "-----------");
			visitChild(n, n.returnType());			
			System.out.println("this.hashcode()=" + this.hashCode() + ", before visit formals ----------" + method_name + "-----------");
			visitChildren(n, n.formals());

			System.out.println("this.hashcode()=" + this.hashCode() + ", before BuildMethodSupportEnd ----------" + method_name + "-----------");
           JNI.cactionBuildMethodSupportEnd(method_name, method_index, // method index 
												 false, false, false, 0, formals.size(),
                                           true, /* user-defined-method */
					new JavaToken(n.name().id().toString(), new JavaSourcePositionInformation(n.position().line())), 
					new JavaToken(n.name().id().toString()+"_args", new JavaSourcePositionInformation(n.position().line())));
           System.out.println("this.hashcode()=" + this.hashCode() + ", after BuildMethodSupportEnd -----------" + method_name + "----------");
		}
...
//			JNI.cactionTypeReference("", ret, this, createJavaToken(n, ret));
			System.out.println("formals begin " + n.formals().size());
			visitChildren(n, n.formals());
...
//			}
			System.out.println("formals end");
...
//				System.out.println("X10ClassDecl : " + type + ", class_name=" + class_name);
				if (type.equals(class_name) || type.toLowerCase().equals(class_name)) {
					System.out.println("this.hashcode()=" + this.hashCode() + ", Type matched for " + type);
					isClassMatched = true;
					if (isPackageMatched) {
...
//						visitDefinitions((X10ClassDecl_c)n); // so far, only declarations should be resolved
				...
				if (true) { //so far, always true, n.toString().equals(packageName)) {
...
//					System.out.println("fileName =" + fileName);
					for (Job job : jobList) {
						...
						if (tVisitor.isFound()) {
							System.out.println(typeName + " found in TypeVisitor, ast=" + ast);
							tVisitor.createTypeReferenceEnd();	
							return;
						}
					}
				...
				public void visit(AmbTypeNode_c n) {
			       ...
			       String ambTypeName = raw.replaceAll("\\{amb\\}", "");
					...
					// 
						...
						System.out.println("className=" + className + ", pkg=" + pkg + ", type=" + type + ", ambType=" + ambTypeName + ", raw=" + raw);
						// So far, I use a representation without package name such as "Rail". 
						// If I use a representation such as "x10.lang.Rail", lookupTypeByName() function tries
						// to look for a type name whether the type name is already registered or not. (and, there is
						// no registration for Rail, so ROSE compiler fails.)
...
//					JNI.cactionTypeDeclaration("", n.nameString(), false, false, false, false, false, false, false, false, false, false);
				...
				public void visit(X10Formal_c n) {
					toRose(n, "TypeVisitor.formal: ", n.name().id().toString());
