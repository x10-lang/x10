  ...
  // Program constructs an array of target places
    public static void main(String[] args) {
        ...
        for (int i = 0; i < args.length; ++i) {
          ...
          Transpose2b trans = new Transpose2b(dimX,dimY,blockP,blockQ,debug);
       ...
       if(destIndex == srcIndex){//special case of diagonal
         ...
         for(int i = 0; i < _x;++i)
            for(int j = 0; j < _y;++j){
               ...
               _destMatrix[destIndex,calculatedDestOffset] = _srcMatrix[srcIndex,calculatedSrcOffset];
            }
       }
       else {
	    ...
	    transposeBlock(destIndex,_destMatrix,copyBlock);
         }
      }
   }   
   public Transpose2b(int x, int y, int P, int Q,boolean debug){
       _debug = debug;
     	_x = x;
	_y = y;
	_P = P;
	...
	dist d = dist.factory.blockCyclic(r,elementArraySize*_numBlocksPerPlace);
    ...
    public Transpose2b(){ //default required for multivm
    ...
    //
    void transposeBlock(int blockId,double[.] destArray,final double[] srcBlock){
     for(int i = 0; i < _x;++i)
       for(int j = 0; j < _y;++j){
          final int calculatedSrcOffset = i*_y +j;
          final int calculatedDestOffset = j*_x+i;
          _destMatrix[blockId,calculatedDestOffset] = srcBlock[calculatedSrcOffset];
        }
