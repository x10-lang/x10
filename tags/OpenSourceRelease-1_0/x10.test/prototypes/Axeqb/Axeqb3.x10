public class Axeqb3 {
    public static void main(String[] args) {
        ...
        Axeqb3 solver = new Axeqb3(N, debug);
        solver.setup();
        ...
        if (debug) solver.print();
        if (!Axeqb3.computeYDirectly) solver.LyeqPb();
        if (debug) {
            solver.print();
            solver.checkLy();
        }
    ...
    Axeqb3(int N_, boolean debug_) {
        N = N_;
        A = new double[dist.factory.blockCyclic([0:N-1,0:N-1], N)];
        column_dist = dist.factory.blockCyclic([0:N-1], 1);
        ...
        for (int colx = 0; colx < N; ++colx) {
            ...
            finish async(A.distribution[diag_index,0]) {
                ...
                if (largest_row != diag_index) {
                    ...
                    // to the left
                    ateach (point[xcol] : column_dist | [0 : diag_index-1]) {
                ...
                if (A[diag_index, diag_index] == 0) {
                    ...
                    ateach (point[xcol] : column_dist | [diag_index+1 : N-1]) {
        ...
        finish async(place.FIRST_PLACE) {
            for (int i = 0; i < N; ++i) {
                ...
                finish async(A.distribution[diag_index, 0]) {
                    ...
                    double sm = 0;
                    final future<double>[] sm_f = new future<double>[diag_index+1];
                    sm_f[0] = future(b.distribution[diag_index]) { -(b[diag_index]) };
                    finish for(int xxcol = 0; xxcol < diag_index; ++xxcol) {
                        ...
                        sm_f[xcol+1] = future(A.distribution[xcol, diag_index]) { A[xcol, diag_index] };
                    }
                    finish for(int xcol = 0; xcol <= diag_index; ++xcol) {
                        sm -= sm_f[xcol].force();
                    }
                    final double yi = sm;
                    async(place.FIRST_PLACE) {
        ...
        finish async(place.FIRST_PLACE) {
            for (int i = N-1; i >= 0; --i) {
                ...
                finish async(A.distribution[diag_index, 0]) {
                    ...
                    double sm = 0;
                    final future<double>[] sm_f = new future<double>[N-diag_index];
                    sm_f[0] = future(b.distribution[diag_index]) { -(y[diag_index]) };
                    finish for (int xxcol = diag_index+1; xxcol < N; ++xxcol) {
                        final int xcol = xxcol;
                        // here is where we would have to multiply by xi
                        // if we hadn't done so earlier
                        sm_f[xcol-diag_index] = future(A.distribution[xcol, diag_index]) { A[xcol, diag_index] };
                    }
                    finish for (int xcol = diag_index; xcol < N; ++xcol) {
                        sm -= sm_f[xcol-diag_index].force();
                    }
                    final double xi = sm/A[diag_index, diag_index];
                    async(place.FIRST_PLACE) {
                        x[diag_index] = xi;
                    }
    ...
    boolean check() {
        ...
        for (int xrow = 0; xrow < N; ++xrow) {
            ...
            if (debug || Math.abs(s - bv) > 0.000000001) {
                System.err.println("s[" + xrow + "] = " + s +
                                   " b[" + xrow + "] = " + bv);
                if (Math.abs(s - bv) > 0.000000001) rc = false;
            }
        }
        return rc;
    }
