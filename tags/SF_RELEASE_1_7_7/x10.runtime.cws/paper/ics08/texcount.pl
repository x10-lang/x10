...
my $versionnumber=1.8;
my $versiondate="2007 Jun 20";
...
my %TeXgroup=('document'=>1,'letter'=>1,'titlepage'=>0,
     ...
     'abstract'=>1,'quote'=>1,'quotation'=>1,'verse'=>1,
...
sub get_next_token {
  ...
  (defined ($next=get_token($tex,'\w+([\-\']\w+)*',1))) && return $next;
  (defined ($next=get_token($tex,'[\"\'\`:\.,\(\)\[\]!\+\-\*=/\^\_\@\<\>\~\#\&]',2)))
&& return $next;
  #(defined ($next=get_token($tex,'\\\\([a-zA-Z_]+|\\\\|[\^\'\`\&!\[\]\(\)\%])',3)))
&& return $next;
  (defined ($next=get_token($tex,'\\\\([a-zA-Z_]+|[^a-zA-Z_])',3))) &&
return $next;
