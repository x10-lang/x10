...
public class BasicTest1Double {
	...
	private void initialize(int size){
		...
		_sumarray = new double[d] (point p[i,j]){return theSize*i +j;};
	...
	private void run(){
		...
		do{
			...
			if(trace)System.out.println("tempRegion:"+tempRegion);
			...
			async(p1) {
				...
				if(trace)System.out.println("sum is:"+tempSum);
				{
					...
					async(rootPlace){
						...
						if(trace)System.out.println("Xhere="+here+" this="+this+"("+this.hashCode()+") rootplace="+rootPlace+" Updating "+_totalSum+" with "+tempS);
					}
				}
			}
			//return tempSum;
	...
	private void verify(){
		...
		double expected_result = (size * size + size)/2;
	...
	public static void main(String[] args) {
		...
		try {
			...
			BasicTest1Double x = new BasicTest1Double();
