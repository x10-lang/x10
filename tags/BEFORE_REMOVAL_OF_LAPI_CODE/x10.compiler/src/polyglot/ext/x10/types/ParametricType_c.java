	...
	implements ParametricType
{
	 ...
	 * @param pos
	 */
	public ParametricType_c(TypeSystem ts, Position pos, X10ReferenceType base,
							List typeparameters, DepParameterExpr parameters)
	{
		super(ts, pos);
		assert base != null;
		this.base = base;
	...
	public boolean equalsImpl(TypeObject t) {
		...
		if (pt.typeparameters != typeparameters) {
			if ((pt.typeparameters == null) || (typeparameters == null))
				return false;
		...
		if (pt.parameters == parameters)
			return true;
		if ((pt.parameters == null) || (parameters == null))
			return false;
	...
	public List memberClasses() {
		...
		while (it.hasNext()) {
			...
			bo.add(new ParametricType_c(this.ts, ct.position(),
										(X10ReferenceType) ct, typeparameters,
										parameters));
		}
		return bo;
	}
	...
	public ClassType memberClassNamed(String name) {
		...
		if (ct == null)
			return null;
		return new ParametricType_c(this.ts, ct.position(),
									(X10ReferenceType) ct, typeparameters,
									parameters);
	}
	 ...
	 * This is used by Polyglot when pretty-printing TypeNodes, so for now this
	 * has to be in a form that Javac will parse and understand.
	 * As a result, this toString() cannot print the full dependent
	 * type signature.
	 * TODO: The proper fix for this would be to erase the type parameters
	 * explicitly in a visitor pass before code generation.
	 *
