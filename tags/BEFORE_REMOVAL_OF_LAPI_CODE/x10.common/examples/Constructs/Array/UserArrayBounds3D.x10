...
/**
 * User defined array bounds test - 3D
 *
...
boxedInt[.] a =new boxedInt[[lb1:ub1,lb2:ub2,lb3:ub3]->here]
  (point [i,j,k]){return new boxedInt(0);};

boolean withinBounds=true;
try {
	a[i,j,k]= new boxedInt(0xabcdef07);
	chk(a[i,j,k].equals(new boxedInt(0xabcdef07)));
...
private int ranInt(int lb,int ub) {
    ...
    public static void main(String[] args) {
        ...
        try {
                finish async b.val=(new UserArrayBounds3D()).run();
        } catch (Throwable e) {
                e.printStackTrace();
                b.val=false;
        }
