...
public class X10Caster extends AscriptionVisitor {
		...
		if ((e instanceof Cast) || (e instanceof Instanceof)) {
			...
			if (cast.isDepTypeCheckingNeeded()) {
				...
				if (e instanceof DepCast) {
					if (codeIsInlinable) {
						mc = new InlineCastChecking(tb, ar, tc, p);
					} else {						
						mc = new InnerClassCastChecking(tb, ar, tc, p);
					}
				} else {
					if (codeIsInlinable) {
						mc = new InlineInstanceofChecking(tb, ar, tc, p);
					} else {						
						mc = new InnerClassInstanceofChecking(tb, ar, tc, p);
					}
				}
				...
				if (cast.notNullRequired() && (e instanceof Cast)) {
					...
					AbstractRuntimeChecking mc = new InlineCastChecking(tb, ar, tc, p);	
	...
	protected abstract class InnerClassChecking extends AbstractRuntimeChecking {
		...
		public Node getRuntimeCheckingExpression(X10CastInfo castOrInstanceof) throws SemanticException {
			...
			Expr objToCast = nf.Local(p,OBJ_TO_CAST);
			...
			if (isBoxedCode) {
				...
				if (xts.isNullable(incomingExprType)) {
					...
					if (nullType.base().isPrimitive()) {
						nullType = nullType.base(xts.boxedType((X10PrimitiveType)nullType.base()));
				...
				exprToCheck = nf.Call(p,castedExpr,((X10TypeSystem) ts).getGetterName(destType.type()));
			} else {
			...
			Formal mthParamFormal = nf.Formal(p, Flags.FINAL, paramType, OBJ_TO_CAST);
			...
			MethodDecl checkCastMethod =   
				nf.MethodDecl(p, Flags.PUBLIC, this.checking.getExprReturnType(castOrInstanceof), 
						CHECK_CAST_MTH_NAME, paramFormalList, Collections.EMPTY_LIST, methodBody);

			// This method is part of an inner class body.
			List<MethodDecl> classBodyMembers = new LinkedList();
			...
			checkingCall = nf.Call(p, newAnonClass, CHECK_CAST_MTH_NAME, incomingExpr);
	...
	private abstract class Checking {
		...
		public Checking(TypeBuilder tb, AmbiguityRemover ar, 
	...
	private class InstanceofChecking extends Checking{
		...
		public InstanceofChecking(TypeBuilder tb, AmbiguityRemover ar, 
				TypeChecker tc, Position p) {
			super(tb,ar,tc,p);
		}
		...
		public NullableCheckBuilder getNullableCheckBuilder() {
			...
			return new NullableCheckInstanceofBuilder(tb,ar,tc,p);
		}
	...
	private class CastChecking extends Checking {
		...
		public CastChecking(TypeBuilder tb, AmbiguityRemover ar, 
				TypeChecker tc,Position p) {
			super(tb,ar,tc,p);
		}
		...
		public NullableCheckBuilder getNullableCheckBuilder() {
			...
			return new NullableCheckCastBuilder(tb,ar,tc,p);
		}
	...
	private class NullableCheckInstanceofBuilder extends NullableCheckBuilder {
		...
		public Expr buildNullableChecking(Expr exprToReturn, boolean mustBeNotNull, boolean toTypeIsNullable, Expr constraintExpr, TypeNode toType) {
			...
			X10NodeFactory xnf = (X10NodeFactory) nf; 
			Expr isAssignable = 
				xnf.ParExpr(p, nf.Call(p, nf.ClassLit(p, toType), "isAssignableFrom", nf.Call(p,exprToReturn,"getClass")));
