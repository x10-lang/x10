...
import x10me.opt.util.SpaceEffGraph;
...
public class LSTGraph extends SpaceEffGraph {
  ...
  public static void perform(IR ir) {
    if (ir.dumpFile.current != null) {
      ir.dumpFile.current.println("LSTGraph:" + ir.method);
    }
    ir.loopStructureTree = new LSTGraph(ir);
    if (ir.dumpFile.current != null) {
  ...
  */
  ...
  private LSTGraph(IR ir) {
    ...
    cfg.clearDFS();
    findBackEdges(ir, entry, ir.cfg.numberOfNodes());
      ...
      for (SpaceEffGraphEdge edge = node.firstInEdge(); edge != null; edge = edge.getNextIn()) {
        if (edge.backEdge()) {
          ...
          if (header == null) {
            ...
            header.loop = loop;
            if (ir.dumpFile.current != null) { 
              ir.dumpFile.current.println("header" + header);
            }
          } else {
            loop = header.loop;
          }
    ...
    if (ir.dumpFile.current != null) {
      for (SpaceEffGraphNode node = _firstNode; node != null; node = node.getNext()) {
        ir.dumpFile.current.println(node);
      }
    }
   ...
   */
  private void findBackEdges(IR ir, BasicBlock bb, int numBlocks) {
    ...
    while (!stack.empty()) {
      ...
      if (e == null) {
        if (ir.dumpFile.current != null) { 
          ir.dumpFile.current.println(" Initial processing of " + bb);
        }
        bb.setDfsVisited();
        e = bb.outEdges();
      } else {
        if (ir.dumpFile.current != null) { 
          ir.dumpFile.current.println(" Resuming processing of " + bb);
        }
      }
      ...
      while (e.hasMoreElements()) {
        ...
        if (LTDominatorInfo.isDominatedBy(bb, outbb)) {   // backedge
          ...
          outEdge.setBackEdge();
          if (ir.dumpFile.current != null) {
            ir.dumpFile.current.println("backedge from " +
                               bb.scratch +
                               " ( " + bb + " ) " +
                               outbb.scratch +
                               " ( " + outbb + " ) ");
          ...
          if (outbb.scratch < bb.scratch) {
            throw new Error("irreducible loop found!");
          }
