...
/**
 * The TableSwitchParent instruction class.
 *
 * The header comment for {@link Instruction} contains
 * an explanation of the role of these interior class in 
 * opt compiler's IR.
 */
public abstract class TableSwitchParent extends Instruction
  ...
  /**        
    * low use IntConstantOperand.
    */
  private IntConstantOperand low;

  /**
   * Get the operand called low from this instruction. Note that the
   * returned operand will still point to its containing instruction.
   *
   * @return the operand called low
   */
  public final IntConstantOperand getLow() {
    return low;
  }

  /**
   * Get the operand called low from this instruction clearing its 
   * instruction pointer. The returned operand will not point to 
   * any containing instruction.
   * @return the operand called low
   */
  public final IntConstantOperand getClearLow() {
    IntConstantOperand tmp = low;
    low = null;
    clearOperand (tmp);
    return tmp;
  }
   ...
   * @param o the operand to store.
   */
  public final void setLow(IntConstantOperand o) {
    if (o == null) {
      low = null;
    } else {      
      assert o.instruction == null;
  ...
  /**        
    * high use IntConstantOperand.
    */
  private IntConstantOperand high;

  /**
   * Get the operand called high from this instruction. Note that the
   * returned operand will still point to its containing instruction.
   *
   * @return the operand called high
   */
  public final IntConstantOperand getHigh() {
    return high;
  }

  /**
   * Get the operand called high from this instruction clearing its 
   * instruction pointer. The returned operand will not point to 
   * any containing instruction.
   * @return the operand called high
   */
  public final IntConstantOperand getClearHigh() {
    IntConstantOperand tmp = high;
    high = null;
    clearOperand (tmp);
    return tmp;
  }
   ...
   * @param o the operand to store.
   */
  public final void setHigh(IntConstantOperand o) {
    if (o == null) {
      high = null;
    } else {      
      assert o.instruction == null;
  ...
  /**
    * Constructor for TableSwitchParent.
    *
    * @param value
    * @param unknown1
    * @param unknown2
    ...
    */
  TableSwitchParent (Operand value, Operand unknown1, Operand unknown2, IntConstantOperand low, IntConstantOperand high, BranchOperand defaultTarget, BranchProfileOperand defaultBranchProfile, int numVarOps) {
    this.value = value;
  ...
  public final Operand getOperand(int i) {
    switch (i) {
      ...
      case 6:
        return defaultBranchProfile;
      default:
        if (i < (7 + ((0 + 1) * this.count)))
	  return this.target[(7 + (0 * this.count)) + i];
        else
        if (i < (7 + ((1 + 1) * this.count)))
	  return this.branchProfile[(7 + (1 * this.count)) + i];
        else
  ...
  public final void putOperand(int i, Operand op) {
    ...
    switch (i) {
      ...
      break;
      case 3:
        low = (IntConstantOperand)op;
      break;
      case 4:
        high = (IntConstantOperand)op;
      break;
      case 5:
      ...
      default:
        if (i < (7 + ((0 + 1) * this.count)))
	  this.target[(7 + (0 * this.count)) + i] = (BranchOperand)op;
        else
        if (i < (7 + ((1 + 1) * this.count)))
	  this.branchProfile[(7 + (1 * this.count)) + i] = (BranchProfileOperand)op;
        else
        throw new ArrayIndexOutOfBoundsException ();
    }
  }
