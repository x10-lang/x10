...
public class RandomAccess_Dist {
	...
	public static class localTable {
		...
		public localTable(long size) {
			tableSize=size;
			mask=tableSize-1;
			array=(long[:self.rect && self.zeroBased && self.rank==1])
				new long[[0:(int)mask]];
		}
...
//		final dist UNIQUE = dist.UNIQUE;
//		final int NUMPLACES = place.MAX_PLACES;
//		final int PLACEIDMASK = NUMPLACES-1;
...
//		}
		...
		// SPMD code
//		final int[.] VERIFY = new int[UNIQUE];
//		final boolean[.] doIO = new boolean[UNIQUE], embarrassing=new boolean[UNIQUE];
//		final int[.] logTableSize = new int[UNIQUE];
//		finish ateach (point p:UNIQUE) {
//			VERIFY[here.id] = UPDATE;
//			doIO[here.id] = false;
//			embarrassing[here.id]=false;
//			logTableSize[here.id]= 10; //20
//			for (int q = 0; q < args.length; ++q) {
//				if (args[q].equals("-o")) {
//					doIO[here.id] = true;
//				}
//				if (args[q].equals("-e")) {
//					embarrassing[here.id] = true;
//				}
//				if (args[q].equals("-m")) {
//					++q;
//					logTableSize[here.id] = java.lang.Integer.parseInt(args[q]);
//				}
//				if (args[q].equals("-v")) {
//					++q;
//					VERIFY[here.id] = java.lang.Integer.parseInt(args[q])%3;
//				}
//			}
//		} // End finish/ateach

		// Hard-coded values
		final int VERIFY = UPDATE;
		final boolean doIO = true;
		final boolean embarrassing=false;
		final int logTableSize = 20;

		//long[:self.rect && self.zeroBased && self.rank==1] Table;
		final long tableSize=(1<<logTableSize);
//		final long tableSize=(1<<logTableSize[here.id]);
		final long numUpdates=tableSize*4*NUMPLACES;
...
//			System.out.println("Distributed table size   = 2^" +logTableSize[here.id] + "*"+NUMPLACES+" = " +
...
//		System.out.println ("The mode of update (0 = update; 1 = parallel verification; 2 = update and sequential verification): "+VERIFY[here.id]);

		final long LogTableSize = logTableSize;
//		final long LogTableSize = logTableSize[here.id];
		final boolean Embarrassing = embarrassing;
//		final boolean Embarrassing = embarrassing[here.id];
		final long NumUpdates=tableSize*4;

		//System.err.println("cputime = "+cputime);
		if (VERIFY == VERIFICATION_P)
//		if (VERIFY[here.id] == VERIFICATION_P)
			finish ateach(point [p] : UNIQUE) {
				long ran=HPCC_starts (p*NumUpdates);
				for (long i=0; i<NumUpdates; i++) {
					final int placeID;
					...
					final long temp=ran;
					//async (UNIQUE[placeID]) Table[placeID].verify(temp);
					async (UNIQUE[placeID]) Table[placeID].array[(int)(temp & Table[placeID].mask)]++;
			...
			finish ateach(point [p] : UNIQUE) {
				//System.err.println("In ateach["+p+"]; NumUpdates="+NumUpdates);
				long ran=HPCC_starts (p*NumUpdates);
				for (long i=0; i<NumUpdates; i++) {
					//System.err.println("In ateach["+p+"]");
					final int placeID;
					if (Embarrassing)
						placeID=p;
					else
					...
					final long temp=ran;
					//System.err.println("Spawning async at "+placeID);
					//async (UNIQUE[placeID]) Table[placeID].update(temp);
					async (UNIQUE[placeID]) Table[placeID].array[(int)(temp & Table[placeID].mask)] ^= temp;
					ran = (ran << 1) ^ ((long) ran < 0 ? POLY : 0);
				}
				//System.err.println("Updates done");
			}
...
//		if (VERIFY[here.id] == UPDATE_AND_VERIFICATION) {
...
//		if (VERIFY[here.id] == UPDATE) System.out.println("    "+GUPs + " Billion(10^9) Updates    per second [GUP/s]");
...
//@@X101X@@TSTAMP@@20082605.111729
//@@X101X@@TCASE@@RandomAccess_Dist
//@@X101X@@VCODE@@SUCCEED
//@@X101X@@TOUT@@0 60
//@@X101X@@SRCS@@RandomAccess_Dist.cc
//@@X101X@@NPLACES@@1
//@@X101X@@FLAGS@@LIMITATION
//@@X101X@@ARGS@@
//@@X101X@@DATA@@Distributed table size   = 2^20*1 = 1048576 words
//@@X101X@@DATA@@Number of total updates = 4194304
//@@X101X@@DATA@@The mode of update (0 = update; 1 = parallel verification; 2 = update and sequential verification): 0
//@@X101X@@DATA@@    CPU time used  = 0.842738 seconds
//@@X101X@@DATA@@    0.004977 Billion(10^9) Updates    per second [GUP/s]
