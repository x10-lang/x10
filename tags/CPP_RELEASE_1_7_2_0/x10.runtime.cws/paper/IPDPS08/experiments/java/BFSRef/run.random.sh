...
#
# $Id: run.random.sh,v 1.1 2008-01-18 15:09:15 srkodali Exp $
# Interactive script for running BFS (Java).
#

prog_name=CWS-BFS.Random
. ../config/run.header
...
do
	...
	do
		...
		do
			...
			printf "\n### nproc: %d\n" $nproc 2>&1| \
				tee -a $OUT_FILE
			printf "${CMD} -d64 ${XOPTS} -cp ../xws.jar BFSRef $nproc E $size\n" \
					2>&1| tee -a $OUT_FILE
			${CMD} -d64 ${XOPTS} -cp ../xws.jar BFSRef \
