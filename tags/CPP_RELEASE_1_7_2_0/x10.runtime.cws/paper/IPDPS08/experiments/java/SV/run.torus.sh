...
# Interactive script for running SV (traversal).
#

prog_name=CWS-SV.Torus
. ../config/run.header
...
do
	...
	do
		...
		do
			...
			printf "\n### nproc: %d\n" $nproc 2>&1| \
				tee -a $OUT_FILE
			printf "${CMD} -d64 ${XOPTS} -cp ../xws.jar SV $nproc T $size\n" \
					2>&1| tee -a $OUT_FILE
			${CMD} -d64 ${XOPTS} -cp ../xws.jar SV $nproc T $size \
					2>&1| tee -a $OUT_FILE
			let 'nproc = nproc * 2'
		done
		let 'size = size * 3'
