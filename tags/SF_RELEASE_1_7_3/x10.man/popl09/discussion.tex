...
entailment checks.
This might done by analyzing the program to identify which pairs of
...
be substituted for \xcd"this" in $a$ and 
\xcd"self" in $t$.

Finally, the language can be restricted so that entailment
checking can be implemented without a run-time constraint solver.
As an instance, note an instance of a generic class \xcd"C[X]"
...
\noindent
one can test if \xcd"xs" is an instance of \xcd"List{X==String}"
by checking that \xcd"head" is an instance of \xcd"String"
and that \xcd"tail", recursively, is an instance of \xcd"List{X==String}".
Importantly, this is only a sound check if all such fields are
(transitively) final;
otherwise, the type membership test is not invariant.  For
instance, a value that is not a \xcd"String" could be assigned 
into the \xcd"head" field.  Care must be taken to implement the
test so that it terminates for cyclic data structures.
This implementation is inefficient for large data structures.

This solution is sound, but implements a slightly weaker semantics than is desired.
The problem is best illustrated by considering an empty list
class:
{
\begin{xten}
class Nil[X] { }
