...
constraints
via the rules for ``\Xcd{x} \Xcd{has} \Xcd{I}'' in
...
Unity~\cite{unity} is another language with structural subtyping.
\todo{Malayeri and Aldrich, ECOOP 08}.
%
...
Another approach is to modify the language
\eat{ Finally, the language can be restricted }
so that entailment
checking can be implemented without a run-time constraint solver.
As an example, note that an instance of a generic class \xcd"C[X]"
...
\todo{similarly \Xcd{List\{X==Number\}} will return true
even if instantiated on \Xcd{List\{X==Integer\}} if the type is
immutable.  But check if \xcd"List" takes \xcd"X" as a param even it if
doesn't mutate an \xcd"X"}

\todo{Also, rule out instanceof constrained type with subtype
constraints.  But this breaks when doing instanceof X}
