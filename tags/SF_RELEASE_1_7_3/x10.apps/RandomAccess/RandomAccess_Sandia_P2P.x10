...
public class RandomAccess_Sandia_P2P {
	...
	public static void  main(String[] args) {
		...
		if (VERIFY == VERIFICATION_P)
			...
			final int [.] Counter=new int [UNIQUE] (point [i]){return -1;};
			ateach(point [p] : UNIQUE) {
				...
				int j,k;
				final int LogNumPlacesx2 = 2*LogNumPlaces;
				for (long i=0; i<NumUpdates/CHUNK; i++) {
					...
					for (k = 0; k < LogNumPlaces; k++){
						...
						/* the following for loop should be replaced with the array copy method*/
							...
							finish async (destPlace){
								Data[destID].dataLength = remoteLength;
							}
							async (destPlace) atomic Counter[destID]++;
							//next;
						await ((Counter[p]%LogNumPlacesx2) == (2*k+1));
