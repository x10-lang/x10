...
#ifdef SPARC
extern int CASW(int *a, int e, int n);
#else
extern int Xchg(int *a, int b);
#endif
...
#define INDEX(a,b) ((a-1)*(THREADS)+(b-1))
  ...
  stack_M = (int **)node_malloc(THREADS*sizeof(int *),TH);
  top_M = (int **) node_malloc(THREADS*sizeof(int *),TH);
  bottom_M=(int **)node_malloc(THREADS*sizeof(int *),TH);
  ...
  while(first_time || work_to_steal)
      ...
      while(!is_empty( stack, &top, &bottom))
	{
	  ...
	  for(i=0;i<graph[n].n_neighbors;i++)
	    {
#ifdef SPARC
	      if((t=CASW(&color[graph[n].my_neighbors[i]],0,MYCOLOR))==0){
#else
	      if((t=Xchg(&color[graph[n].my_neighbors[i]],MYCOLOR))==0){
#endif
