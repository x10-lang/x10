...
class LU {
    ...
    def exchange(row:Int, row2:Int, min:Int, max:Int, dest:Int) {
        ...
        val _A = A;
        finish{
        	val remote_handle = buffers as Any as PlaceLocalHandle[Rail[Double]];
        	...
        	async(remote_place){
        		...
        		for (var i:Int=0 ; i<size ; ++i) {
        			dst_data(dst_off+i) = to_serialize(i);
            	}
         		val local_size = _A().swapRow(row2, min, max, _buffers());
            	...
            	async(source){
                	...
                	val local_off = local_pair.second; 
                	for (var i:Int=0 ; i<local_size ; ++i) {
                    	local_data(local_off+i) = local_serialize(i);
                	}
    ...
    def memget(I:Int, J:Int) {
        if (A_here.hasBlock(I, J)) {
            ...
            val _B = B;
            finish{
                val p = Place.place(A_here.placeOfBlock(I,J));
                @TailAsync(true)
                async(p){
