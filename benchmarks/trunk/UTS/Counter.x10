...
public class Counter {
  ...
  static struct Event (timeStamp:Long, state:Int) {
    ...
    global safe public def toString() = timeStamp + ": " + 
	(state == 0 ? "DEAD"
	: state == 1 ? "COMPUTING"
	: state == 2 ? "STEALING"
   ...
   */
	...
	def this(c:Counter!) {
	    ...
	    maxDepth=c.maxDepth;
	    // normalize when copying
	    lifeStory = 
		ValRail.make[Event] (c.lifeStory.size(), 
				     (i:Int) => {
					 val z = c.lifeStory(0); 
					 val e = c.lifeStory(i); 
					 Event(e.timeStamp-z.timeStamp, e.state)
					     });
	}
	    ...
	    if (lifeStory != null && lifeStory.length > 0)
		Console.OUT.println("Lifestory starts at "+ lifeStory(0).timeStamp + " ends at " + 
				    lifeStory(lifeStory.length-1).timeStamp);
