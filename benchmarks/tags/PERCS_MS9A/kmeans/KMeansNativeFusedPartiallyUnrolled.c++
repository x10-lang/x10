...
int main (int argc, char **argv)
{
        ...
        for (int i=0 ; i<iterations ; ++i) {
                ...
                for (int p=0 ; p<num_slice_points ; p+=UR) {
                        ...
                        for (int i=0 ; i<UR ; ++i) for (int k=0 ; k<num_clusters ; ++k) {
                                float dist = 0;
                                for (int d=0 ; d<dim ; ++d) {
                                        float tmp = host_points[(p+i)+d*num_slice_points_stride] - old_clusters[k*dim+d];
                                        //float tmp= host_points[(p+i)*dim+d] - old_clusters[k*dim+d];
                                        dist += tmp * tmp;
                                }
