...
E* r_graph_L(int n,int *pm)
{
    int i,j,v1,v2,v,top,tail,r, n_edges,m;
    struct s_edge *L;
    char * filename ;
    FILE * fp;
    char * color;
    ...
    m = *pm;
    El=malloc(sizeof(E)*2*m);
    if(El==NULL) printf(" not enough memory\n");
    j=0; 
    for(i=0;i<m;i++)
     {
#if USE_MY_RAND
