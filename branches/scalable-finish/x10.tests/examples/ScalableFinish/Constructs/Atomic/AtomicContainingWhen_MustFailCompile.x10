 ...
 */
public class AtomicContainingWhen_MustFailCompile   {
	...
	public static def main(var args: Rail[String]): void = {
		new AtomicContainingWhen_MustFailCompile().run ();
	}
	...
	static class Y {
		static def test(val f: foo, val l: lockStruct): void = {
			...
			f.apply(l); // it is hard to determine if f.run s an async or when
		}
	}
