...
import x10me.opt.util.SortedGraphNode;
import x10me.opt.util.SpaceEffGraphEdge;
import x10me.opt.util.SpaceEffGraphNode;
import x10me.runtime.Entrypoints;
...
public class BasicBlock extends SortedGraphNode {
  ...
  protected BasicBlock(int i, InlineSequence position, int num) {
    setNumber(num);
    start = new Label ( new BasicBlockOperand(this));
    start.bcIndex = i;

    start.position = position;
    end = new BBend(new BasicBlockOperand(this));
    ...
    end.bcIndex = BCconstants.UNKNOWN_BCI;
   ...
   */
  public final void printExtended(DumpFile df) {
    df.current.println("Basic block " + toString());
    ...
    if (!e2.hasMoreElements()) {
      df.current.println("<none>");
    } else {
      bb2 = e2.next();
      df.current.print(bb2.toString());
    ...
    if (hasExceptionHandlers()) {
      df.current.print("\tIn scope exception handlers: ");
	...
	df.current.print("<none>");
      }
      df.current.println();
    }

    if (start != null) {
      df.current.println("\tInstructions:");
      Instruction inst = start;
      while (inst != end) {
	df.current.println(inst.bcIndex + ":\t" + inst);
	inst = inst.getNext();
      }	
      df.current.println(inst.bcIndex + ":\t" + inst);
    }
    ...
    // print out set.
    if (getNext() != null) {
      df.current.println("\tNext in code order: " + getNext().toString());
    }

    e2 = getNormalOut();
    df.current.print("\tnormal out: ");
    if (!e2.hasMoreElements()) {
      df.current.println("<none>");
    } else {
      bb2 = e2.next();
    ...
    df.current.print("\texceptional out: ");
    if (!e2.hasMoreElements()) {
      df.current.println("<none>");
    } else {
      bb2 = e2.next();
    ...
    if (mayThrowUncaughtException()) {
      df.current.println("\tMay throw uncaught exceptions, implicit edge to EXIT");
    }
  ...
  public final Instruction makeGOTO() {
    return new Goto(makeJumpTarget());
  }

  /**
   * @return the first instruciton of the basic block (the label)
   */
  public final Label firstInstruction() {
    return start;
  }
  ...
  public final boolean hasGoto() {
    ...
    return lastRealInstruction () instanceof Goto;
  }
  ...
  public final boolean hasReturn() {
    if (isEmpty()) return false;
    return lastRealInstruction() instanceof Return;
  }
  ...
  public final boolean hasSwitch() {
    ...
    return s instanceof TableSwitch || s instanceof LowTableSwitch || s instanceof LookupSwitch;
  }
  ...
  public final boolean hasAthrowInst() {
   ...
   if (s instanceof Athrow) {
      return true;
    }
    MethodOperand mop = null;
    if (s instanceof Call) {
      mop = ((Call)s).getMethod();
    }
  ...
  public final boolean hasNonReturningCall() {
    ...
    if (s instanceof Call) {
      MethodOperand methodOp = ((Call)s).getMethod();
  ...
  public final BasicBlock getNotTakenNextBlock() {
    ...
    if (last instanceof Goto) {
      return ((Goto)last).getBranchTarget();
    } else {
      return nextBasicBlockInCodeOrder();
    }
  }
  ...
  public void killFallThrough() {
    ...
    if (fallThrough != null) {
      lastInstruction().insertBefore(new Goto(fallThrough.makeJumpTarget()));
    }
  }
  ...
  public final void prependInstructionRespectingPrologue(Instruction i) {
    ...
    if ((first != null) && (first instanceof Prologue)) {
      first.insertAfter(i);
    } else {
      start.insertAfter(i);
    }
  ...
  public final void appendInstructionRespectingTerminalBranch(Instruction i) {
    Instruction s = end;
    while (s.getPrev().isBranch()) s = s.getPrev();
    s.insertBefore(i);
  }
    ...
    if (instr.isPEI()) {
      ...
      switch (instr.getOpcode()) {
        case Operators.Athrow:
          Type type = ((Athrow)instr).getValue().getType();
          addTargets(e, type);
          break;
        case Operators.Checkcast:
        case Operators.CheckcastNotnull:
        case Operators.CheckcastUnresolved:
          addTargets(e, KnownTypes.CLASS_CAST_EXCEPTION_TYPE);
          break;
        case Operators.NullCheck:
          addTargets(e, KnownTypes.CLASS_CAST_EXCEPTION_TYPE);
          break;
        case Operators.BoundsCheck:
          addTargets(e, KnownTypes.INDEX_OUT_OF_BOUNDS_EXCEPTION_TYPE);
          break;
        case Operators.IntZeroCheck:
        case Operators.LongZeroCheck:
          addTargets(e, KnownTypes.ARITHMETIC_EXCEPTION_TYPE);
          break;
        case Operators.ObjArrayStoreCheck:
          addTargets(e, KnownTypes.ARRAY_STORE_EXCEPTION_TYPE);
          break;
        default:
  ...
  private void addTargets(ComputedBBEnum e, Type thrownException) {
    for (SpaceEffGraphEdge ed = _outEdgeStart; ed != null; ed = ed.getNextOut()) {
      ...
      if (bb.isExceptionHandlerBasicBlock()) {
        ...
        if (eblock.mayCatchException(thrownException) != OptConstants.NO) {
  ...
  public final void recomputeNormalOut(IR ir) {
    ...
    for (InstructionEnumeration e = enumerateBranchInstructions(); e.hasMoreElements();) {
      ...
      BasicBlockEnumeration targets = ((HasTargets)branch).getBranchTargets();
  ...
  public final BasicBlock segregateInstruction(Instruction target, IR ir) {
    assert this == target.getBasicBlock();
  ...
  public final BasicBlock splitNodeAt(Instruction last_instr_BB1, IR ir) {
    assert this == last_instr_BB1.getBasicBlock();
    ...
    if (BB3 == null) {
      ...
      assert BB1.next == BB2 && BB2.prev == BB1;
  ...
  public final BasicBlock replicateThisOut(IR ir, BasicBlock b, BasicBlock pred) {
    ...
    if (bFallThrough != null) {
      Instruction g = new Goto(bFallThrough.makeJumpTarget());
  ...
  public final void redirectOuts(BasicBlock b, BasicBlock bCopy, IR ir) {
    ...
    if (getFallThroughBlock() == b) {
      Instruction g = new Goto(copyTarget); //no copy needed.
      appendInstruction(g);
    }
  ...
  public final boolean mergeFallThrough(IR ir) {
    ...
    if (OptOptions.verifyAssertions > OptOptions.NOVERIFY) {
      for (InstructionEnumeration e = enumerateBranchInstructions(); e.hasMoreElements();) {
        BasicBlockEnumeration targets = ((HasTargets)e.next()).getBranchTargets();
        while (targets.hasMoreElements()) {
          BasicBlock target = targets.next();
          assert target == succBB;
        }
      }
    }
   ...
   * @param ir the containing IR object
   */
  public final void unfactor(IR ir) {
   ...
   */
  public final void pruneExceptionalOut(IR ir) {
  ...
  */
  ...
  public static final class ComputedBBEnum implements BasicBlockEnumeration {
    private BasicBlock[] blocks;
    private int numBlocks;
    private int current;

    public ComputedBBEnum(int maxBlocks) {
      blocks = new BasicBlock[maxBlocks];
    }

    public void addElement(BasicBlock b) {
      blocks[numBlocks++] = b;
    }

    public void addPossiblyDuplicateElement(BasicBlock b) {
  ...
  public static BasicBlock makeBlockOnEdge(BasicBlock in, BasicBlock out, IR ir) {
    ...
    for (InstructionEnumeration e = in.reverseRealInstrEnumerator(); e.hasMoreElements();) {
      Instruction s = e.next();
      if (s instanceof IfCmp2) {
	IfCmp2 ic = (IfCmp2)s;
        if (ic.getTarget1().similar(outTarget)) {
          ic.setTarget1((BranchOperand) target.copy());
        }
        if (ic.getTarget2().similar(outTarget)) {
          ic.setTarget2((BranchOperand) target.copy());
        }
      } else if (s instanceof IfCmp) {
	IfCmp ic = (IfCmp)s;
        if (ic.getTarget().similar(outTarget)) {
          ic.setTarget((BranchOperand) target.copy());
        }
      } else if (s instanceof InlineGuard) {
	InlineGuard ig = (InlineGuard)s;
        if (ig.getTarget().similar(outTarget)) {
          ig.setTarget((BranchOperand) target.copy());
        }
      } else if (s instanceof Goto) {
	Goto g = (Goto)s;
        foundGoto = true;
        if (g.getTarget().similar(outTarget)) {
          g.setTarget((BranchOperand) target.copy());
        }
      } else if (s instanceof TableSwitch) {
	TableSwitch ts = (TableSwitch)s;
        foundGoto = true;
        if (ts.getDefaultTarget().similar(outTarget)) {
          ts.setDefaultTarget((BranchOperand) target.copy());
        }
        for (int i = 0; i < ts.getNumberOf(); i++) {
          if (ts.getTarget(i).similar(outTarget)) {
            ts.setTarget(i, (BranchOperand) target.copy());
          }
        }
      } else if (s instanceof LowTableSwitch) {
	LowTableSwitch lts = (LowTableSwitch)s;
        foundGoto = true;
        for (int i = 0; i < lts.getNumberOf(); i++) {
          if (lts.getTarget(i).similar(outTarget)) {
            lts.setTarget(i, (BranchOperand) target.copy());
          }
        }
      } else if (s instanceof LookupSwitch) {
	LookupSwitch ls = (LookupSwitch)s;
        foundGoto = true;
        if (ls.getDefaultTarget().similar(outTarget)) {
          ls.setDefaultTarget((BranchOperand) target.copy());
        }
        for (int i = 0; i < ls.getNumberOf(); i++) {
          if (ls.getTarget(i).similar(outTarget)) {
            ls.setTarget(i, (BranchOperand) target.copy());
          }
        }
      } else {
    ...
    // 4. Add a goto bb->out
    Instruction s = new Goto(out.makeJumpTarget());
    ...
    if (out != next) {
      ...
      if (!foundGoto) {
        ...
        s = new Goto(jumpTarget.makeJumpTarget());
