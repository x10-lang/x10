...
class RayTracer {
	...
	val datasizes: ValRail[int] = [1, 64]; //reducing data size
	...
	public def render(val interval: Interval!): void = @ ClockedM(c) {
		...
		// For each line
		var yy: int = 0;
		for (yy = interval.yfrom; yy < interval.yto; yy++) {
		val y = yy; 
		//finish for(val (y): Point in [interval.yfrom..interval.yto-1]) 
		async clocked(c) {
			var lchecksum: long = 0;
		    ...
		    checksum = lchecksum;
    		    Console.OUT.println( Runtime.workerTid());
		} // end for (y)
		}
		next;
		}
...
public class RayTrace extends RayTracer {

	public def JGFsetsize(var size: int): void =  @ClockedM (RayTracer.c) {
		this.size = size;
	}

	public def JGFinitialise(): void = @ClockedM (RayTracer.c) {
		//JGFInstrumentor.startTimer("Section3:RayTracerr:Init");

		// set image size
		width = datasizes(size);
		height = datasizes(size + 1);
	...
	public def JGFapplication(): void = @ClockedM (RayTracer.c) {
	...
	public def JGFtidyup(): void = @ClockedM (RayTracer.c) {
		/*
		scene = null;
		lights = null;
		prim = null;
	...
	public def JGFrun(var size: int): void = @ClockedM(RayTracer.c) {
