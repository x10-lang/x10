...
import clocked.*;
	...
	private const SPARSE_NUM_ITER: int = 160000;
	...
	public def JGFinitialise(): void = {
		...
		y = Rail.make[double @ Clocked[double](c, op, 0.0D)](ds_M, (i: int) => 0.0D);      // distributed -- cvp
	...
	public def JGFkernel(): void = @ClockedM(c) {
		test();
	}

	public def JGFvalidate(): void = {
		//double refval[] = { 75.02484945753453, 150.0130719633895, 749.5245870753752 };
	        val refval: ValRail[double] = [131.87370565033834 ,  150.57514642658575, 749.5245870753752 ];
	...
	public def JGFrun(var size: int): void = @ ClockedM(c) {
	...
	public  def test(): void = @ClockedM(c) {
