...
class IdeaTest {
 ...
 class IDEA extends IdeaTest {
	...
	public def JGFinitialise(): void = @ClockedM(super.c){
		array_rows = datasizes(size);
		buildTestData();
	}

	public def JGFkernel(): void = @ ClockedM(super.c) {
		Do();
	}

	public def JGFvalidate(): void = @ ClockedM(super.c) {
		for (var i: int = 0; i < array_rows; i++) {
