...
\begin{abstract}
The advent of multicore processors has made concurrent programming models manda- 
tory. However, most concurrent programming models come with a repertoire of problems.
...
Non-deterministic functional behavior arising from scheduling choices is one of the biggest problems of concurrent programming. The program in~\figref{non-det} is non-deterministic. It uses 
...
Such non-determinism makes debugging all but impossible because unwanted behavior is 
...
and will never deadlock. We start by discussing our model in~\secref{model} and a few examples in~\secref{examples}. 
 semantics in~\secref{semantics} and provide a proof of correcness (determinism and deadlock-freedom) in~\secref{proof}. 
...
is clocked  with a reduction operator $+$ in~\lineref{declx}.
...
A task may offer multiple values to the same variable in one phase. These values are also reduced using the commutative associative operator along with the values offered by other tasks. For instance, in~\figref{ddc-multiple}, after the execution of 
...
~\lineref{declb} forces all locations of b to be independently shared.
...
\begin{figure*}[htbp]
\includegraphics[angle=-90]{figures/next.eps}
