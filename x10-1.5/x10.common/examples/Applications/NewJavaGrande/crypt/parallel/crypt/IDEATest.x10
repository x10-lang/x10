...
public class IDEATest {
	...
	int /*byte*/ [] plain1;       // Buffer for plaintext data.
	int /*byte*/ [] crypt1;       // Buffer for encrypted data.
	int /*byte*/ [] plain2;       // Buffer for decrypted data.

	int /*short*/ [] userkey;     // Key for encryption/decryption.
	...
	void buildTestData()
	{
		...
		plain1 = new /* byte*/ int [array_rows];
		crypt1 = new /* byte*/ int [array_rows];
		plain2 = new /* byte*/ int [array_rows];
		...
		userkey = new int /*short*/ [8];  // User key has 8 16-bit shorts.
	 ...
	 * fit in 16 bits.
	 */
	private void cipher_idea(final int /*byte*/ [] text1, final int /*byte*/ [] text2, final int [] key)
	{
		// for (int i = 0; i < text1.length; i += 8)
		finish foreach (point[ii] : [0:(text1.length-1)/8]) {
			int i = 8 * ii;
			int i1 = i;                 // Index into first text array.
			int i2 = i;                 // Index into second text array.
