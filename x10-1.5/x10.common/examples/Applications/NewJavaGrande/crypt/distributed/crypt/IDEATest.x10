...
public class IDEATest {
	...
	int[] userkey;         // Key for encryption/decryption.
	int value[.] ZR;       // Encryption subkey (userkey derived).
	int value[.] DKR;      // Decryption subkey (userkey derived).
	...
	void buildTestData()
	{
		...
		userkey = new int[8];  // User key has 8 16-bit shorts.
		...
		plain1 = new byte[D] (point p[i]) { return (byte) i; };
		crypt1 = new byte[D];
		plain2 = new byte[D];
	}
	...
	private void calcEncryptKey()
	{
		...
		final int[] Z = new int[52];

		for (point [i] : [0:7])  // First 8 subkeys are userkey itself.
		{
	...
	private void calcDecryptKey()
	{
		...
		final int[] DK = new int[52];
		final int value[.] Z = ZR;
	 ...
	 * fit in 16 bits.
	 */
	private void cipher_idea(final byte[.] text1,
			final byte[.] text2,
			final int value[.] key)
	{
