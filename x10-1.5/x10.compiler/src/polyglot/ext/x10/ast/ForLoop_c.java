...
public class ForLoop_c extends X10Loop_c implements ForLoop {
	...
	public Node typeCheck(TypeChecker tc) throws SemanticException {
		...
		if (ts.isDistribution(type)) {
			newDomain = (Expr) ((X10Del) tc.nodeFactory().Field(
					domain.position(), newDomain,
					tc.nodeFactory().Id(domain.position(), "region")).del())
					.setGenerated(true).typeCheck(tc);
			// newDomain = (Expr) tc.nodeFactory().Field(n.position(),
			// newDomain, tc.nodeFactory().Id(n.position(),
			// "region")).del().typeCheck(tc);
		}
