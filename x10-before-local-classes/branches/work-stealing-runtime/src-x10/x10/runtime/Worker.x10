 ...
 * @author tardieu
 */
public final class Worker(pool:Pool) implements ()=>Void {
	...
	// inner loop to help j9 jit
	private def loop(cond:FinishState):Boolean {
		for (var i:Int = 0; i < BOUND; i++) {
			if (cond.done() || pool.shutdown()) return false;
			activity = poll();
			if (activity == null) {
				activity = pool.scan(random, cond);
				if (activity == null) return false;
			}
			NativeRuntime.runAtLocal(activity.location.id, ()=>activity.run());
		}
