...
import x10.util.Map;
import x10.util.ValMap;

// This class provides a default implementation of the mege method based on getInitialValue() and reduce(). The merge method gets reused in the apply method of MapReducer.
abstract class MapGenerator[K, V] {
    public def getInitialValue() : V = {
        throw new UnsupportedOperationException();
    }

    public def reduce(v1 : V, v2 : V) : V = {
        throw new UnsupportedOperationException();
    }

    public def merge(m : Map[K, V], k : K, v : V) = {
        if (!m.containsKey(k)) {
            m.put(k, getInitialValue());
        }
        m.put(k, reduce(m.get(k)(), v));
    }
}

// MapReducer implments the Reducible class to provide the identity element and reduction operator for merging maps. 
abstract class MapReducer[K, V] extends MapGenerator[K, V] implements Reducible[ValMap[K, V]] {

    abstract public def accumulate() : Void;

    abstract public def toValMap(m : Map[K, V]) : ValMap[K, V];

    abstract public global safe def getValMap() : ValMap[K, V];

    abstract public global safe def zeroMap() : Map[K, V];

    public global safe def zero() = {
        return toValMap(zeroMap());
    }

    public global safe def apply(m1: ValMap[K, V], m2: ValMap[K, V]) = {
        val map = zeroMap();
...
// This class customizes the MapReducer class for hash maps by providing the identity elements specific to hash maps.
abstract class HashMapReducer[K, V] extends MapReducer[K, V] {

    public def toValMap(m : Map[K, V]) = {
        return ValHashMap.make[K, V](m);
    }

    public global safe def zeroMap() : Map[K, V] = {
        return new HashMap[K, V]();
    }
}

// This class provides a default mechanism for distributing objects that contain hash maps across places.
abstract class Distributor[K, V, T]{T <: MapReducer[K, V]} {
    val dist : Dist;
    val plh : PlaceLocalHandle[T];

    public def this(dist : Dist, init : () => T!) : Distributor[K, V, T]{T <: MapReducer[K, V]} = {
        this.dist = dist;
        this.plh = PlaceLocalHandle.make[T](dist, init);
    }

    public abstract def getReducer() : MapReducer[K, V];
    ...
    public def reduce() : ValHashMap[K, V] = {
        return finish (getReducer()) {
            ateach (p in dist) {
                offer plh().getValMap();
            }
        };
