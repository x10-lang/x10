...
This class implements the Poisson solver using the multigrid algorithm. For study purposes, 
different versions of the implementation of those stencil operators are presented. The choice
of the one where the computations/communications are overlapped can be made by setting the 
corresponding flag in class Util. To fully express the concurrency in the algorithm, foreach 
loop should be used to iterate over local arrays at each place. In the current implementation, 
doing so might not be optimal (too many threads) compared with using for loop. However, the
production version of X10 runtime should be responsible for optimizing foreach loops based on 
the available hardware resources. Also, it is nice to have the functionality of preprocessor
so that the code can switch to for or foreach loop without hand modification.
