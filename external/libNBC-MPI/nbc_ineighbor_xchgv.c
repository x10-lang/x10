...
int NBC_Ineighbor_xchgv(void *sbuf, int *scounts, int *sdispls, MPI_Datatype stype,
...

  {
    ...
    for(i = 0; i < nneighbors; i++) {
      res = NBC_Sched_recv((char*)rbuf+rdispls[i]*rcvext, false, rcounts[i], rtype, neighbors[i], schedule);
      if (NBC_OK != res) { printf("Error in NBC_Sched_recv() (%i)\n", res); return res; }
      res = NBC_Sched_send((char*)sbuf+sdispls[i]*sndext, false, scounts[i], stype, neighbors[i], schedule);
