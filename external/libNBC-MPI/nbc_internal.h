...
typedef struct {
    ...
    int             key;
    NBC_Handle *    handle;
} NBC_Icomm_split_args;

typedef enum {
    NBC_THREAD_COMM_SPLIT = 1,
    NBC_THREAD_EXIT
} NBC_Thread_pool_msg_t;

typedef struct {
    pthread_t       tids[1]; /* Future config/runtime size array */
    int             msg_type;
    union {
        NBC_Icomm_split_args    comm_split_args;
        /** ... potentially other types of thread args **/
    } arg_u;
    pthread_mutex_t lock;
    pthread_cond_t  cond;
} NBC_Thread_pool;
extern NBC_Thread_pool GNBC_thread_pool;
