  ...
  // Program constructs an array of target places
    public static void main(String[] args) {
        ...
        for (int i = 0; i < args.length; ++i) {
          ...
          Transpose2d trans = new Transpose2d(dimX,dimY,blockP,blockQ,debug);
   ...
   void swapAndTranspose(final int destIndex,int srcIndex){
       ...
       double srcArray[] = (double []) _srcMatrix[srcIndex];
   ...
   public Transpose2d(int x, int y, int P, int Q,boolean debug){
       _debug = debug;
     	_x = x;
	_y = y;
	_P = P;
	...
	startTimer(ARRAY_CONSTRUCTION);
         place cur_place = place.FIRST_PLACE;
          finish do {
             ...
             async(cur_place){
               int placeId = here.id;
	       _srcMatrix[placeId] = (java.lang.Object)(new double[elementArraySize]);
	       _destMatrix[placeId] = (java.lang.Object)(new double[elementArraySize]);
             }
    ...
    public Transpose2d(){ //default required for multivm
    ...
    public void initialize(){
         ...
         place cur_place = place.FIRST_PLACE;
          finish do {
	       ...
	       double [] srcArray = (double [])_srcMatrix[here.id];
               for(int i = 0;i < _x;++i)
                 for(int j = 0; j < _y;++j){
    ...
    //
    void transposeBlock(final double[] destArray,final double[] srcBlock){
     for(int i = 0; i < _x;++i)
       for(int j = 0; j < _y;++j){
     ...
     public void transpose(){
		...
		if(true)System.out.println("Elapsed time for "+here.id+"    :"+(localElapsed/A_BILLION)+" total:"+(elapsed/A_BILLION));
             }
