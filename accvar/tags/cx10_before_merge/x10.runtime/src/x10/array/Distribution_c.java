...
public abstract class Distribution_c extends dist /*implements Distribution*/ {
	...
	transient protected /*final*/ Set/*<place>*/ places;    
	...
	static final class Arbitrary extends Distribution_c {
		...
		transient private /*final*/ Map map_;
		
		private void writeObject(ObjectOutputStream out) throws IOException {
			out.defaultWriteObject();
			Object[] points = map_.keySet().toArray();
			int[] pids = new int[points.length];
			for(int k =0; k<points.length; k++) {
				pids[k]  = ((place)map_.get(points[k])).id;
			}
			out.writeObject(points);
			out.writeObject(pids);
			
			if(Report.should_report("cluster", x10.cluster.Debug.distarr))
				Report.report(x10.cluster.Debug.distarr, "Arbitrary.writeObject: "+ this);
		}
		private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
			in.defaultReadObject();
			Object[] points = (Object[]) in.readObject();
			int[] pids = (int[]) in.readObject();        	        
			
			map_ = new HashMap();
			places = new HashSet(); //parent's state
			point pt;
			place pl;
			
			for(int k=0; k<points.length; k++) {
				pt = (point) points[k];
				pl = Runtime.places()[pids[k]];
				places.add(pl); //parent state
				map_.put(pt, pl); //(key, val)
			}
