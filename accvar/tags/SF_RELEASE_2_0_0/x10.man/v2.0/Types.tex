...
Functions are instances of {\em function types|} {\Sref{Functions})
  ...
  in the envirnonment that are referenced in the body of the function
...
\index{\Xcd{x10.lang.Object}}
        ...
        extend the class \xcd"x10.lang.Objet".
...
public class Object {
    ...
    public def loc():Place;
    public def at(p:Place):boolean;
    public def at(o:Object):boolean;
}
\end{xten}
...
Variables of reference class type may contain the value \xcd"null". 
...
\index{properties!value properties}
        ...
        public final instance fields bound to object creation.  
%
...
The expression \xcdmath"X[S$_1$, $\dots$, S$_m$](e$_1$, $\dots$, e$_n$)"
...
\xcd":>") expressions over constants and types, and existential quantification over typed variables.
...
To ensure that type-checking is decidable, we introduce restrictions to ensure that
it is impossible to create cycles using just properties. Concretely, it is not possible
in \Xten{} to write a program which can create at runtime a set of objects such that
the graph formed by taking objects as nodes and adding an edge from $m$ to $n$ if
$m$ has a property whose value is $n$ has a cycle in it. 

The restirction is implemented by using the {\em proto} rules for visibility of {\tt this} during constructor execution (\Sref{Protorules}).
...
For convenience, a value \xcd"f" of a function type
\xcd"(x1:T1,...,xn:Tn){c}=>T" has two other functions associated with
it: \xcd"toString(): ()=> String" and \xcd"typeName():()=$>$String". These
functions are called using the method invocation syntax,
viz. \xcd"f.toString()" and \xcd"f.typeName()". The first returns an
implementation-defined string. The second returns a string
representation of the function type, \xcd"(x1:T1,...xn:Tn,)=$>$T". These
functions can also be named using method selector notation,
viz. \xcd"f.toString.()" and \xcd"f.typeName.()".


Function types do not support equality. Specifically, it is a
compile-time error for a value of a function type to be passed as
the first or second argument of an invocation of the operators \xcd"==" or
\xcd"!=". An important consequence is that the creation of a function does
...
implements clause; this declares an abstract method def
"apply(x1:T1,...,xn:Tn)\{c\}:T" on that class. Similarly, an interface
...
\xcd"null" is not a legal value for a function type. 
...
The type \xcd"x10.lang.Box[T]" is a \emph{boxed type},
and allows a struct type \xcd"T" to be used as a 
reference type. The programmer may wish to define his/her own 
conversions from structs to objects.
     ...
     implements Equals
{
    ...
    public def equals(x:Object) = this==x;
