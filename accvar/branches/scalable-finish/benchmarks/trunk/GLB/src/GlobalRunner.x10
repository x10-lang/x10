...
public final class GlobalRunner[T, Z]  implements Runner[T,Z] {
     ...
     */
    def this(maker:() => TaskFrame[T,Z]) 
	throws x10.util.OptionsParser.Err {
	this(Rail.make[String](0,(i:Int)=>""), Dist.makeUnique(), maker);
    }
    def this(dist:Dist, maker:() => TaskFrame[T,Z]) 
	throws x10.util.OptionsParser.Err {
	this(Rail.make[String](0,(i:Int)=>""), dist, maker);
    }
    def this(args:Rail[String]!, maker:() => TaskFrame[T,Z]) 
	throws x10.util.OptionsParser.Err {
	this(args, Dist.makeUnique(), maker);
    }
    def this(args:Rail[String]!, dist:Dist, maker: ()=> TaskFrame[T, Z]) 
	throws x10.util.OptionsParser.Err {
    ...
    final static class LocalRunner[T, Z] {
		...
		while  (true) {
		    ...
		    // steal that got processed in the async. Or some
		    // loot may have arrived.
		    // So reset lootSize.
		    lootSize = stack.size();
		    if (lootSize <= 2) {
	    ...
	    val n = myLifelines.length();
            for (var i:Int=0; (i<n) && (noLoot) && (0<=myLifelines(i)); 
                 ++i) {
		...
		val loots = ValRail.make[ValRail[T]](P-1,
			     (i:Int)=> stack.pop(lootSize));
