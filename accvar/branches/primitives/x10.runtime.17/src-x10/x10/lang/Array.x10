    ...
    implements (Point{self.rank==dist.region.rank})=>T
{
    ...
    public static def make[T](region:Region): Array[T](region)
        = makeVar[T](region, null as Nullable[(Point)=>T]) as Array[T](region);

    public static def make[T](dist: Dist): Array[T](dist)
        = makeVar[T](dist, null as Nullable[(Point)=>T]) as Array[T](dist);

    public static def make[T](region:Region, init: (Point)=>T): Array[T](region)
        = makeVar[T](region, new Nullable[(Point)=>T](init)) as Array[T](region);

    public static def make[T](dist: Dist, init: (Point)=>T): Array[T](dist)
        = makeVar[T](dist, new Nullable[(Point)=>T](init)) as Array[T](dist);
    ...
    public static def makeVar[T](region: Region, init: Nullable[(Point)=>T]): Array[T](region)
        = BaseArray.makeVar1[T](region, init) as Array[T](region);

    public static def makeVar[T](dist: Dist, init: Nullable[(Point)=>T]): Array[T](dist)
        = BaseArray.makeVar1[T](dist, init);

    public static def makeVal[T](region: Region): Array[T](region)
        = makeVal[T](region, null as Nullable[(Point)=>T]) as Array[T](region);

    public static def makeVal[T](dist: Dist): Array[T](dist)
        = makeVal[T](dist, null as Nullable[(Point)=>T]) as Array[T](dist);

    public static def makeVal[T](region: Region, init: Nullable[(Point)=>T]): Array[T](region)
        = BaseArray.makeVal1[T](region, init) as Array[T](region);

    public static def makeVal[T](dist: Dist, init: Nullable[(Point)=>T]): Array[T](dist)
    ...
    public static def make[T](size: nat, init: (Point)=>T): Array[T](1)
        = makeVar[T](0..size-1, new Nullable[(Point)=>T](init)) as Array[T](1);

    public static def makeFast[T](region: Region)
        = makeVar[T](region, null as Nullable[(Point)=>T])
            as FastArray[T]{region==region};

    public static def makeFast[T](dist: Dist)
        = makeVar[T](dist, null as Nullable[(Point)=>T])
            as FastArray[T]{dist==dist};

    public static def makeFast[T](region: Region, init: (Point)=>T)
        = BaseArray.makeVar1[T](region, new Nullable[(Point)=>T](init))
            as FastArray[T]{region==region};

    public static def makeFast[T](dist: Dist, init: (Point)=>T)
        = BaseArray.makeVar1[T](dist, new Nullable[(Point)=>T](init))
            as FastArray[T]{dist==dist};
