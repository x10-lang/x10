...
/**
 * Uses the {@link x10dt.ui.editor.formatting.X10HeuristicScanner} to
 ...
 * </p>
 */
public class X10Indenter {
	/**
	 * The X10 formatting preferences.
	 */
	private final class X10Prefs {
		final boolean prefSpacesForTab;
		final int prefTabSize;
		final int prefIndentationSize;
		// Most of the following are interpreted as multiples of fIndentationSize
		...
		private boolean isStandalone() {
			return X10DTUIPlugin.getInstance() == null;
		}
		 ...
		 */
		private String getFormattingOption(String key) {
			if (fProject == null)
				return JavaCore.getOption(key);
			return fPrefsSvc.getStringPreference(key);
		}

		X10Prefs(IProject project) {
			fProject= project;
			fPrefsSvc= new PreferencesService(fProject, X10DTCorePlugin.kLanguageName);
			if (isStandalone()) {
				prefSpacesForTab= false;
				prefTabSize= 4;
				...
				prefSpacesForTab= fPrefsSvc.getBooleanPreference(X10Constants.P_SPACESFORTABS);
		...
		private int prefIndentationSize() {
			return fPrefsSvc.getIntPreference(X10Constants.P_INDENTWIDTH);
		}
		...
		private int prefArrayIndent() {
			String option= getFormattingOption(DefaultCodeFormatterConstants.FORMATTER_ALIGNMENT_FOR_EXPRESSIONS_IN_ARRAY_INITIALIZER);
			try {
		...
		private boolean prefTernaryDeepAlign() {
			String option= getFormattingOption(DefaultCodeFormatterConstants.FORMATTER_ALIGNMENT_FOR_CONDITIONAL_EXPRESSION);
			try {
				return DefaultCodeFormatterConstants.getIndentStyle(option) == DefaultCodeFormatterConstants.INDENT_ON_COLUMN;
			} catch (IllegalArgumentException e) {
				// ignore and return default
		...
		private int prefCaseIndent() {
			if ("true".equals(getFormattingOption(DefaultCodeFormatterConstants.FORMATTER_INDENT_SWITCHSTATEMENTS_COMPARE_TO_SWITCH)))
		...
		private int prefCaseBlockIndent() {
			if ("true".equals(getFormattingOption(DefaultCodeFormatterConstants.FORMATTER_INDENT_SWITCHSTATEMENTS_COMPARE_TO_CASES)))
				return prefBlockIndent();
			else
				return 0;
		}
		...
		private boolean prefMethodDeclDeepIndent() {
			String option= getFormattingOption(DefaultCodeFormatterConstants.FORMATTER_ALIGNMENT_FOR_PARAMETERS_IN_METHOD_DECLARATION);
			try {
				return DefaultCodeFormatterConstants.getIndentStyle(option) == DefaultCodeFormatterConstants.INDENT_ON_COLUMN;
			} catch (IllegalArgumentException e) {
				// ignore and return default
		...
		private boolean prefMethodCallDeepIndent() {
			String option= getFormattingOption(DefaultCodeFormatterConstants.FORMATTER_ALIGNMENT_FOR_ARGUMENTS_IN_METHOD_INVOCATION);
			try {
				return DefaultCodeFormatterConstants.getIndentStyle(option) == DefaultCodeFormatterConstants.INDENT_ON_COLUMN;
			} catch (IllegalArgumentException e) {
				// ignore and return default
		...
		private int prefBlockIndent() {
			String option= getFormattingOption(X10Constants.P_INDENTBLOCKSTATEMENTS);
		...
		private int prefMethodBodyIndent() {
			if ("false".equals(getFormattingOption(X10Constants.P_INDENTMETHODBODY)))
				return 0;
		...
		private int prefTypeIndent() {
			String option= getFormattingOption(X10Constants.P_INDENTTYPEBODY);
			if ("false".equals(option))
				return 0;
		...
		private boolean prefIndentBracesForBlocks() {
			return DefaultCodeFormatterConstants.NEXT_LINE_SHIFTED.equals(getFormattingOption(DefaultCodeFormatterConstants.FORMATTER_BRACE_POSITION_FOR_BLOCK));
		}

		private boolean prefIndentBracesForArrays() {
			return DefaultCodeFormatterConstants.NEXT_LINE_SHIFTED.equals(getFormattingOption(DefaultCodeFormatterConstants.FORMATTER_BRACE_POSITION_FOR_ARRAY_INITIALIZER));
		}

		private boolean prefIndentBracesForMethods() {
			return DefaultCodeFormatterConstants.NEXT_LINE_SHIFTED.equals(getFormattingOption(DefaultCodeFormatterConstants.FORMATTER_BRACE_POSITION_FOR_METHOD_DECLARATION));
		}

		private boolean prefIndentBracesForTypes() {
			return DefaultCodeFormatterConstants.NEXT_LINE_SHIFTED.equals(getFormattingOption(DefaultCodeFormatterConstants.FORMATTER_BRACE_POSITION_FOR_TYPE_DECLARATION));
		}

		private int prefContinuationIndent() {
			try {
				return Integer.parseInt(getFormattingOption(DefaultCodeFormatterConstants.FORMATTER_CONTINUATION_INDENTATION));
	 ...
	 */
	private final X10HeuristicScanner fScanner;
	/**
	 * The X10 formatting preferences.
	 */
	private final X10Prefs fPrefs;
	 ...
	 */
	public X10Indenter(IDocument document, X10HeuristicScanner scanner) {
	 ...
	 * @since 3.1
	 */
	public X10Indenter(IDocument document, X10HeuristicScanner scanner, IProject project) {
		Assert.isNotNull(document);
		Assert.isNotNull(scanner);
		fDocument= document;
		fScanner= scanner;
		fPrefs= new X10Prefs(project);
	}
...
//			return null;
	...
	public StringBuffer computeIndentation(int offset, boolean assumeOpeningBrace) {
		...
		if (fAlign != X10HeuristicScanner.NOT_FOUND) {
			try {
	...
	private StringBuffer createIndent(int start, final int indent, final boolean convertSpaceRunsToTabs) {
		final boolean convertTabs= fPrefs.prefSpacesForTab && convertSpaceRunsToTabs;
	...
	public int findReferencePosition(int offset, int nextToken) {
		...
		if (offset < fDocument.getLength()) {
			try {
				...
				int prevToken= fScanner.previousToken(prevPos, X10HeuristicScanner.UNBOUND);
				boolean bracelessBlockStart= fScanner.isBracelessBlockStart(prevPos, X10HeuristicScanner.UNBOUND);
	...
	public int findReferencePosition(int offset, boolean danglingElse, boolean matchBrace, boolean matchParen, boolean matchCase) {
		...
		fAlign= X10HeuristicScanner.NOT_FOUND;
		fPosition= offset;
			...
			case Symbols.TokenGREATERTHAN:
				...
				switch (fToken) {
					case Symbols.TokenIDENT:
						...
						try {
							isGenericStarter= !X10HeuristicScanner.isGenericStarter(getTokenContent());
						} catch (BadLocationException e) {
							return false;
						}
						if (isGenericStarter)
	...
	private void nextToken(int start) {
		fToken= fScanner.previousToken(start - 1, X10HeuristicScanner.UNBOUND);
		fPreviousPos= start;
