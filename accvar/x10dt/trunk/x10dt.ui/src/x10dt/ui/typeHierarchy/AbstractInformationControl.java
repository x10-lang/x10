...
//	private CustomFiltersActionGroup fCustomFiltersActionGroup;
	 ...
	 * @since 3.5
	 */
	private ITypeInfo fInitiallySelectedType;
	 ...
	 */
	public AbstractInformationControl(UniversalEditor editor, Shell parent, int shellStyle, int treeStyle, String invokingCommandId, boolean showStatusField) {
...
//		fCustomFiltersActionGroup= new CustomFiltersActionGroup(getId(), fTreeViewer);
	 ...
	 * @param treeStyle the additional styles for the tree widget
	 */
	public AbstractInformationControl(UniversalEditor editor, Shell parent, int shellStyle, int treeStyle) {
		this(editor, parent, shellStyle, treeStyle, null, false);
	}
	...
	private TreeItem findElement(TreeItem[] items, TreeItem[] toBeSkipped, boolean allowToGoUp) {
		...
		for (int i= 0; i < items.length; i++) {
			...
			ITypeInfo element= (ITypeInfo)item.getData();
			if (element != null) {
...
//		fCustomFiltersActionGroup.fillViewMenu(viewMenu);
	...
	protected void inputChanged(Object newInput, Object newSelection) {
		...
		if (newSelection instanceof ITypeInfo) {
			ITypeInfo javaElement= ((ITypeInfo)newSelection);
//			if (javaElement.getElementType() == ITypeInfo.TYPE)
				fInitiallySelectedType= (ITypeInfo)javaElement;
//			else
//				fInitiallySelectedType= (ITypeInfo)javaElement.getAncestor(ITypeInfo.TYPE);
		}
	...
	protected IDialogSettings getDialogSettings() {
		...
		IDialogSettings settings= X10DTUIPlugin.getInstance().getDialogSettings().getSection(sectionName);
		if (settings == null)
			settings= X10DTUIPlugin.getInstance().getDialogSettings().addNewSection(sectionName);
