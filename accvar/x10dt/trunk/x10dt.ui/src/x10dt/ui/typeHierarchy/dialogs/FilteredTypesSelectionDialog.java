...
import x10dt.ui.typeHierarchy.DefaultTypeNameMatch;
import x10dt.ui.typeHierarchy.ITypeInfoFilterExtension;
import x10dt.ui.typeHierarchy.ITypeInfoImageProvider;
import x10dt.ui.typeHierarchy.ITypeSelectionComponent;
import x10dt.ui.typeHierarchy.Messages;
import x10dt.ui.typeHierarchy.SearchUtils;
import x10dt.ui.typeHierarchy.Strings;
import x10dt.ui.typeHierarchy.TextFieldNavigationHandler;
import x10dt.ui.typeHierarchy.TypeFilter;
import x10dt.ui.typeHierarchy.TypeInfoFilter;
import x10dt.ui.typeHierarchy.TypeInfoRequestorAdapter;
import x10dt.ui.typeHierarchy.TypeNameMatch;
import x10dt.ui.typeHierarchy.TypeNameMatchLabelProvider;
...
//		int matchRule= typeSearchFilter.getMatchRule();
		...
		try {
			...
			throw new CoreException(new Status(IStatus.ERROR, X10DTUIPlugin.PLUGIN_ID, e.getMessage(), e));
		} finally {
	...
	protected IStatus validateItem(Object item) {
		...
		if (item == null)
			return new Status(IStatus.ERROR, X10DTUIPlugin.PLUGIN_ID, IStatus.ERROR, "", null); //$NON-NLS-1$

		if (fValidator != null) {
			ITypeInfo type= ((TypeNameMatch) item).getType();
			if (!type.exists(new NullProgressMonitor())) {
				String qualifiedName= SearchUtils.getFullyQualifiedName(type);
				return new Status(IStatus.ERROR, X10DTUIPlugin.PLUGIN_ID, IStatus.ERROR, MessageFormat.format(Messages.FilteredTypesSelectionDialog_error_type_doesnot_exist, qualifiedName), null);
			}
			Object[] elements= { type };
			return fValidator.validate(elements);
		} else
			return new Status(IStatus.OK, X10DTUIPlugin.PLUGIN_ID, IStatus.OK, "", null); //$NON-NLS-1$
	}
	...
	private static class ConsistencyRunnable implements IRunnableWithProgress {
		public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
			if (fgFirstTime) {
				...
				manager.join(X10DTUIPlugin.PLUGIN_ID, monitor);
			}
