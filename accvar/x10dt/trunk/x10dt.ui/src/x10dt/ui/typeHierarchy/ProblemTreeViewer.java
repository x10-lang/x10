...
public class ProblemTreeViewer extends TreeViewer implements ResourceToItemsMapper.IContentViewerAccessor {
	...
	public void addFilter(ViewerFilter filter) {
		if (filter instanceof X10ViewerFilter) {
			((X10ViewerFilter) filter).filteringStart();
		}
	...
	public void removeFilter(ViewerFilter filter) {
		...
		if (filter instanceof X10ViewerFilter) {
			((X10ViewerFilter) filter).filteringEnd();
		}
	}
	...
	public void setFilters(ViewerFilter[] filters) {
		...
		for (int i= 0; i < filters.length; i++) {
			...
			if (curr instanceof X10ViewerFilter && !findAndRemove(oldFilters, curr)) {
				((X10ViewerFilter) curr).filteringStart();
			}
		}
	...
	private void endFilterSessions(ViewerFilter[] filters) {
		for (int i= 0; i < filters.length; i++) {
			ViewerFilter curr= filters[i];
			if (curr instanceof X10ViewerFilter) {
				((X10ViewerFilter) curr).filteringEnd();
			}
		}
	}
