...
public class OpenTypeHierarchyUtil {
	...
	public static TypeHierarchyViewPart open(IMemberInfo[] candidates, IWorkbenchWindow window) {
		...
		try {
			if (X10Constants.OPEN_TYPE_HIERARCHY_IN_PERSPECTIVE.equals(X10DTUIPlugin.getInstance().getPreferenceStore().getString(EditorActionDefinitionIds.OPEN_TYPE_HIERARCHY))) {
