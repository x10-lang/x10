...
/**
 * Implements a job that runs a command remotely or locally. It takes care of parsing the standard and error outputs of 
 * the process created to run the command.
 * 
 * @author egeay
 */
public abstract class AbstractX10RuntimeSystemJob extends Job {
  ...
  protected final IStatus run(final IProgressMonitor monitor) {
    ...
    if (monitor.isCanceled()) {
      ...
      return new Status(IStatus.OK, RMSCoreActivator.PLUGIN_ID, Messages.AXRSJ_JobCancelationMsg);
    }
    ...
    if (this.fJobProcess == null) {
      ...
      return new Status(IStatus.CANCEL, RMSCoreActivator.PLUGIN_ID, Messages.AXRSJ_JobCanceled);
    }
    ...
    return new Status(IStatus.OK, RMSCoreActivator.PLUGIN_ID, NLS.bind(Messages.AXRSJ_SuccessfulRunMsg,
  ...
  private void doExecutionStarted(final IProgressMonitor monitor) {
    if (! monitor.isCanceled()) {
      ...
      this.fStdOutThread = new Thread(new Runnable() {
        ...
        public void run() {
          try {
            processStandardOutput(outReader, ipJob);
          } catch (IOException except) {
            RMSCoreActivator.log(IStatus.ERROR, Messages.AXRSJ_OutputStreamReadingError, except);
          }
        }
      ...
      this.fStdErrThread = new Thread(new Runnable() {
        ...
        public void run() {
          try {
            processStandardError(errReader, ipJob);
          } catch (IOException except) {
            RMSCoreActivator.log(IStatus.ERROR, Messages.AXRSJ_ErrorStreamReadingError, except);
          }
        }
