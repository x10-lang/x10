...
//#ifdef HAVE_OFED   SS: We need to probe MPI for comm create
...
//#endif
  ...
  if (NBC_COMM_SPLIT_REQ == handle->request_type) {
    if (pthread_mutex_lock(&GNBC_thread_pool.lock)) {
        perror("pthread_mutex_lock");
        return NULL;
    }
    ret = * (volatile int *) &GNBC_thread_pool.arg_u.comm_split_args.handle->comm_split_status;
...
int NBC_Init_handle(NBC_Handle *handle, MPI_Comm comm) {
  ...
  /** Set type **/
  handle->request_type = NBC_COLLECTIVE_REQ;
...
NBC_Thread_pool GNBC_thread_pool;

int NBC_Init() {
    if (pthread_create(&GNBC_thread_pool.tids[0], NULL, 
                NBC_Helper_thread, NULL)) {
        perror("pthread_create");
    }
    if (pthread_mutex_init(&GNBC_thread_pool.lock, NULL)) {
        perror("pthread_mutex_init");
    }
    if (pthread_cond_init(&GNBC_thread_pool.cond, NULL)) {
...
int NBC_Finalize() {
    ...
    GNBC_thread_pool.msg_type = NBC_THREAD_EXIT;
    ...
    if (pthread_join(GNBC_thread_pool.tids[0], &status)) {
        perror("pthread_join");
    }

    if (pthread_cond_destroy(&GNBC_thread_pool.cond)) {
        perror("pthread_cond_destroy");
    }

    if (pthread_mutex_destroy(&GNBC_thread_pool.lock)) {
