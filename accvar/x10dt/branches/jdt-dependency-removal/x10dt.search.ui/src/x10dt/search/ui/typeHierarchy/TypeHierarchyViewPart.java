...
//	private SelectAllAction fSelectAllAction;
	...
	public TypeHierarchyViewPart() {
		...
		UISearchPlugin.getDefault().getPreferenceStore().addPropertyChangeListener(fPropertyChangeListener);
		...
		fDialogSettings= UISearchPlugin.getDefault().getDialogSettings();
...
//				}
	...
	private void updateInput(IMemberInfo inputElement) {
		...
		if (inputElement == null) {
			...
			try {
				fHierarchyLifeCycle.ensureRefreshedTypeHierarchy(inputElement, UISearchPlugin.getDefault().getWorkbench().getActiveWorkbenchWindow());
...
//				ExceptionHandler.handle(e, getSite().getShell(), TypeHierarchyMessages.TypeHierarchyViewPart_exception_title, TypeHierarchyMessages.TypeHierarchyViewPart_exception_message);
	...
	public void dispose() {
		...
		if (fPropertyChangeListener != null) {
			UISearchPlugin.getDefault().getPreferenceStore().removePropertyChangeListener(fPropertyChangeListener);
				...
				fRestoreStateJob= new Job(label) {
					protected IStatus run(IProgressMonitor monitor) {
						try {
							...
							return new Status(IStatus.ERROR, UISearchPlugin.PLUGIN_ID, e.getMessage());
						}
