...
public class MembersOrderPreferenceCache implements IPropertyChangeListener {
	...
	public void install(IPreferenceStore store) {
		...
		fSortByVisibility= store.getBoolean(X10Constants.APPEARANCE_ENABLE_VISIBILITY_SORT_ORDER);
	}
	...
	public static boolean isMemberOrderProperty(String property) {
		return X10Constants.APPEARANCE_MEMBER_SORT_ORDER.equals(property)
			|| X10Constants.APPEARANCE_VISIBILITY_SORT_ORDER.equals(property)
			|| X10Constants.APPEARANCE_ENABLE_VISIBILITY_SORT_ORDER.equals(property);
	}

	public void propertyChange(PropertyChangeEvent event) {
		String property= event.getProperty();

		if (X10Constants.APPEARANCE_MEMBER_SORT_ORDER.equals(property)) {
			fCategoryOffsets= null;
		} else if (X10Constants.APPEARANCE_VISIBILITY_SORT_ORDER.equals(property)) {
			fVisibilityOffsets= null;
		} else if (X10Constants.APPEARANCE_ENABLE_VISIBILITY_SORT_ORDER.equals(property)) {
			fSortByVisibility= fPreferenceStore.getBoolean(X10Constants.APPEARANCE_ENABLE_VISIBILITY_SORT_ORDER);
		}
	}
	...
	private int[] getCategoryOffsets() {
		...
		String key= X10Constants.APPEARANCE_MEMBER_SORT_ORDER;
	...
	private int[] getVisibilityOffsets() {
		...
		IPreferenceStore store= fPreferenceStore;
		String key= X10Constants.APPEARANCE_VISIBILITY_SORT_ORDER;
