...
import blas.MatrixBlas; //Including the package of BLAS native functions

//public class SummaTeam {

public class SummaTeamBlas { 
// This is BLAS-based X10-Team SUMMA implementation
// Class: Compute C = alaph * A * B + beta * C
	...
	//
	val rowTeam:Array[Team](1); //Collective communicator among places (processes) in the same row
	val colTeam:Array[Team](1); //Collective communicator among places (processes) in the same colomn
	//
	...
	public def this(ps:Int,                /* Size of panel */
		...
		//
		// Create a team for each block row
		for (var r:Int=0; r<rowC_Block; r++) {
			val rid = r;
			// Create collective communicator for places in the same block row
			val p = new Array[Place](colC_Block, 
									 (i:Int)=>Place.place(i*rowC_Block+rid));
			rowTeam(r) = Team(p);
		}
		//
		// Create a team for each block column
		for (var c:Int=0; c<colC_Block; c++) {
			val cid = c;
			// Create collective communicator for places in the same block column
