...
public class RunSummaXT {
	...
	//
	dist = Dist.makeUnique(); //Create unique distribution, each place is assigned with one data index
	//
	//------------- Distribute matrix data in block among all places -----------
	A = DistArray.make[Array[Double](1)](dist);  // Initial the unique data distribution for matrix. 
	setDistMatrix(A, rowBs, colBs); // Allocate and set data of a matrix block on each place
	//
	...
	Console.OUT.printf("Starting X10 SUMMA-Team: Matrix:%ix%i, %ix%i blocks(places), panel:%i\n", 
	...
	Console.OUT.printf("X10 SUMMA-Team (%ix%x): Run time: %3.3f (s)  MFLOPS: %6.3f (%6.3f for each proc)\n", 
	...
	val sM = args.size > 0 ? Int.parse(args(0)):8; //The first option set the size of matrix, default is 8
	val sP = args.size > 1 ? Int.parse(args(1)):1; //The second set the panel size, default is 1
