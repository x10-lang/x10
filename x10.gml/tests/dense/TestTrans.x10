...
public class TestTrans extends x10Test {
	...
	DenseMatrixBLAS.compTransMult(ET(1.0), a, v, ET(0.0), c1);
	...
	DenseMatrixBLAS.compTransMult(ET(1.0), a, v, ET(1.0), c1);
	...
	DenseMatrixBLAS.compTransMultTrans(ET(1.0), a, b, ET(0.0), cm);
	...
	bm.initRandom();
	val cm = DenseMatrix.make(M,N); 
	DenseMatrixBLAS.compTransMult(ET(1.0), am, bm, ET(0.0), cm);
	...
	val cm = DenseMatrix.make(M,N);
	DenseMatrixBLAS.compMultTrans(ET(1.0), am, bm, ET(0.0), cm);
	...
	bm.initRandom();
	val cm = DenseMatrix.make(M,N);
	DenseMatrixBLAS.compTransMultTrans(ET(1.0), am, bm, ET(0.0), cm);
