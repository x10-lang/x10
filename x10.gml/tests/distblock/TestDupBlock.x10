...
public class TestDupBlock extends x10Test {
	...
	val ddm = DupBlockMatrix.makeDense(M, N, bM, bN).init((r:Long, c:Long)=>ET(1.0+r+c));
	...
	dupblk.init((r:Long,c:Long)=>ET(1.0*(r+c)%2*(r+c)));
	...
	val dm1:DupBlockMatrix = dm  * ET(-1.0);
	val dm0 = dm + dm1;
	...
	var ret:Boolean=true;
	val a = DupBlockMatrix.makeDense(grid).init((r:Long,c:Long)=>ET(1.0*((r+c))));
	val b = DupBlockMatrix.makeDense(grid).init((r:Long,c:Long)=>ET(1.0*((r+c))));;
	val c = DupBlockMatrix.makeSparse(grid, nzp).init((r:Long,c:Long)=>ET(1.0*((r+c)%3)));
	val c1 = a + b + c;
	ret &= c1.checkSync();
	val c2 = a + (b + c);
	ret &= c2.checkSync();
