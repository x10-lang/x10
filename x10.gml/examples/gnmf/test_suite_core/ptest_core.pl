...
sub PrintOut {
  ...
  #------------------------------------------------------------------------
  printf("# $ProcPerNode per node, total proc (places), matrix size: %i, nonzero:%5.1f million\n",
	 $msz, $msz*100000*$NZDensity/1000/1000);
  print("#             |");
  ...
  foreach my $test (@TestExecList) {
	...
	foreach my $ms (@MatrixSizeList) {
	  ...
	  foreach my $nn (@NodeList) {
		...
		#print "Test on $np proc, matrix size $ms x $ms, panel size $ps\n";
		my @retval = RunTest("$launch -N$nn -n$np $BatchModeOpt $wrap $testexe $ms $NZDensity $itnum");
		$dnp{$nn} = [ @retval ]; #{ %dps };
	  }
	  $dms{$ms} = { %dnp };
	}
