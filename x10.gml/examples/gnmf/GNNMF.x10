...
public class GNNMF {
    ...
    public static def make(gridV:Grid, gridW:Grid, gridH:Grid, blockMap:DistMap, nzd:Float, it:Int) {
    ...
    public static def make(vM:Long, vN:Long, nzd:Float, it:Int, vRowBs:Long, vColBs:Long) {
        //Preset parameters
        val wColBs = 1;
    ...
    public def printInfo():void {
        val nzc =  V.getTotalNonZeroCount();
        val nzd =  nzc / (V.M * V.N *1.0f);
        val gV = V.getGrid();
        ...
        Console.OUT.printf("V nonzero density: %f, total nonzero count: %d\n", nzd, nzc);
