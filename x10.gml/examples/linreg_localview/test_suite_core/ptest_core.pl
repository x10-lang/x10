...
# Users have no need to reset the following variables in most of cases
#
...
$LogFile = "./TestOut_core.log";
print "# Test output log can be found at $LogFile\n";
#
my @TestLog = ();
my %Result ;
...
sub ParseData {
  ...
  while(<TEST_IN>) {
      if ($_ =~ m/$cmd/) {
          while (<TEST_IN>) {
                if ($_ =~ m/Parallel linear regression --- total:([\d.]+) ms commuTime:([\d.]+) ms paraComp:([\d.]+) ms/ ) {
                  return ($1, $2, $3);
                }
          }
      }
...
sub PrintOut {
  ...
  my $msz = $MatrixSizeList[0];
  #------------------------------------------------------------------------
  print("# Node list [ @NodeList ] ($ProcPerNode per node)\n");
  printf("# Sparse matrix V, rows:%i, nonzero: %2.1f Million\n",
                 $msz, $msz*$MatrixV_cols*$NZDensity/1000/1000);
 ...
 print("\n");
  ...
  #-----------------------------------------
  for my $np (@NodeList)      {
        printf("  %9i   |", $np*$ProcPerNode);
        for my $ms (@MatrixSizeList) {
          for my $t (@TestExecList)  {
                my $exe = $t->{name};
                my $t = 1.0 * $Result{$exe}{$ms}{$np}[0] / $itnum/1000;
                my $s = 1.0 * $Result{$exe}{$ms}{$np}[1] / $itnum/1000;
                my $p = 1.0 *  $Result{$exe}{$ms}{$np}[2] / $itnum/1000;
                printf("| %10.4f | %10.4f |", $t, $s);
          }
        }
        print "\n";
  }
	...
	foreach my $ms (@MatrixSizeList) {
	  ...
	  foreach my $nn (@NodeList) {
		my $np = $nn * $ProcPerNode;
                my $exe_param = "-m $ms -n $MatrixV_cols --density $NZDensity --iterations $itnum";
