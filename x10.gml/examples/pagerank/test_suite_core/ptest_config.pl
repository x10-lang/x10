...
printf "\#PageRank scalability benchmark test on topology size (core/place)\n";
...
#
@TestExecList = (
	{name => "../RunPageRank_mpi", 	alloc => "salloc",  wrapper => "$gml_path/scripts/srun.mvapich"},
	{name => "../RunPageRank_sock", alloc => "salloc",  wrapper => "$gml_path/scripts/srun.x10sock"},
	{name => "RunPageRank",         alloc => "salloc",  wrapper => "$gml_path/scripts/srun.x10java x10 -mx 2048M $jopts"}
);
