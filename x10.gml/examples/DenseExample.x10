...
public class DenseExample{
    ...
    public static class RunDenseExample(M:Long, N:Long) {
        ...
        public def testScale():Boolean{
            ...
            val dm1  = dm * (2.5 as ElemType);             //Perform scaling operation on all elements using operator overloading
            dm1.scale((1.0/2.5) as ElemType);              //Perform scaling operation using method call
        ...
        public def testAdd():Boolean {
            val dm = DenseMatrix.make(M, N);
            dm.initRandom();
            val dm1 = dm * (-1.0 as ElemType);
            val dm0 = dm + dm1;              //Perform matrix cell addition 
            val ret = dm0.equals(0.0 as ElemType);
            if (!ret)
