...
# All definations overwrite the default values in ptest script.
#-----------------------------------------------------------------

printf "\#Linear regression scalability benchmark test on topology size (core/place)\n";
...
@MatrixSizeList = ( 20000000 );
printf "\#Set matrix dimension (col/row) list : ( @MatrixSizeList )\n";

$MatrixV_cols = 100000;
printf "\#Set matrix V columns                : $MatrixV_cols\n";
...
$ProcPerNode   = 8;               # Process/core/place running on each node
...
$LogFile = "./TestOut_core.log";  # Output log file name
...
#
@TestExecList = (
	{name => "../RunLinReg_mpi",  alloc => "salloc",  wrapper => "$gml_path/scripts/srun.mvapich"},
	{name => "../RunLinReg_sock", alloc => "salloc",  wrapper => "$gml_path/scripts/srun.x10sock"},
	{name => "RunLinReg",         alloc => "salloc",  wrapper => "$gml_path/scripts/srun.x10java x10 -mx 2048M $jopts"}
);

#printf "Configuration read done\n";
