...
int* partition_matrix(int total_n, int block_n, int* bdim)
{
  ...
  //PPOUT4("P%i set dim %i %i\n", myid, total_n, block_n);
  for (i=0; i< block_n; i++)
	{
	  if (k) 
		{
				              ...
				              int* Ccols, double* Cdata) /* Crows == Arows */
{
  ...
  panel_size = ps;
  //if (myid == 0)
  //	printf("P%i set the panel size to %i\n", myid, panel_size);
  //fflush(stdout);
  //work1 = (double*) malloc (sizeof(double)*sM*panel_size);
  work1 = new double[sM*panel_size];
  //work2 = (double*) malloc (sizeof(double)*sN*panel_size);
...
/*   fflush(stdout); */
  ...
  //printf("P%i before free temp space\n", myid); fflush(stdout);
  //free(work2);
  //free(work1);
  delete work2;
  delete work1;
  //printf("P%i Done free temp space\n", myid); fflush(stdout);
}
