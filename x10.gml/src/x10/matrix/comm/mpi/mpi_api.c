...
{
#ifdef MPI_COMMU
  int64_t* pbuf = (off>0)?(buf+off):buf;
...
{
#ifdef MPI_COMMU
  int64_t* sbuf = (sendoff>0)?(sendbuf+sendoff):sendbuf;
  int64_t* rbuf = (recvoff>0)?(recvbuf+recvoff):recvbuf;
...
//--------------------------
void mpi_reduce_sum_long(int64_t* sendbuf, int soff, int64_t* recvbuf, int roff,  int cnt, int root)
{
#ifdef MPI_COMMU
  //int MPI_Reduce(void* sendbuf, void* recvbuf, int count, 
  //			   MPI_Datatype datatype, MPI_Op op, int root, MPI_Comm comm) 
  int64_t* sbuf = (soff>0)?(sendbuf+soff):sendbuf;
  int64_t* rbuf = (roff>0)?(recvbuf+roff):recvbuf;
...
//------------------------
void mpi_allreduce_sum_long(int64_t*sendbuf, int soff, int64_t* recvbuf, int roff, int cnt)
{
#ifdef MPI_COMMU
