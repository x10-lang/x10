...
public class SparseCSRBuilder {
	...
	public static def makeTranspose(spamat:SparseCSR): SparseCSCBuilder {
	...
	public def findIndex(r:Int, c:Int) : Int {
		var i:Int = findLastIndexInRow(r);
		for (; i>=0; i--) {
	...
	public def findLastIndexInRow(c:Int) : Int {
		var i:Int = 0;
	...
	public def sortRowMajor() {
		nonzeros.sort((nz1:NonZeroEntry,nz2:NonZeroEntry)=>cmpRowMajor(nz1,nz2));
	}
	...
	public def toSparseCSR():SparseCSR {
		val spa = SparseCSR.make(M, N, nonzeros.size());
		toSparseCSR(spa);
		return spa;
	}
	...
	public def toSparseCSR(spa: SparseCSR) {
		...
		for (var l:Int=0; l<M; l++) {
			...
			for (var c:Int=0; c<rowsizes(l); c++, idx++) {
				val nzv = nonzeros.get(idx).value;
				//Remove all zero entries
				if (MathTool.isZero(nzv)) continue;
				ca.index(cnt) = nonzeros.get(idx).col;
	...
	public def toString():String {
		...
		str.add("Sparse matrix in CSR ["+M+","+N+"] nonzero entry list:\n");
