...
#
# $Id: mk.plot.2.sh,v 1.1 2008-06-26 12:25:41 srkodali Exp $
#
...
out_file=dfs_torus_altair_color.jpg
...
printf "set title \"Size: 1000\"\n" >> $tmp_file
printf "set noylabel\n" >> $tmp_file
printf "set noytics\n" >> $tmp_file
printf "set xlabel \" \"\n" >> $tmp_file
printf "set origin 0.25,0\n" >> $tmp_file
printf "set size 0.25,1\n" >> $tmp_file
printf "plot \"${java_tbl}\" index 1 using 3:5 title \"XWS\" with linespoints lw 2 pt 4, \\" >> $tmp_file
printf "\n" >> $tmp_file
printf "\"${c_tbl}\" index 1 using 3:5 title \"C\" with linespoints lw 2 pt 4, \\" >> $tmp_file
printf "\n" >> $tmp_file
printf "\"${cilk_tbl}\" index 1 using 3:5 title \"Cilk\" with linespoints lw 2 pt 4" >> $tmp_file
printf "\n" >> $tmp_file

printf "set xlabel \" \"\n" >> $tmp_file
printf "set title \"Size: 2000\"\n" >> $tmp_file
