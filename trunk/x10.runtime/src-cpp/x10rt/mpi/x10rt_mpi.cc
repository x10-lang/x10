...
typedef struct _x10rt_net_get_req {
    int                       type;
    int                       dest_place;
    void                    * msg;
    int                       msg_len;
...
typedef struct _x10rt_net_put_req {
    int                       type;
    void                    * msg;
    int                       msg_len;
    int                       len;
...
/* differentiate from x10rt_net_{get|put}_req
 * to save precious bytes from packet size
 * for each PUT/GET */
typedef struct _x10rt_net_nw_req {
    int                       type;
    int                       msg_len;
    int                       len;
} x10rt_net_nw_req;
...
class x10rt_net_req {
        ...
        void                * buf;
        union {
            x10rt_net_get_req         get_req;
            x10rt_net_put_req         put_req;
        } u;
    public:
        x10rt_net_req()  { next = prev = NULL;  buf = NULL; type = -1; }
        ~x10rt_net_req() { next = prev = NULL;  buf = NULL; type = -1; }
        ...
        void setGetReq(x10rt_net_get_req * r) {
        ...
        void setPutReq(x10rt_net_put_req * r) {
            u.put_req.type    = r->type;
            u.put_req.msg     = r->msg;
            u.put_req.msg_len = r->msg_len;
            u.put_req.len     = r->len;
        }
        x10rt_net_get_req * getGetReq() {
            assert(X10RT_GET_INCOMING_DATA == type);
            return &u.get_req;
        }
        x10rt_net_put_req * getPutReq() {
            assert(X10RT_PUT_INCOMING_DATA == type);
            return &u.put_req;
        }
        friend class x10rt_net_req_queue;
};

class x10rt_net_req_queue {
    ...
    public:
        ...
        x10rt_net_req * next(x10rt_net_req * r) {
            x10rt_net_req * n;
            if(pthread_mutex_lock(&lock)) {
                perror("pthread_mutex_lock");
                exit(EXIT_FAILURE);
        ...
        void addRequests(int num) {         /* wrap around enqueue (which is thread safe) */
            for(int i=0; i<num; ++i) {
                char * mem = ChkAlloc<char>(sizeof(x10rt_net_req));
                x10rt_net_req * r = new(mem) x10rt_net_req();
                enqueue(r);
            }
        }
        void enqueue(x10rt_net_req * r) {       /* thread safe */
        ...
        x10rt_net_req * pop() {                 /* thread safe */
            if(pthread_mutex_lock(&lock)) {
                perror("pthread_mutex_lock");
                exit(EXIT_FAILURE);
            }

            x10rt_net_req * r = head;
            if(NULL != head) { 
                head = head->next; 
                len --; 
                if(NULL == head) {
        ...
        void remove(x10rt_net_req * r) {        /* thread safe */
            if(pthread_mutex_lock(&lock)) {
                perror("pthread_mutex_lock");
                exit(EXIT_FAILURE);
            }
        ...
        x10rt_net_req * popNoFail() {           /* wrap around pop (which is thread safe) */
            x10rt_net_req * r = pop();
            if(NULL == r) {
...
typedef void *(*putCb1)(const x10rt_msg_params &, x10rt_copy_sz len);
typedef void (*putCb2)(const x10rt_msg_params &, x10rt_copy_sz len);
typedef void *(*getCb1)(const x10rt_msg_params &, x10rt_copy_sz len);
typedef void (*getCb2)(const x10rt_msg_params &, x10rt_copy_sz len);

class x10rt_net_internal_state {
    public:
        bool                init;
        bool                finalized;
        pthread_mutex_t     lock;
        ...
        x10rt_net_req_queue     free_list;
        x10rt_net_req_queue     pending_list;

        x10rt_net_internal_state() {
            init                = false;
            finalized           = false;
...
static x10rt_net_internal_state     global_state;
...
void *x10rt_net_msg_realloc (void *old, size_t, size_t new_sz)
{
    return ChkRealloc<void>(old, new_sz);
}
void *x10rt_net_get_realloc (void *old, size_t, size_t new_sz)
{
    return ChkRealloc<void>(old, new_sz);
}
void *x10rt_net_put_realloc (void *old, size_t, size_t new_sz)
{
    return ChkRealloc<void>(old, new_sz);
}
...
void x10rt_net_send_get (x10rt_msg_params &p, void *buf, unsigned long len)
{
    int                 get_msg_len;
    x10rt_net_req         * req;
    x10rt_net_nw_req      * get_msg;
    x10rt_net_get_req       get_req;
     ...
     *  <--- x10rt_net_nw_req --->
     */
    get_msg_len         = sizeof(x10rt_net_nw_req) + p.len;
    get_msg             = ChkAlloc<x10rt_net_nw_req>(get_msg_len);
    get_msg->type       = p.type;
...
void x10rt_net_send_put (x10rt_msg_params &p, void *buf, unsigned long len)
{
    int put_msg_len;
    x10rt_net_put_req * put_msg;
    assert(global_state.init);
    assert(!global_state.finalized);

    x10rt_net_req * req = global_state.free_list.popNoFail();
     ...
     *  <------ x10rt_net_put_req ----->
     */
    put_msg_len         = sizeof(x10rt_net_put_req) + p.len;
    put_msg             = ChkAlloc<x10rt_net_put_req>(put_msg_len);
    put_msg->type       = p.type;
    put_msg->msg        = p.msg;
...
static void send_completion(x10rt_net_req_queue * q, x10rt_net_req * req)
{
    free(req->getBuf());
    q->remove(req);
...
static void recv_completion(int ix, int bytes, x10rt_net_req_queue * q, x10rt_net_req * req)
{
    assert(ix>0);
...
static void get_incoming_data_completion(x10rt_net_req_queue * q, x10rt_net_req * req)
{
    x10rt_net_get_req * get_req = req->getGetReq();
...
static void get_outgoing_req_completion(x10rt_net_req_queue * q, x10rt_net_req * req)
{
    free(req->getBuf());
    q->remove(req);
    global_state.free_list.enqueue(req);
...
static void get_incoming_req_completion(int dest_place, x10rt_net_req_queue * q, x10rt_net_req * req)
{
    /*      GET Message
     * +-------------------------------------+
     * | type | msg_len | len | <- msg ... ->|
     ...
     *  <--- x10rt_net_nw_req --->
     */
    x10rt_net_nw_req * get_nw_req = (x10rt_net_nw_req *) req->getBuf();
...
static void get_outgoing_data_completion(x10rt_net_req_queue * q, x10rt_net_req * req)
{
    q->remove(req);
    global_state.free_list.enqueue(req);
}

static void put_outgoing_req_completion(x10rt_net_req_queue * q, x10rt_net_req * req)
{
    free(req->getBuf());
    q->remove(req);
    global_state.free_list.enqueue(req);
...
static void put_outgoing_data_completion(x10rt_net_req_queue * q, x10rt_net_req * req)
{
    q->remove(req);
    global_state.free_list.enqueue(req);
}

static void put_incoming_req_completion(int src_place, x10rt_net_req_queue * q, x10rt_net_req * req)
{
    /*      PUT Message
     * +-------------------------------------------+
     * | type | msg | msg_len | len | <- msg ... ->|
     ...
     *  <------ x10rt_net_put_req ----->
     */
    x10rt_net_put_req * put_req = (x10rt_net_put_req *) req->getBuf();
    int len = put_req->len;
...
static void put_incoming_data_completion(x10rt_net_req_queue * q, x10rt_net_req * req)
{
    x10rt_net_put_req   * put_req = req->getPutReq();
...
void check_pending(x10rt_net_req_queue * q)
{
    int num_checked = 0;
    ...
    x10rt_net_req * req = q->start();
    while((NULL != req) &&
            num_checked < X10RT_MAX_PEEK_DEPTH) {
        int complete = 0;
        x10rt_net_req * req_copy = req;
