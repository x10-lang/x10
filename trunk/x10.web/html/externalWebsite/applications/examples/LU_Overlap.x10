...
The three steps (pivot, exchange, update) are sequential in LU, although there is parallelism in each of them. 
...
public class LU_Overlap extends x10Test {
	final static double eps=0.00000000001;
	double [.] m_A; //Can this array be referred to globally? Right now, it can be (error?). Otherwise, it has to be in a wrapper class.
	region m_R;
	dist m_D;
	int m,n;
	int m_pivotInfo; 
	int [] m_p; //Can be also be referred to globally. It looks like an error.
	//flags for updating part1(the first column) and part 2 (the rest columns) of the submatrix
	final boolean [] m_update=new boolean [2];
