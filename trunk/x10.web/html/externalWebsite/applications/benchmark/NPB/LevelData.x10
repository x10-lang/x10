...
expressiveness rather than to generate performance numbers. But we use X10 VM's support for 
abstract performance metrics to study the potential performance impacts of expressing the 
concurrency in the MG algorithm explicitly in X10.

This class implements the distributed array for one level of refinement in the MG benchmark. 
It is implemented using the encapsulation apprach rather than through a distribution. The 
reason is that each sub-array overlapps with each other because of the adding of a one-deep 
layer of ghost cells. The method exchange is used to update these ghost cells and to apply 
the periodic physical boundary condtion.

For study purposes, different versions of the methods in this class are presented. The choice
of whether update the ghost cells after each operation that modifies the values on inner region
or do so only when the update is needed can be made by setting the corresponding flag in class 
Util. To fully express the concurrency in the algorithm, foreach loop should be used to iterate 
over local arrays at each place when the oder of the iteration does not matter. In the current 
implementation, doing so might not be optimal (too many threads) compared with using for loop. 
However, the production version of X10 runtime should be responsible for optimizing foreach 
loops based on the available hardware resources. Also, it is nice to have the functionality of 
preprocessor so that the code can switch to for or foreach loop without hand modification.
...
public value LevelData {	
	...
	public LevelData(final int a_problemDomainSize, final boolean a_isParallel){
		...
		m_REGIONs=new region [m_numPlaces];
		m_boundaries=new region [6][m_numPlaces];
