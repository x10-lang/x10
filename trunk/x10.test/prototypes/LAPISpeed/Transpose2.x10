  ...
  // Program constructs an array of target places
    public static void main(String[] args) {
		...
		x10.lang.Runtime.setExitCode(99);
          ...
          Transpose2 trans = new Transpose2(dimX,dimY,blockP,blockQ,debug);
     ...
     public Transpose2(int x, int y, int P, int Q,boolean debug){
       _debug = debug;
     	_x = x;
	_y = y;
	_P = P;
    ...
    public Transpose2(){ //default required for multivm
    ...
    void transposeBlock(int placeId,double[.] destArray,final double[] srcBlock){
     for(int i = 0; i < _x;++i)
       for(int j = 0; j < _y;++j){
          ...
          _destMatrix[placeId,calculatedDestOffset] = srcBlock[calculatedSrcOffset];
        }
                ...
                if(here.id == destinationPlaceId){
                  ...
                  for(int i = 0; i < _x;++i)
                    for(int j = 0; j < _y;++j){
                      final int calculatedSrcOffset = i*_y +j;
                      final int calculatedDestOffset = j*_x+i;
                      _destMatrix[destinationPlaceId,calculatedDestOffset] = _srcMatrix[destinationPlaceId,calculatedSrcOffset];
                   }
                }
                else {
                  ...
                  for(int i = 0;i < _x;++i)
                    for(int j = 0; j < _y;++j){
                      int calculatedOffset = i*_y +j;
                      tempBlock[index++] = _srcMatrix[here.id,calculatedOffset];
                    }
