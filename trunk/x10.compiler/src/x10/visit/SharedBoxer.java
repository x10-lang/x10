...
public class SharedBoxer extends ContextVisitor {
    ...
    protected Node leaveCall(Node parent, Node old, Node n, NodeVisitor v) throws SemanticException {
        ...
        if (n instanceof X10Local_c) {
            if (!(parent instanceof X10LocalAssign_c && X10Flags.toX10Flags(((X10LocalAssign_c) parent).local().flags()).isShared())) {
    ...
    private Node visitLocalAssign(X10LocalAssign_c n) {
        if (X10Flags.toX10Flags(n.local().flags()).isShared()) {
            ...
            try {
                ...
                mi = xts.findMethod(type, xts.MethodMatcher(right.type(), Name.make("set"), typeArguments, context));
            } catch (SemanticException e) {
                throw new InternalCompilerError("Something is terribly wrong", e);
            }
            if (bo == null) { // i.e. =         a = b -> a.set(b)
                return xnf.Call(position, local, xnf.Id(position, "set"), right).methodInstance(mi).type(right.type());
            } else {          // i.e. += *= ... a += b -> a.set(a.get() + b)
                return xnf.Call(position, local, xnf.Id(position, "set"), xnf.Binary(position, createWrappingCall(local), bo, right).type(right.type()))
