...
\bard{What is this para here for?}
Properties are described in (\Sref{properties}): in brief they are immutable
instance fields of values that can be used at compile type to express static
constraints. Property methods (\Sref{properties}) permit more complex
expressions to be defined using properties and other property methods. A
method is \Xcd{safe} if it has certain behavioral characteristics
\Sref{SafeAnnotation}. It is \Xcd{global} if it can be invoked at any place.


\bard{Squish this list into text?}
...
A class, interface, or struct or  with one or more type parameters is a
\emph{generic class},  \emph{generic interface}, or \emph{generic struct}.
(\Sref{Generics}).
...
\label{properties}
        ...
        public \xcd`val` instance fields bound on object creation.  
%
        For example, the following code declares a class named
        \xcd"Coords" with properties \xcd"x" and \xcd"y"
...
%~~neg
        ...
        \xcd"Coords{x==0}" is the set of all points whose \xcd"x"
        property is \xcd"0".
...
\index{types!generic types}
        ...
        declared with one or more type parameters.  
(Structs and interfaces may be generic as well.)
Generic
