...
\bard{This is a troublesome topic.  It is necessary to compute some common
ancestor of types: the conditional expression \xcd`t ? a : b` 
and a function with multiple return sites 
\xcd`def f(x:Int) { ... return a; ... return b; ...}`
will be given some type generalizing both 
\xcd`typeof(a)` and \xcd`typeof(b)`.
However, both the constraint system and covariant generic types allow the
possibility of types without a {\bf least} common ancestor. 
Current concept: X10 may compute any common ancestor, possibly all the way to
\xcd`Any`. However, it will try to compute a good one. 
guarantee that if the types of \xcd`a` and \xcd`b` are
identical, the common ancestor chosen is that type too.  If the types are
\xcd`T{e}` and \xcd`T{f}`, the common ancestor will be \xcd`T{g}` for some
\xcd`g` (entailed by both \xcd`e` and \xcd`f` -- perhaps just \xcd`true`). 
}
