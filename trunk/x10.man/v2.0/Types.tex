...
In \XtenCurrVer{} value constraints may be equalities 
(\xcd"=="), disequalities (\xcd"!=") and conjunctions thereof.
The terms over which these constraints are specified include
literals and (accessible, immutable) variables and fields,
and the special constants {\tt here}, {\tt self}, and {\tt this}.
...
To ensure that type-checking is decidable, we introduce restrictions
to ensure that it is impossible to create cycles using just
properties. Concretely, it is not possible in \Xten{} to write a
program which can create at runtime a set of objects such that the
...
The type \xcd"Box[T]" is unrelated to \xcd"T".
