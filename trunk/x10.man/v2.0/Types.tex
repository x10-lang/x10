...
%~fails~neg
\noindent
Inside \xcd`ConsTy`, \xcd`T` is constrained to be a subtype of \xcd`U`.
Outside of \xcd`ConsTy`, even if \xcd`T` and \xcd`U` are both defined as type
variables, there 
is no reason to expect any relationship between them. 

\limitation{Subtyping of type variables does not currently work.}

Subtyping of types that have no free variables does not depend
on the context, however.    For example, \xcd`Int{self != 0} <: Int` is always
...
\xcd`class C extends D{d} implements I1, ..., In {...}`
then \xcd`C` is a subtype of \xcd`D{d}`, \xcd`I1`, ..., \xcd`In`. 

\item
{\bf Interface types:}  
Given the definition 
\xcd`interface I extends I1, ... In {...}`
then \xcd`I` is a subtype of \xcd`I1`, ..., \xcd`In`.

\item 
{\bf Struct types:} 
Given the definition 
\xcd`struct S implements I1, ..., In {...}`
then \xcd`S` is a subtype of \xcd`I1`, ..., \xcd`In`. 
...
\item 
{\bf Type Variables:}
In the scope of a constraint which entails a relations between type
expressions  \xcd`A <: B`, \xcd`A` is a subtype of \xcd`B`. 
