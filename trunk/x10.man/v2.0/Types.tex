...
\bard{Explain the reason for this discontinuous behavior!}
...
Intuitively, type \xcdmath"T$_1$" is a subtype of type \xcdmath"T$_2$" if
...
This section formalizes the concept of subtyping. 

The subtyping relation \xcd`T <: U` is the 
...
\bard{
I reformulated the following definition to be entirely in terms of direct
subtyping.  It would be good to know that I haven't broken anything.
}

\begin{itemize}
\item
{\bf Class types:}  
A class type is a direct subtype of
any class it is declared to extend.  A class type is direct subtype
of any interfaces it is declared to implement.

\item
{\bf Interface types:}  An interface type is a direct
subtype of any interfaces it is declared to extend.

\item 
{\bf Struct types:} 
A struct type is a direct subtype of any interface it is declared to
implement. 
...
if: 
\begin{enumerate}
\item Each \xcdmath"T$_i$" is a direct subtype of or equal to the corresponding
      \xcdmath"T$'_i$";
\item \xcd`c` entails \xcdmath"c$'$";
\item \xcdmath"T$'$" is a direct subtype of or equal to \xcdmath"T";
\item Every \xcdmath"EX$_j$" is a direct subtype of or equal to some \xcdmath"EX$'_{j'}$".
\end{enumerate}

\item
{\bf Constrained types:}
\xcd`T{c}` is a direct subtype of \xcd`T{d}` if \xcd`c` entails \xcd`d`. 

\item {\bf Any:} 
Every type \xcd`T` is a direct subtype of \xcd`x10.lang.Any`.

\item 
{\bf Type Variables}
\bard{I don't know that this clause even makes sense -- are type variables
types?  Also it defines direct subtyping }
A type variable \xcd`X` appearing as a parameter of a class, interface,
struct, or method is a direct subtype of another type \xcd`Y` if the
constraints on the class etc. imply that \xcd`X <: Y`.
...
must be computed.  For example, the type of 
%~~exp~~`~~`~~b:Boolean ~~
\xcd`b ? 1 : true`
has to be something which admits both \xcd`1` and \xcd`true` as values. 

Given types \xcd`T` and \xcd`U`, if there exists a 
type \xcd`V` such that \xcd`T <: V`, and \xcd`U <: V`, and, for all types
\xcd`W` such that \xcd`T <: W` and \xcd`U <: W` it is the case that 
\xcd`V <: W`, then \xcd`V` is a least common ancestor of \xcd`T` and \xcd`U`.  
If there is no such \xcd`V`, then \xcd`x10.lang.Any` is defined to be the
least common ancestor of \xcd`T` and \xcd`U`. 

While there are, in general, many least common ancestors of \xcd`T` and
\xcd`U`, they are all equivalent.  (If \xcd`V` and \xcd`W` are least common
ancestors, then \xcd`V <: W` and \xcd`W <: V` by the definition of ``a least
common ancestor'', whence \xcd`V == W`.)  The X10 compiler may pick any one of
them when it needs to do this computation. 

\bard{This definition is flawed at best.  There is a [near]infinite 
descending chain of common ancestors for \xcd`0U` and \xcd`1U`, but no
reasonable least one.  The matter is under consideration now.}
...
The package \xcd"x10.lang" provides a number of built-in class, struct, and
