...
called {\em clocks}.  Clocks are structured to allow barrier synchronization
without introducing either deadlocks or race conditions.

%%CLOCK-INTRO%% Clocks permit dynamically created
%%CLOCK-INTRO%% activities to register and deregister. An activity may be registered
%%CLOCK-INTRO%% with multiple clocks at the same time. In particular, nested clocks
%%CLOCK-INTRO%% are permitted: an activity may create a nested clock and within one
%%CLOCK-INTRO%% phase of the outer clock schedule activities to run to completion on
%%CLOCK-INTRO%% the nested clock.  Nevertheless the design of clocks ensures that
%%CLOCK-INTRO%% deadlock cannot be introduced by using clock operations, and that
%%CLOCK-INTRO%% clock operations do not introduce any races.
