...
class.)  For example, a (ridiculous) way to print the sum of two numbers is: 
%~~gen
% package Extern.or_current_turn;
%~~vis
\begin{xten}
...
\bard{What constructs can be used in native code?  return? throw? type
variables?}
...
\begin{itemize}
\item The first \# means to use the default compiler for the architecture (from
      x10rt properties file). 
\item The second \# is substituted for the .cc files and CXXFLAGS that would
      ordinarily be used. 
\item The third \# is substituted for the libraries and LDFLAGS that would
      ordinarily used.
\item For the second and third, if a \% is used instead of a \# then the
      parameters are suppressed and suitable parameters must be provided
      manually. This allows a complete override of the postcompiler behaviour.
\end{itemize}
