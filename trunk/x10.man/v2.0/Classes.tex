...
\item Guarantee that fields are read only after they are initialized.

\item Ensure that immutable cyclic object graphs can be
  created.\footnote{(Mutable graphs can be created without escaping \Xcd{this}
    ny initializing the backpointer to \Xcd{null} and then
    changing it later.}  This requires that it be possible to pass an
  object under construction into a constructor invocation.

\item Ensure that user-defined methods can be called during object
...
\item Ensure the design is minimally invasive. Most programmers should
