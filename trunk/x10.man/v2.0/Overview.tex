...
(\Sref{ReferenceClasses}) and \emph{structs}
(\Sref{Structs}). Containers may have type parameters, have fields,
methods and inner containing types and can implement one or more
interfaces. A method or an immutable field of a
container may be marked \Xcd{global}.  For a given entity (instance of
...
\paragraph{Functions.}
\Xten{} provides functions (\Sref{Closures}) to allow code to be used
...
\xcd"Vector" class with an \xcd"add" method.
%~~gen
%~~vis
\begin{xten}
struct Vector(x: Int, y: Int) {
    def add(p: Vector) = Vector(x+p.x,y+p.y);
}
\end{xten}
%~~siv
%~~neg
...
The dependent type \xcd"Vector{x==0}" is the type of all points
