...
as \Xcd{global}. The value of these fields is accessible at every place the object can be referenced.
...
\item Method invocation (\Sref{MethodInvocation}).  A method may be
  ...
  \Xcd{safe}, and \Xcd{pure}.


A method may be marked \Xcd{nonblocking}. 
A \Xcd{nonblocking} method
may not contain any \Xcd{when} statement whose condition is not
statically equivalent to \Xcd{true}m, and must call only
\Xcd{nonblocking} methods.


A method may be marked \Xcd{sequential}. A \Xcd{sequential} method 
...
\item  If the \Xcd{global} qualifier is omitted, the field is considered non-global. 
...
The \Xten{} compiler ensures that non-global methods on \Xcd{o} can only be invoked in a place where \Xcd{here== o.home()}, i.e. the place where \Xcd{o} was created.
