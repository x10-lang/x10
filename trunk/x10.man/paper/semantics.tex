...
&&&\\
\end{tabular}
\caption{Syntax for {\sf CFJ}}\label{CFJ-syntax}
\end{figure*}
...
\end{array}
$$
\caption{\CFJ{} subtyping judgement}\label{CFJ-subtyping}
\end{figure*}
...
we define $\mathit{fields}({\tt T},\theta)$ to be $\bar{S}\theta\ \bar{f}$.
...
\\ \quad \\
\rname{T-Field}%
\from{\Gamma \vdash {\tt T}_0\ {\tt e} \ \ \ \mathit{fields}({\tt T}_0,{\tt z}_0)= \bar{\tt U}\ \bar{\tt f}_i \ \ \ \mbox{(${\tt z}_0$ fresh)}} 
...
\mathit{mtype}({\tt T}_0,{\tt m},{\tt z}_0)= \tt {\tt Z}_{1:n}\ {\tt z}_{1:n}:c \rightarrow {\tt S} \\
...
\from{
  ...
  \theta=[\bar{\tt f}/\this.\bar{\tt f}] & 
    \mathit{fields}(C,\theta)=\bar{\tt Z}\ \bar{\tt f} \\
...
\caption{{\sf CFJ} type judgement}\label{CFJ-type}
\end{figure*}
...
\begin{array}{ccc}
\rname{{\sc R-Field}}%
\from{\mathit{fields}(C)=\bar{C}\ \bar{f}}
\infer{(\new\ {\tt C}(\bar{\tt e})).{\tt f}_i \derives {\tt e}_i} &
\rname{{\sc R-Invk}}%
\from{\mathit{mbody}({\tt m},{\tt C})=\bar{x}. {\tt e}_0}
\infer{(\new\ {\tt C}(\bar{\tt e})).{\tt m}(\bar{\tt d}) \derives 
[\bar{d}/\bar{x},\new\ C(\bar{e})/\this]{\tt e}_0} &
\rname{{\sc R-Cast}}%
\from{\epsilon \vdash C \subtype T[\new\ C(\bar{\tt d})/\self]}
...
provided that $\mathit{fields}({\tt C})=\bar{\tt T}\ \bar{\tt f}$ (for some 
...
{\tt this}---{\tt this} is permitted to occur in the clause of a type
{\tt T} only
...
\TUCast{}, \TDCast{} and\TSCast{} for upwards cast, downwards
cast, and ``stupid'' cast respectively. 
...
the given static type (${\tt T}_0$). $\mathit{fields}({\tt T}_0,{\tt o})$ is
...
type $T$ and method name {\tt m}, let $\mathit{mtype}({\tt T},{\tt m},{\tt o})$
...
z}$) have the static type of the actuals)\footnote{This
is stronger than assuming $\bar{\tt Z}$.}
...
If $\mathit{mtype}({\tt T}_0,{\tt m})=\bar{\tt T}\ \bar{\tt x} : {\tt c}
\rightarrow {\tt S}$, and $\mathit{mbody}({\tt m}, {\tt T}_0)=\bar{\tt x}.{\tt
