...
The design also
supports the definition of dependent methods.  XXX remove
%Dependent types interact
%naturally with generic types.

As in staged languages~\cite{???}, the design distinguishes between
...
permitted.
These casts involve arithmetic, not
...
The design is implemented in the \Xten{} compiler.
No extension of an
...
parameter list}, a list of typed parameters, or {\em properties},
...
Given such a class definition for a class {\tt C},
types can be constructed by {\em constraining} the properties of
{\tt C}.
If the class {\tt C} has no properties, the only type that can be
constructed is the type {\tt C}.  On very general
principles one can argue that {\em any} boolean expression on the
properties should specify a type: the type of all instances of the
class which satisfy the boolean expression. Thus,
{\tt List(:length == 3)} should be a permissible type, as should
{\tt List(:length <= 41)} or even
{\tt List(:length >= f())}.

Accordingly we define a {\em parametric type} to be of the form
{\tt C(:e)}, the name of
a class or interface {\tt C} followed by a where clause {\tt e}, a boolean expression
on the properties of that class. We call {\tt C}
the {\em base class} (XXX root class) and {\tt e} the {\em condition}
of the parametric type. The denotation
...
non-empty the condition of $t$ must be consistent with the where clause
(if any) of the base class of $t$.  The compiler is required to
...
Constructors and methods may specify preconditions
...
\begin{tightcode}
    List(:length+arg.length) append(final List arg) \{ ... \}
\end{tightcode}
...
XXX huh?  What's the different btwn these and final formals.  --njn
...
XXX lots of redundancy WRT previous section
...
\begin{verbatim}
  public value class List( nat n ) <Node> {
    ...
    public static List(m)<nat> gen(nat m) {
         return gen(0,m);
    }
    /** Return a list of (m-i) elements, from i to m-1. */
    public static List(m-i)<nat> gen(nat i, nat m: i <= m) {
      return (i == m)
             ? new List(m-i)<nat>()
...
For instance, a
nullary constructor for {\tt List} is only defined when the property
...
\subsection{Creating instances of a dependent class}
...
where Interface may be a parametric interface {\tt I(:e)}. For such a
...
interface {\tt I}.  (A parameteric class or interface {\tt I} is taken as
shorthand for {\tt I(:true)}.) Further, to every method specified in the
...
\subsection{Types defined by dependent classes and interfaces}
...
Subtyping is undecidable.  Type-checker can report "yes", "no",
