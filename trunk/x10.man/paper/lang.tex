...
is the constraintnsolver, which operates on its own vocabulary of
...
Constrained types may occur in a class cast \xcd{(T) o}.  Code is
generated to check at run time that \xcd{o} satisfies \xcd{T}.
...
Let us define the real-clause of a constrained type \xcd{C(:c)} to be
the set of constraints that must be satisfied by any instance of
\xcd{
C(:c)}. This includes \xcd{c} but also includes constraints that hold
for all instances of \xcd{C}, as determined by the definition of
\xcd{
C}. Let us use the notation $\rc({\tt C})$ for the {\em real
clause} of \xcd{
...
What is $\rc({\tt C}, {\tt X})$ (we have drawn out \xcd{X} as the
...
\xcd{0} for which the real clause \xcd{self != null &
self.n == this.n-1 & self.n >= 0} is not satisfiable.
