...
A property is a \xcd{public} final instance field of the
...
\xcd{float} properties \xcd{x} and \xcd{y}). The type
\xcd{C\{self != null\}} represents all instances of \xcd{C}. When there is no
...
parameters, including the implicit parameter \xcd{this}.
...
\eat{
Note that the constraint may constrain the implicit parameter in
the properties of \xcd{this}; thus, it may be used to
specify that a method appears to be {\em conditionally} available on
some objects
...
\xcd{Region} with a \xcd{boolean} property \xcd{rect}:
\begin{xten}
...
The precondition \xcd{this.rect}
says that
...
It is a static error if \xcd{m1}'s erased signature is the same
as \xcd{m2}'s, but \xcd{m1} does not override \xcd{m2}.

It is also a static error if the method precondition on \xcd{m2} does
not entail the precondition on \xcd{m1}.  This restriction
ensures that if the a call
is type-checked against \xcd{m2} at the superclass type \xcd{D},
the precondition is satisfied if the method is dispatched to the
method \xcd{m1} in the subclass \xcd{C}.
...
\xcd{class} \xcd{C($\dots$)} \xcd{extends} \xcd{D\{d\}}.
