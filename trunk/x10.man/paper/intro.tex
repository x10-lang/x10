...
\tcd{(T1 x1, \ldots, Tk xk)} similar in syntactic structure to
...
types such as \xcd{int} and \xcd{double} are object types.}
...
We also permit the syntax \tcd{C(e1,\ldots,ek)} for
the type \tcd{C(:x1==e1 \& \ldots\ \& xk==ek)} (assuming that
the property list for \xcd{C} specifies the \xcd{k} properties
\xcd{x1}, \ldots, \xcd{xk}, and each term \xcd{ei} is of the correct
type).
...
In a constraint, the name \xcd{self} refers to the type being constrained.
The class has two
...
the \xcd{append} and \xcd{reverse} method, the return type
...
the method or constructor body.  A parameters may also depend on
another parameter in the argument list.

The use of constraints makes existential types very natural.
Consider the return type of \xcd{filter}: it specifies
that the list returned is of some unknown length. The only thing
known about it is that its size is bounded by \xcd{n}.
Thus,
...
library~\cite{omega} and the CVC3 theorem prover~\cite{cvc}.  A
separate set-based constraint system has been built using CVC3.
These constraint systems are described in
Section~\ref{sec:examples}; the implementation is
...
language.
We claim that the design is natural, easy to use, useful. Many
...
A formal semantics for core language with constrained types 
