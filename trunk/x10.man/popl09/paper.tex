...
and value-dependent types in an object-oriented language.  The
framework is instantiated on a constraint system $\cal O$ that
captures the inheritance, subtyping, and class membership
relationships of the program.  Further instantiations augment
the constraint system with dependent types and generic types.
Types in the framework are comprised of classes and a constraint
over types and values.  

The use of constraints on type properties allows the design to
capture many features of generics in languages like Java 5 and
\csharp and then to extend these features with new more
expressive power.
