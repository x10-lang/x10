...
Explain: main difference between CFJ and CFGJ.
Treat subclassing as a constraint.
Subtyping rule now:
\infrule{
\Gamma \vdash_{\cal C} {\tt T}_1 \subtype {\tt T}_2
}{
\Gamma \vdash {\tt T}_1 \subtype {\tt T}_2
}

Baked in constraints:
\infrule{
\mbox{\Xcdmath{class C[$\bar{\tt X}$]($\bar{\tt x}$: $\bar{\tt T}$)\{c\} extends T \{$\dots$\}}}
%}} \\
}{
%\vdash_{\cal C} \mbox{\Xcdmath{C\{c\}}} \subtype {\tt T}
\vdash_{\cal C} {\tt C} \subtype {\tt T}
}
...
\if 0
\infrule{
\mbox{\xcdmath"class C[$\bar{\tt X}$]($\bar{\tt x}$: $\bar{\tt T}$){c}"} \\
\mbox{\xcdmath"  extends T { $\bar{\tt K}$ $\bar{\tt M}$ $\bar{\tt F}$ }"}
\\
\mathit{mtype}(T,m) =
\mbox{\xcdmath"[$\bar{\tt X}'$]($\bar{\tt x}'$: $\bar{\tt T}'$){c'} => T'"} \\
\\
\mathit{mtype}(C,m) =
\mbox{\xcdmath"[$\bar{\tt X}$]($\bar{\tt x}$: $\bar{\tt T}$){c} => T"} \\
\\
...
For each property T, a static member class named T is introduced.
References to the type T continue to refer to T.

Constraints are erased from type references.
