...
formulas; we write \Xcd{p.f} (\Xcd{p.X}) as sugar for \Xcd{f(p)}
(\Xcd{f(X)}).
...
\andalso
\Gamma, \Xcd{x} \ty \Xcd{T}_0 \vdash \tbar{e} \ty \tbar{T}
}{
\Gamma \vdash \Xcd{\{ val x = e}_0\Xcd{;}~\tbar{e}~\Xcd{ \}} \ty \Xcd{T}_n
}
...
\Gamma \vdash \Xcd{\{ val x} \ty \Xcd{T}_0~\Xcd{= e}_0\Xcd{;}~\tbar{e}~\Xcd{ \}} \ty \Xcd{T}_n
}

\infrule[T-assign]{
\Gamma \vdash \Xcd{x} \ty \Xcd{T}
\andalso
\Gamma \vdash \Xcd{e} \ty \Xcd{T}
}{
\Gamma \vdash \Xcd{x}~\Xcd{=}~\Xcd{e} \ty \Xcd{T}
}

\infrule[T-if]{
\Gamma \vdash \Xcd{e}_0 \ty
\exists\Xcd{z}\ty\Xcd{Boolean}.~\Xcd{Boolean\{self==z\}}
\\
\Gamma, \Xcd{z==true} \vdash \Xcd{e}_1 \ty \Xcd{T}
\\
\Gamma, \Xcd{z==false} \vdash \Xcd{e}_2 \ty \Xcd{T}
}{
\Gamma \vdash \Xcd{if}~\Xcd{(e}_0\Xcd{)}~\Xcd{e}_1~\Xcd{else}~\Xcd{e}_2 \ty \Xcd{T}
}
}
...
\subsection{A graph-based inference algorithm}
...
\eat{
Credits: Nate Nystrom had the idea, designed the type
system and the type inference algorithm,
implemented the type checker, and sketched the
translation; Igor Peshansky implemented the translation; Vijay
Saraswat worked out the formal semantics and did the proof, and
implemented the constraint solver.  All authors contributed to the text.
}
