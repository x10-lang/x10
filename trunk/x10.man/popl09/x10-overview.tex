...
As implemented, \Xten simplifies the 
semantics presented here, limiting expressiveness in favor of usability.
After implementing the design presented here
and using it to implement part of the \Xten runtime library,
we found that the semantics are
often difficult for the uninitiated to use.
%---indeed, they
%provide comparable expressive power to Java wildcards, whose usability
%issues are well-known~\cite{wildcards-are-evil}.
We explore usability issues in Section~\ref{sec:usability}.
The semantics presented here do not have the restrictions
...
In the shortened syntax for types (e.g., \xcd"List[T]"),
...
As previously mentioned, after using the \Xten design described above,
we encountered some usability problems.
The fundamental problem is that programmers are used to
\emph{type parameters} rather than \emph{type properties};
...
% of a type rather than at the definition site, can lead to 
% subtle design bugs.
...
\end{xten}
This introduces a new set of type properties named \xcd"K" and
...
type information about all four properties in each instance
of \xcd"HashMap".

To correct this problem, the revised \Xten design imposes
restrictions on type properties so that they 
behave similarly to type parameters.
We make the syntax \xcd"e.X"
illegal and permit type properties to be accessible only from
within the body of their class definition via the
implicit \xcd"this" qualifier, and we require that type
constraints be written using the shortened \xcd"C[T]"
(or \xcd"C[+T]", etc.) syntax 
rather than as \xcd"C{X==T}" (\xcd"C{X<=T}").  The compiler also handles the
inheritance problem by ensuring inherited type properties are
not visible from subclasses.


\todo{This is still all a lie.  We support definition-site variance only}
