...
\Xcd{?} \Xcd{extends} \Xcd{T} and \Xcd{?} \Xcd{super} \Xcd{T} respectively.
%
...
constraints
via the rules for ``\Xcd{x} \Xcd{has} \Xcd{I}''.
...
Consider again the \xcd"List" class in Figure~\ref{fig:list0}.
The expression
...
If the check fails, the cast throws a
...
With type constraints the generated code must be able to check
that a given type is a subtype of another.  Since subtyping
of constrained types requires checking entailment of
constraints, the generated code must be be able to check
entailment at runtime.
One way to achieve this is to embed
the constraint solver itself into the runtime system.
This solution, however, can result in inefficient run-time casts
...
entailment checks.
This is done by analyzing the program to identify which pairs of
...
checking is thus implemented as reachability checking. 
This solution is a whole program analysis; all
constraints must be visible to generate the graph.

Finally, the language can be restricted so that entailment checking is
efficient.
For equality constraints on types, one can simply test equality on
the base types and on 
canonical representations of each constraint.
%
We restrict subtyping constraints to be used only on value types.  Since
the value cannot been mutated we can use the value itself rather than the
value's type properties to test entailment.
For instance, for an immutable \xcd"List" class, for the cast
\xcd"xs as List{T==int}", we test that \xcd"xs.head" is an
instance of \xcd"int" and recursively test \xcd"xs.tail".
Since all elements of \xcd"xs" are \xcd"int" and since the list
is immutable, it is safe to treat \xcd"xs" as 
\xcd"List{T==int}".
