...
\subsection{Run-time constraint solving}

Covariance, mutability, erasure.

Constraints are normally evaluated symbolically at compile time.
However, with run-time casts, constraints may be evaluated at
run time.  With constraints on values, this evaluation consists
of simply evaluating the constraint expression.
With constraints on types, however, subtyping tests must be
perfomed at run-time.  This can require constraint solving.
