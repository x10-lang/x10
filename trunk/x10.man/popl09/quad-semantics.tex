...
We then demonstrate other languages in the family with features such as dependent types, \FGJ-like generic types, or structural subtyping constraints. Each language is derived from \FXGL{\cdot} in a rigorous way. %Finally, we prove the soundness of their type systems.
...
\item specifing what types the variables of constrainted type-parameters types are subtypes of.
\item specifing the member of variables of constrainted type-parameters types.
\end{itemize}
...
\infrule {\Gamma\vdash \phi}{\Gamma\vdash\false} whenever $\phi$ (or its consequences) would violates any of the above.

Formally, the proof of the soudness result is built on top of lemmas so that the main body of the proof does not directly depend on member lookup rules and subtyping rules for types parameters but do it through lemmas that exactely formalize points 1 and 2 above.
