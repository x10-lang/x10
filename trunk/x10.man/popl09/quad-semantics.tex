...
\infrule[C-Extends]
	{\Gamma\vdash{\tt T}:\type,{\tt U}:\type}
	{\cj{\Gamma}{{\tt T}\extends {\tt U}}}

\infrule[X-Subtype]
	{\Gamma\vdash{\tt T}:\type,{\tt U}:\type,{\tt T}\subtype {\tt U}}
	{\Gamma\vdash {\tt T}\extends {\tt U}}
...
\quad\typicallabel{XXX}
\infrule[C-Pred]
	{{\tt p}(\tbar{T})\in \mathcal{A} \andalso \Gamma\vdash\tbar{t}:\tbar{U},\tbar{U}<:\tbar{T}} 
	{\cj{\Gamma}{{\tt p}(\tbar{t})}}
\end{minipage}%
\begin{minipage}{.55\textwidth}
\quad\typicallabel{XXX}
\infrule[T-Fun]
	{{\tt f}(\tbar{T})\Rightarrow{\tt U}\in \mathcal{A} \andalso \Gamma\vdash\tbar{e}:\tbar{V},\tbar{V}<:\tbar{T}\andalso \tbar{y}~\rm fresh} 
	{\Gamma\vdash{{\tt e}(\tbar{t})}:\exists\tbar{y}:\tbar{V}.~{\tt U}\{\self=={\tt f}(\tbar{y})\}}
\end{minipage}%
\caption{\FXD}
\end{figure*}
