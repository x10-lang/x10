...
In the previous section we presented X10. The subtle issues we encountered in the process of designing and implementing the X10 type system led us to believe we needed a formal framework in which to explore the design space and that would let us reason about fundamental issues such as soundness, completeness, and decidability. The framework we came up with consists in a family of formal languages---the \FXG family---that share a common base language, operational semantics, and type system.

Our framework models a lot of the relevant bits of X10 but not all.
Following \FJ, our it does not account for mutations. Proving the soundness of the type system for an imperative calculus is beyond the scope of this paper.
...
\subsection{\FXGL{X,\bullet}}

The grammar for \FXGL{X} is shown in Figure~\ref{fig:fxg-grammar}. The syntax is essentially that of \FGJ. We use $\bar{x}$ to denote a list $x_1, \dots, x_n$, and use $\bullet$ to denote the empty list.
...
Each language in the \FXGL{L} family is defined over a given untyped input constraint system that is required to support the constraint permitted by the \FXGL{L} grammar---\true{}, \false{}, conjunction, and equality constraints for \FXGL{X}.
...
In order to access the constraint system from the type system, we define the {\em constraint projection} $\sigma(\Gamma)$ that, in essence, strips out all type information from the $\Gamma$.
%
\begin{center}
\begin{tabular}{ll}
...
By rule {\sc X-Proj} in Figure~\ref{fig:object}, we specify that $\Gamma\vdash{\tt c}$ if the constraint projection of $\Gamma$ entails $c$ in the input constraint system. In \FXGL{X}, there is no other way to prove entailment.
...
\infrule[X-Proj]
  ...
  {\Gamma\vdash c}
\end{minipage}%
\caption{\FXGL{X} object constraint system.}
\label{fig:object}
\end{figure*}
...
\infrule[W-And]
	{\cj{\Gamma}{\tt c} \andalso \cj{\Gamma,{\tt c}}{\tt d}}
	{\cj{\Gamma}{{\tt c},{\tt d}}}
...
\quad\typicallabel{XXXX}
\infrule[X-Bound]
	{\Gamma\vdash{\tt X}\extends{\tt G}\comma{\tt X}\extends{\tt H} \andalso
	  {\tt G}\neq{\tt H}}
	{\Gamma\vdash\false}
\end{minipage}%
...
\infrule[W-Struct]
	...
	{\wj{\Gamma}{{\tt X}~\underline\has~\msign{Y}{y}{G}{c}{H}}}
