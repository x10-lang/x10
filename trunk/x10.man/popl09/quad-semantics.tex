...
\infrule[Exists]
	{\Gamma \vdash {\tt t}: {\tt V} \andalso \Gamma \vdash {\tt T}[{\tt t}/{\tt x}]: \type}
	{\Gamma \vdash \exists {\tt x}:{\tt V}.~{\tt T}: \type} 
...
\infax[Fields-B]
{{\tt x}:{\tt Object} \vdashO \fields({\tt x})=\bullet}

\infrule[Fields-I]
{\Gamma, {\tt x}:{\tt D} \vdashO \fields({\tt x})=\tbar{g}: \tbar{V}
 \\
 {\tt class}~{\tt C}(\tbar{f}:\tbar{V})\{{\tt c}\}~{\tt extends}~{\tt D}~\{~\tbar{M}~\} \in {\tt P}}
{\Gamma, {\tt x}:{\tt C} \vdashO \fields({\tt x})=\tbar{g}: \tbar{V}, \tbar{f}: \tbar{U}[{\tt x}/{\tt this}]}

\infrule[Fields-C,E]
{\Gamma, {\tt x}: {\tt S} \vdashO \fields({\tt x})=\tbar{f}: \tbar{V}}
{\Gamma, {\tt x}: {\tt S}\{{\tt d}\} \vdashO \fields({\tt x}) = \tbar{f}:
                                  \tbar{V}\{{\tt d}[{\tt x}/{\tt self}]\}
\\
\Gamma, {\tt x}: \exists {\tt y}: {\tt U}.~{\tt S} \vdashO
\fields({\tt x})=\tbar{f}:\exists {\tt y}:{\tt U}.~\tbar{V}
	}

\infrule[Method-B]
{\Gamma, {\tt x}:{\tt C}\vdashO\klass({\tt C})
 \andalso
 \theta=[{\tt x}/\this]
 \\
 {\tt class}~{\tt C}(\tbar{f}:\tbar{V})\{{\tt c}\}~{\tt extends}~{\tt D}~\{~\tbar{M}~\} \in {\tt P}
 \\
 {\tt def}~{\tt m}(\tbar{z}:\tbar{V})\{{\tt c}\}:{\tt V} = {\tt e} \in \tbar{M}}
{\Gamma, {\tt x}:{\tt C} \vdashO {\tt x}~\has~({\tt m}(\tbar{z}:\tbar{V}\theta)\{{\tt c}\theta\}:{\tt V}\theta={\tt e})}

\infrule[Method-I]
{\Gamma, {\tt x}:{\tt D} \vdashO {\tt x}~\has~({\tt m}(\tbar{z}:\tbar{V})\{{\tt c}\}:{\tt V}={\tt e})
\\
 {\tt class}~{\tt C}(\tbar{f}:\tbar{V})\{{\tt c}\}~{\tt extends}~{\tt D}~\{~\tbar{M}~\} \in {\tt P}
 \andalso
 {\tt m} \not\in \tbar{M}}
{\Gamma, {\tt x}:{\tt C} \vdashO {\tt x}~\has~({\tt m}(\tbar{z}:\tbar{V})\{{\tt c}\}:{\tt V}={\tt e})}

\infrule[Method-C,E]
{\Gamma, {\tt x}:{\tt S} \vdashO {\tt x}~\has~({\tt m}(\tbar{z}:\tbar{V})\{{\tt c}\}:{\tt V}={\tt e})}
{\Gamma, {\tt x}: {\tt S}\{{\tt d}\} \vdashO 
                                 {\tt x}~\has~({\tt
                                 m}(\tbar{z}:\tbar{V})\{{\tt
                                 c}\}:{\tt V\{d[{\tt x}/{\tt self}]\}}={\tt e})
\\
\Gamma, {\tt x}: \exists {\tt y}: {\tt U}.~{\tt S} \vdashO
                                 {\tt x}~\has~({\tt
                                 m}(\tbar{z}:\tbar{V})\{{\tt
                                 c}\}:{\tt \exists {\tt y}: {\tt U}.~{\tt V}}={\tt e})}
