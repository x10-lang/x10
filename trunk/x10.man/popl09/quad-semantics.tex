...
\newcommand\cj[2]{{#1} \vdash {\sf constraint}({#2})}
\newcommand\cjj[3]{{#1} \vdash {\sf constraint}({#2},{#3})}
\newcommand\wj[2]{{#1} \vdash {\sf type}({#2})}
...
\begin{tabular}{r@{\quad}rcl}
  ...
  (Generic type)& {\tt A}, {\tt B}, {\tt C}, {\tt F}, {\tt G}, {\tt H} &{::=}& a type {\tt T} without existential constraints or types \\
...
\infrule[\RField]
	{\vdash\fields({\tt C}[\tbar{V}])=\tbar{f}:\tbar{F}}
	{\new~{\tt C}[\tbar{V}](\tbar{v}).{\tt f}_i \derives {\tt v}_i}
...
\infrule[\RCNewArg]
	...
	{\new~{\tt C}[\tbar{V}](\ldots,{\tt e}_i,\ldots)\derives\new~{\tt C}[\tbar{V}](\ldots,{\tt e}'_i,\ldots)}

\infrule[\RInvk]
	{\vdash{\sf methods}({\tt C}[\tbar{V}]) \ni {\tt m}[\tbar{W}](\tbar{x}:\tbar{G})\{{\tt c}\}:{\tt H}={\tt e}}
	{\new~{\tt C}[\tbar{V}](\tbar{v}).{\tt m}[\tbar{W}](\tbar{a})\derives {\tt e}[\new~{\tt C}[\tbar{T}](\tbar{v}),\tbar{W},\tbar{a}/\this,\tbar{Y},\tbar{x}]}

\infrule[\RCInvkArg]
	{{\tt a}_i\derives {\tt a}'_i}
	{{\tt v}.{\tt m}(\ldots,{\tt a}_i,\ldots)\derives {\tt v}.{\tt m}(\ldots,{\tt a}'_i,\ldots)}

\infrule[\RCast]
	{\vdash{\tt C}[\tbar{V}]\{\self==\new~{\tt C}[\tbar{V}](\tbar{v})\}\subtype {\tt W}}
	{\new~{\tt C}[\tbar{V}](\tbar{v})~\as~{\tt U}\derives\new~{\tt C}[\tbar{V}](\tbar{v})}
\end{minipage}
...
\quad\typicallabel{XXXXXX}
\infrule[T-Var]
  {\wj{\Gamma}{\tt T} \andalso {\tt x}~\rm fresh}
...
\quad{}\typicallabel{XXXXXX}
\infrule[T-Field]
	{\Gamma\vdash {\tt e}:{\tt T} \andalso \Gamma,{\tt x}:{\tt T}\vdash {\tt x}~\has~{\tt f}:{\tt F} \andalso {\tt x}~\rm fresh}
	{\Gamma\vdash {\tt e}.{\tt f}:\exists {\tt x}:{\tt T}.~{\tt F}\{\self=={\tt x}.{\tt f}\}}

\infrule[T-New]
	{\Gamma\vdash\tbar{e}:\tbar{T} \andalso \wj{\Gamma}{{\tt C}[\tbar{A}]} \andalso 
	  \vdash\fields({\tt C}[\tbar{A}])=\tbar{f}:\tbar{F} \andalso 
	  \Gamma\vdash \tbar{T}\subtype\tbar{F} \andalso 
	  \Gamma,\tbar{x}:\tbar{T}\vdash{\sf inv}({\tt C}[\tbar{A}])[\tbar{x}/\this.\tbar{f}] \andalso
    \tbar{x}~\rm fresh}
	{\Gamma\vdash\new~{\tt C}[\tbar{A}](\tbar{e}):\exists\tbar{x}:\tbar{T}.~{\tt C}[\tbar{A}]\{\self==\new~{\tt C}[\tbar{A}](\tbar{x})\}}
        
\infrule[T-Invk]
	{\Gamma\vdash {\tt e}:{\tt T},\tbar{a}:\tbar{U} \andalso \wj{\Gamma,{\tt x}:{\tt T}}{\tbar{A}} \andalso
	  \Gamma,{\tt x}:{\tt T}\vdash {\tt
          x}~\has~{\tt m}[\tbar{A}](\tbar{y}:\tbar{G})\{{\tt k}\}:{\tt H}={\tt b},\tbar{U}\subtype\tbar{G}\andalso \Gamma,{\tt x}:{\tt E},\tbar{y}:\tbar{U}\vdash {\tt k} \andalso {\tt x},\tbar{y}~\rm fresh}
	{\Gamma\vdash {\tt e}.{\tt m}[\tbar{A}](\tbar{a}):\exists {\tt x}:{\tt T}.~\exists\tbar{y}:\tbar{U}.~{\tt H}}

\infrule[OK-Method]
  {{\tt class}~{\tt C}[\tbar{X}\cdot\tbar{Y}](\tbar{f}:\tbar{F})\{{\tt k}\}~{\tt extends}~{\tt D}[\tbar{X}]~\{~\tbar{M}~\}\in {\tt P} \andalso
    {\tt def}~{\tt m}[\tbar{Z}](\tbar{x}:\tbar{G})\{{\tt l}\}:{\tt H}={\tt e}\in\tbar{M}\andalso
    \wj{\tbar{X}\cdot\tbar{Y}\cdot\tbar{Z}:\type}{\tbar{G}}\\
    \cjj{\tbar{X}\cdot\tbar{Y}\cdot\tbar{Z}:\type,\this:{\tt C}[\tbar{X}\cdot\tbar{Y}],\tbar{x}:\tbar{G}}{\tt l}{\tbar{Z}}\andalso
   \tbar{X}\cdot\tbar{Y}\cdot\tbar{Z}:\type,\this:{\tt C}[\tbar{X}\cdot\tbar{Y}],\tbar{x}:\tbar{G},{\tt l}\vdash {\tt e}:{\tt A},{\tt A}\subtype {\tt H}}
  {\vdash{\tt def}~{\tt m}[\tbar{Z}](\tbar{x}:\tbar{G})\{{\tt l}\}:{\tt H}={\tt e}~{\rm OK~in}~{\tt C}[\tbar{X}\cdot\tbar{Y}]}

\infrule[OK-Class]
  {\wj{\tbar{X}\cdot\tbar{Y}:\type}{\tbar{F}}\andalso \wj{\tbar{X}:\type}{{\tt D[\tbar{X}]}} \andalso
    \tbar{x}~\rm fresh\\
  \cjj{\tbar{X}\cdot\tbar{Y}:\type,\tbar{x}:\tbar{F}}{{\tt k}[\tbar{x}/\this.\tbar{f}]}{\tbar{X}\cdot\tbar{Y}} \andalso 
  \tbar{M}~{\rm OK~in}~{\tt C}[\tbar{X}\cdot\tbar{Y}]}
  {\vdash {\tt class}~{\tt C}[\tbar{X}\cdot\tbar{Y}](\tbar{f}:\tbar{F})\{{\tt k}\}~{\tt extends}~{\tt D}[\tbar{X}]~\{~\tbar{M}~\}~\rm OK}
\end{minipage}
...
\quad\typicallabel{XXXXXX}
\infrule[X-Inv]
  {\Gamma\vdash {\tt t}:{\tt T} \andalso \Gamma,{\sf inv}({\tt T})[{\tt t}/\this]\vdash {\tt c}}
  {\Gamma\vdash {\tt c}}
\end{minipage}
\begin{minipage}{\textwidth}
\quad{}\typicallabel{XXXXXX}
\infrule[X-Sel]
  {\Gamma,{\tt x}:{\tt C}[\tbar{T}]\vdash{\tt x}~\has~\tbar{f}:\tbar{F}
  	\andalso
...
\infrule[W-Dep]
        {\wj{\Gamma}{\tt T} \andalso \cjj{\Gamma, \self \ty {\tt T}}{\tt c}{\emptyset}}
	{\wj{\Gamma}{{\tt T}\{{\tt c}\}}}
\infrule[W-Class]
  {{\tt class}~{\tt C}[\tbar{X}\cdot\tbar{Y}](\tbar{f}:\tbar{G})\{{\tt k}\}~{\tt extends}~{\tt D}[\tbar{X}]~\{~\tbar{M}~\} \in {\tt P} \andalso \wj{\Gamma}{\tbar{T}}}
  {\wj{\Gamma}{\tt C[\tbar{T}]}}
\infrule[W-Exists]
        {\wj{\Gamma}{\tt T} \andalso \wj{\Gamma,{\tt x}:{\tt T}}{\tt U} \andalso {\tt x}~\rm fresh}
	{\wj{\Gamma}{\exists {\tt x}:{\tt T}.~{\tt U}}}
\end{minipage}
\caption{{\sf FXG} well-formed types}
\label{fig:well}
\end{figure*}
...
\quad\typicallabel{XXX}
\infrule[S-Trans]
	{\Gamma\vdash {\tt S}\subtype {\tt T}, {\tt T}\subtype {\tt U}}
	{\Gamma\vdash {\tt S}\subtype {\tt U}}
\end{minipage}%
\begin{minipage}{.45\textwidth}
\quad\typicallabel{XXXX}
\infrule[S-Const-L]
	{\wj{\Gamma}{{\tt T}\{{\tt c}\}} \andalso\Gamma,{\tt c}\vdash{\tt T}\subtype {\tt U}}
...
\infrule[S-Class]
  {{\tt class}~{\tt C}[\tbar{X}\cdot\tbar{Y}](\tbar{f}:\tbar{G})\{{\tt k}\}~{\tt extends}~{\tt D}[\tbar{X}]~\{~\tbar{M}~\}\in {\tt P}\\
  \wj{\Gamma}{\tbar{A}} \andalso \wj{\Gamma}{\tbar{B}}}
  {\Gamma\vdash {\tt C}[\tbar{A}\cdot\tbar{B}]\subtype {\tt D}[\tbar{A}]}

\infrule[S-Exists-L]
  {\wj{\Gamma}{\tt U} \andalso \Gamma,{\tt x}:{\tt U}\vdash {\tt S}\subtype {\tt T} \andalso {\tt x}~\rm fresh}
  {\Gamma\vdash\exists {\tt x}:{\tt U}.~{\tt S}\subtype {\tt T}}
\end{minipage}%
\begin{minipage}{.5\textwidth}
\quad\typicallabel{XXXXX}
\infrule[S-Const-R]
	{\wj{\Gamma}{{\tt T}\{{\tt c}\}}\andalso\Gamma,\self:{\tt S}\vdash {\tt c},{\tt S}\subtype {\tt T}}
	{\Gamma\vdash {\tt S}\subtype {\tt T}\{{\tt c}\}}

\infrule[S-Exists-R]
  {\Gamma\vdash {\tt t}:{\tt U},{\tt S}\subtype {\tt T}[{\tt t}/{\tt x}]}
  {\Gamma\vdash {\tt S}\subtype\exists {\tt x}:{\tt U}.~{\tt T}}
\end{minipage}        
...
\quad\typicallabel{XXXXXX}
\infrule[L-Member-C]
  {\Gamma,{\tt x}:{\tt E},{\tt c}\vdash {\tt x}~\has~{\tt I}}
  {\Gamma,{\tt x}:{\tt E}\{{\tt c}\}\vdash {\tt x}~\has~{\tt I}}
...
\quad\typicallabel{XXXXXX}
\infrule[L-Member-E]
  {\Gamma,{\tt y}:{\tt E},{\tt x}:{\tt F}\vdash {\tt x}~\has~{\tt I} \andalso {\tt y}~\rm fresh}
  {\Gamma,{\tt x}:\exists {\tt y}:{\tt E}.~{\tt F}\vdash {\tt x}~\has~{\tt I}}

\infrule[L-Field-H]
  {\Gamma\vdash\fields({\tt C}[\tbar{A}])=\tbar{f}:\tbar{F}}
  {\Gamma,{\tt x}:{\tt C}[\tbar{A}]\vdash{\tt x}~\has~{\tt f}_i:{\tt F}_i}
\end{minipage}

\begin{minipage}{\textwidth}
\quad\typicallabel{XXXXXX}
\infrule[L-Fields-I]
  {{\tt class}~{\tt C}[\tbar{X}\cdot\tbar{Y}](\tbar{f}:\tbar{F})\{{\tt k}\}~{\tt extends}~{\tt D}[\tbar{X}]~\{~\tbar{M}~\}\in {\tt P}
    \andalso \wj{\Gamma}{\tbar{A}\cdot\tbar{B}} \andalso
   \Gamma\vdash\fields({\tt D}[\tbar{A}])=\tbar{g}:\tbar{G}}
   {\Gamma\vdash\fields({\tt C}[\tbar{A}\cdot\tbar{B}])=\tbar{g}\cdot\tbar{f}:\tbar{G}\cdot\tbar{F}[\tbar{A}\cdot\tbar{B}/\tbar{X}\cdot\tbar{Y}]}

\infrule[L-Method-B]
  {{\tt class}~{\tt C}[\tbar{X}\cdot\tbar{Y}](\tbar{f}:\tbar{F})\{{\tt k}\}~{\tt extends}~{\tt D}[\tbar{X}]~\{~\tbar{M}~\}\in {\tt P}
   \andalso
   {\tt def}~{\tt m}[\tbar{Z}](\tbar{x}\ty \tbar{G})\{{\tt l}\}\ty {\tt H}={\tt e}\in \tbar{M} \andalso
   \wj{\Gamma}{\tbar{A}\cdot\tbar{B}\cdot\tbar{C}}}
  {\Gamma\vdash {\sf methods}({\tt C}[\tbar{A}\cdot\tbar{B}]) \ni {\tt m}[\tbar{C}](\tbar{x}:\tbar{G}[\tbar{A}\cdot\tbar{B}\cdot\tbar{C}/\tbar{X}\cdot\tbar{Y}\cdot\tbar{Z}]/)\{{\tt l[\tbar{A}\cdot\tbar{B}\cdot\tbar{C}/\tbar{X}\cdot\tbar{Y}\cdot\tbar{Z}]}\}:{\tt H[\tbar{A}\cdot\tbar{B}\cdot\tbar{C}/\tbar{X}\cdot\tbar{Y}\cdot\tbar{Z}]}={\tt e}[\tbar{A}\cdot\tbar{B}\cdot\tbar{C}/\tbar{X}\cdot\tbar{Y}\cdot\tbar{Z}]}

\infrule[L-Method-I]
  {{\tt class}~{\tt C}[\tbar{X}\cdot\tbar{Y}](\tbar{f}:\tbar{F})\{{\tt k}\}~{\tt extends}~{\tt D}[\tbar{X}]~\{~\tbar{M}~\}\in {\tt P}
   \andalso
   \Gamma\vdash {\sf methods}({\tt D}[\tbar{A}]) \ni {\tt m}[\tbar{C}](\tbar{x}:\tbar{G})\{{\tt l}\}:{\tt H}={\tt e}
   \andalso
   \wj{\Gamma}{\tbar{B}}
   \andalso
   {\tt m}\not\in\tbar{M}}
  {\vdash {\sf methods}({\tt C}[\tbar{A}\cdot\tbar{B}]) \ni {\tt m}[\tbar{C}](\tbar{x}:\tbar{G})\{{\tt d}\}:{\tt H}={\tt l}}

\infrule[L-Method-H]
  {\Gamma\vdash {\sf methods}({\tt C}[\tbar{A}]) \ni {\tt m}[\tbar{B}](\tbar{x}:\tbar{E})\{{\tt d}\}:{\tt G}={\tt e}}
  {\Gamma,{\tt x}:{\tt C}[\tbar{A}]\vdash{\tt x}~\has~{\tt m}[\tbar{B}](\tbar{x}:\tbar{E})\{{\tt d}\}:{\tt G}={\tt e}}
\end{minipage}%
...
\quad\typicallabel{XXXXXX}
\infrule[A-Sub]
	{\Gamma\vdash {\tt G}\extends {\tt H} \andalso \wj{\Gamma}{\tt G} \andalso \wj{\Gamma}{\tt H}}
...
\quad\typicallabel{XXX}
\infrule[A-Inc]
{\Gamma\vdash bad~bounds}{\Gamma\vdash{\tt false}}
\end{minipage}%
\caption{{\sf FXG}$(<=)$}
\label{fig:FXG}
\end{figure*}
