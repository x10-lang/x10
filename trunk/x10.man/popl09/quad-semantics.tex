...
The modification of the $\underline\has$ lookup predicate is necessary to permit typing method invocations with receivers of generic types. It has the unfortunate side effect that we can no longer ensure that type derivations hence types are unique. For instance, given the class definitions
%
\begin{xten}
...
\end{xten}
%
occurrences of $\this.{\tt f}$ in {\tt D} has bound both by {\tt A} and {\tt B} hence 
