...
We now describe the semantics of languages in the \FXG family. We start with the \FXGL{X,\bullet} language that is the common core of all languages  in the family. Each language \FXGL{X',T} of the family is rigourously derived from the core language by:
\begin{itemize}
...
The use of the subtyping relation to check that the cast is satisfied is the only reason the semantics has to keep track of type parameters. The fields and methods of value $\new~{\tt C}[\tbar{A}](\tbar{v})$ do not depend on the types \tbar{A}.
...
{\sc R-Invk} does not check method guards. We will establish that there is no need for a run-time check in well-typed programs.
...
The type system of the \FXG language is parametrized in the constraint system $\cal X$ that is used to decide constraint entailment. $\cal X$ must be expressive enough to handle the constraints permitted by the \FXG grammar and provide judgments of the form $\tbar{c} \vdashX {\tt d}$.

A minimal $\cal X$ would simply have function symbols ``$.{\tt f}$''$\,:{\tt t}\mapsto{\tt t}.{\tt f}$ and ``$\new~{\tt C}[\tbar{A}]$''$\,:\tbar{t}\mapsto\new~{\tt C}[\tbar{A}](\tbar{t})$ and satisfy\footnote{Formally, we consider a constraint system generator $\mathcal{X}$ that given a program {\tt P} produces a constraint system $\mathcal{X}_{\tt P}$ that satisfies {\sc X-Sel}.} the {\sc X-Sel} inference rule of Figure~\ref{fig:object}.


\eat{
 and decide about the term equality

. Formally, $\cal X$ has variables, unary function symbols $.{\tt f}\ty{\tt t}\mapsto{\tt t}.{\tt f}$ and n-ary function symbols $\new~{\tt C}[\tbar{A}]\ty\ty\tbar{t}\mapsto\new~{\tt C}[\tbar{A}](\tbar{t})$ an

$\cal X$ axiomatization...
family is defined over a given untyped input constraint system that is required to support the constraint permitted by the \FXGL{L} grammar---\true{}, \false{}, conjunction, and equality constraints for \FXGL{X}.

Formally, $\cal X$ %has nullary predicates $\true$ and $\false$, a 2-ary conjunction predicate over constraints, a 2-ary equality predicate over terms. Moreover, $\cal X$ 
...
In order to expose the current typing context to $\cal X$, we define the {\em constraint projection} $\sigma(\Gamma)$ that, in essence, strips out all type information from the $\Gamma$.
%
\begin{center}
\begin{tabular}{ll}
...
By rule {\sc X-Proj} in Figure~\ref{fig:object}, we specify that $\Gamma\vdash{\tt c}$ if the constraint projection of $\Gamma$ entails $c$ in the input constraint system. %In \FXG, there is no other way to prove entailment.
...
\paragraph{2. Well-formedness.} The rules of well-formedness are specified in Figure~\ref{fig:well}. A constraint term, constraint, or type $\alpha$ is well formed in context $\Gamma$, written $\wj{\Gamma}{\alpha}$, iff its free variables are declared in $\Gamma$ and all the type parameters of all the generic classes in $\alpha$ satisfy the guards of these classes. Notice this depends on $\cal X$ but not on any typing judgments.

In all inference rules presented below, we make the implicit assumption that the context $\Gamma$ of every premise of a constraint-entailment, lookup, typing, or subtyping judgment are well-formed. If $\Gamma$ is not then the rule cannot be used. However, we make no such assumption for the premises of well-formedness judgements.
...
\infrule[L-Method-I]
  ...
  {\methods({\tt C}[\tbar{A}])\ni\minst{B}{x}{G}{d}{H}{e}}
\end{minipage}%
\caption{\FXGL{X} fields and methods.}
\label{fig:members}
\end{figure*}
...
\end{minipage}
\caption{\FXGL{X} operational semantics. \tbar{A} and \tbar{B} are lists of ground types (no type variables, no existentials).}
\label{fig:sos}
\end{figure*}
...
\infrule[W-Class]
  ...
  {\wj{\Gamma}{\tt C}[\tbar{A}]}
\end{minipage}%
\caption{\FXGL{X} well-formedness.}
\label{fig:well}
\end{figure*}
...
\end{minipage}%
\caption{\FXGL{X} member lookup. {\tt I} ranges over members (fields and methods).}
\label{fig:lookup}
\end{figure*}
...
\quad\typicallabel{XXXXXX}
\infrule[S-Const-R]
	{\wj{\Gamma}{{\tt T}\{{\tt c}\}} \andalso \Gamma\vdash {\tt c}[{\tt x}/\self]\comma{\tt x}\subtype {\tt T}}
...
\end{minipage}%
\caption{\FXGL{X} subtyping rules.}\label{fig:subtyping}
\end{figure*}
...
\infrule[OK-Class]
  ...
  {\vdash \cdecl~\rm OK}
\end{minipage}%
\caption{\FXGL{X} typing rules.}\label{fig:FX}
\end{figure*}
...
\infrule[W-Pred]
  {\tj{\Gamma}{\tbar{t}} \andalso {\tt p}\in\cal Q}
  {\cj{\Gamma}{{\tt p}(\tbar{t})}}
\end{minipage}%
...
As explained in the previous section, the intent of the subtyping relation is to combine subclassing and constraint entailment: type ${\tt C}[\tbar{A}]\{{\tt c}\}$ is a subtype of ${\tt D}[\tbar{A}]\{{\tt d}\}$ iff {\tt C} is a subclass of {\tt D} and {\tt c} entails {\tt d} in the underlying constraint system. In the formal model, subtyping is invariant in the type parameters, that is, ${\tt C}[\tbar{A}]\{{\tt c}\}$ is a subtype of ${\tt C}[\tbar{B}]\{{\tt c}\}$ only if $\tbar{A}=\tbar{B}$.
...
\subsection{\FXGL{B}}
We now turn to showing how \FGJ{}-style bounds can be supported in the \FXGL{X} family.
\FXGL{B} is the language obtained by adding to \FXGL{X} the productions and rules of Figure~\ref{fig:FXGB}.
...
\begin{theorem}[Method guards] If $\Gamma$ is a well-formed context and $\Gamma \vdash {\tt e}.{\tt m}[\tbar{B}](\tbar{a})\ty{\tt T}$ and ${\tt e} \derives^*	 \new~{\tt C}[\tbar{A}](\tbar{v})$ and $\tbar{a} \derives^* \tbar{w}$ and $\methods({\tt C}[\tbar{A}])\ni\minst{B}{x}{G}{c}{H}{e}$ then $\Gamma\vdash {\tt c}[\new~{\tt C}[\tbar{A}](\tbar{v}),\tbar{w}/\this,\tbar{x}]$.
\end{theorem}

Second, we prove the type system is sound.

\begin{lemma}[Subject Reduction] If $\Gamma$ is a well-formed context, ${\tt e} \derives {\tt e'}$, and $\Gamma \vdash {\tt e}\ty{\tt T}$ then there exists a type {\tt S} such that $\Gamma\vdash{\tt e'}\ty{\tt S}$. Moreover, $\Gamma,{\tt x}\ty{\tt S} \vdash {\tt x}\subtype{\tt T}$.
\end{lemma}
