...
A minimal $\cal X$ would simply have function symbols ``$.{\tt f}$''$\,:{\tt t}\mapsto{\tt t}.{\tt f}$ and ``$\new~{\tt C}[\tbar{A}]$''$\,:\tbar{t}\mapsto\new~{\tt C}[\tbar{A}](\tbar{t})$ and satisfy\footnote{Formally, we consider a constraint system generator $\mathcal{X}$ that given a program {\tt P} produces a constraint system $\mathcal{X}_{\tt P}$ that satisfies {\sc X-Sel}.} the {\sc X-Sel} inference rule of Figure~\ref{fig:object}. TODO: axiomatization.
...
\subsection{\FXG+primitive types}
...
\subsection{Results}
The following results hold for all instantiations of \FXG on a constraint system $\cal X$ satisfying the prerequisites. Below, the typing contexts are always assumed to be consistent and well formed.
...
\end{tabular}
\end{center}
\item We adopt a constraint system that can handle the new constraints. Here, we only have to require the constraint system to propagate the new constraints. There is no particular inference rule it must satisfy about them. The extension with structural subtyping constraint we discuss below gives a better example of that step.
...
\infrule[S-Bound]
	{\cj{\Gamma}{{\tt X}\extends{\tt G}} \andalso \Gamma\vdash {\tt X}\extends {\tt G}}
	{\Gamma,{\tt x}\ty{\tt X}\vdash {\tt x}\subtype {\tt G}}
\item We deal with inconsistent bounds, for instance by means of:
\infrule
	{{\tt G}\neq{\tt H}}
	{{\tt X}\extends{\tt G}\comma{\tt X}\extends{\tt H}\vdash\false}
\end{enumerate}
...
Whatever the extension, in order to preserve the soundness of the type system and the property of principal types, that is, the proof structure we have previously described, one need only ensure that:
\begin{enumerate}
