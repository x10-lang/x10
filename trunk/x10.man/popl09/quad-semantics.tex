...
\infrule[T-Invk]
	{\Gamma\vdash {\tt e}:{\tt T},\tbar{a}:\tbar{U} \andalso
	  \Gamma,{\tt x}:{\tt T}\vdash {\tt
          x}~\has~{\tt m}[\tbar{A}](\tbar{y}:\tbar{G})\{{\tt k}\}:{\tt H}={\tt b},\tbar{U}\subtype\tbar{G}\andalso \Gamma,{\tt x}:{\tt T},\tbar{y}:\tbar{U}\vdash {\tt k} \andalso {\tt x},\tbar{y}~\rm fresh}
	{\Gamma\vdash {\tt e}.{\tt m}[\tbar{A}](\tbar{a}):\exists {\tt x}:{\tt T}.~\exists\tbar{y}:\tbar{U}.~{\tt H}}

\infrule[OK-Method]
  {\wj{\tbar{X},\tbar{Z}:\type,\this:{\tt C}[\tbar{X}]}{\tbar{G}}\\
    \cj{\tbar{X},\tbar{Z}:\type,\this:{\tt C}[\tbar{X}],\tbar{x}:\tbar{G}}{\tt k}\andalso
   \tbar{X},\tbar{Z}:\type,\this:{\tt C}[\tbar{X}],\tbar{x}:\tbar{G},{\tt k}\vdash {\tt e}:{\tt E},{\tt E}\subtype {\tt H}}
  {\vdash{\tt def}~{\tt m}[\tbar{Z}](\tbar{x}:\tbar{G})\{{\tt k}\}:{\tt H}={\tt e}~{\rm OK~in}~{\tt C}[\tbar{X}]}
