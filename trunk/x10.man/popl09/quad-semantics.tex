...
\centering
${\sf inv}({\tt T})=\wedge\{{\tt l}~s.t~\exists {\tt C}[\tbar{A}],{\tt T}\subtype{\tt C}[\tbar{A}], {\tt class}~{\tt C}[\tbar{X}](\tbar{f}:\tbar{F})\{{\tt l}\}~{\tt extends}~{\tt D}[\tbar{E}]~\{~\tbar{M}~\}\in {\tt P}, {\tt l}={\tt k}[\tbar{A}/\tbar{X}]\}$
...
\infrule[T-New]
	{\Gamma\vdash\tbar{e}:\tbar{T} \andalso
	  ...
	  \tbar{x}~{\rm fresh} \andalso 
	  \Gamma,\tbar{x}:\tbar{T}\vdash\tbar{T}\subtype\tbar{F}[\tbar{x}/\this.\tbar{f}],{\sf inv}({\tt C}[\tbar{A}])[\tbar{x}/\this.\tbar{f}]}
...
\infrule[T-Invk]
	{\Gamma\vdash {\tt e}:{\tt T},\tbar{a}:\tbar{U}  \andalso {\tt x},\tbar{y}~{\rm fresh} \andalso
	  \Gamma,{\tt x}:{\tt T}\vdash {\tt
          x}~\has~{\tt m}[\tbar{A}](\tbar{y}:\tbar{G})\{{\tt k}\}:{\tt H}={\tt b}\andalso \Gamma,{\tt x}:{\tt T},\tbar{y}:\tbar{U}\vdash {\tt k},\tbar{U}\subtype\tbar{G}}
...
\infrule[OK-Method]
  {\wj{\tbar{X}:*,\tbar{Y}:*,\this:{\tt C}[\tbar{X}],\tbar{x}:\tbar{G}}{\tbar{G}} \andalso
    \cj{\tbar{X}:*,\tbar{Y}:*,\this:{\tt C}[\tbar{X}],\tbar{x}:\tbar{G}}{\tt k}\andalso
   \tbar{X}:*,\tbar{Y}:*,\this:{\tt C}[\tbar{X}],\tbar{x}:\tbar{G},{\tt k}\vdash {\tt e}:{\tt E},{\tt E}\subtype {\tt H}}
  {\vdash{\tt def}~{\tt m}[\tbar{Y}](\tbar{x}:\tbar{G})\{{\tt k}\}:{\tt H}={\tt e}~{\rm OK~in}~{\tt C}[\tbar{X}]}

\infrule[OK-Class]
  {\wj{\tbar{X}:*}{\tt D[\tbar{E}]}\andalso
    \tbar{x}~{\rm fresh} \andalso
   \wj{\tbar{X}:*,\tbar{x}:\tbar{F}}{\tbar{F}[\tbar{x}/\this.\tbar{f}]}\andalso 
  \cj{\tbar{X}:*,\tbar{x}:\tbar{F}}{{\tt k}[\tbar{x}/\this.\tbar{f}]} \andalso 
...
\infrule[S-Class]
  {{\tt class}~{\tt C}[\tbar{X}](\tbar{f}:\tbar{G})\{{\tt k}\}~{\tt extends}~{\tt D}[\tbar{E}]~\{~\tbar{M}~\}\in {\tt P}\\
...
\quad\typicallabel{XXXXXX}
\infrule[S-Const-L]
	{\wj{\Gamma}{{\tt T}\{{\tt c}\}} \andalso\Gamma,{\tt x}:{\tt T}\{{\tt c}\}\vdash{\tt T}\{\self=={\tt x}\}\subtype {\tt U}}
	{\Gamma\vdash {\tt T}\{{\tt c}\}\subtype {\tt U}}

\infrule[S-Exists-L]
  {\wj{\Gamma}{\exists{\tt x}:{\tt U}.~{\tt S}} \\
  \Gamma,{\tt x}:{\tt U}\vdash {\tt S}\subtype {\tt T} \andalso {\tt x}~\rm not~free~in~{\tt T}}
...
\infrule[S-Exists-R]
  {\wj{\Gamma}{\exists{\tt x}:{\tt U}.~{\tt T}} \\
  \Gamma,{\tt y}:{\tt S}\vdash {\tt t}:{\tt U},{\tt S}\{\self=={\tt y}\}\subtype {\tt T}[{\tt t}/{\tt x}]}
