...
challenge of allowing developers to write
high-performance applications for such machines while building on the
...
class \Xcd{C} we permit a type \Xcd{C\{c\}} where \Xcd{c} is a
constraint on the immutable fields, or \emph{properties}, of the class. Thus, 
...
statically. Subtyping is easily defined: a type \Xcd{C\{c\}} is a
subtype of \Xcd{D\{d\}} 
...
\Xcd{T} and a value property \Xcd{r} of type
\Xcd{Region}. The \Xcd{get} method for the array requires a
point \Xcd{p} which must lie in the object's region.\footnote{Within the body of
...
\Xcd{this.r} and \Xcd{this.T} can be replaced by \Xcd{r} and
\Xcd{T}
respectively.}
\end{example}
...
\Xcdmath{Array\{self.T $\extends$ Number,} \Xcd{self.r.rank==2\}}
