...
%\begin{lem}\label{uni}
%If $\Gamma\vdash v:T$ then $\Gamma,self:T\vdash self==v$.
%\end{lem}
...
where & $R$ is $\exists\overline{x}:\overline{V}.C[\overline{A}]\{self=={\tt new}~C(\overline{x})\}$
\\
{\sc H-Method} & $\Gamma, r:C[\overline{A}]~{\tt has}~m[\overline{B}](\overline{f}:\overline{F})\{c\}:M$
\\
{\sc T-Invk} & $\Gamma\vdash{\tt new}~C[\overline{A}](\overline{v}).m[\overline{B}](\overline{w}):T$
\\
& $\Gamma\vdash\overline{w}:\overline{W}$
\\
& $\Gamma,x:C[\overline{A}],\overline{f}:\overline{W}\vdash c, \overline f<:\overline{F}$
\\
where & $T$ is $\exists r:R.\exists \overline{f}:\overline{W}.M$
\\
Lemma~\ref{sub} & $\Gamma\vdash e':S$
\\
& $\Gamma,x:S\vdash x<:\exists r:R.\exists \overline{f}:\overline{W}.E$
\\
& $\Gamma,r:R,\overline{f}:\overline{W},x:E\vdash x<:M$
\\
Lemma~\ref{sud} & $\Gamma,x:\exists r:R.\exists \overline{f}:\overline{W}.E\vdash x<:T$
\\
...
TODO: handle $\theta$ between methods and has in the above
