...
that all the types are well-formed (under the assumption {\tt this:C}.
...
\begin{tabular}{p{0.50\textwidth}p{0.47\textwidth}}
\infrule[Method OK]
{\this:C, \bar{x}:\bar{T} \vdash T \ \type, \bar{T} \ \type\\
\andalso \this:C, \bar{x}:\bar{T}, c \vdash e:U, U \subtype T}
{{\tt def}\ m(\bar{x}:\bar{T})\{c\}:T= e;\ \mbox{OK in}\ C}
&
\infrule[Class OK]
{\bar{M}\ \mbox{OK in}\ C \andalso \this:C \vdash \bar{T}\ \type\\
\andalso \vdash T\ \type \andalso (acyclicity)
}
{\mbox{\tt class}\ C(\bar{f}:\bar{T})\ \mbox{\tt extends}\ T\{\bar{M}\} \ \mbox{OK in}\ C}
\end{tabular}}
\caption{Method and Class OK}
\end{figure*}


\subsection{Subtype judgement}

Note that {\tt self} may be free in $c,d$ and must be declared.

\infrule[Dep type]
{\Gamma \vdash C \subtype D, C\{c\}\ \type, D\{d\}\ \type \\
