...
\begin{thm}[Method guards] If $\Gamma$ is a well-formed context and $\Gamma \vdash e.m[\overline{B}](\overline{a}):T$ and $e \rightarrow^*{\tt new}~C[\overline{A}](\overline{v})$ and $\overline{a} \rightarrow^* \overline{w}$ and ${\sf methods}(C[\overline{A}])\ni m[\overline{B}](\overline{f}:\overline{F})\{c\}:M=e$ then $\Gamma\vdash c[{\tt new}~C[\overline{A}](\overline{v}),\overline{w}/this,\overline{x}]$.
\end{thm}
...
$\Gamma, x:R\vdash x<:E$ and $\Gamma, \overline{f}:\overline{W}\vdash \overline{f}<:\overline{A}$. By Lemma~\ref{sum}, ${\tt new}~C[\overline{A}]
}