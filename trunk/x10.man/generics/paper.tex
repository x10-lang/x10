...
X10 performs (or will perform) type inference of local variable
types and of type arguments for method and constructor calls
\footnote{We describe the results the type
...
Another possible algorithm to base this on use is the inference algorithm
for C\# described by Bierman et al.~\cite{csharp3}.
A key difference is that X10 supports where clauses that
constraint method type arguments (and value arguments).
...
\end{displayxten}
Any \xcd{Set} can be passed in: for argument \xcd{e}, the method
...
\end{displayxten}
However, this breaks the connection between the element types of the
argument type and of the return type.
...
The problem can be resolved by introducing explicit structural constraints
into X10:
\begin{displayxten}
class SortedList[T](compare: (T,T) => int) {
