...
The \grammarrule{WhereClause} in a class or interface declaration specifies an
explicit condition on the properties of the type, and is discussed further
in \Sref{DepType:Where}.
...
may appear in the \grammarrule{WhereClause}. A where clause \xcd"c" with
...
class invariant associated with \xcd"C" (\Sref{DepType:TypeInvariant}). However,
...
The compiler must check that every invocation \xcdmath"C(e$_1$,..., e$_n$)" to a
...
entail the \grammarrule{WhereClause} in the parameter list of the constructor.
\end{staticrule}
...
contain a \grammarrule{WhereClause}.
...
The guard (specified by \grammarrule{WhereClause})
...
equivalent to the types \xcdmath"C{sc}, S$_1${t$_1$}, ...., T$_n${t$_n$}"
respectively.
\end{staticrule*}
