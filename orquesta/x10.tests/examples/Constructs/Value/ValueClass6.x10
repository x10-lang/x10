...
public class ValueClass6 extends x10Test {
	...
	public def run(): boolean = {
		final val D: dist = [0..9]->here;
		// different value arrays whose elements are the same
		// reference objects, must be equal
		final val X1: Array[foo] = new Array[foo](D, (p(i): point): foo => new foo());
		final val Y1: Array[foo] = new Array[foo](D, (p(i): point): foo => X1(i));
		System.out.println("1");
		if (X1 != Y1) return false;
		// different value arrays whose elements are different
		// value objects
		// that have the same contents, must be ==
		final val X2: Array[complex] = new Array[complex](D, (p: point): complex {
			System.out.println("The currentplace for X2" + i  + " is " + ( here));
			return new complex(i,i);
		});
		final val Y2: Array[complex] = new Array[complex](D, (p: point): complex => {
		...
		final val X3: Array[foo] = new Array[foo](D, (p(i): point): foo => {
			System.out.println("The currentplace for X3" + i  + " is " + ( here));
			return new foo();
		});
		final val Y3: Array[foo] = new Array[foo](D, (p(i): point): foo => {
		...
		final val X4: Array[foo] = new Array[foo](D, (p(i): point) => new foo());
		final val Y4: Array[foo] = new Array[foo](D, (p(i): point) => X4(i));
