...
//   lapi_am_t        am;
  ...
  void Lock()   { tf.mutex_lock(hndl); } 
  void Unlock() { tf.mutex_unlock(hndl); }
...
#if 0
  virtual void NbPut(void *src, void *dst, int bytes, int proc, void *nbh) {
...
//       TheComm().Put(&LU_col, dst, sizeof(int), proc);
  ...
  /*FIXME: optimize the updates through diag_readyBelowCount to happen
    ...
    this column*/
  void reportReadyBelowCount(int I, int J, int mulSubCount) {
    ...
    TheComm().AtomicAdd(one, dst, proc);
  }

  /*FIXME: optimize to report pivot candidate for all blocks in this
    column at this proc*/
  void reportPivotCandidate(int I, int J, double mColV, int mRow) {
...
//  	    <<" val="<<val<<" row="<<mRow<<" nupdates="<<nupdates<<endl;
	...
	*pivotCount=0;
	...
	TheComm().AtomicAdd(nupdates, dstpivotCount, proc);      	
      }
    }
#endif
  }
...
private:
  ...
  void computeMax(int col, int start_row) {
    ...
    M->cv->reportPivotCandidate(I, J, fabs(maxColV), maxRow);
  }
...
//     cerr<<"Permuting rows "<<row1<<" and "<<row2<<endl;
  ...
  bool backSolve(Profiler &prof) {
    ...
    const int diag = min(I,J);
    M->getData(diag, diag, buf);

    assert(M->pv->ready(I,J));
...
//   cerr<<"StepLU. I="<<I<<" J="<<J<<endl;
  ...
  //cerr<<"StepLU. I="<<I<<" J="<<J<<" Readybelowcount reached"<<endl;
  

  if(I == J) {
    if(LU_col>=0) {
      //cerr<<"diag. I="<<I<<" LU_col>=0"<<endl;
	...
	assert(pivotCount <= M->px*M->nx - I);
      ...
      if(pivotCount < M->px*M->nx - I) {
// 	cerr<<"I="<<I<<" J="<<J<<" Not enough pivots"
// 	    <<"(got="<<pivotCount<<" expected="
// 	    <<(M->px*M->nx-I)
// 	    <<")"<<endl;
	return false;
      }
      ...
      double maxv = -1.0;
      for(int i=0; i<M->px; i++) {
// 	cerr<<"pivot input. pi="<<i<<" val="<<maxColV[i]<<" row="<<maxRow[i]<<endl;
 	if(maxColV[i] > maxv) {
	  maxv = maxColV[i];
	  mrow = maxRow[i];
	}
...
//       cerr<<"I="<<I<<" J="<<J<<". Computed pivots. row="<<I*B+LU_col<<" pivot="<<mrow<<" pivot-val="<<maxv<<endl;      
      ...
      if(I*B+LU_col != mrow) {
//  	cerr<<"I="<<I<<" J="<<J<<". Permuting rows"<<I*B+LU_col<<" and "<<mrow<<endl;	
	permute(I*B+LU_col, mrow, prof);
//  	cerr<<"I="<<I<<" J="<<J<<". DONE permuting rows"<<I*B+LU_col<<" and "<<mrow<<endl;	
      }
//       cerr<<"I="<<I<<" J="<<J<<". Doing LU"<<endl;
      LU(LU_col, prof);
//       cerr<<"I="<<I<<" J="<<J<<". Done LU"<<endl;
      if(LU_col==B-1)  {
//  	cerr<<"I="<<I<<" J="<<J<<". Setting ready"<<endl;	
	setReady();
//  	cerr<<"I="<<I<<" J="<<J<<". Resetting LU_col"<<endl;
	M->cv->resetLU_col(I, J);
//  	cerr<<"I="<<I<<" J="<<J<<". Done setting ready"<<endl;
	//cerr<<"I="<<I<<" J="<<J<<endl;
      }
      //MEM_BARRIER();
    }
    ...
    if(LU_col1<=B-1)	{
//       cerr<<"I="<<I<<" J="<<J<<" updating LU_col="<<LU_col1<<endl;
      updateLU_col(LU_col1);
      computeMax(LU_col1, LU_col1);
//       cerr<<"I="<<I<<" J="<<J<<" Computed max"<<endl;
    }
  }
  else {
    if(LU_col>=0) {
//       cerr<<"I="<<I<<" J="<<J<<". LU_col>=0"<<endl;

      int diag_LU_col = M->cv->getShadowLU_col(J, J);

      if(!(diag_LU_col > LU_col) && !M->rv->ready(J,J)) {
// 	cerr<<"I="<<I<<" J="<<J<<". Diag not done. diag_LU_col="
// 	    <<diag_LU_col<<" ready(diag)="<<M->rv->ready(J,J)<<endl;	
	return false;
      }
//       cerr<<"I="<<I<<" J="<<J<<". Computing lower"<<endl;      
      lower(LU_col, prof);
      if(LU_col==B-1) {
	//M->cv->resetReadyBelowCount(I, J);
	//M->cv->resetLU_col(I, J);
    ...
    int LU_col1 = (LU_col==-1?0:LU_col+1);
    if(LU_col1 <= B-1) {
//       cerr<<"StepLU. I="<<I<<" J="<<J<<". Computing max LU_col1="<<LU_col1<<endl;
      computeMax(LU_col1);
      //MEM_BARRIER(); //A store barrier?
      updateLU_col(LU_col1);
//       cerr<<"StepLU. I="<<I<<" J="<<J<<". Updated LU_col"<<endl;
    }
  }
