...
public class Basic {
	...
	private static class RunnableWithBuf implements Serializable,
		...
		public void run(byte[] buf) {
			switch (id) {
			case Ping:
				Basic.recv_msg_ping(buf);
				break;
			case Pong:
				Basic.recv_msg_pong(buf);
				break;
			case Quit:
				Basic.recv_quit(buf);
				break;
		...
		public static RunnableWithBuf $_deserialize_body(RunnableWithBuf _obj,
		...
		public static RunnableWithBuf $_deserializer(
				x10.serialization.X10JavaDeserializer deserializer)
				throws java.io.IOException {
	...
	public void run(String[] argv) {
		if (X10RT.numPlaces() == 1) {
			System.err
					.println("This is a communications test so needs at least 2 hosts.");
		...
		for (int i = 0; i < argv.length; ++i) {
			if (!strcmp(argv[i], "--help")) {
				...
				if (X10RT.here() == 0) {
					System.err
							.println("Didn't understand: \"" + argv[i] + "\"");
					show_help(System.err, getClass().getName());
				}
				System.exit(EXIT_FAILURE);
			}
		...
		if (X10RT.here() == 0) {
			...
			if (automatic) {
				...
				for (int l = 0; l < 512 * 1024; l = (l * 2) > 0 ? (l * 2) : 1) {
					...
					double micros = 0;
					for (int j = 1; j <= 16; ++j) {
						micros = run_test(iterations / j, j, l, put, get) / 1E3
								/ (iterations / j * j) / 2
								/ (X10RT.numPlaces() - 1);
				...
				double micros = run_test(iterations, window, len, put, get)
						/ 1E3 / (iterations * window) / 2
						/ (X10RT.numPlaces() - 1);
				System.out.println("Half roundtrip time: " + micros
						+ " us  Bandwidth: " + (len / micros) + " MB/s");
			}
	...
	private static class Handler implements VoidFun_0_0 {
		...
		public static Handler $_deserialize_body(Handler _obj,
				x10.serialization.X10JavaDeserializer deserializer)
				throws java.io.IOException {
		...
		public static Handler $_deserializer(
				x10.serialization.X10JavaDeserializer deserializer)
				throws java.io.IOException {
	...
	public static void sendMsg(int place, RunnableWithBuf runner, byte[] buf,
			int len) {
		try {
