...
public class Topology {
	...
	public static int nchildren(int i) {
		return 0;
	}

	public static int child(int i, int j) {
		return i;
	}

	public static boolean is_host(int i) {
		return true;
	}

	public static boolean is_spe(int i) {
		return false;
	}

	public static boolean is_cuda(int i) {
		return false;
	}
	...
	public void run(String[] argv) {
		...
		for (int i = 0; i < X10RT.numPlaces(); ++i) {
			...
			System.out.printf((last ? "`-" : "|-") + "--%02d(%02d)--", i,
					parent(i), typestr(i));
			if (nchildren(i) > 0) {
				System.out.println("---.");
			}
