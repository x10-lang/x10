...
public abstract class ULongUtils {
	 ...
	 * 
	 * @param a
	 *            unboxed ULong value represented as long
	...
	public static float toFloat(long a) {
		...
		if (a >= 0)
			...
			return (float) (v - 2.0 * java.lang.Long.MIN_VALUE);
	}
	...
	public static double toDouble(long a) {
		...
		if (a >= 0)
			...
			return v - 2.0 * java.lang.Long.MIN_VALUE;
	}
	...
	private static java.math.BigInteger toULONG(long a) {
		byte[] bytes = new byte[9]; // set zero to bytes[0] to make the value
									// positive
		for (int i = 0; i < 8; ++i) {
	...
	public static long parseULong(java.lang.String s, int radix) {
		...
		if (ulong.signum() < 0 || ulong.compareTo(ULONG_MAX) > 0) {
			throw new java.lang.NumberFormatException("For input string: \""
					+ s + "\"");
		}
