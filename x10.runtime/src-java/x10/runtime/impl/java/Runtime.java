...
public abstract class Runtime implements x10.core.fun.VoidFun_0_0 {
	...
	/**
	 * Body of main java thread (only called in non-library mode)
	 */
	protected void start(final String[] args) {
		...
		try {
			...
			// Note: since this isn't user visible,
			// java.lang.InterruptedException is used.
		}
	...
	static class $Closure$Main implements x10.core.fun.VoidFun_0_0 {
		...
		public void $_serialize(X10JavaSerializer $serializer)
			...
			throw new java.lang.UnsupportedOperationException(
					"Serialization not supported for " + getClass());
		}
	...
	public void $apply() {
		...
		final x10.array.Array<String> aargs = new x10.array.Array<String>(
				(java.lang.System[]) null, Types.STRING)
				.x10$array$Array$$init$S((args == null) ? 0 : args.length);
		if (args != null) {
		...
		try {
			...
			// XTENLANG=2686: Unwrap UnknownJavaThrowable to get the original
			// Throwable object
			if (t instanceof x10.lang.WrappedThrowable)
				t = t.getCause();
			t.printStackTrace();
			setExitCode(1);
		}
	 ...
	 * Disable Assertions
	 */
	public static final boolean DISABLE_ASSERTIONS = Boolean
			.getBoolean("x10.DISABLE_ASSERTIONS");
	 ...
	 * Trace static init
	 */
	public static final boolean TRACE_STATIC_INIT = Boolean
			.getBoolean("X10_TRACE_STATIC_INIT");

	/**
	 * Emit detail serialization traces for java serialization. Using for
	 * debugging in preliminary stage
	 */
	public static final boolean TRACE_SER_DETAIL = Boolean
			.getBoolean("x10.TRACE_SER_DETAIL");

	public static final boolean X10_TRACE_ANSI_COLORS = Boolean
			.getBoolean("X10_TRACE_ANSI_COLORS");
	...
	private static void print(String type, String col, String message) {
		System.out.println(ANSI_BOLD + X10RT.here() + ": " + col + type + ": "
				+ ANSI_RESET + message);
	}

	public static void runAsyncAt(int place, x10.core.fun.VoidFun_0_0 body,
			FinishState finishState, x10.lang.Runtime.Profile prof, int endpoint) {
		runAsyncAt(place, body, finishState, prof);
	}

	public static void runAsyncAt(int place, x10.core.fun.VoidFun_0_0 body,
			FinishState finishState, x10.lang.Runtime.Profile prof) {
		try {
			byte[] bytes = serialize(body, finishState, prof);
			long start = prof != null ? System.nanoTime() : 0;
			x10.x10rt.MessageHandlers.runSimpleAsyncAtSend(place, bytes.length,
					bytes);
			if (prof != null) {
				prof.communicationNanos += System.nanoTime() - start;
			}
		} catch (java.io.IOException e) {
			java.lang.RuntimeException xe = x10.runtime.impl.java.ThrowableUtils
					.ensureX10Exception(e);
			xe.printStackTrace();
			throw xe;
		}
	}
	 ...
	 * Synchronously executes body at place(id)
	 */
	public static void runClosureCopyAt(int place,
			x10.core.fun.VoidFun_0_0 body, x10.lang.Runtime.Profile prof) {
		runAt(place, body, prof);
	}
	...
	public static <T> T deepCopy(T body, x10.lang.Runtime.Profile prof) {
		...
		try {
			...
			if (TRACE_SER_DETAIL) {
				System.out.println("Done with deserialization for deepCopy of "
						+ body.getClass());
			}
			return body;
		} catch (java.io.IOException e) {
	...
	public static <T> byte[] serialize(T body, x10.lang.Runtime.Profile prof)
			throws java.io.IOException {
		if (TRACE_SER_DETAIL) {
			System.out.println("Starting serialization for runAtAll  "
					+ body.getClass());
		}
		long start = prof != null ? System.nanoTime() : 0;
		...
		if (TRACE_SER_DETAIL) {
			System.out.println("Done with serialization for runAtAll "
					+ body.getClass());
		}
		return ba;
	}
	...
	private static byte[] serialize(x10.core.fun.VoidFun_0_0 body,
			FinishState finishState, x10.lang.Runtime.Profile prof)
			throws java.io.IOException {
		if (TRACE_SER_DETAIL) {
			System.out.println("Starting serialization for runAtAll  "
	...
	public static x10.util.HashMap<String, String> loadenv() {
		...
		x10.util.HashMap<String, String> map = new x10.util.HashMap<String, String>(
				(java.lang.System[]) null, Types.STRING, Types.STRING)
				.x10$util$HashMap$$init$S();
		for (Map.Entry<String, String> e : env.entrySet()) {
			map.put__0x10$util$HashMap$$K__1x10$util$HashMap$$V(e.getKey(),
					e.getValue());
		}
		return map;
	}
	...
	public static Reader execForRead(String command) {
		try {
			...
			return new x10.io.InputStreamReader(new x10.core.io.InputStream(
					proc.getInputStream()));
		} catch (java.io.IOException e) {
			java.lang.RuntimeException xe = x10.runtime.impl.java.ThrowableUtils
					.ensureX10Exception(e);
	...
	public static Writer execForWrite(String command) {
		try {
			Process proc = java.lang.Runtime.getRuntime().exec(command);
			return new x10.io.OutputStreamWriter(new x10.core.io.OutputStream(
	...
	public static void main(String[] args) throws Throwable {
		...
		for (int i = 0; i < args.length; i++) {
			...
			if (arg.equals("-v") || arg.equals("-verbose")
				...
				int eq = arg.indexOf('=');
				String key = "x10."
						+ (eq < 0 ? arg.substring(1) : arg.substring(1, eq));
				String value = eq < 0 ? "true" : arg.substring(eq + 1);
				System.setProperty(key, value);
			} else {
				int dotx10 = arg.indexOf(".x10");
				className = (dotx10 < 0 ? arg : arg.substring(0, dotx10))
						+ "$$Main";
		...
		if (verbose) {
			System.err.println("Invoking user class: " + className
					+ " with classpath '"
	...
	public short $_get_serialization_id() {
		throw new java.lang.UnsupportedOperationException("Cannot serialize "
				+ getClass());
	}
	 ...
	 * The property is a value in milliseconds, we convert to nanoSeconds for
	 * efficiency when using System.nanoTime. The default value is 10ms.
	 */
	public static final long PROF_SER_FILTER = 1000 * 1000 * Long.getLong(
			"x10.PROF_SER_FILTER", 10);
