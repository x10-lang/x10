...
/**
 * Commonly serialized types whose serialization ids are known at all places and
 * therefore do not have to be assigned on a per-message basis.
 * 
 * We start with an initial set of built-in types, that are extensible by the
 * user on the command line by setting the property x10.ADDITIONAL_SHARED_TYPES.
 * 
 * TODO: Future work: The design is intended to support future dynamic
 * optimization where the serialization subsystem can use profiling to identify
 * commonly serialized types and use a multi-phase protocol to promote such
...
class SharedDictionaries {
	...
	private static synchronized void doInitialization() {
		...
		serializationDict = new SerializationDictionary(
				SerializationConstants.FIRST_SHARED_ID);
		...
		String[] builtInTypes = new String[] {
				...
				"x10.lang.Clock",
				"x10.lang.Complex",
				/* "x10.lang.GlobalRef", */// TODO: @NativeRep to
											// x10.core.GlobalRef
				...
				/* "x10.util.IndexedMemoryChunk", */// TODO: @NativeRep to
													// x10.core.IndexedMemoryChunk
				"x10.util.Pair",
				/* "x10.util.RemoteIndexedMemoryChunk", */// TODO: @NativeRep to
															// x10.core.RemoteIndexedMemoryChunk

				/* Managed X10 implementation classes */
				// TODO: For mixed mode, we need to map these to the
				// backend neutral concepts (eg, x10.lang.GlobalRef not
				// x10.core.GlobalRef)
				"x10.core.Boolean",
				"x10.core.Byte",
				"x10.core.Char",
				...
				/*
				 * Types that correspond to closures in XRX annotated with
				 * @RemoteInvocation
				 */
				/*
				 * TODO: This list should be machine generated; this is way too
				 * fragile!
				 */
	...
	private static void processType(String type) {
		try {
			...
			if (Runtime.TRACE_SER) {
				Runtime.printTraceMessage("Assigned shared serialization id "
						+ id + " to " + type);
			}
		} catch (ClassNotFoundException e) {
			if (Runtime.TRACE_SER) {
				Runtime.printTraceMessage("Unable to load common type " + type
						+ ". It will not have a preassigned id");
			}
		}
	}
	...
	static Class<?> getClassForID(short sid) {
		if (!initialized)
			doInitialization();
		assert SerializationConstants.FIRST_SHARED_ID <= sid
				&& sid < SerializationConstants.FIRST_DYNAMIC_ID;
