...
class DeserializationDictionary {
	...
	DeserializationDictionary(X10JavaDeserializer jds) {
		...
		try {
			...
			if (Runtime.TRACE_SER) {
				Runtime.printTraceMessage("\tReceiving " + numEntries
						+ " serialization ids");
			}
			for (short i = 0; i < numEntries; i++) {
				...
				if (Runtime.TRACE_SER) {
					Runtime.printTraceMessage("\tserialization id: " + id
							+ " = " + name);
				}
				addEntry(id, name);
			}
			...
			throw new RuntimeException(
					"Failure while reading message dictionary", e);
		}
	}
	...
	void addEntry(short id, String name) {
		...
		try {
			...
			System.err.println("readMessageDictionary: failed to load class "
					+ name);
		...
		if (x10.serialization.X10JavaSerializable.class.isAssignableFrom(clazz)
			...
			try {
				m = clazz
						.getMethod("$_deserializer", X10JavaDeserializer.class);
			} catch (NoSuchMethodException e) {
				throw new RuntimeException(
						"DeserializationDictionary: class "
								+ clazz
								+ " implements X10JavaSerializable but does not have a $_deserializer method",
						e);
			}
	...
	Class<?> getClassForID(short sid) {
		...
		if (clazz == null) {
			throw new RuntimeException("DeserializationDictionary: id " + sid
					+ " is not mapped to a class!");
		}
		return clazz;
	}
