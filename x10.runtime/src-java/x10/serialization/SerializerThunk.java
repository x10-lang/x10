...
/**
 * An instance of a SerializerThunk knows how to serialize a specific class. The
 * various subclasses of this abstract class encode different cases of the
...
abstract class SerializerThunk {
	...
	private static ConcurrentHashMap<Class<?>, SerializerThunk> thunks = new ConcurrentHashMap<Class<?>, SerializerThunk>(
			50);
	 ...
	 * 
	 * @param clazz
	 *            The class for which to find a SerializerThunk instance
	 * @return a SerializerThunk instance for clazz
	 */
	static SerializerThunk getSerializerThunk(Class<? extends Object> clazz)
		...
		if (ans == null) {
			...
			if (Runtime.TRACE_SER) {
				Runtime.printTraceMessage("Creating serialization thunk "
	 ...
	 *            The class of obj
	 * @param xjs
	 *            The serializer into which the object should be serialized.
	 */
	<T> void serializeObject(T obj, Class<? extends Object> clazz,
			X10JavaSerializer xjs) throws IllegalAccessException, IOException,
	...
	abstract <T> void serializeBody(T obj, Class<? extends Object> clazz,
			X10JavaSerializer xjs) throws IllegalAccessException, IOException,
			IllegalArgumentException, InvocationTargetException,
			SecurityException, NoSuchFieldException;

	private static SerializerThunk getSerializerThunkHelper(
			Class<? extends Object> clazz) throws SecurityException,
			NoSuchFieldException, NoSuchMethodException {

		// We need to handle these classes in a special way because their
		// implementation of serialization/deserialization is not straight
		// forward.
		if ("java.lang.String".equals(clazz.getName())) {
			return new SerializerThunk.JavaLangStringSerializerThunk(clazz);
		} else if ("x10.rtt.NamedType".equals(clazz.getName())) {
			SerializerThunk superThunk = getSerializerThunk(clazz
					.getSuperclass());
		...
		boolean isX10JavaSerializable = x10.serialization.X10JavaSerializable.class
		...
		// Error checking. We don't support classes that try to implement both
		// Hadoop and X10 serialization protocols
		if (isHadoopSerializable) {
			...
			if (isX10JavaSerializable) {
				throw new RuntimeException(
						"serializer: "
								+ clazz
								+ " implements both x10.serialization.X10JavaSerializable and org.apache.hadoop.io.Writable.");
			}
		}
		...
		// We are going to serialize if via reflective access to its instance
		// fields.
	...
	/**
	 * A thunk for a vanilla X10 class (supports compiler-generated
	 * serialization code).
	 */
	private static class X10JavaSerializableSerializerThunk extends
			SerializerThunk {
		protected final Method serializeMethod;

		public X10JavaSerializableSerializerThunk(Class<?> clazz) {
			super(null); // The compiler-generated serialization code will
							// invoke the superclass serializer directly
			try {
				serializeMethod = clazz.getMethod("$_serialize",
						X10JavaSerializer.class);
			} catch (NoSuchMethodException e) {
				System.err.println("SerializerThunk: class " + clazz
						+ " does not have a $_serialize method");
		...
		@Override
		<T> void serializeBody(T obj, Class<? extends Object> clazz,
				X10JavaSerializer xjs) throws IllegalArgumentException,
	...
	private static class FieldBasedSerializerThunk extends SerializerThunk {
		...
		FieldBasedSerializerThunk(Class<? extends Object> clazz,
				SerializerThunk st) {
			super(st);
		...
		<T> void serializeBody(T obj, Class<? extends Object> clazz,
				X10JavaSerializer xjs) throws IllegalAccessException,
				IOException {
	...
	private static class HadoopSerializerThunk extends SerializerThunk {
		...
		HadoopSerializerThunk(Class<? extends Object> clazz)
	...
	private static class CustomSerializerThunk extends SerializerThunk {
		...
		CustomSerializerThunk(Class<? extends Object> clazz)
				throws SecurityException, NoSuchFieldException {
			super(null);
	...
	private static class JavaLangStringSerializerThunk extends SerializerThunk {
		...
		<T> void serializeBody(T obj, Class<? extends Object> clazz,
				X10JavaSerializer xjs) throws IOException, SecurityException,
