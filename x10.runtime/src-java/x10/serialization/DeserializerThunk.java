 ...
 * The various subclasses of this abstract class encode different cases of the
 * serialization protocol and are matched 1:1 with subclasses of
 * SerializerThunk.
 * <p>
 * 
 * The motivation for this software architecture is to improve the runtime
 * efficiency of serialization/deserialization by doing as much of the protocol
 * and meta-data operations once (at Thunk creation time) and amortizing that
 * cost across all the uses of the Thunk object.
 * 
 * @see SerializerThunk
 */
abstract class DeserializerThunk {
	...
	protected static ConcurrentHashMap<Class<?>, DeserializerThunk> thunks = new ConcurrentHashMap<Class<?>, DeserializerThunk>(
			50);
	...
	/**
	 * Factory method to find or create the DeserializerThunk instance for
	 * clazz.
	 * 
	 * @param clazz
	...
	static DeserializerThunk getDeserializerThunk(Class<? extends Object> clazz)
		...
		if (ans == null) {
			...
			if (Runtime.TRACE_SER) {
				Runtime.printTraceMessage("Creating deserialization thunk "
						+ ans.getClass() + " for " + clazz);
			}
		}
		return ans;
	}
	...
	/**
	 * Create an instance of clazz and initialize it by reading values for its
	 * instance fields from jds.
	 * 
	 * @param clazz
	 *            The class to create
	 * @param jds
	 *            The deserializer from which to obtain the new object's state.
	 * @return the deserialized object.
	 */
	@SuppressWarnings("unchecked")
	<T> T deserializeObject(Class<?> clazz, X10JavaDeserializer jds)
			throws IOException, IllegalArgumentException,
			IllegalAccessException, InvocationTargetException,
			InstantiationException {
		T obj = null;

		// If the class is java.lang.Class we cannot create an instance in the
		// following manner so we just skip it
	...
	/**
	 * Initialize obj, an instance of clazz, by reading its instance state from
	 * jds.
	 * 
	 * @param clazz
	 *            The class that is being deserialized
	 * @param obj
	 *            The object instance of the class to initialize
	 * @param i
	 *            The id of this object (in case deserialization needs to
	 *            replace it)
	 * @param jds
	 *            The deserializer from which to obtain the new object's state.
	 * @return The deserialized object.
	 */
	<T> T deserializeObject(Class<?> clazz, T obj, int i,
			X10JavaDeserializer jds) throws IOException,
			IllegalArgumentException, IllegalAccessException,
			InvocationTargetException, InstantiationException {
		if (superThunk != null) {
			obj = superThunk.deserializeObject(clazz.getSuperclass(), obj, i,
					jds);
		}
		return deserializeBody(clazz, obj, i, jds);
	}

	protected abstract <T> T deserializeBody(Class<?> clazz, T obj, int i,
			X10JavaDeserializer jds) throws IOException,
			IllegalArgumentException, IllegalAccessException,
			InvocationTargetException, InstantiationException;
	...
	private static DeserializerThunk getDeserializerThunkHelper(Class<?> clazz)
		...
		// We need to handle these classes in a special way cause there
		// implementation of serialization/deserialization is
		// not straight forward. Hence we just call into the custom
		...
		if ("java.lang.String".equals(clazz.getName())) {
			...
			return new SpecialCaseDeserializerThunk(null);
		} else if ("x10.core.IndexedMemoryChunk$$Closure$0".equals(clazz
				.getName())) {
			return new SpecialCaseDeserializerThunk(null);
		} else if ("x10.core.IndexedMemoryChunk$$Closure$1".equals(clazz
				.getName())) {
			return new SpecialCaseDeserializerThunk(null);
		...
		if (isCustomSerializable && isHadoopSerializable) {
			throw new RuntimeException(
					"deserializer: "
							+ clazz
							+ " implements both x10.io.CustomSerialization and org.apache.hadoop.io.Writable.");
		}
		...
		if (!("java.lang.Object".equals(superclass.getName())
				|| "x10.core.Ref".equals(superclass.getName()) || "x10.core.Struct"
					.equals(superclass.getName()))) {
	...
	private static class FieldBasedDeserializerThunk extends DeserializerThunk {
		...
		FieldBasedDeserializerThunk(Class<? extends Object> clazz,
				DeserializerThunk st) {
			super(st);

			// XTENLANG-2982,2983 transient fields may be initialized with
			// readObject method.
			Method readObjectMethod = null;
			try {
				readObjectMethod = clazz.getDeclaredMethod("readObject",
			...
			for (Field field : declaredFields) {
				...
				if (Modifier.isStatic(modifiers)
						|| (Modifier.isTransient(modifiers) && readObjectMethod == null)) {
					continue;
				}
		...
		protected <T> T deserializeBody(Class<?> clazz, T obj, int i,
				X10JavaDeserializer jds) throws IOException,
			...
			for (Field field : fields) {
				...
				if (type.isPrimitive()) {
					...
					field.set(obj, jds.readArrayUsingReflection(type
							.getComponentType()));
	...
	private static class HadoopDeserializerThunk extends DeserializerThunk {
		...
		HadoopDeserializerThunk(Class<? extends Object> clazz)
		...
		<T> T deserializeObject(Class<?> clazz, X10JavaDeserializer jds)
				...
				InstantiationException {
			if (Runtime.TRACE_SER) {
				Runtime.printTraceMessage("Calling hadoop deserializer with object of type "
						+ clazz);
			}

			// Hadoop assumes that the default constructor will be used to
			// create the instance.
			// The default constructor will execute field initialization
			// expressions.
		...
		@Override
		protected <T> T deserializeBody(Class<?> clazz, T obj, int i,
				X10JavaDeserializer jds) throws IllegalArgumentException,
	...
	private static class CustomDeserializerThunk extends DeserializerThunk {
		...
		CustomDeserializerThunk(Class<? extends Object> clazz)
			...
			// Even though this class implements a custom serialization
			// protocol,
			// the runtime needs to invisibly serialize those instance fields
			// that
			// are used to provide RTT information for generic types (not
			// visible at the user-level).
			TypeVariable<? extends Class<? extends Object>>[] typeParameters = clazz
					.getTypeParameters();
			if (typeParameters.length > 0) {
				...
				for (TypeVariable<? extends Class<? extends Object>> typeParameter : typeParameters) {
					Field field = clazz.getDeclaredField(typeParameter
							.getName());
					field.setAccessible(true);
					flds.add(field);
				}
			...
			// We can't use the same method name in all classes cause it creates
			// an endless loop cause when super.init is called it calls back to
			// this method
			makeMethod = clazz.getMethod(clazz.getName().replace(".", "$")
					+ "$"
					+ DeserializerThunk.CONSTRUCTOR_METHOD_NAME_FOR_REFLECTION,
					SerialData.class);
	...
	private static class JavaLangStringDeserializerThunk extends
			DeserializerThunk {
	...
	private static class SpecialCaseDeserializerThunk extends DeserializerThunk {
		...
		protected <T> T deserializeBody(Class<?> clazz, T obj, int i,
			...
			if ("x10.rtt.NamedType".equals(clazz.getName())) {
				...
				X10JavaSerializable x10JavaSerializable = RuntimeType
						.$_deserialize_body((RuntimeType) obj, jds);
				...
				return (T) imc;
					...
					.getName())) {
				return (T) x10.core.IndexedMemoryChunk.$Closure$0
						.$_deserialize_body(
								(x10.core.IndexedMemoryChunk.$Closure$0) obj,
								jds);
			} else if ("x10.core.IndexedMemoryChunk$$Closure$1".equals(clazz
					.getName())) {
				return (T) x10.core.IndexedMemoryChunk.$Closure$1
						.$_deserialize_body(
								(x10.core.IndexedMemoryChunk.$Closure$1) obj,
								jds);
			} else if (x10.core.GlobalRef.class.getName().equals(
					clazz.getName())) {
				return (T) x10.core.GlobalRef.$_deserialize_body(
						(x10.core.GlobalRef) obj, jds);
			} else if ("java.lang.Throwable".equals(clazz.getName())) {
				if (X10JavaSerializer.THROWABLES_SERIALIZE_MESSAGE) {
					try {
						String message = (String) jds.readRef();
						Field detailMessageField = java.lang.Throwable.class
				...
				if (X10JavaSerializer.THROWABLES_SERIALIZE_STACKTRACE) {
					java.lang.StackTraceElement[] trace = (java.lang.StackTraceElement[]) jds
							.readArrayUsingReflection(java.lang.StackTraceElement.class);
					java.lang.Throwable t = (java.lang.Throwable) obj;
					t.setStackTrace(trace);
				}
				if (X10JavaSerializer.THROWABLES_SERIALIZE_CAUSE) {
					try {
						java.lang.Throwable cause = (java.lang.Throwable) jds
								.readRef();
						Field causeField = java.lang.Throwable.class
								.getDeclaredField("cause");
			...
			throw new RuntimeException("Unhandled type in special case thunk: "
					+ obj.getClass());
		}
	}
}