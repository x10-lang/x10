...
/**
 * X10IdentityHashMap is an implementation of Map. All optional operations are
 * supported, adding and removing. Keys and values can be any objects.
 * 
...
class X10IdentityHashMap<K, V> extends AbstractMap<K, V> implements Map<K, V>,
	...
	/**
	 * MapEntry is an internal class which provides an implementation of
	 * Map.Entry.
	 */
	...
	static class HashMapEntrySet<KT, VT> extends AbstractSet<Map.Entry<KT, VT>> {
		...
		private static boolean valuesEq(Entry<?, ?> entry,
				Map.Entry<?, ?> oEntry) {
			return (entry != null)
	...
	/**
	 * Constructs a new instance of X10IdentityHashMap with the specified
	 * capacity.
	 * 
	 * @param capacity
	...
	/**
	 * Constructs a new instance of X10IdentityHashMap containing the mappings
	...
	/**
	 * Answers a new X10IdentityHashMap with the same mappings and size as this
	 * X10IdentityHashMap.
	 * 
	...
	public Object clone() {
		try {
			X10IdentityHashMap<K, V> map = (X10IdentityHashMap<K, V>) super
					.clone();
			map.elementCount = 0;
	 ...
	 * @return true if <code>key</code> is a key of this X10IdentityHashMap,
	 *         false otherwise
	 */
	@Override
	 ...
	 * @param value
	 *            the object to search for
	 * @return true if <code>value</code> is a value of this X10IdentityHashMap,
	 *         false otherwise
	 */
	@Override
	...
	/**
	 * Answers a Set of the mappings contained in this X10IdentityHashMap. Each
	 * element in the set is a Map.Entry. The set is backed by this
	 * X10IdentityHashMap so changes to one are reflected by the other. The set
	...
	final Entry<K, V> findNonNullKeyEntry(Object key, int index, int keyHash) {
		...
		if (key instanceof Integer) {
			...
			while (m != null
					&& (m.storedKeyHash != storedKeyHash || !areEqualKeys(key,
							m.key))) {
				m = m.next;
			}
		}
	...
	/**
	 * Answers a Set of the keys contained in this X10IdentityHashMap. The set
	 * is backed by this X10IdentityHashMap so changes to one are reflected by
		...
		if (keySet == null) {
			keySet = new AbstractSet<K>() {
				...
				public boolean remove(Object key) {
					Entry<K, V> entry = X10IdentityHashMap.this
							.removeEntry(key);
					return entry != null;
				}
	...
	final Entry<K, V> removeEntry(Object key) {
		...
		if (key != null) {
			...
			if (key instanceof Integer) {
				...
				int storedKeyHash = keyHash & 0xFFFFFFFE;
				while (entry != null
						&& (entry.storedKeyHash != storedKeyHash || !areEqualKeys(
								key, entry.key))) {
					last = entry;
					entry = entry.next;
				}
	...
	/**
	 * Answers a Collection of the values contained in this X10IdentityHashMap.
	 * The collection is backed by this X10IdentityHashMap so changes to one are
	...
	private void readObject(ObjectInputStream stream) throws IOException,
		...
		for (int i = elementCount; --i >= 0;) {
			...
			int index = (null == key) ? 0
					: (System.identityHashCode(key) & (length - 1));
