...
public final class X10JavaDeserializer implements SerializationConstants {
	...
	public int record_reference(Object obj) {
		if (Runtime.TRACE_SER) {
			...
			Runtime.printTraceMessage("\t\tRecorded new reference of type "
					+ Runtime.ANSI_CYAN + Runtime.ANSI_BOLD + className
					+ Runtime.ANSI_RESET + " at " + counter
					+ " (absolute) in map");
		}
	...
	public Object getObjectAtPosition(int pos) {
		...
		if (Runtime.TRACE_SER) {
			Runtime.printTraceMessage("\t\tRetrieving repeated reference of type "
					+ Runtime.ANSI_CYAN
					+ Runtime.ANSI_BOLD
	...
	public Object readRef(short serializationID) throws IOException {
		...
		if (serializationID <= MAX_HARDCODED_ID) {
			return X10JavaDeserializer.deserializePrimitive(serializationID,
					this);
		}
			...
			if (componentTypeID == INTEGER_ID) {
				...
				if (componentType.isArray()) {
					for (int i = 0; i < length; ++i) {
						Array.set(obj, i,
								readArrayUsingReflection(componentType));
					}
				} else {
					for (int i = 0; i < length; ++i) {
		...
		if (Runtime.TRACE_SER) {
			Runtime.printTraceMessage("Deserializing non-null value with id "
					+ serializationID);
		}
		...
		try {
			...
			if (method != null) {
				// A class that implements X10JavaSerializable. Call
				// _deserialize
				return method.invoke(null, this);
			} else {
				Class<?> clazz = dict.getClassForID(serializationID);
				DeserializerThunk dt = DeserializerThunk
						.getDeserializerThunk(clazz);
				return dt.deserializeObject(clazz, this);
			}
		} catch (InvocationTargetException e) {
			// This should never happen
			throw new RuntimeException(
					"Error in deserializing non-null value with id "
							+ serializationID, e);
	...
	public int readInt() throws IOException {
		int v = in.readInt();
		if (Runtime.TRACE_SER) {
			Runtime.printTraceMessage("Deserializing [****] an "
					+ Runtime.ANSI_CYAN + "int" + Runtime.ANSI_RESET + ": " + v);
		}
		return v;
	}
	...
	public boolean readBoolean() throws IOException {
		...
		if (Runtime.TRACE_SER) {
			Runtime.printTraceMessage("Deserializing [*] a "
					+ Runtime.ANSI_CYAN + "boolean" + Runtime.ANSI_RESET + ": "
					+ v);
		}
		return v;
	}
	...
	public char readChar() throws IOException {
		...
		if (Runtime.TRACE_SER) {
			Runtime.printTraceMessage("Deserializing [**] a "
					+ Runtime.ANSI_CYAN + "char" + Runtime.ANSI_RESET + ": "
					+ v);
		}
		return v;
	}
	...
	public byte readByte() throws IOException {
		...
		if (Runtime.TRACE_SER) {
			Runtime.printTraceMessage("Deserializing [*] a "
					+ Runtime.ANSI_CYAN + "byte" + Runtime.ANSI_RESET + ": "
					+ v);
		}
		return v;
	}
	...
	public short readShort() throws IOException {
		...
		if (Runtime.TRACE_SER) {
			Runtime.printTraceMessage("Deserializing [**] a "
					+ Runtime.ANSI_CYAN + "short" + Runtime.ANSI_RESET + ": "
					+ v);
		}
		return v;
	}
	...
	public String readString() throws IOException {
		...
		if (Runtime.TRACE_SER) {
			Runtime.printTraceMessage("Deserializing a " + Runtime.ANSI_CYAN
	...
	public Object readRefUsingReflection() throws IOException {
		...
		try {
			DeserializerThunk thunk = DeserializerThunk
					.getDeserializerThunk(clazz);
	...
	// superclass.
	public <T> Object deserializeClassUsingReflection(
			Class<? extends Object> clazz, T obj, int i) throws IOException {
		try {
			DeserializerThunk thunk = DeserializerThunk
					.getDeserializerThunk(clazz);
	...
	<T> void readPrimitiveUsingReflection(Field field, T obj)
	...
	public Object readArrayUsingReflection(Class<?> componentType)
			throws IOException {
		short serializationID = readShort();
		if (serializationID == NULL_ID) {
			if (Runtime.TRACE_SER) {
	...
	public static Object deserializePrimitive(short serializationID,
			X10JavaDeserializer deserializer) throws IOException {
		if (Runtime.TRACE_SER) {
			Runtime.printTraceMessage("Deserializing primitive/special value with id "
					+ serializationID);
		}
		Object obj = null;
		...
		case STRING_ID:
			...
			deserializer.record_reference(obj); // TODO: consider avoid
												// recording this as a
												// reference; it can't be
												// cyclic.
			break;
		case FLOAT_ID:
		...
		default:
			throw new RuntimeException(
					"Unhandled hard-wired serialization id in readPrimitive!");
		}
		return obj;
	}
}
