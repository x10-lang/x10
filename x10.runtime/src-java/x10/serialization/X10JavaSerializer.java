...
public final class X10JavaSerializer implements SerializationConstants {
	...
	// N.B. use custom IdentityHashMap class, as standard one has poor
	// performance on J9
	X10IdentityHashMap<Object, Integer> objectMap = new X10IdentityHashMap<Object, Integer>();
	int counter = 0;

	// [GlobalGC] Table to remember serialized GlobalRefs, set and used in
	// GlobalRef.java and InitDispatcher.java
	X10IdentityHashMap<x10.core.GlobalRef<?>, Integer> grefMap = new X10IdentityHashMap<x10.core.GlobalRef<?>, Integer>(); // for
																															// GlobalGC

	public void addToGrefMap(x10.core.GlobalRef<?> gr, int weight) {
		grefMap.put(gr, weight);
	}

	public java.util.Map<x10.core.GlobalRef<?>, Integer> getGrefMap() {
		return grefMap;
	}

	// Build up per-message id dictionary
	protected SerializationDictionary idDictionary = new SerializationDictionary(
			FIRST_DYNAMIC_ID);
	...
	public byte[] toMessage() throws IOException {
		...
		if (Runtime.TRACE_SER) {
			Runtime.printTraceMessage("Sending per-message serialization ids: "
					+ idDictionary);
		}
		...
		System.arraycopy(dataBytes, 0, message, dictBytes.length,
				dataBytes.length);
		return message;
	}
	...
	public void write(X10JavaSerializable obj) throws IOException {
		...
		if (i <= MAX_HARDCODED_ID) {
			...
			case RTT_USHORT_ID:
				out.writeShort(i);
				if (Runtime.TRACE_SER) {
					Runtime.printTraceMessage("Optimized serializing [**] a "
							+ Runtime.ANSI_CYAN + "serialization_id_t"
							+ Runtime.ANSI_RESET + ": " + i);
				}
				return;
			...
			default:
				...
				throw new RuntimeException(
						"Unhandled hard-wired serialization id in write(X10JavaSerializable)");
			}
		}
		...
		if (Runtime.TRACE_SER) {
			Runtime.printTraceMessage("Serializing [**] a " + Runtime.ANSI_CYAN
					+ "serialization_id_t" + Runtime.ANSI_RESET + ": " + i);
		}
		out.writeShort(i);
		if (Runtime.TRACE_SER) {
			Runtime.printTraceMessage("Serializing a " + Runtime.ANSI_CYAN
					+ Runtime.ANSI_BOLD + obj.getClass().getName()
					+ Runtime.ANSI_RESET);
		}
		obj.$_serialize(this);
	}
	...
	public void write(int i) throws IOException {
		if (Runtime.TRACE_SER) {
			Runtime.printTraceMessage("Serializing [****] an "
					+ Runtime.ANSI_CYAN + "int" + Runtime.ANSI_RESET + ": " + i);
		}
		out.writeInt(i);
	}
	...
	public void write(Integer p) throws IOException {
		...
		if (Runtime.TRACE_SER) {
			Runtime.printTraceMessage("Serializing an " + Runtime.ANSI_CYAN
					+ "Integer" + Runtime.ANSI_RESET + ": " + p);
		}
	...
	public void write(Character p) throws IOException {
		...
		if (Runtime.TRACE_SER) {
			Runtime.printTraceMessage("Serializing a " + Runtime.ANSI_CYAN
					+ "Character" + Runtime.ANSI_RESET + ": " + p);
		}
	...
	public void write(long l) throws IOException {
		if (Runtime.TRACE_SER) {
			Runtime.printTraceMessage("Serializing [********] a "
					+ Runtime.ANSI_CYAN + "long" + Runtime.ANSI_RESET + ": "
					+ l);
		}
		out.writeLong(l);
	}
	...
	public void write(double d) throws IOException {
		if (Runtime.TRACE_SER) {
			Runtime.printTraceMessage("Serializing [********] a "
					+ Runtime.ANSI_CYAN + "double" + Runtime.ANSI_RESET + ": "
					+ d);
		}
		out.writeDouble(d);
	}
	...
	public void write(float f) throws IOException {
		if (Runtime.TRACE_SER) {
			Runtime.printTraceMessage("Serializing [****] a "
					+ Runtime.ANSI_CYAN + "float" + Runtime.ANSI_RESET + ": "
					+ f);
		}
		out.writeFloat(f);
	}
	...
	private Integer previous_position(Object obj, boolean writeRef)
			throws IOException {
		Integer pos = objectMap.get(obj);
		if (pos != null) {
			if (Runtime.TRACE_SER) {
				Runtime.printTraceMessage("\t\tFound repeated reference of type "
						+ Runtime.ANSI_CYAN
						+ Runtime.ANSI_BOLD
						+ obj.getClass().getName()
						+ Runtime.ANSI_RESET
						+ " at " + pos + " (absolute) in map");
			}
			// We have serialized this object before hence no need to do it
			// again
			if (writeRef) {
			...
			if (Runtime.TRACE_SER) {
				Runtime.printTraceMessage("\t\tRecorded new reference of type "
						+ Runtime.ANSI_CYAN + Runtime.ANSI_BOLD
						+ obj.getClass().getName() + Runtime.ANSI_RESET
						+ " at " + counter + " (absolute) in map");
			}
			counter++;
		}
		return pos;
	...
	// This method is called from generated code when an X10 class has a Java
	// superclass
	public <T> void serializeClassUsingReflection(T obj, Class<T> clazz)
			throws IOException {
		try {
	...
	public void writeArrayUsingReflectionWithType(Object obj)
			throws IOException {
		if (obj == null) {
			writeNull();
			return;
	...
	<T> void writeStringUsingReflection(Field field, T obj)
			throws IllegalAccessException, IOException {
		String str = (String) field.get(obj);
		write(str);
	}

	<T> void writePrimitiveUsingReflection(Field field, T obj)
			throws IllegalAccessException, IOException {
