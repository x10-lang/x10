...
public class TeamSupport {
	...
	private static int getTypeCode(IndexedMemoryChunk<?> chunk) {
		...
		if (chunkRaw instanceof byte[]) {
			...
			throw new java.lang.UnsupportedOperationException(
					"Unsupported type of src array " + chunk.T.typeName()
							+ " in nativeAllReduce");
		}
		return typeCode;
	}
	...
	private static void aboutToDie(String methodName) {
		...
		throw new java.lang.UnsupportedOperationException("About to die in "
				+ methodName);
	}

	public static void nativeMake(IndexedMemoryChunk<Place> places, int count,
			IndexedMemoryChunk<x10.core.Int> result) {
	...
	public static void nativeScatter(int id, int role, int root,
			IndexedMemoryChunk<?> src, int src_off, IndexedMemoryChunk<?> dst,
		...
		if (!X10RT.forceSinglePlace) {
			...
			try {
				nativeScatterImpl(id, role, root, srcRaw, src_off, dstRaw,
						dst_off, count, typeCode, fs);
			} catch (UnsatisfiedLinkError e) {
	...
	public static void nativeBcast(int id, int role, int root,
			IndexedMemoryChunk<?> src, int src_off, IndexedMemoryChunk<?> dst,
			int dst_off, int count) {
		if (!X10RT.forceSinglePlace) {
			Object srcRaw = src.getBackingArray();
			...
			try {
				nativeBcastImpl(id, role, root, srcRaw, src_off, dstRaw,
						dst_off, count, typeCode, fs);
			} catch (UnsatisfiedLinkError e) {
	...
	public static void nativeAllToAll(int id, int role,
			IndexedMemoryChunk<?> src, int src_off, IndexedMemoryChunk<?> dst,
			int dst_off, int count) {
		if (!X10RT.forceSinglePlace) {
			Object srcRaw = src.getBackingArray();
			...
			try {
				nativeAllToAllImpl(id, role, srcRaw, src_off, dstRaw, dst_off,
						count, typeCode, fs);
			} catch (UnsatisfiedLinkError e) {
	...
	public static void nativeAllReduce(int id, int role,
			IndexedMemoryChunk<?> src, int src_off, IndexedMemoryChunk<?> dst,
			int dst_off, int count, int op) {
		if (!X10RT.forceSinglePlace) {
			Object srcRaw = src.getBackingArray();
			Object dstRaw = dst.getBackingArray();
			...
			try {
				nativeAllReduceImpl(id, role, srcRaw, src_off, dstRaw, dst_off,
						count, op, typeCode, fs);
			} catch (UnsatisfiedLinkError e) {
	...
	public static void nativeIndexOfMax(int id, int role,
			IndexedMemoryChunk<?> src, IndexedMemoryChunk<?> dst) {
		if (!X10RT.forceSinglePlace) {
			...
			try {
				...
				x10.util.Team.DoubleIdx dstTuple = ((x10.util.Team.DoubleIdx[]) dst
						.getBackingArray())[0];
			...
			x10.util.Team.DoubleIdx dstTuple = ((x10.util.Team.DoubleIdx[]) dst
					.getBackingArray())[0];
			x10.util.Team.DoubleIdx srcTuple = ((x10.util.Team.DoubleIdx[]) src
					.getBackingArray())[0];
	...
	public static void nativeIndexOfMin(int id, int role,
			IndexedMemoryChunk<?> src, IndexedMemoryChunk<?> dst) {
		if (!X10RT.forceSinglePlace) {
			double v = ((x10.util.Team.DoubleIdx[]) src.getBackingArray())[0].value;
			double[] value = new double[] { v };
	...
	public static void nativeSplit(int id, int role, int color, int new_role,
			IndexedMemoryChunk<x10.core.Int> result) {
		if (!X10RT.forceSinglePlace) {
			int[] nr = result.getIntArray();
	...
	private static native void nativeMakeImpl(int[] places, int count,
			int[] result, FinishState fs);

	private static native int nativeSizeImpl(int id);

	private static native void nativeBarrierImpl(int id, int role,
			FinishState fs);

	private static native void nativeScatterImpl(int id, int role, int root,
			Object srcRaw, int src_off, Object dstRaw, int dst_off, int count,
			int typecode, FinishState fs);

	private static native void nativeBcastImpl(int id, int role, int root,
			Object srcRaw, int src_off, Object dstRaw, int dst_off, int count,
			int typecode, FinishState fs);

	private static native void nativeAllToAllImpl(int id, int role,
			Object srcRaw, int src_off, Object dstRaw, int dst_off, int count,
			int typecode, FinishState fs);

	private static native void nativeAllReduceImpl(int id, int role,
			Object srcRaw, int src_off, Object dstRaw, int dst_off, int count,
			int op, int typecode, FinishState fs);

	private static native void nativeIndexOfMaxImpl(int id, int role,
			double[] value, int[] idx, FinishState fs);

	private static native void nativeIndexOfMinImpl(int id, int role,
			double[] value, int[] idx, FinishState fs);

	private static native void nativeSplitImpl(int id, int role, int color,
			int new_role, int[] nr, FinishState fs);
