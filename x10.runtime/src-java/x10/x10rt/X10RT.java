...
public class X10RT {
	private enum State {
		UNINITIALIZED, INITIALIZED, RUNNING, TEARING_DOWN, TORN_DOWN
	};
	...
	/**
	 * Initialize the X10RT runtime. This method, or the standard init() method
	 * below must be called before any other methods on this class or on any
	 * other X10RT related class can be successfully invoked.
	 */
	public static synchronized String init_library() {
		...
		if (libName.equals("disabled")) {
			...
			try {
				...
				System.err.println("Unable to load " + libName
						+ ". Forcing single place execution");
				forceSinglePlace = true;
			}
		}
		 ...
		 * x10.lang.Runtime.get$NTHREADS(); x10.lang.Runtime.get$MAX_THREADS();
	...
	/*
	 * This method is the second phase of the init_library() call above.
	 * Init_library only initializes internal variables, minus what is needed
	 * for communication with other places.
	 * 
	 * myPlace is which place this runtime is in the whole computation.
	 * connectionInfo is an array the size of nplaces, and contains the
	 * connection string for each remote place. The connection string for
	 * myPlace may be null.
	 */
	public static synchronized void connect_library(int myPlace,
	...
	public static synchronized void init() {
		...
		if (forceSinglePlace) {
			...
			// TODO: For now we are not trying to plumb the command line
			// arguments from
			// the program's main method into X10RT. We really can't easily do
			// this
			// until we change this code to be run via an explicit static method
			// in
			// X10RT instead of doing it in the class initializer.
			// bherta: fix in progress, via the init_library() and
			// connect_library() methods above
			...
			// Add a shutdown hook to automatically teardown X10RT as part of
			// JVM teardown
				...
				public void run() {
					synchronized (X10RT.class) {
						...
						if (X10_EXITING_NORMALLY) {
							if (VERBOSE)
								System.err
										.println("Normal exit; x10rt_finalize called");
							x10rt_finalize();
							if (VERBOSE)
								System.err
										.println("Normal exit; x10rt_finalize returned");
						} else {
							if (VERBOSE)
								System.err
										.println("Abnormal exit; skipping call to x10rt_finalize");
						}
	...
	/**
	 * This is a non-blocking call. Checks network for incoming messages and
	 * returns.
	 */
	...
	/*
	 * Native methods exported from x10rt_front.h that are related to
	 * initialization and finalization of the X10RT library. See X10RT API at
	 * x10-lang.org for semantics.
	 */
	...
	/*
	 * Subset of x10rt_front.h API related to messages that actually needs to be
	 * exposed at the Java level (as opposed to being used in the native code
	 * backing the native methods of MessageHandlers.
	 */
