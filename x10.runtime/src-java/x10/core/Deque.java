...
public class Deque extends Ref {
	...
	public static x10.serialization.X10JavaSerializable $_deserialize_body(
			Deque $_obj, x10.serialization.X10JavaDeserializer $deserializer)
			throws java.io.IOException {
		// TODO need check
		$_obj.base = $deserializer.readInt();
		$_obj.sp = $deserializer.readInt();
		$_obj.queue = (java.lang.Object[]) $deserializer
				.readArrayUsingReflection(java.lang.Object.class);
		return $_obj;
	}
	 ...
	 * power of two. Initial size must be at least 2, but is padded to minimize
	 * cache effects.
	 */
	 ...
	 * << 28 to ensure lack of index wraparound. (This is less than usual
	 * bounds, because we need leftshift by 3 to be in int range).
	 */
	private static final int MAXIMUM_QUEUE_CAPACITY = 1 << 28;

	/**
	 * The work-stealing queue array. Size must be a power of two. Initialized
	 * when thread starts, to improve memory locality.
	 */
	private Object[] queue;

	/**
	 * Index (mod queue.length) of next queue slot to push to or pop from. It is
	 * written only by owner thread, via ordered store. Both sp and base are
	 * allowed to wrap around on overflow, but (sp - base) still estimates size.
	 */
	...
	/**
	 * Add in store-order the given task at given slot of q to null. Caller must
	 * ensure q is nonnull and index is in range.
	 */
	private void setSlot(Object[] q, int i, Object t) {
		// DAVE G: Egregious hack to get code running on IBM SDKs that don't
		// have putOrderedObject.
		// _unsafe.putOrderedObject(q, (i << qShift) + qBase, t);
		_unsafe.putObject(q, (i << qShift) + qBase, t);
	}

	/**
	 * CAS given slot of q to null. Caller must ensure q is nonnull and index is
	 * in range.
	 */
	 ...
	 * 
	 * @param t
	 *            the task. Caller must ensure nonnull
	 */
	...
	/**
	 * Tries to take a task from the base of the queue, failing if either empty
	 * or contended.
	 * 
	...
	public final Object steal() {
		...
		int i;
		int b;
		if (sp != (b = base) && (q = queue) != null && // must read q after b
				(t = q[i = (q.length - 1) & b]) != null && casSlotNull(q, i, t)) {
			base = b + 1;
			return t;
		}
		return null;
	...
	/**
	 * Returns a popped task, or null if empty. Ensures active status if
	 * nonnull. Called only by current thread.
	 */
	...
	public static final RuntimeType<Deque> $RTT = NamedType.<Deque> make(
			"x10.lang.Deque", Deque.class);

	public RuntimeType<Deque> $getRTT() {
		return $RTT;
	}
	...
	static {
		try {
			...
			baseOffset = _unsafe.objectFieldOffset(Deque.class
					.getDeclaredField("base"));
			spOffset = _unsafe.objectFieldOffset(Deque.class
					.getDeclaredField("sp"));
