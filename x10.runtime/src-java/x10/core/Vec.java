...
public final class Vec<T> extends x10.core.Struct {
	...
	public static final RuntimeType<Vec<?>> $RTT = NamedType.<Vec<?>> make(
			"x10.util.Vec", Vec.class, RuntimeType.INVARIANTS(1),
	...
	public final Vec<T> x10$util$Vec$$init$S(final Type<T> T, final int s) {
		this.T = T;
		this.size = s;
		this.backing = new x10.array.Array<T>((java.lang.System[]) null, T)
				.x10$array$Array$$init$S(size);
		return this;
	}
	...
	public final Vec<T> x10$util$Vec$$init$S(final Type<T> T, Vec<T> other) {
		...
		this.backing = new x10.array.Array<T>((java.lang.System[]) null, T)
				.x10$array$Array$$init$S(other.backing,
						(x10.array.Array.__0$1x10$array$Array$$T$2) null);
		return this;
	}
	...
	// return this.equals_0$1x10$util$Vec$$T$2((Vec)
	// Types.asStruct(ParameterizedType.make(Vec.$RTT, T), other));
	// }
	//
	// final public boolean equals_0$1x10$util$Vec$$T$2(Vec other) {
	// if (this.size != other.size) return false;
	// for (int i = 0; i < this.size; ++i) {
	// if (!this.backing.$apply$G(i).equals(other.backing.$apply$G(i))) return
	// false;
	// }
	// return true;
	// }
	...
	final public boolean _struct_equals$O(java.lang.Object otherObj) {
		if (!Vec.$RTT.isInstance(otherObj, T))
			return false;
		Vec<T> other = (Vec<T>) Types.asStruct(
				ParameterizedType.make(Vec.$RTT, T), otherObj);
		// optimized
		// if (this.size != other.size) return false;
		// for (int i = 0; i < this.size; ++i) {
		// if (!this.backing.$apply$G(i).equals(other.backing.$apply$G(i)))
		// return false;
		// }
		// return true;
		...
		if (T == Types.BYTE || T == Types.UBYTE) {
			return java.util.Arrays.equals((byte[]) thisValue,
					(byte[]) otherValue);
		}
		if (T == Types.SHORT || T == Types.USHORT) {
			return java.util.Arrays.equals((short[]) thisValue,
					(short[]) otherValue);
		}
		if (T == Types.INT || T == Types.UINT) {
			return java.util.Arrays.equals((int[]) thisValue,
					(int[]) otherValue);
		}
		if (T == Types.LONG || T == Types.ULONG) {
			return java.util.Arrays.equals((long[]) thisValue,
					(long[]) otherValue);
		}
		if (T == Types.FLOAT) {
			return java.util.Arrays.equals((float[]) thisValue,
					(float[]) otherValue);
		}
		if (T == Types.DOUBLE) {
			return java.util.Arrays.equals((double[]) thisValue,
					(double[]) otherValue);
		}
		if (T == Types.CHAR) {
			return java.util.Arrays.equals((char[]) thisValue,
					(char[]) otherValue);
		}
		if (T == Types.BOOLEAN) {
			return java.util.Arrays.equals((boolean[]) thisValue,
					(boolean[]) otherValue);
		}
		return java.util.Arrays.equals((Object[]) thisValue,
				(Object[]) otherValue);
	}
	...
	public static X10JavaSerializable $_deserialize_body(Vec $_obj,
			X10JavaDeserializer $deserializer) throws IOException {
