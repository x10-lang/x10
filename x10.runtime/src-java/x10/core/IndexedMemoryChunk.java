...
public final class IndexedMemoryChunk<T> extends x10.core.Struct implements
		X10JavaSerializable {
	...
	public final IndexedMemoryChunk<T> x10$util$IndexedMemoryChunk$$init$S(
			Type<T> T, int length, Object value) {
		this.T = T;
		this.length = length;
		this.value = value;
		return this;
	...
	public static <T> IndexedMemoryChunk<T> allocate(Type/* <T> */T,
			long length, boolean zeroed) {
		if (length > Integer.MAX_VALUE) {
			// TODO
			throw new java.lang.OutOfMemoryError(
					"Array length must be shorter than 2^31");
		}
		return new IndexedMemoryChunk<T>(T, (int) length, zeroed);
	}

	public static <T> IndexedMemoryChunk<T> allocate(Type/* <T> */T, int length,
			boolean zeroed) {
	...
	public static <T> void asyncCopy(IndexedMemoryChunk<T> src,
			final int srcIndex, final RemoteIndexedMemoryChunk<T> dst,
			final int dstIndex, final int numElems) {
		// synchronous version for the same place
		if (dst.home.id == x10.lang.Runtime.home().id) {
			System.arraycopy(src.value, srcIndex, dst.$apply$G().value,
					dstIndex, numElems);
			return;
		}

		// extra copy here simplifies logic and allows us to do this entirely at
		// the Java level.
		// We'll eventually need to optimize this by writing custom native/JNI
		// code instead of treating
		...
		VoidFun_0_0 copyBody = new $Closure$0(dataToCopy, dst.id, dstIndex,
				numElems);
	...
	public static class $Closure$0 extends x10.core.Ref implements VoidFun_0_0 {
		...
		public static final RuntimeType<$Closure$0> $RTT = x10.rtt.StaticVoidFunType
				.<$Closure$0> make($Closure$0.class,
						new Type[] { VoidFun_0_0.$RTT });
		...
		// TODO Keith This is not compatible with C++ at the moment cause the
		// java backend does not implement send_put
		public void $_serialize(X10JavaSerializer $serializer)
				throws IOException {
			$serializer.write(this.numElems);
			if (this.numElems > 0) {
				Class<?> componentType = this.srcData.getClass()
						.getComponentType();
				if (componentType.isPrimitive()) {
					$serializer
							.write(SerializationConstants.JAVA_OBJECT_STREAM_ID);
		...
		public static X10JavaSerializable $_deserialize_body($Closure$0 $_obj,
				X10JavaDeserializer $deserializer) throws IOException {
	...
	public static <T> void asyncCopy(IndexedMemoryChunk<T> src, int srcIndex,
			RemoteIndexedMemoryChunk<T> dst, int dstIndex, int numElems,
			VoidFun_0_0 notifier) {
		// synchronous version for the same place
		if (dst.home.id == x10.lang.Runtime.home().id) {
			System.arraycopy(src.value, srcIndex, dst.$apply$G().value,
					dstIndex, numElems);
		...
		throw new java.lang.UnsupportedOperationException(
				"asyncCopy with notifier not implemented for multivm");
		// notifier.$apply();
	}

	public static <T> void asyncCopy(final RemoteIndexedMemoryChunk<T> src,
			final int srcIndex, IndexedMemoryChunk<T> dst, final int dstIndex,
			final int numElems) {
		// synchronous version for the same place
		if (src.home.id == x10.lang.Runtime.home().id) {
			System.arraycopy(src.$apply$G().value, srcIndex, dst.value,
					dstIndex, numElems);
			return;
		}

		// A really bad implementation! Leaks dst!! Non-optimized copies! Extra
		// distributed async/finish traffic!
		final RemoteIndexedMemoryChunk<T> dstWrapper = RemoteIndexedMemoryChunk
				.wrap(dst);

		VoidFun_0_0 copyBody1 = new $Closure$1<T>(src, srcIndex, dstWrapper,
				dstIndex, numElems);
	...
	public static class $Closure$1<T> extends x10.core.Ref implements
			VoidFun_0_0 {
		private static final long serialVersionUID = 1L;
		public Type<T> srcT;
		public int srcId;
		...
		$Closure$1(RemoteIndexedMemoryChunk<T> src, int srcIndex,
				RemoteIndexedMemoryChunk<T> dstWrapper, int dstIndex,
				int numElems) {
			this.srcT = src.T;
			this.srcId = src.id;
		...
		public void $apply() {
			// This body runs at src's home. It accesses the data for src and
			// then does
			// another async back to dstWrapper's home to transfer the data.
			Object srcData = RemoteIndexedMemoryChunk.getValue(srcId);

			// extra copy here simplifies logic and allows us to do this
			// entirely at the Java level.
			// We'll eventually need to optimize this by writing custom
			// native/JNI code instead of treating
			// it as just another async to execute remotely.
			final Object dataToCopy;
			...
			VoidFun_0_0 copyBody2 = new $Closure$0(dataToCopy, dstWrapperId,
					dstIndex, numElems);

			x10.lang.Runtime.runAsync(dstWrapperHome, copyBody2, null);
		}

		public static final RuntimeType<$Closure$1<?>> $RTT = x10.rtt.StaticVoidFunType
				.<$Closure$1<?>> make($Closure$1.class,
						new Type[] { VoidFun_0_0.$RTT });
		...
		public static X10JavaSerializable $_deserialize_body($Closure$1 $_obj,
				X10JavaDeserializer $deserializer) throws IOException {
	...
	public static <T> void asyncCopy(RemoteIndexedMemoryChunk<T> src,
			int srcIndex, IndexedMemoryChunk<T> dst, int dstIndex,
	...
	public static <T> void copy(IndexedMemoryChunk<T> src, int srcIndex,
			IndexedMemoryChunk<T> dst, int dstIndex, int numElems) {
	...
	public RemoteIndexedMemoryChunk<T> getCongruentSibling(x10.lang.Place p) {
		throw new java.lang.UnsupportedOperationException(
				"Remote operations are not implemented.");
	}

	public static final RuntimeType<IndexedMemoryChunk<?>> $RTT = NamedType
			.<IndexedMemoryChunk<?>> make("x10.util.IndexedMemoryChunk",
					IndexedMemoryChunk.class, RuntimeType.INVARIANTS(1),
	...
	public void $_serialize(X10JavaSerializer $serializer) throws IOException {
		...
		// If the T is a java primitive type, we use default java serialization
		// here
	...
	public static X10JavaSerializable $_deserializer(
			X10JavaDeserializer $deserializer) throws IOException {
		IndexedMemoryChunk $_obj = new IndexedMemoryChunk(
	...
	public static X10JavaSerializable $_deserialize_body(
			IndexedMemoryChunk $_obj, X10JavaDeserializer $deserializer)
			throws IOException {
		...
		if (Types.isPrimitiveType($_obj.T)) {
			...
			java.lang.String[] values = (java.lang.String[]) $_obj.T
					.makeArray($_obj.length);
