...
public class NamedStructType<T> extends RuntimeType<T> implements
		X10JavaSerializable {
	...
	// }
	//
	// protected NamedStructType(String typeName, Class<?> javaClass, Variance[]
	// variances) {
	// super(javaClass, variances);
	// this.typeName = typeName;
	// }
	//
	// protected NamedStructType(String typeName, Class<?> javaClass, Type<?>[]
	// parents) {
	// super(javaClass, parents);
	// this.typeName = typeName;
	// }

	// N.B. this is also used to implement readResolve() in place for
	// Types.COMPARABLE
	protected NamedStructType(String typeName, Class<?> javaClass,
	...
	public static <T> NamedStructType/* <T> */make(String typeName,
			Class<?> javaClass) {
		if (useCache) {
			NamedStructType<?> type = typeCache.get(javaClass);
			if (type == null) {
				NamedStructType<?> type0 = new NamedStructType<T>(typeName,
						javaClass, null, null);
	...
	public static <T> NamedStructType/* <T> */make(String typeName,
			Class<?> javaClass, Variance[] variances) {
		if (useCache) {
			NamedStructType<?> type = typeCache.get(javaClass);
			if (type == null) {
				NamedStructType<?> type0 = new NamedStructType<T>(typeName,
						javaClass, variances, null);
				type = typeCache.putIfAbsent(javaClass, type0);
				if (type == null)
					type = type0;
			}
	...
	public static <T> NamedStructType/* <T> */make(String typeName,
			Class<?> javaClass, Type<?>[] parents) {
		if (useCache) {
			NamedStructType<?> type = typeCache.get(javaClass);
			if (type == null) {
				NamedStructType<?> type0 = new NamedStructType<T>(typeName,
						javaClass, null, parents);
				type = typeCache.putIfAbsent(javaClass, type0);
				if (type == null)
					type = type0;
			}
	...
	public static <T> NamedStructType/* <T> */make(String typeName,
			Class<?> javaClass, Variance[] variances, Type<?>[] parents) {
		if (useCache) {
			NamedStructType<?> type = typeCache.get(javaClass);
			if (type == null) {
				NamedStructType<?> type0 = new NamedStructType<T>(typeName,
						javaClass, variances, parents);
				type = typeCache.putIfAbsent(javaClass, type0);
				if (type == null)
					type = type0;
			}
			...
			return new NamedStructType<T>(typeName, javaClass, variances,
					parents);
		}
	}
	...
	public static X10JavaSerializable $_deserialize_body(NamedStructType nt,
			X10JavaDeserializer deserializer) throws IOException {
