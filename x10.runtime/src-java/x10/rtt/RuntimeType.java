...
public class RuntimeType<T> implements Type<T>, X10JavaSerializable {
	...
	public static <T> RuntimeType/* <T> */make(Class<?> javaClass,
		...
		if (useCache) {
			RuntimeType<?> type = typeCache.get(javaClass);
			if (type == null) {
				RuntimeType<?> type0 = new RuntimeType<T>(javaClass, variances,
						null);
				type = typeCache.putIfAbsent(javaClass, type0);
				if (type == null)
					type = type0;
	...
	public static <T> RuntimeType/* <T> */make(Class<?> javaClass,
		...
		if (useCache) {
			RuntimeType<?> type = typeCache.get(javaClass);
			if (type == null) {
				RuntimeType<?> type0 = new RuntimeType<T>(javaClass, null,
						parents);
				type = typeCache.putIfAbsent(javaClass, type0);
				if (type == null)
					type = type0;
	...
	public boolean equals(Object o) {
		...
		if (o instanceof RuntimeType<?>) {
			...
			// N.B. for given javaClass, we assume variances and parents are
			// unique.
	...
	public boolean isAssignableTo(Type<?> superType) {
		...
		if (superType instanceof ParameterizedType) {
			...
			if (pt.getRawType().isAssignableFrom(pt.getActualTypeArguments(),
					this, null)) {
				return true;
			}
		}
	...
	private static final boolean subtypeTestForParam(Variance variance,
			Type<?> superParam, Type<?> subParam) {
		switch (variance) {
		case INVARIANT:
	...
	public final boolean isInstance(Object o, Type<?>... thisParams) {
		 ...
		 * @NativeRep'ed type return checkParents(o, thisParams); }
		 */
		else {
			return false;
		}
	...
	private boolean checkAnonymous(Class<?> target) {
		...
		if (target.getSuperclass() != java.lang.Object.class
				&& target.getSuperclass() == javaClass) {
			return true;
		}
		if (target.getInterfaces().length == 1
				&& target.getInterfaces()[0] == javaClass) {
			return true;
		}
		return false;
	}
	...
	// instantiateCheck(Type<?>[] params, RuntimeType<?> rtt, Type<?>[]
	// paramsRTT)
	// e.g. C[T1,T2]:Super[Int, T1] -> C[Int,Double]:Super[Int,Int]
	private final boolean instantiateCheck(Type<?>[] params,
			RuntimeType<?> rtt, Object o) {
		if (rtt.parents != null) {
			for (Type<?> t : rtt.parents) {
				if (javaClass.isAssignableFrom(t.getJavaClass())) {
					if (t instanceof ParameterizedType<?>) {
						ParameterizedType<?> pt = (ParameterizedType<?>) t;
						Type<?>[] origTypeArgumentsT = pt
								.getActualTypeArguments();
						Type<?>[] resolvedTypeArgumentsT = new Type<?>[origTypeArgumentsT.length];
						for (int i = 0; i < origTypeArgumentsT.length; i++) {
							if (origTypeArgumentsT[i] != null
									&& origTypeArgumentsT[i] instanceof UnresolvedType) {
								int index = ((UnresolvedType) origTypeArgumentsT[i])
										.getIndex();
								assert (index == -1);
						...
						if (isAssignableFrom(params, pt.getRawType(),
								resolvedTypeArgumentsT)) {
							return true;
						}
					}
				}
					...
					if (t instanceof ParameterizedType<?>) {
						...
						for (int i = 0; i < origTypeArgumentsT.length; i++) {
							if (origTypeArgumentsT[i] != null
								...
								int index = ((UnresolvedType) origTypeArgumentsT[i])
										.getIndex();
								resolvedTypeArgumentsT[i] = index == -1 ? rtt
										: any.$getParam(index);
							} else {
								resolvedTypeArgumentsT[i] = origTypeArgumentsT[i];
							}
	...
	// e.g. C[T1,T2]:Super[Int, T1] -> C[Int,Double]:Super[Int,Int]
	private final boolean instantiateCheck(Type<?>[] params,
			RuntimeType<?> rtt, Type<?>[] paramsRTT) {
		if (rtt.parents != null) {
			for (Type<?> t : rtt.parents) {
				if (javaClass.isAssignableFrom(t.getJavaClass())) {
					if (t instanceof ParameterizedType<?>) {
	...
	final boolean isAssignableFrom(Type<?>[] thisParams,
			RuntimeType<?> subType, Type<?>[] subParams) {
	...
	public Object makeArray(int dim0, int dim1, int dim2, int dim3) {
		return Array.newInstance(javaClass,
				new int[] { dim0, dim1, dim2, dim3 });
	}
	...
	private static String typeName(Class<?> javaClass) {
		...
		if (javaClass.isArray()) {
			return X10_INTEROP_JAVA_ARRAY + "["
					+ typeName(javaClass.getComponentType()) + "]";
	...
	public boolean isInstance(Object o, Type<?> thisParam0) {
		...
		/*
		 * else if (o instanceof String || o instanceof Number) { //
		 * @NativeRep'ed type return checkParents(o, thisParam0); }
		 */
		else {
			return false;
		}
	...
	// for shortcut
	public final boolean isInstance(Object o, Type<?> thisParam0,
			Type<?> thisParam1) {
		if (o == null) {
			return false;
		}
		...
		/*
		 * else if (o instanceof String || o instanceof Number) { //
		 * @NativeRep'ed type return checkParents(o, thisParam0, thisParam1); }
		 */
		else {
			return false;
		}
	...
	// for shortcut
	public final boolean isInstance(Object o, Type<?> thisParam0,
			Type<?> thisParam1, Type<?> thisParam2) {
		if (o == null) {
			return false;
		}
		Class<?> target = o.getClass();
	...
	public static X10JavaSerializable $_deserializer(
		...
		X10JavaSerializable x10JavaSerializable = $_deserialize_body(rt,
				deserializer);
	...
	public static X10JavaSerializable $_deserialize_body(RuntimeType rt,
			X10JavaDeserializer deserializer) throws IOException {
