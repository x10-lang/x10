...
public class NamedType<T> extends RuntimeType<T> implements X10JavaSerializable {
	...
	public static <T> NamedType/* <T> */make(String typeName,
			Class<?> javaClass) {
		if (useCache) {
			NamedType<?> type = typeCache.get(javaClass);
			if (type == null) {
				NamedType<?> type0 = new NamedType<T>(typeName, javaClass,
						null, null);
	...
	public static X10JavaSerializable $_deserialize_body(NamedType nt,
			X10JavaDeserializer deserializer) throws IOException {
