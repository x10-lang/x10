...
public final class ParameterizedType<T> implements Type<T>, X10JavaSerializable {
	...
	private ParameterizedType(RuntimeType<T> rawType,
	...
	public static <T> ParameterizedType/* <T> */make(RuntimeType<T> rawType,
			Type<?> actualTypeArgument0) {
		if (useCache && rawType != null
				&& actualTypeArgument0 != null
				// N.B. guard from NPE with recursive type. see
				// XTENLANG_423.W[U], which extends Z[W[U]], causes NPE for
				// hashCode.
				&& (!(actualTypeArgument0 instanceof ParameterizedType) || ((ParameterizedType<?>) actualTypeArgument0).rawType != null)) {
			ConcurrentHashMap<Type<?>, ParameterizedType<?>> typeCache10 = typeCache1
					.get(rawType);
			if (typeCache10 == null) {
	...
	private static final ConcurrentHashMap<RuntimeType<?>, ConcurrentHashMap<Type<?>, ConcurrentHashMap<Type<?>, ParameterizedType<?>>>> typeCache2 = new ConcurrentHashMap<RuntimeType<?>, ConcurrentHashMap<Type<?>, ConcurrentHashMap<Type<?>, ParameterizedType<?>>>>();

	public static <T> ParameterizedType/* <T> */make(RuntimeType<T> rawType,
			Type<?> actualTypeArgument0, Type<?> actualTypeArgument1) {
		if (useCache
				&& rawType != null
				&& actualTypeArgument0 != null
				&& actualTypeArgument1 != null
				// N.B. guard from NPE with recursive type. see
				// XTENLANG_423.W[U], which extends Z[W[U]], causes NPE for
				// hashCode.
				&& (!(actualTypeArgument0 instanceof ParameterizedType) || ((ParameterizedType<?>) actualTypeArgument0).rawType != null)
				&& (!(actualTypeArgument1 instanceof ParameterizedType) || ((ParameterizedType<?>) actualTypeArgument1).rawType != null)) {
			ConcurrentHashMap<Type<?>, ConcurrentHashMap<Type<?>, ParameterizedType<?>>> typeCache20 = typeCache2
					.get(rawType);
			if (typeCache20 == null) {
				...
				if (typeCache20 == null)
					typeCache20 = typeCache20_;
			}
			ConcurrentHashMap<Type<?>, ParameterizedType<?>> typeCache21 = typeCache20
					.get(actualTypeArgument0);
			if (typeCache21 == null) {
				ConcurrentHashMap<Type<?>, ParameterizedType<?>> typeCache21_;
				typeCache21_ = new ConcurrentHashMap<Type<?>, ParameterizedType<?>>();
				typeCache21 = typeCache20.putIfAbsent(actualTypeArgument0,
						typeCache21_);
				if (typeCache21 == null)
					typeCache21 = typeCache21_;
			}
	...
	public final boolean isAssignableTo(Type<?> superType) {
		...
		if (superType instanceof ParameterizedType) {
			...
			if (pt.getRawType().isAssignableFrom(pt.actualTypeArguments,
					rawType, actualTypeArguments)) {
				return true;
			}
		} else if (superType instanceof RuntimeType) {
			if (((RuntimeType<?>) superType).isAssignableFrom(null, rawType,
					actualTypeArguments)) {
				return true;
			}
		}
	...
	public static X10JavaSerializable $_deserialize_body(ParameterizedType pt,
			X10JavaDeserializer deserializer) throws IOException {
