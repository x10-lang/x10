    ...
    private static class LocalTeamState(places:PlaceGroup, teamid:Int, myIndex:Long) {
        ...
        private def collective_impl[T](collType:Int, root:Place, src:Rail[T], src_off:Long, dst:Rail[T], dst_off:Long, count:Long, operation:Int, offsets:Rail[Int], counts:Rail[Int]):void {
            ...
            if (local_child1Index == -1) { // no children to wait for
                this.phase.compareAndSet(PHASE_INIT, PHASE_SCATTER);
            ...
            sleepUntil(() => this.phase.get() == PHASE_SCATTER, "upward");
            if (Team.state(teamidcopy).isValid()) {
                ...
                try { // try/catch for DeadPlaceExceptions associated with the 'at' statements
	                ...
	                if (myLinks.parentIndex == -1) { // this is the root
	                    ...
	                    if (collType == COLL_BROADCAST)
	                        ...
	                        local_temp_buff = src;
	                    this.phase.compareAndSet(PHASE_SCATTER, PHASE_DONE); // the root node has no parent, and can skip its own state ahead
	                } else {
	                    ...
	                    if (collType >= COLL_ALLTOALL) {
	                        ...
	                        if (collType == COLL_ALLTOALL) {
	                            ...
	                            @Pragma(Pragma.FINISH_ASYNC) at (places(myLinks.parentIndex)) async {
	                                if (waitForParentToReceive()) {
	                                    ...
	                                    finish Rail.asyncCopy(gr, dst_off+(count*sourceIndex), Team.state(teamidcopy).local_dst as Rail[T], Team.state(teamidcopy).local_dst_off+(count*sourceIndex), totalData);
	                            ...
	                            @Pragma(Pragma.FINISH_ASYNC) at (places(myLinks.parentIndex)) async {
                                    if (waitForParentToReceive()) {
                                        ...
                                        if (target != null)
                                            finish Rail.asyncCopy(gr, dst_off, target, off, count);
                                            incrementParentPhase();
                                    }
	                            }
	                            ...
	                            @Pragma(Pragma.FINISH_ASYNC) at (places(myLinks.parentIndex)) async {
	                                if (waitForParentToReceive()) {
                                        sleepUntil(() => Team.state(teamidcopy).dstLock.tryLock(), "lock");
                            ...
                            if (DEBUGINTERNALS) Runtime.println(here+":team"+teamidcopy+" about to increment parent " + places(myLinks.parentIndex));
	                        @Pragma(Pragma.FINISH_ASYNC) finish at (places(myLinks.parentIndex)) async {
                                if (waitForParentToReceive())
                                    incrementParentPhase();                 
	                        }
                            if (DEBUGINTERNALS) Runtime.println(here+":team"+teamidcopy+" finished incrementing parent " + places(myLinks.parentIndex));
	                    }
	                    
	                    sleepUntil(() => this.phase.get() == PHASE_DONE, "downward");
	                }
	                ...
	                if (dper.size > 0) {
	                    if (DEBUGINTERNALS) Runtime.println(here+" caught DPE (multiple) updating child: "+dper(0));
	                    Team.state(teamidcopy).markInvalid();
	                } else {
                        throw me;
                    }
	                    ...
	                    if (collType == COLL_ALLTOALL) {
	                        ...
	                        @Pragma(Pragma.FINISH_SPMD) finish {
	                            at (places(local_child1Index)) async {
	                                ...
	                                finish Rail.asyncCopy(grDst, dst_off, Team.state(teamidcopy).local_dst as Rail[T], Team.state(teamidcopy).local_dst_off, Team.state(teamidcopy).local_count);
	                            }
	                            if (local_child2Index != -1) {
	                        ...
	                        val copyToChild = () => @NoInline {
	                            ...
	                            finish Rail.asyncCopy(grTmp, myOffset, Team.state(teamidcopy).local_temp_buff as Rail[T], 0, totalData);
	                        };
	                ...
	                if (local_child1Index != -1) { // free the first child, if it exists
	                    ...
	                    val freeChild1 = () => @NoInline {
	                        if (!Team.state(teamidcopy).phase.compareAndSet(PHASE_SCATTER, PHASE_DONE))
	                            Runtime.println("ERROR setting the first child "+here+":team"+teamidcopy+" to PHASE_DONE");
	                    };
	                    ...
	                    if (local_child2Index != -1) {
	                        ...
	                        val freeChild2 = () => @NoInline {
	                            if (!Team.state(teamidcopy).phase.compareAndSet(PHASE_SCATTER, PHASE_DONE))
	                                Runtime.println("ERROR setting the second child "+here+":team"+teamidcopy+" to PHASE_DONE");
	                        };
