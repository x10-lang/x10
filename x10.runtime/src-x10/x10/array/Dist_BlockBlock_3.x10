 ...
 */
public class Dist_BlockBlock_3 extends Dist(3) {
    ...
    @NonEscaping protected final def computeLocalIndices():DenseIterationSpace_3{self!=null} {
        val local_m = BlockingUtils.partitionBlockBlock(globalIndices.min(0), globalIndices.min(1),
                                                        globalIndices.max(0), globalIndices.max(1), 
                                                        pg.numPlaces(), pg.indexOf(here));
        return new DenseIterationSpace_3(local_m.min(0), local_m.min(1), globalIndices.min(2),
                                         local_m.max(0), local_m.max(1), globalIndices.max(2));
    }
