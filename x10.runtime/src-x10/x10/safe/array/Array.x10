...
import safe.util.IndexedMemoryChunk;
...
public final class Array[T] (
   ...
   implements (Point(rank))=>T,
    ...
    public @Inline def this(backingStore:IndexedMemoryChunk[T])
    {
        ...
        property(myReg, Int.asInt(1) as Int{self == Int.asInt(1)}, Boolean.TRUE(), Boolean.TRUE(), Boolean.TRUE(), backingStore.length());
    ...
    public @Inline def this(size:Int) {T haszero}
    {
        ...
        property(myReg, Int.asInt(1) as Int{self == Int.asInt(1)}, Boolean.TRUE(), Boolean.TRUE(), Boolean.TRUE(), size);
    ...
    public @Inline def this(init:Array[T])
    {
        ...
        // FIXME: why not implicit cast?
        IndexedMemoryChunk.copy(init.raw, Int.asInt(0), r, Int.asInt(0), r.length());
        raw = r;
    }
 ...
 // TODO: propagate the typeArg of the target (this) to the call in ConstructorSplitterVisitor
    ...
    public def toString(): String {
        if (rail()) {
            ...
            sb.add("[");
            val sz:Int = Math.min(Int.asX10Int(size), 10);
            for (var i:Int = 0; (i < sz)(); i = i + 1) {
                if ((i > 0)()) sb.add(",");
                sb.add("" + raw(i));
            }
    ...
    // public def sequence(){this.rank==Int.asInt(1)}:Sequence[T] = new Sequence[T]() {
    ...
    public @Inline operator this(i0:Int){rank==Int.asInt(1)}:T {
        if (rail()) {
     ...
     * @see #set(T, Int, Int)
     */
    public @Inline operator this(i0:Int, i1:Int){rank==Int.asInt(2)}:T {
     ...
     */
    public @Inline operator this(i0:Int, i1:Int, i2:Int){rank==Int.asInt(3)}:T {
     ...
     */
    public @Inline operator this(i0:Int, i1:Int, i2:Int, i3:Int){rank==Int.asInt(4)}:T {
    ...
    public @Inline operator this(i0:Int)=(v:T){rank==Int.asInt(1)}:T{self==v} {
        if (rail()) {
            // Bounds checking by backing IndexedMemoryChunk is sufficient
            raw(i0) = v;
        } else {
     ...
     * @see #set(T, Point)
     */
    public @Inline operator this(i0:Int,i1:Int)=(v:T){rank==Int.asInt(2)}:T{self==v} {
        if (CompilerFlags.checkBounds() && !region.contains(i0, i1)()) {
            raiseBoundsError(i0, i1);
        }
        var offset:Int  = i0 - layout_min0;
     ...
     * @see #set(T, Point)
     */
    public @Inline operator this(i0:Int, i1:Int, i2:Int)=(v:T){rank==Int.asInt(3)}:T{self==v} {
        if (CompilerFlags.checkBounds() && !region.contains(i0, i1, i2)()) {
            raiseBoundsError(i0, i1, i2);
        }
     ...
     * @see #set(T, Point)
     */
    public @Inline operator this( i0:Int, i1:Int, i2:Int, i3:Int)=(v:T){rank==Int.asInt(4)}:T{self==v} {
        if (CompilerFlags.checkBounds() && !region.contains(i0, i1, i2, i3)()) {
            raiseBoundsError(i0, i1, i2, i3);
        }
    ...
    public static def copy[T](src:Array[T], dst:Array[T]) {
        ...
        IndexedMemoryChunk.copy(src.raw, Int.asInt(0), dst.raw, Int.asInt(0), src.raw.length());
    }
    ...
    private static struct LayoutHelper {
        ...
        def this(reg:Region) {
            if (reg.isEmpty()()) {
                ...
                if (Int.sequals(reg.rank, 1)()) {
                    min0 = reg.min(0);
                    stride1 = 0;
                    min1 = 0;
                    size = reg.max(0) - reg.min(0) + 1;
                    layout = null;
                } else if (Int.sequals(reg.rank, 2)()) {
                    min0 = reg.min(0);
                    min1 = reg.min(1);
