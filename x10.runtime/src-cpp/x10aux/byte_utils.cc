...
String* x10aux::byte_utils::toString(x10_byte value, x10_int radix) {
...
String* x10aux::byte_utils::toString(x10_byte value) {
    return to_string(value);
}

String* x10aux::byte_utils::toString(x10_ubyte value, x10_int radix) {
    if (0 == value) return String::Lit("0");
    if (radix < 2 || radix > 36) radix = 10;
...
String* x10aux::byte_utils::toString(x10_ubyte value) {
    return to_string(value);
}

x10_byte x10aux::byte_utils::parseByte(String* s, x10_int radix) {
...
x10_ubyte x10aux::byte_utils::parseUByte(String* s, x10_int radix) {
    const char *start = nullCheck(s)->c_str();
    char *end;
    errno = 0;
