...
/////
String* LongNatives::toString__tm__(x10tm::TMThread *SelfTM, x10_long value, x10_int radix) {
    if (0 == value) return String::Lit("0");
    if (radix < 2 || radix > 36) radix = 10;
    // worst case is binary of Long.MIN_VALUE -- - plus 64 digits and a '\0'
    char buf[66] = ""; //zeroes entire buffer (S6.7.8.21)
...
String* LongNatives::toString__tm__(x10tm::TMThread *SelfTM, x10_long value) {
    return to_string(value);
}

x10_long LongNatives::parseLong__tm__(x10tm::TMThread *SelfTM, String* s, x10_int radix) {
    const char *start = nullCheck(s)->c_str();
    char *end;
    errno = 0;
    x10_long ans = strtoll(start, &end, radix);
...
x10_long LongNatives::highestOneBit__tm__(x10tm::TMThread *SelfTM, x10_long x) {
    x |= (x >> 1);
    x |= (x >> 2);
    x |= (x >> 4);
    x |= (x >> 8);
...
x10_long LongNatives::lowestOneBit__tm__(x10tm::TMThread *SelfTM, x10_long x) {
    return x & (-x);
}

x10_int LongNatives::numberOfLeadingZeros__tm__(x10tm::TMThread *SelfTM, x10_long x) {
    x |= (x >> 1);
    x |= (x >> 2);
    x |= (x >> 4);
    x |= (x >> 8);
...
x10_int LongNatives::numberOfTrailingZeros__tm__(x10tm::TMThread *SelfTM, x10_long x) {
    return bitCount(~x & (x-1));
}

x10_int LongNatives::bitCount__tm__(x10tm::TMThread *SelfTM, x10_long x) {
    x10_ulong ux = (x10_ulong)x;
    ux = ux - ((ux >> 1) & 0x5555555555555555LL);
    ux = (ux & 0x3333333333333333LL) + ((ux >> 2) & 0x3333333333333333LL);
    ux = (ux & 0x0F0F0F0F0F0F0F0FLL) + ((ux >> 4) & 0x0F0F0F0F0F0F0F0FLL);
...
x10_long LongNatives::rotateLeft__tm__(x10tm::TMThread *SelfTM, x10_long x, x10_int distance) {
    return (x << distance) | (((x10_ulong)x) >> (64 - distance));
}

x10_long LongNatives::rotateRight__tm__(x10tm::TMThread *SelfTM, x10_long x, x10_int distance) {
    return (((x10_ulong)x) >> distance) | (x << (64 - distance));
}

x10_long LongNatives::reverse__tm__(x10tm::TMThread *SelfTM, x10_long x) {
    x10_ulong ux = (x10_ulong)x;
    ux = ((ux & 0x5555555555555555LL) << 1) | ((ux >> 1) & 0x5555555555555555LL);
    ux = ((ux & 0x3333333333333333LL) << 2) | ((ux >> 2) & 0x3333333333333333LL);
    ux = ((ux & 0x0F0F0F0F0F0F0F0FLL) << 4) | ((ux >> 4) & 0x0F0F0F0F0F0F0F0FLL);
...
x10_long LongNatives::reverseBytes__tm__(x10tm::TMThread *SelfTM, x10_long x) {
    x10_ulong ux = x;
    x10_ulong ans = ux << 56;
    ans |= (ux & 0xFF00L) << 40;
    ans |= (ux & 0xFF0000L) << 24;
