...
///
String* IntNatives::toString__tm__(x10tm::TMThread *SelfTM, x10_int value, x10_int radix) {
    if (0 == value) return String::Lit("0");
    if (radix < 2 || radix > 36) radix = 10;
    // worst case is binary of Int.MIN_VALUE -- - plus 32 digits and a '\0'
    char buf[34] = ""; //zeroes entire buffer (S6.7.8.21)
...
String* IntNatives::toString__tm__(x10tm::TMThread *SelfTM, x10_int value) {
    return to_string(value);
}

x10_int IntNatives::parseInt__tm__(x10tm::TMThread *SelfTM, String* s, x10_int radix) {
    const char *start = nullCheck(s)->c_str();
    char *end;
    errno = 0;
    x10_int ans = strtol(start, &end, radix);
...
x10_int IntNatives::highestOneBit__tm__(x10tm::TMThread *SelfTM, x10_int x) {
    x |= (x >> 1);
    x |= (x >> 2);
    x |= (x >> 4);
    x |= (x >> 8);
...
x10_int IntNatives::lowestOneBit__tm__(x10tm::TMThread *SelfTM, x10_int x) {
    return x & (-x);
}

x10_int IntNatives::numberOfLeadingZeros__tm__(x10tm::TMThread *SelfTM, x10_int x) {
    x |= (x >> 1);
    x |= (x >> 2);
    x |= (x >> 4);
    x |= (x >> 8);
...
x10_int IntNatives::numberOfTrailingZeros__tm__(x10tm::TMThread *SelfTM, x10_int x) {
    return bitCount(~x & (x-1));
}

x10_int IntNatives::bitCount__tm__(x10tm::TMThread *SelfTM, x10_int x) {
    x10_uint ux = (x10_uint)x;
    ux = ux - ((ux >> 1) & 0x55555555);
    ux = (ux & 0x33333333) + ((ux >> 2) & 0x33333333);
    ux = (ux + (ux >> 4)) & 0x0F0F0F0F;
...
x10_int IntNatives::rotateLeft__tm__(x10tm::TMThread *SelfTM, x10_int x, x10_int distance) {
    return (x << distance) | (((x10_uint)x) >> (32 - distance));
}

x10_int IntNatives::rotateRight__tm__(x10tm::TMThread *SelfTM, x10_int x, x10_int distance) {
    return (((x10_uint)x) >> distance) | (x << (32 - distance));
}

x10_int IntNatives::reverse__tm__(x10tm::TMThread *SelfTM, x10_int x) {
    x10_uint ux = (x10_uint)x;
    ux = ((ux & 0x55555555) << 1) | ((ux >> 1) & 0x55555555);
    ux = ((ux & 0x33333333) << 2) | ((ux >> 2) & 0x33333333);
    ux = ((ux & 0x0F0F0F0F) << 4) | ((ux >> 4) & 0x0F0F0F0F);
...
x10_int IntNatives::reverseBytes__tm__(x10tm::TMThread *SelfTM, x10_int x) {
    x10_long value = 0;
    if (x<0) {
        value = 0x80000000;
        x &= 0x7FFFFFFF;
